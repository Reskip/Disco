...
describe('multi-tenancy config and tenant resolution', () => {
  ...
  it('resolves required tenant from trusted header when configured', () => {
    const ctx = resolveTenantContext(
      { multi_tenancy: { mode: 'required_from_auth', trusted_header: 'x-disco-tenant-id' } },
      { headers: { 'X-Disco-Tenant-Id': 'tenant-b' } }
    );
  ...
  it('rejects conflicting trusted headers from request and Feathers params', () => {
    expect(() =>
      resolveTenantContext(
        ...
        {
          headers: { 'x-disco-tenant-id': 'tenant-a' },
          params: { headers: { 'X-Disco-Tenant-Id': 'tenant-b' } },
        }
      )
    ).toThrow(/Conflicting tenant identities/);
  });
  ...
  it.each([
    {
      ...
      headers: { 'x-disco-tenant-id': ['tenant-a', 'tenant-b'] },
    },
    {
  ...
  it.each([
    {
      ...
      headers: { 'x-disco-tenant-id': ['tenant-a', 'tenant-a'] },
    },
    {
