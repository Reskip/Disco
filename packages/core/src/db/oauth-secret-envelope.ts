...
export function sealMCPOAuthSecret(
  ...
  if (!masterSecret) throw new Error('MCP OAuth secret sealing requires DISCO_MASTER_SECRET');
...
export function openMCPOAuthSecret(
  ...
  if (!masterSecret) throw new Error('MCP OAuth secret opening requires DISCO_MASTER_SECRET');
