...
import type { ContentBlock } from '@disco/core/types';
...
const BROAD_SEARCH_GUIDANCE =
  'Broad cross-session message search must be scoped to sessionId/taskId or bounded with createdAfter. Use a window of 32 days or less; add createdBefore for historical searches. Example: { "search": "SEO", "createdAfter": "2026-06-01T00:00:00Z", "createdBefore": "2026-06-15T00:00:00Z" }. This prevents full-table scans on large message databases.';

// A full calendar month is commonly expressed as the first day at 00:00
// through the following month's first day at 23:59:59. Time-zone offsets make
// that interval almost 32 exact days for a 31-day month. Keep the query tightly
// bounded while accepting that natural month-shaped request instead of making
// the model retry an otherwise safe read-only search.
const MAX_CROSS_SESSION_SEARCH_WINDOW_MS = 32 * 24 * 60 * 60 * 1000;
...
export function registerMessageTools(server: McpServer, ctx: McpContext): void {
  ...
  server.registerTool(
    'disco_messages_list',
    {
      ...
      inputSchema: z.object({
        ...
        search: mcpOptionalString(
          'search',
          'Keyword search across message content. Space-separated terms are AND\'d, pipe (|) for OR. Example: "OAuth middleware" requires both; "OAuth | JWT" matches either. Cross-session search must include sessionId/taskId or a createdAfter-bounded window of 32 days or less.'
        ),
    ...
    async (args) => {
      ...
      // Always restrict the query to Sessions owned by the current Disco user.
      // Global roles do not grant access to a different user's conversation history.
