...
import type { MCPServer, MCPServerID, Session, SessionID, UUID } from '@disco/core/types';
...
function createSessionData(overrides?: Partial<Session>): Partial<Session> {
  return {
    ...
    working_directory: overrides?.working_directory ?? `/tmp/session-mcp-${generateId()}`,
...
/**
 * Set up a Session and MCP servers.
 */
async function setupTestData(db: any) {
  const sessionRepo = new SessionRepository(db);
  const mcpServerRepo = new MCPServerRepository(db);

  const session = await sessionRepo.create(createSessionData());
...
describe('SessionMCPServerRepository error handling', () => {
  dbTest('should handle multiple sessions with same server', async ({ db }) => {
    ...
    // Create two sessions
    const session1 = await sessionRepo.create(createSessionData());
    const session2 = await sessionRepo.create(createSessionData());
