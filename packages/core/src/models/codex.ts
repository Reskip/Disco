...
const _CODEX_MODEL_REGISTRY = {
  ...
  'gpt-6-astra': {
    name: 'GPT-6 Astra',
    description: 'Frontier Codex model with a 1.05M context window and advanced tool use',
    status: 'current',
    selectable: true,
    // Availability is still being rolled out by account. The current Disco
    // provider account is verified, but other installations may not be yet.
...
export function isUnsupportedDiscoCodexModel(model?: string): boolean {
...
export function formatUnsupportedDiscoCodexModelMessage(model: string): string {
  ...
  const replacement = lifecycle?.replacement ?? DEFAULT_CODEX_MODEL;
  return `Codex model "${model}" is a legacy alias that is not supported for Disco Codex sessions. Remove it from the request, parent session, or user defaults; omit modelConfig to use the default (${DEFAULT_CODEX_MODEL}) or use "${replacement}".`;
}
