...
The PostgreSQL Compose profile runs Disco against PostgreSQL for schema parity,
...
docker compose -f docker-compose.yml -f docker-compose.postgres.yml \
  --profile postgres up --build postgres disco-dev
```

The checked-in `.disco.yml` variants provide the usual branch-specific ports and
...
DISCO_RBAC_ENABLED=true
DISCO_UNIX_USER_MODE=simple|sandbox|delegated
...
docker compose --profile postgres exec postgres \
  psql -U disco_app -d disco -c 'select count(*) from users;'
```
