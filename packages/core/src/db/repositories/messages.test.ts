...
import type { Message, MessageID, SessionID, TaskID, UUID } from '@disco/core/types';
...
// Counter to ensure unique workspace paths across tests.
let testCounter = 0;
...
async function createTestSession(
  db: any,
  overrides?: { session_id?: UUID; created_by?: UUID }
): Promise<SessionID> {
  ...
  const session = await sessionRepo.create({
    ...
    working_directory: `/test/session/${uniqueId}`,
  });
...
describe('MessagesRepository.findAll', () => {
  ...
  dbTest('should restrict by visibleToUserId through direct Session ownership', async ({ db }) => {
    ...
    await users.create({
      user_id: viewerId,
      username: 'messages-visible@example.com',
      name: 'Messages Viewer',
    });
    const visibleSession = await sessions.create({
      title: 'visible',
      created_by: viewerId,
      working_directory: `/tmp/visible-${testCounter++}`,
    });
    const hiddenSession = await sessions.create({
      title: 'hidden',
      created_by: 'test-user' as UUID,
      working_directory: `/tmp/hidden-${testCounter++}`,
    });
