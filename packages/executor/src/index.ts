/**
 * DiscoExecutor - New Feathers/WebSocket-based architecture
 *
...
import { resolveSdkWatchdogConfig } from '@disco/core/config';
...
import { type DiscoClient, createFeathersClient } from './services/feathers-client.js';
...
const DEBUG_EXECUTOR =
  process.env.DISCO_DEBUG_EXECUTOR === '1' || process.env.DEBUG?.includes('executor');
...
export class DiscoExecutor {
  private client: DiscoClient | null = null;
  ...
  private async handleWatchdogDecision(
    evidence: Omit<import('@disco/core/types').SdkHealthFailureInput, 'task_id'>
  ): Promise<void> {
    if (!this.client) return;
