...
import type {
  DiscoConfig,
  DiscoDeploymentMode,
  DiscoExecutorSessionWorkspaceStorage,
  DiscoExecutorUserHomeStorage,
  DiscoHaSupportProfile,
} from './types';
...
export interface ResolvedExecutorStorageSettings {
  userHome: DiscoExecutorUserHomeStorage;
  sessionWorkspace: DiscoExecutorSessionWorkspaceStorage;
}
...
export type ResolvedDeploymentConfig =
  ...
  | {
      ...
      capabilities: {
        ...
        discoManagedInteractivePermissions: true;
        scheduler: true;
        sessionQueue: true;
...
export function resolveEnvironmentHealthMonitorSettings(
  ...
  const settings = {
    scanIntervalMs:
      envInteger(env, 'DISCO_HA_ENV_HEALTH_SCAN_INTERVAL_MS') ??
      raw?.scan_interval_ms ??
      DEFAULT_ENVIRONMENT_HEALTH_SCAN_INTERVAL_MS,
    maxIdleIntervalMs:
      envInteger(env, 'DISCO_HA_ENV_HEALTH_MAX_IDLE_INTERVAL_MS') ??
      raw?.max_idle_interval_ms ??
      DEFAULT_ENVIRONMENT_HEALTH_MAX_IDLE_INTERVAL_MS,
    startupOffsetMaxMs:
      envNonNegativeInteger(env, 'DISCO_HA_ENV_HEALTH_STARTUP_OFFSET_MAX_MS') ??
      raw?.startup_offset_max_ms ??
      DEFAULT_ENVIRONMENT_HEALTH_STARTUP_OFFSET_MAX_MS,
    scanBatchSize:
      envInteger(env, 'DISCO_HA_ENV_HEALTH_SCAN_BATCH_SIZE') ??
      raw?.scan_batch_size ??
      DEFAULT_ENVIRONMENT_HEALTH_SCAN_BATCH_SIZE,
    maxInFlight:
      envInteger(env, 'DISCO_HA_ENV_HEALTH_MAX_IN_FLIGHT') ??
      raw?.max_in_flight ??
      DEFAULT_ENVIRONMENT_HEALTH_MAX_IN_FLIGHT,
    httpTimeoutMs:
      envInteger(env, 'DISCO_HA_ENV_HEALTH_HTTP_TIMEOUT_MS') ??
      raw?.http_timeout_ms ??
      DEFAULT_ENVIRONMENT_HEALTH_HTTP_TIMEOUT_MS,
    claimLeaseMs:
      envInteger(env, 'DISCO_HA_ENV_HEALTH_CLAIM_LEASE_MS') ??
      raw?.claim_lease_ms ??
      DEFAULT_ENVIRONMENT_HEALTH_CLAIM_LEASE_MS,
    shutdownDrainTimeoutMs:
      envInteger(env, 'DISCO_HA_ENV_HEALTH_SHUTDOWN_DRAIN_TIMEOUT_MS') ??
...
function effectiveDatabaseDialect(config: DiscoConfig, env: DeploymentEnv): 'sqlite' | 'postgresql' {
  if (env.DISCO_DB_DIALECT === 'postgresql' || env.DISCO_DB_DIALECT === 'sqlite') {
    return env.DISCO_DB_DIALECT;
  }
...
export function resolveDeploymentConfig(
  ...
  const mode = (env.DISCO_DEPLOYMENT_MODE ??
    config.deployment?.mode ??
    'standalone') as DiscoDeploymentMode;
  ...
  const supportProfile = (env.DISCO_HA_SUPPORT_PROFILE ?? config.deployment?.ha?.support_profile) as
    | DiscoHaSupportProfile
    | undefined;
  ...
  if (!env.DISCO_JWT_SECRET || env.DISCO_JWT_SECRET.length < 32) {
    throw new Error(
      'Config error: HA requires an explicitly supplied DISCO_JWT_SECRET of at least 32 characters'
    );
  }
  if (!env.DISCO_MASTER_SECRET || env.DISCO_MASTER_SECRET.length < 32) {
    throw new Error(
      'Config error: HA requires an explicitly supplied DISCO_MASTER_SECRET of at least 32 characters'
    );
  }
  if (!env.DISCO_ADMIN_PASSWORD) {
    throw new Error(
      'Config error: HA requires an explicitly supplied DISCO_ADMIN_PASSWORD for race-free first-run admin bootstrap'
    );
  }
  const executionTopology = (env.DISCO_HA_EXECUTION_TOPOLOGY ??
  ...
  const sharedFilesystem =
    envBoolean(env, 'DISCO_HA_SHARED_FILESYSTEM') ??
    config.deployment?.ha?.shared_filesystem ??
    false;
  const ingressAffinity =
    envBoolean(env, 'DISCO_HA_INGRESS_AFFINITY') ?? config.deployment?.ha?.ingress_affinity ?? false;
  ...
  if (
    ...
    !executorStorage.session_workspace
  ) {
    throw new Error(
      'Config error: HA requires explicit execution.executor_storage.user_home and session_workspace guarantees'
    );
  }
  ...
  if (executionTopology === 'shared-local') {
    if (executorStorage.session_workspace === 'replica-local') {
      throw new Error(
        'Config error: HA shared-local execution requires a shared or persistent-per-session execution.executor_storage.session_workspace'
      );
    }
  } else if (executorStorage.session_workspace === 'replica-local') {
    throw new Error(
      'Config error: HA external execution requires a shared or persistent-per-session execution.executor_storage.session_workspace'
    );
  }
  ...
  const keyPrefix = validateRedisKeyPrefix(env.DISCO_REDIS_KEY_PREFIX ?? redis?.key_prefix ?? '');
  const connectTimeoutMs = positiveInteger(
    envInteger(env, 'DISCO_REDIS_CONNECT_TIMEOUT_MS') ??
  ...
  const reconnectBaseDelayMs = positiveInteger(
    envInteger(env, 'DISCO_REDIS_RECONNECT_BASE_DELAY_MS') ??
  ...
  const reconnectMaxDelayMs = positiveInteger(
    envInteger(env, 'DISCO_REDIS_RECONNECT_MAX_DELAY_MS') ??
  ...
  return {
    ...
    executorStorage: {
      ...
      sessionWorkspace: executorStorage.session_workspace,
    },
    topology:
