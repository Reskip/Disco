...
import { feathers, feathersExpress } from '@disco/core/feathers';
...
describe('OpenAPI routes', () => {
  ...
  it('serves the machine-readable schema without a bundled documentation UI', async () => {
    ...
    expect(document).toMatchObject({
      openapi: '3.0.3',
      info: { title: 'Disco API', version: '1.2.3' },
    });
  ...
  it.each(['/docs', '/docs/'])('redirects %s to the maintained reference', async (path) => {
    ...
    expect(response.headers.get('location')).toBe('https://disco.live/api-reference');
  });
});
