...
export function createPublishManifest(source) {
  ...
  const clientVersion = manifest.dependencies?.['@disco-live/client'];
  if (typeof clientVersion !== 'string') {
    throw new Error('@disco-live/client must be a release dependency');
  }
  if (clientVersion.startsWith('workspace:')) {
    manifest.dependencies['@disco-live/client'] = manifest.version;
  } else if (clientVersion !== manifest.version) {
    throw new Error(
      `@disco-live/client ${clientVersion} does not match disco-live ${manifest.version}`
    );
  }
  if (manifest.scripts?.preinstall || manifest.scripts?.install || manifest.scripts?.postinstall) {
    throw new Error(
      'disco-live release packages must not require npm lifecycle installation scripts'
    );
  }
...
async function assertMaterializedInternalPackage(internalRoot, bundledPackage) {
  const expectedName = `@disco/${bundledPackage.name}`;
  ...
  if (!isContainedPath(canonicalRoot, canonicalDirectory)) {
    throw new Error(`${expectedName} resolves outside the disco-live package`);
  }
...
export async function packRelease({ packageRoot = scriptRoot, destination, internalPackageRoot }) {
  ...
  const internalRoot = resolve(internalPackageRoot ?? join(root, 'node_modules', '@disco'));
  ...
  const expectedNames = new Set(
    BUNDLED_INTERNAL_PACKAGES.map((bundledPackage) => `@disco/${bundledPackage.name}`)
  );
  if (
  ...
  for (const bundledPackage of BUNDLED_INTERNAL_PACKAGES) {
    ...
    const packageName = `@disco/${bundledPackage.name}`;
  ...
  const stageParent = await mkdtemp(join(tmpdir(), 'disco-live-pack-'));
  ...
  try {
    ...
    for (const { bundledPackage, source } of internalPackages) {
      const target = join(stage, 'node_modules', '@disco', bundledPackage.distDirectory);
