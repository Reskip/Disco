...
export async function repairManagedIntegrationPermissions(
  ...
  for (const directory of [root, join(root, discoVersion)]) {
    await repairExistingSharedManagedDirectory(directory);
  }
  const installDirectory = getAgenticToolInstallDir(tool, discoVersion);
  try {
...
export async function assertManagedIntegrationPermissions(
  ...
  await assertManagedPackageTreeShared(getAgenticToolInstallDir(tool, discoVersion));
}
...
export async function acquireAgenticToolInstallLock(): Promise<() => Promise<void>> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ELOCKED') {
      throw new Error(
        'Another `disco install` is already updating agentic tools. Try again when it finishes.'
      );
    }
    throw error;
  }
...
export async function readManagedIntegrationManifest(
  ...
  try {
    const value = JSON.parse(
      await readFile(
        join(getAgenticToolInstallDir(tool, discoVersion), 'disco-integration.json'),
        'utf8'
      )
    ) as ManagedIntegrationManifest;
    if (
      value.discoVersion !== discoVersion ||
      value.packageName !== AGENTIC_TOOL_INTEGRATIONS[tool].packageName ||
      value.packageVersion !== discoVersion
    ) {
      return undefined;
    }
    return value;
...
/**
 * Sort key for an Disco version directory name. Non-numeric segments (e.g. the
...
/**
 * Disco versions that have a managed tool directory on disk, oldest first.
 * Staging/backup directories are dot-prefixed and therefore skipped.
 */
export async function listManagedDiscoVersions(): Promise<string[]> {
  try {
...
/**
 * Tools with a verified manifest under a given Disco version. Directories whose
...
export async function listInstalledAgenticTools(
  ...
  let entries: string[];
  try {
    entries = (await readdir(join(getAgenticToolsRoot(), discoVersion), { withFileTypes: true }))
  ...
  const verified = await Promise.all(
    tools.map(async (tool) =>
      (await readManagedIntegrationManifest(tool, discoVersion)) ? tool : undefined
    )
  );
...
export async function listManagedToolDirectories(
  ...
  try {
    return (await readdir(join(getAgenticToolsRoot(), discoVersion), { withFileTypes: true }))
...
export async function removeManagedDiscoVersion(version: string): Promise<void> {
...
async function runNpmInstall(prefix: string, packageSpec: string): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    // Recent Node releases do not execute Windows `.cmd` shims directly and
    // return EINVAL. Let the platform shell resolve npm.cmd on Windows; all
    // arguments here are Disco-owned package metadata and generated paths.
...
export async function installManagedIntegration(
  ...
  const destination = getAgenticToolInstallDir(tool, discoVersion);
  const parent = join(getAgenticToolsRoot(), discoVersion);
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify(createManagedAgenticToolInstallManifest(tool, discoVersion), null, 2)}\n`,
      'utf8'
    );
    await runNpmInstall(staging, `${definition.packageName}@${discoVersion}`);
    ...
    if (
      ...
      installedPackage.version !== discoVersion
    ) {
      throw new Error(
        `Installed ${installedPackage.name ?? 'unknown package'}@${installedPackage.version ?? 'unknown'}; expected ${definition.packageName}@${discoVersion}`
      );
    }
    await assertManagedAgenticToolInstallReady(tool, discoVersion, staging);
    const manifest: ManagedIntegrationManifest = {
      discoVersion,
      packageName: definition.packageName,
      packageVersion: discoVersion,
      installedAt: new Date().toISOString(),
    };
    await writeFile(
      join(staging, 'disco-integration.json'),
