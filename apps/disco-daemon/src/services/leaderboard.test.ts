...
import { TaskStatus, type TokenUsageSample } from '@disco/core/types';
...
async function seedUser(db: Database, id: string, name: string, email: string): Promise<void> {
  await insert(db, users)
    .values({
      ...
      emoji: 'emoji',
      role: 'member',
      data: { avatar_url: `https://cdn.example.test/${id}.png` },
    })
    .run();
}
...
async function seedSession(
  db: Database,
  opts: { sessionId: string; tool: 'claude-code' | 'codex' | 'gemini' }
): Promise<void> {
  await insert(db, sessions)
    .values({
      ...
      working_directory: `/tmp/${opts.sessionId}`,
...
async function seedTask(
  ...
  const data = {
    ...
    normalized_sdk_response: {
      ...
      tokenUsageSamples: opts.tokenUsageSamples,
...
async function seedCanonicalDataset(db: Database): Promise<{
  ...
  await seedSession(db, { sessionId: claudeSessionId, tool: 'claude-code' });
  await seedSession(db, { sessionId: codexSessionId, tool: 'codex' });
  ...
  return { aliceId, bobId, claudeSessionId, codexSessionId };
}

// ---------------------------------------------------------------------------
// User-first public contract
// ---------------------------------------------------------------------------

describe('LeaderboardService user-first contract', () => {
  dbTest('defaults to one row per user without repository carrier fields', async ({ db }) => {
    ...
    for (const row of result.data) {
      ...
      expect(row).not.toHaveProperty('repoId');
  ...
  dbTest('totalTokens still sums normalized totalTokens', async ({ db }) => {
    ...
    expect(alice?.userAvatarUrl).toBe('https://cdn.example.test/user-alice.png');
  });
});

// ---------------------------------------------------------------------------
// Permanent accounting lifecycle
// ---------------------------------------------------------------------------

describe('LeaderboardService permanent accounting lifecycle', () => {
  dbTest('deleting a Session removes its Tasks but preserves all usage totals', async ({ db }) => {
    const { aliceId, claudeSessionId } = await seedCanonicalDataset(db);
    const service = new LeaderboardService(db);

    const before = await service.find({ query: { groupBy: 'user' } });
    const beforeAlice = before.data.find((row) => row.userId === aliceId);
    expect(beforeAlice).toMatchObject({
      totalInputTokens: 7_000,
    ...
    await deleteFrom(db, sessions).where(eq(sessions.session_id, claudeSessionId)).run();

    expect(
      await select(db).from(tasks).where(eq(tasks.session_id, claudeSessionId)).all()
    ).toHaveLength(0);
    expect(
      await select(db)
        .from(taskUsageLedger)
        .where(eq(taskUsageLedger.session_id, claudeSessionId))
        .all()
    ).toHaveLength(3);

    const after = await service.find({ query: { groupBy: 'user' } });
    expect(after.data.find((row) => row.userId === aliceId)).toEqual(beforeAlice);
  });

  dbTest(
    'repeated Task updates overwrite one ledger row instead of double-counting',
    async ({ db }) => {
      const { claudeSessionId } = await seedCanonicalDataset(db);
      const task = await select(db)
        .from(tasks)
        .where(eq(tasks.session_id, claudeSessionId))
        .limit(1)
        .one();
      ...
      const data = {
        ...task!.data,
        normalized_sdk_response: {
          ...task!.data.normalized_sdk_response!,
          tokenUsage: {
            inputTokens: 9_000,
            outputTokens: 1_000,
            totalTokens: 10_000,
      ...
      await update(db, tasks).set({ data }).where(eq(tasks.task_id, task!.task_id)).run();
      await update(db, tasks).set({ data }).where(eq(tasks.task_id, task!.task_id)).run();

      const ledgerRows = await select(db)
        .from(taskUsageLedger)
...
describe('LeaderboardService bucketing', () => {
  dbTest('bucket: "30s" merges tasks inside each half-minute window', async ({ db }) => {
    const { aliceId, claudeSessionId } = await seedCanonicalDataset(db);
    await seedTask(db, {
      sessionId: claudeSessionId,
      createdBy: aliceId,
      createdAt: new Date('2026-04-15T12:00:20.000Z'),
      model: 'claude-sonnet-4-6',
      inputTokens: 200,
      outputTokens: 100,
    });
    await seedTask(db, {
      sessionId: claudeSessionId,
      createdBy: aliceId,
      createdAt: new Date('2026-04-15T12:00:35.000Z'),
      model: 'claude-sonnet-4-6',
      inputTokens: 300,
      outputTokens: 150,
    });
    ...
    const result = await service.find({ query: { bucket: '30s', groupBy: '' } });

    expect(result.data).toHaveLength(5);
    expect(result.data[0].bucket).toBe('2026-04-15T12:00:00.000Z');
    expect(result.data[0].taskCount).toBe(2);
    expect(result.data[0].totalTokens).toBe(1_800);
    expect(result.data[1].bucket).toBe('2026-04-15T12:00:30.000Z');
    expect(result.data[1].taskCount).toBe(1);
    expect(result.data[1].totalTokens).toBe(450);
  });

  dbTest(
    'timeSource:samples attributes cumulative deltas to their observed 30s buckets',
    async ({ db }) => {
      const { aliceId, claudeSessionId } = await seedCanonicalDataset(db);
      ...
      await seedTask(db, {
        ...
        createdBy: aliceId,
        createdAt,
        completedAt: new Date('2026-04-25T12:01:45.000Z'),
        model: 'gpt-5.6-sol',
        inputTokens: 500,
        outputTokens: 60,
        tokenUsageSamples: [
          {
            observedAt: '2026-04-25T12:00:10.000Z',
            inputTokens: 100,
            outputTokens: 10,
            totalTokens: 110,
          },
          {
            observedAt: '2026-04-25T12:00:40.000Z',
            inputTokens: 250,
            outputTokens: 30,
            totalTokens: 280,
          },
          {
            observedAt: '2026-04-25T12:01:10.000Z',
            inputTokens: 400,
            outputTokens: 50,
            totalTokens: 450,
          },
      ...
      const result = await service.find({
        query: {
          ...
          endDate: '2026-04-25T12:02:00.000Z',
          sortBy: 'tokens',
          sortOrder: 'asc',
        },
      });

      expect(result.data.map((row) => [row.bucket, row.totalTokens])).toEqual([
        ['2026-04-25T12:00:00.000Z', 110],
        ['2026-04-25T12:00:30.000Z', 170],
        ['2026-04-25T12:01:00.000Z', 170],
        ['2026-04-25T12:01:30.000Z', 110],
      ]);
      expect(result.data.reduce((sum, row) => sum + row.totalTokens, 0)).toBe(560);
    }
  );
