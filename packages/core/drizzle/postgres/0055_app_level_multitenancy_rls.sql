...
-- when `disco.tenant_id` has not been set. Cloud deployments should set
-- `disco.tenant_id` for each tenant-scoped transaction; app-layer required
...
CREATE POLICY "tenant_isolation_sessions" ON "sessions"
  USING ("tenant_id" = COALESCE(NULLIF(current_setting('disco.tenant_id', true), ''), 'default'))
  WITH CHECK ("tenant_id" = COALESCE(NULLIF(current_setting('disco.tenant_id', true), ''), 'default'));
--> statement-breakpoint
