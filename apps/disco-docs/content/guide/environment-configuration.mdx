...
Disco's managed environments let a repo ship a **library of named variants** (`lean`, `postgres`, `full`, …) that every branch can pick from. Variants are Handlebars-templated and get **rendered into a per-branch snapshot** that the Start/Stop/Nuke/Logs buttons execute against.
...
```text
  ┌────────────────────────────────────────────────────────┐
  │ 1. .disco.yml — repo-shared, committed to git           │
...
- **Layer 1** is authored by repo maintainers and lives in `.disco.yml`. It's portable: clone the repo, `Import .disco.yml`, done.
- **Layer 2** is where each Disco deployment pins its own values (host IP, internal registry, AWS profile) without touching the shared file. Stored in the DB, never exported.
...
_Picking a variant. `sqlite` is the default for fastest boot; `full` mirrors production with PostgreSQL, RBAC, and the configured execution boundary._
...
Every field in a variant is a Handlebars template. The render context is built in [`packages/core/src/templates/handlebars-helpers.ts`](https://github.com/preset-io/disco/blob/main/packages/core/src/templates/handlebars-helpers.ts).
...
| `{{branch.path}}` | Branch | `/home/you/disco/feat-new-filter` | Absolute path on disk. |
...
| `{{host.ip_address}}` | Auto-detected | `10.0.1.42` | Fallback order: `template_overrides.host.ip_address` → `daemon.host_ip_address` in `~/.disco/config.yaml` → autodetect. |
...
## The `.disco.yml` file {/* #variants */}

<a id="variants" />

`.disco.yml` lives at the **branch root** (`$BRANCH_PATH/.disco.yml`) and is the shared, committed description of a repo's environment variants. It's a regular repo file: you edit it on a branch, open a PR, and it rolls out when merged.
...
environment:
  ...
  variants:
    lean:
      ...
      start:  "docker compose -f docker-compose-light.yml -p disco-{{branch.name}} up -d"
      stop:   "docker compose -f docker-compose-light.yml -p disco-{{branch.name}} down"
      nuke:   "docker compose -f docker-compose-light.yml -p disco-{{branch.name}} down -v"
      logs:   "docker compose -f docker-compose-light.yml -p disco-{{branch.name}} logs --tail=100"
    ...
    postgres:
      ...
      start:  "docker compose -p disco-{{branch.name}} up -d --build"
    ...
    full:
      ...
      start:  "COMPOSE_PROFILES=full docker compose -p disco-{{branch.name}} up -d --build"
      stop:   "docker compose -p disco-{{branch.name}} down"
      nuke:   "docker compose -p disco-{{branch.name}} down -v"
      logs:   "docker compose -p disco-{{branch.name}} logs --tail=100"
```
...
| Action | Effect |
|---|---|
| **Import `.disco.yml`** | Parses `$BRANCH_PATH/.disco.yml` and **replaces** `environment.variants` + `environment.default` in the DB. `template_overrides` and branch snapshots are untouched. |
| **Export `.disco.yml`** | Writes the current `environment.variants` + `environment.default` to `$BRANCH_PATH/.disco.yml`. `template_overrides` is **stripped** before writing. |
...
1. Iterate on `.disco.yml` on a branch.
...
`template_overrides` is a per-repo, per-deployment block that lives **in the DB, not in `.disco.yml`**. It's where each Disco instance pins the concrete values that the shared variants reference (IPs, registries, profile names).

### Why it exists

Superset's upstream `.disco.yml` shouldn't hard-code `10.0.1.42` as the host IP. That's Preset's infra, not Apache's. Instead, Superset's variants reference `{{host.ip_address}}` and Preset sets it once, per-repo, in `template_overrides`. The shared file stays clean; the deployment-specific values stay in the deployment.
...
The import/export parser **refuses** any `template_overrides:` key found in `.disco.yml` and the exporter **always strips** it. This is a defense-in-depth guarantee that values like `host.ip_address: 10.0.1.42` cannot be accidentally committed upstream to a public repo.
...
2. Click **Render**.
3. Disco re-evaluates `variant + template_overrides` and overwrites the branch snapshot.
...
## Migration from pre-variants Disco
...
| `.disco.yml` v1 (flat `environment: { start, stop, … }`) | Still parses. Loaded as `variants.default`. Export writes v2 going forward. |
| `daemon.host_ip_address` in `~/.disco/config.yaml` | Still works as the fallback for `{{host.ip_address}}`. For new setups, prefer per-repo `template_overrides.host.ip_address`. |
...
Disco instances choose how lifecycle fields are handled. The default is
`hybrid`:
...
must render to explicit `http://` or `https://` URLs. Disco sends a GET request
to the URL.
...
environment:
  ...
  variants:
    remote:
      start: "https://orchestrator.example.com/disco/start?branch={{branch.name}}"
      stop:  "https://orchestrator.example.com/disco/stop?branch={{branch.name}}"
      logs:  "https://orchestrator.example.com/disco/logs?branch={{branch.name}}"
...
In daemon HA mode, lifecycle actions remain webhook-only; Disco does not assign
...
explicitly unobservable (`unknown`); Disco never treats another daemon's local
...
- `.disco.yml` parser source: `packages/core/src/config/disco-yml.ts`
