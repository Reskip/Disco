import type { AgenticToolConfigField, AgenticToolName } from '@disco-live/client';
import {
  CheckCircleOutlined,
  CloseCircleOutlined,
  DeleteOutlined,
  ...
  codex: [
    {
      ...
      helper: (
        <Text type="secondary" style={{ fontSize: 12 }}>
          Alternative: on the machine Disco runs sessions on, run{' '}
          <Text code style={{ fontSize: 12 }}>
            codex login --device-auth
          </Text>{' '}
          to use Codex CLI account auth without storing an OpenAI key in Disco.{' '}
          <Link href="https://disco.live/guide/extended-install#authentication" target="_blank">
            Learn more →
          </Link>
        </Text>
      ),
