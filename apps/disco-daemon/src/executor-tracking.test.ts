...
describe('Windows executor containment', () => {
  it('accepts the owned child exit callback after cooperative quiescence', async () => {
    const owner = {};
    const inspectProcessForTest = vi
      .fn<(pid: number, signal?: 0 | NodeJS.Signals) => 'present' | 'absent' | 'unverified'>()
      .mockReturnValue('present');
    trackExecutorProcess(
      { sessionId: 'session-windows', taskId: 'task-windows', pid: 4242 },
      owner
    );
    try {
      setTimeout(() => markExecutorProcessExited('session-windows', 4242, owner), 5);
      ...
      await expect(
        containExecutorProcess(
          ...
          'task-windows',
          {
            ...
            preSignalGraceMs: 100,
            pollMs: 1,
            inspectProcessForTest,
          },
          owner
        )
      ).resolves.toEqual({ status: 'verified_absent' });
      expect(inspectProcessForTest).not.toHaveBeenCalledWith(4242, 'SIGTERM');
    } finally {
      untrackExecutorProcess('session-windows', 'task-windows', owner);
    }
  });

  it('signals a still-running owned process and verifies its disappearance', async () => {
    const owner = {};
    let terminated = false;
    const inspectProcessForTest = vi.fn((_pid: number, signal: 0 | NodeJS.Signals = 0) => {
      if (signal === 'SIGTERM') {
        terminated = true;
        return 'present' as const;
      }
      return terminated ? ('absent' as const) : ('present' as const);
    });
    trackExecutorProcess(
      { sessionId: 'session-windows-signal', taskId: 'task-windows-signal', pid: 4343 },
      owner
    );
    try {
      await expect(
        containExecutorProcess(
          'session-windows-signal',
          'task-windows-signal',
          {
            platformForTest: 'win32',
            termGraceMs: 20,
      ...
      expect(inspectProcessForTest).toHaveBeenCalledWith(4343, 'SIGTERM');
    } finally {
      untrackExecutorProcess('session-windows-signal', 'task-windows-signal', owner);
    }
  });
});
...
describe.runIf(process.platform === 'linux' || process.platform === 'darwin')(
  ...
  () => {
    ...
    it('reconciles a durable containment fence after in-memory tracking is lost', async () => {
      const root = await mkdtemp(join(tmpdir(), 'disco-containment-fence-'));
    ...
    it('fails closed across restart while a writer has only a durable start intent', async () => {
      const root = await mkdtemp(join(tmpdir(), 'disco-containment-intent-'));
    ...
    it('does not publish a first intent before its namespace entry is durable', async () => {
      const root = await mkdtemp(join(tmpdir(), 'disco-containment-order-'));
