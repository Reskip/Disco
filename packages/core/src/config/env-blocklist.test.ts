...
describe('env-blocklist', () => {
  describe('BLOCKED_ENV_VARS set', () => {
    ...
    it('should contain DISCO_MASTER_SECRET', () => {
      expect(BLOCKED_ENV_VARS.has('DISCO_MASTER_SECRET')).toBe(true);
    });
  ...
  describe('isEnvVarAllowed', () => {
    describe('blocked variables', () => {
      ...
      it('should reject DISCO_MASTER_SECRET', () => {
        expect(isEnvVarAllowed('DISCO_MASTER_SECRET')).toBe(false);
      });
    });
  ...
  describe('getEnvVarBlockReason', () => {
    ...
    it('should return reason for DISCO_MASTER_SECRET', () => {
      const reason = getEnvVarBlockReason('DISCO_MASTER_SECRET');
      expect(reason).not.toBeNull();
  ...
  describe('security-critical variables', () => {
    ...
    it('should protect Disco security infrastructure', () => {
      expect(isEnvVarAllowed('DISCO_MASTER_SECRET')).toBe(false);
      const reason = getEnvVarBlockReason('DISCO_MASTER_SECRET');
      expect(reason).toContain('encryption');
    });
  ...
  describe('process-hijacking blocks (pattern + extended set)', () => {
    ...
    it('blocks PYTHON* except the PYTHON_DISCO_* carve-out', () => {
      expect(isEnvVarAllowed('PYTHONSTARTUP')).toBe(false);
      expect(isEnvVarAllowed('PYTHONFAULTHANDLER')).toBe(false);
      expect(isEnvVarAllowed('PYTHON_DISCO_TEST')).toBe(true);
    });
