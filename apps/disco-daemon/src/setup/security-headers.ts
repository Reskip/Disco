 ...
 *
 * Equivalent to a minimal `helmet()` configuration tailored for the Disco
 * daemon. Sets:
 ...
 *
 * The CSP is resolved from `~/.disco/config.yaml` (`security.csp.*`) via
 * `resolveSecurity()` in @disco/core, so operators can append per-directive
 * sources (`extras`) or replace them wholesale (`override`) without touching
 * code. Built-in defaults are chosen so every bundled Disco feature works out
...
import type { ResolvedCsp } from '@disco/core/config';
...
export function securityHeaders(opts: SecurityHeadersOptions = {}): RequestHandler {
  ...
  // from the `report-to` directive — either our default `disco-csp` or the
