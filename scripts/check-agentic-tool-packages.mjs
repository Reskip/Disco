...
const base = JSON.parse(await readFile(join(root, 'packages/disco-live/package.json'), 'utf8'));
const cli = JSON.parse(await readFile(join(root, 'apps/disco-cli/package.json'), 'utf8'));
...
if (cli.version !== base.version) {
  failures.push(`@disco/cli: ${cli.version} != disco-live ${base.version}`);
}
const EXACT_SEMVER =
  /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?(?:\+[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?$/;
for (const dependency of forbiddenBaseDependencies) {
  if (base.dependencies?.[dependency]) failures.push(`disco-live must not depend on ${dependency}`);
}
...
// devDependencies for TypeScript and tests; those are absent from disco-live.
const runtimeManifestAllowlist = new Set(
  integrations.map((id) => `packages/disco-${id}/package.json`)
);
  ...
  for (const entry of await readdir(join(root, parent), { withFileTypes: true })) {
    ...
    for (const dependency of forbiddenBaseDependencies) {
      ...
      if (pkg.optionalDependencies?.[dependency]) {
        failures.push(
          `${relative}: ${dependency} must not bypass disco install via optionalDependencies`
        );
      }
    }
  }
...
for (const id of integrations) {
  const directory = join(root, `packages/disco-${id}`);
  const pkg = JSON.parse(await readFile(join(directory, 'package.json'), 'utf8'));
  if (pkg.name !== `@disco-live/${id}`) failures.push(`${id}: unexpected package name ${pkg.name}`);
  if (pkg.version !== base.version)
    failures.push(`${pkg.name}: ${pkg.version} != disco-live ${base.version}`);
  ...
  if (!source.includes(`DISCO_INTEGRATION_VERSION = '${base.version}'`)) {
    failures.push(`${pkg.name}: source integration version does not match ${base.version}`);
  }
}

const opencodeWrapper = JSON.parse(
  await readFile(join(root, 'packages/disco-opencode/package.json'), 'utf8')
);
...
//
// The six integration wrappers shipped briefly as `@disco/<tool>`. `@disco` is the
// *private* workspace scope (@disco/core, @disco/git, @disco/agentic-tools, ...) and
...
// are `disco-live` or `@disco-live/*`; anything `@disco/*` is internal and must say
// so with `private: true`.
const PUBLISHABLE_NAME = /^(disco-live|@disco-live\/[a-z0-9-]+)$/;
for (const parent of ['packages', 'apps']) {
  let entries;
  try {
    ...
    if (!PUBLISHABLE_NAME.test(pkg.name ?? '')) {
      failures.push(
        `${parent}/${entry.name}: "${pkg.name}" is publishable but not under a namespace we own ` +
          `(expected disco-live or @disco-live/*) — add "private": true if it is internal`
      );
    }
  }
}
