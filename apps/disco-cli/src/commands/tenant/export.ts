/**
 * `disco tenant export` — write a deterministic, versioned archive of one
 * tenant's database rows and configured filesystem tree to a directory the
 * caller supplies. Disco is agnostic of how that directory is transported or
...
import { createDatabase, getDatabaseUrl } from '@disco/core/db';
import {
  assertValidTenantId,
  exportTenant,
  InvalidTenantIdError,
  TenantPortabilityUnsupportedError,
} from '@disco/core/tenant-portability';
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof TenantPortabilityUnsupportedError) {
        this.logToStderr(
          chalk.dim(
            '  Point the command at a PostgreSQL database (DISCO_DB_DIALECT=postgresql, DATABASE_URL=…).'
          )
        );
      }
      await flushStderr();
