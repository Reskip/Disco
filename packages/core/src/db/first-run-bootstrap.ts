 ...
 *      persistence (writing `~/.disco/admin-credentials`, printing to stderr,
...
import { sessions, tasks, type UserRow, users } from './schema';
...
/**
 * Default username assigned to the bootstrap admin. Exported so the daemon-side
 * orchestrator can include it in the persisted-credentials file.
 */
export const BOOTSTRAP_ADMIN_USERNAME = DEFAULT_ADMIN_USERNAME;
...
export async function reattributeLegacyAnonymousRows(
  ...
  const tablesWithCreatedBy = [sessions, tasks];
  let total = 0;
