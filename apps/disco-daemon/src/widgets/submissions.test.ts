...
import type { Message, MessageID, Session, UserID } from '@disco/core/types';
...
function makeApp(
  records: { message: Message; session: Session },
  options: {
...
describe('canResolveWidget', () => {
  it('allows the session owner', () => {
    ...
    expect(canResolveWidget({ user_id: 'alice' as UserID }, session)).toBe(true);
  });

  it('rejects every non-owner', () => {
    const session = { created_by: 'alice' as UserID };
    expect(canResolveWidget({ user_id: 'bob' as UserID }, session)).toBe(false);
  });
});
...
describe('resolveWidget', () => {
  ...
  it('does not let legacy Branch permission tiers authorize a non-owner', async () => {
    registerTestWidget();
    const fixtures = makeFixtures();
    const { app, resolutionStore } = makeApp(fixtures);
    ...
    await expect(
      resolveWidget(
        ...
        { kind: 'submit', body: { value: 'secret-key', scope: 'global' } },
        { user_id: 'someone-else' as UserID },
        { app: app as never, resolutionStore, runInTenantDatabaseScope }
      )
    ).rejects.toThrow(/Session owner/);
  });
