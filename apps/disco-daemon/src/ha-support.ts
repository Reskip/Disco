import type { ResolvedDeploymentConfig } from '@disco/core/config';
import { Unavailable } from '@disco/core/feathers';
import type { HookContext, PermissionMode, Session } from '@disco/core/types';
import { mapPermissionMode } from '@disco/core/utils/permission-mode-mapper';
...
export const HA_UNSUPPORTED_FEATURES = {
  providerNativeInteractivePermissions:
    'provider-native interactive permission modes without Disco realtime decision routing',
...
/** Identify modes that cannot create either an Disco or provider-native prompt. */
...
export function assertHaTaskPermissionSupported(
  ...
  // executor's Disco PermissionService. The task-scoped Feathers control room
  ...
  // surfaces that Disco does not answer, so retain their noninteractive gate.
  const hasDiscoRealtimeDecisionHandler =
  ...
  if (
    ...
    !hasDiscoRealtimeDecisionHandler &&
