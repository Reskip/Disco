---
title: Using the API
description: Authentication, endpoint conventions, response formats, and examples for the Disco REST and WebSocket APIs.
---

# Using the API

The interactive [API Reference](/api-reference) is generated from the OpenAPI document exposed by the Disco daemon.

> **Need the exact schema for your installed version?** Start Disco and fetch
...
Disco uses JWT-based authentication:
...
You'll be prompted for your email and password. Your token is automatically saved to `~/.disco/cli-token` and is valid for 7 days.
...
local daemon from `disco.live` unless you deliberately add that origin to the
...
**Authentication is always required.** On first daemon start, an admin user is auto-created with a random password. The credentials are written to `~/.disco/admin-credentials` (mode 0600) and printed once to the daemon log. Use those to log in and obtain a JWT.
...
- **TypeScript types**: [`packages/core/src/types`](https://github.com/preset-io/disco/tree/main/packages/core/src/types)
- **Service implementations**: [`apps/disco-daemon/src/services`](https://github.com/preset-io/disco/tree/main/apps/disco-daemon/src/services)
