...
afterEach(() => {
  ...
  delete (window as Window & { __DISCO_INITIAL_LOAD_TIMINGS__?: InitialLoadDebugTimings })
    .__DISCO_INITIAL_LOAD_TIMINGS__;
  vi.restoreAllMocks();
});
...
describe('initial load debug flag', () => {
  it('persists when debugLoad=1 is present', () => {
    ...
    expect(window.localStorage.getItem('disco.debug.initialLoad')).toBe('1');
  });

  it('removes the persisted flag when debugLoad=0 is present', () => {
    window.localStorage.setItem('disco.debug.initialLoad', '1');
    window.history.replaceState({}, '', '/?debugLoad=0');

    expect(syncInitialLoadDebugFlagFromUrl()).toBe(false);
    expect(window.localStorage.getItem('disco.debug.initialLoad')).toBeNull();
  });
...
describe('initial load debug timer', () => {
  it('captures fetch status, counts, stage transitions, and exposes the latest payload', async () => {
    ...
    expect(
      (window as Window & { __DISCO_INITIAL_LOAD_TIMINGS__?: InitialLoadDebugTimings })
        .__DISCO_INITIAL_LOAD_TIMINGS__
    ).toBe(timings);
  });
