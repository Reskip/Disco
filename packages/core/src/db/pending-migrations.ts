 ...
 *
 * Both the CLI (`disco daemon start` pre-flight) and the daemon
 * (`apps/disco-daemon/src/setup/database.ts::checkAndReportMigrations`) need
 * to tell the user "your database has pending migrations, run `disco db
 ...
 * a clear pointer to `disco db migrate` and (for SQLite) a backup command.
 *
