 ...
 * into ~/.disco/logs/daemon.log — so the user sees no error at their terminal
 ...
 * (with a pointer to `disco db migrate`) and the CLI exits with a non-zero
 * status code.
 *
 * The `PendingMigrationsInfo` type and `formatPendingMigrationsMessage`
 * formatter live in `@disco/core/db` and are re-exported here for
 ...
 *   (env vars > ~/.disco/config.yaml > default). Pass an explicit URL when the
