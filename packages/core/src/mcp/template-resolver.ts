 ...
 *
 * The list of user-defined keys is communicated via DISCO_USER_ENV_KEYS env var
...
import { DISCO_USER_ENV_KEYS_VAR } from '../config/env-resolver';
 ...
 * The list of user-defined keys is read from DISCO_USER_ENV_KEYS env var
 * (set by createUserProcessEnvironment when daemon spawns executor).
 *
 * This prevents global MCP configs from accessing system secrets like
 * DISCO_MASTER_SECRET, database credentials, or other daemon internals.
 *
...
export function buildMCPTemplateContextFromEnv(
  ...
  const userEnvKeysStr = env[DISCO_USER_ENV_KEYS_VAR];
