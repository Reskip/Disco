...
Disco can accept a generic external launch handoff when another trusted app has already authenticated the user. The browser carries only an opaque, short-lived `launch_code`; the daemon exchanges that code over a server-to-server backchannel, verifies the returned assertion, maps a local user, and issues the same runtime access and refresh tokens used by normal login.
...
parameter containing the current Disco path, so launch providers can preserve
...
external_launch:
  ...
  audience: disco-runtime:my-instance
  ...
  service_credential_env: DISCO_EXTERNAL_LAUNCH_SERVICE_TOKEN
...
external_launch:
  ...
  audience: disco-runtime:dev
  dev_shared_secret_env: DISCO_EXTERNAL_LAUNCH_SHARED_SECRET
```

The following environment variables can override common fields:

- `DISCO_EXTERNAL_LAUNCH_ENABLED`
- `DISCO_EXTERNAL_LAUNCH_EXCHANGE_URL`
- `DISCO_EXTERNAL_LAUNCH_ISSUER`
- `DISCO_EXTERNAL_LAUNCH_AUDIENCE`
- `DISCO_EXTERNAL_LAUNCH_INSTANCE_ID`
- `DISCO_EXTERNAL_LAUNCH_SERVICE_TOKEN`
- `DISCO_EXTERNAL_LAUNCH_SHARED_SECRET`
...
fixture `apps/disco-daemon/src/auth/__fixtures__/launch-exchange-request.json`: it
...
- If there is no host-local session, the unauthenticated screen sends the
  ...
  endpoint) with two query params: `return_to` (the current **relative** Disco
  ...
  which then runs the normal exchange. Disco only ever sends the browser to the
...
- Launch codes, returned assertions, the exchange bearer credential, cookies and
  ...
  (`apps/disco-daemon/src/auth/launch-redaction.ts`). Exchange failures log only a
