/**
 * Marketplace connect: install one catalog entry and attach it to an existing
 * session the caller can use.
 *
 * The request names a catalog entry and the target session, and
 ...
 * restriction, session ownership, and execution identity all resolve exactly once,
...
import { BadRequest, Forbidden, NotFound } from '@disco/core/feathers';
import { probeRemoteAuthType } from '@disco/core/mcp-catalog';
import type {
  AuthenticatedParams,
...
import { catalogDisplayName, catalogServerSlug } from '@disco/core/types';
 ...
 * dangerous-looking ones. Disco redacts every custom header value on read and
...
export function createMCPCatalogConnectService(
  ...
  return {
    async create(data, params) {
      ...
      try {
        const session = (await service('sessions').get(data.session_id, params)) as Session;
        if (!userId || session.created_by !== userId) {
          throw new Forbidden('The target session must belong to the current user');
        }
