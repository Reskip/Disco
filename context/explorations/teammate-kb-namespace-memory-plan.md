...
**Date:** 2026-06-08
**Scope:** Concise implementation plan for using Disco Knowledge Base namespaces as the backend for Disco Teammates' memory, scratchpad, docs, skills, and file-like storage. This is a planning doc; do not implement from here without a follow-up task.
...
- Existing KB MCP tools are explicit/global (`disco_kb_put`, `disco_kb_edit`, `disco_kb_search`, etc.) and already receive `McpContext` with `userId`, optional `sessionId`, and Feathers service params.
...
- `others_can`: `write` so the default matches Disco's open collaboration model: agents can see and write to each other's teammate namespaces unless an owner narrows access.
...
- `disco_teammate_context` (read-only)
- `disco_teammate_memory_append` (mutating)
- Optional alias later: `disco_teammate_memory_file` if product language lands on "file".

### `disco_teammate_memory_append`
...
<!-- disco-memory-entry id="<uuid-or-idempotency-hash>" hash="sha256:..." -->

- [2026-06-08T15:20:00Z] note: Memory text here. #tag
  - source: disco://session/<id>
  <!-- /disco-memory-entry -->
```
...
- Follow-up: teach `knowledgeUnitsForMarkdown` to split on `disco-memory-entry` comments before heading/auto-split fallback, and reuse unit `content_md5`/embedding hashes so unchanged entries do not re-embed.
...
The teammate is not a separate KB principal in V1. It runs as a real Disco user/session, then the teammate branch grant list narrows where teammate-specific tools can read/write. Crucially, the teammate cannot edit that grant list through MCP; otherwise it could grant itself write access to new namespaces and elevate its own effective scope.

### Defaults

- Primary teammate namespace: open by default (`others_can: write`, docs `visibility: public` by default), matching Disco's default collaboration model.
...
- `disco_kb_put`, `disco_kb_edit`, `disco_kb_get`, `disco_kb_search` continue accepting explicit `namespace`/`uri`.
...
- Optional later: if no namespace is supplied and MCP has a current teammate context, generic tools may suggest `Use disco_teammate_memory_append or pass namespace: <primarySlug>` rather than guessing.

### Teammate-aware wrappers

Add wrappers that are context-aware and bounded:

- `disco_teammate_kb_search`: default search scopes are primary namespace plus branch-granted read namespaces. Accept optional `includeShared: boolean` or explicit `namespaces`, but reject any namespace outside the grant list.
- `disco_teammate_memory_search`: hard-default `kind: memory`, `pathPrefix: memory/`, primary namespace.
- `disco_teammate_memory_append`: write only to primary namespace unless a future `target` is explicitly allowed.
- `disco_teammate_context`: read-only; may return primary namespace and grants, but must not accept mutation arguments.

Explicit non-goal for MCP: no `disco_teammate_namespace_set`, no `disco_teammate_grant_put`, and no generic MCP path that patches `custom_context.teammate.kb`. If MCP needs repair diagnostics, return instructions for a human/API caller instead of performing the mutation.
...
- MCP registrations in a new `apps/disco-daemon/src/mcp/tools/teammate-knowledge.ts` or in `knowledge.ts` if kept small; MCP registrations should be read/append/search only, not configuration mutation.
...
- Invoke it from teammate creation before the bootstrap session starts; use `disco_teammate_context` only for read/diagnostic legacy missing-namespace reporting.
...
- Add `disco_teammate_memory_append` and `disco_teammate_memory_search`.
