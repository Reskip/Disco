...
Disco separates application authorization from the process substrate. The code
is the source of truth; the user-facing guide is
[`apps/disco-docs/content/guide/multiplayer-unix-isolation.mdx`](../../apps/disco-docs/content/guide/multiplayer-unix-isolation.mdx).
...
validation rather than silently downgraded. Disco no longer creates host users or
...
execution mode. Tenant context and immutable Disco user IDs remain the trust
source for branch/session/task access. `others_can`, `others_fs_access`, branch
owners, Disco groups, board grants, and membership records are product RBAC—not
host POSIX groups.
...
- `apps/disco-daemon/src/utils/executor-delegated-identity.ts`
