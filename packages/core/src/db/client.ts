...
import type { DiscoConfig } from '../config/types';
...
export interface DbConfig {
  ...
  /**
   * Database URL
   * - SQLite local file: 'file:~/.disco/disco.db' or 'file:/absolute/path/disco.db'
...
async function configureSQLitePragmas(client: Client): Promise<void> {
  ...
  const silent = process.env.DISCO_SILENT_PRAGMA_LOGS === 'true';
 ...
 * const db = createDatabase({ url: 'file:~/.disco/disco.db' });
 *
 * // Remote Turso
...
export function resolveDatabaseUrl(options: DatabaseUrlResolutionOptions = {}): string {
  ...
  if (env.DISCO_DB_DIALECT === 'postgresql') {
    return env.DATABASE_URL || 'postgresql://localhost:5432/disco';
  }
  if (env.DISCO_DB_PATH) {
    return expandDatabasePathForHome(env.DISCO_DB_PATH, homeDir);
  }
  if (env.DATABASE_URL) {
  ...
  if (dbConfig) {
    ...
    if (dialect === 'postgresql') {
      ...
      if (dbConfig.postgresql?.host) {
        ...
        const database = pg.database || 'disco';
        return `postgresql://${user}${password}@${pg.host}:${port}/${database}`;
      }
      return 'postgresql://localhost:5432/disco';
    }
 ...
 *
 * Priority:
 * 1. DISCO_DB_DIALECT + DATABASE_URL/DISCO_DB_PATH environment variables
 * 2. database.dialect + database.postgresql.url / database.sqlite.path from config.yaml
 * 3. Default SQLite path (~/.disco/disco.db)
 *
...
export function getDatabaseUrl(): string {
  let config: DiscoConfig | undefined;
  try {
