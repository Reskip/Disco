...
beforeEach(async () => {
  scratch = await mkdtemp(join(tmpdir(), 'disco-tenant-archive-'));
});
...
describe('assertManifestTablesMatchCatalog', () => {
  const twoTables = makeManifest([
    { name: 'agents', rowCount: 0, sha256: HEX, bytes: 0 },
    { name: 'sessions', rowCount: 1, sha256: HEX, bytes: 10 },
  ]);

  it('accepts an exact set regardless of order', () => {
    expect(() => assertManifestTablesMatchCatalog(twoTables, ['sessions', 'agents'])).not.toThrow();
  });
