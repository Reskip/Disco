 ...
 * utility classes". Disco compiles them at development time into
 ...
 *    changes without regenerating (`pnpm --filter disco-ui gen:streamdown-css`)
...
describe('streamdown-tailwind.generated.css', () => {
  it('stays in sync with the installed streamdown dist and theme bridge', () => {
    ...
    expect(
      stamp?.[1],
      'Streamdown inputs changed — run `pnpm --filter disco-ui gen:streamdown-css` and commit the result'
  ...
  it('compiles the chrome utilities that regressed unstyled with streamdown 2.5', () => {
    ...
    // Dark mode variants bind to Disco's html.dark convention (ThemeContext).
