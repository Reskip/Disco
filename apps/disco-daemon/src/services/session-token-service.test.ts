...
describe('SessionTokenService runtime scoping', () => {
  it('issues executor-purpose tokens with user/task/session scope and enforces max uses', async () => {
    ...
    expect(decoded).not.toHaveProperty('branch_id');
  ...
  it('binds tenant, user, session, and task before consuming a bounded local token', async () => {
  ...
  it('refuses PostgreSQL mode without an injected database authority', () => {
    vi.stubEnv('DISCO_DB_DIALECT', 'postgresql');
    try {
      expect(
        () =>
