...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'disco-sandbox-migration-'));
...
test('SQLite dry-run resolves only the requested tenant without mutating state', {
  ...
  const db = join(f.dataHome, 'disco.db');
...
`
  ...
  const baseArgs = [
    ...
    '1',
    '--service-name',
    'disco-migration-test-nonexistent',
  ];
  const env = {
    ...process.env,
    DATABASE_URL: '',
...
case "$* $input" in
  ...
  *" -f "*)
    ...
    /usr/bin/grep -q 'CREATE TEMP TABLE disco_sandbox_home_map' "$sql_file" || exit 8
...
`
  ...
  const result = spawnSync(
    ...
    [
      ...
      'root',
      '--database-url',
      'postgresql://user:topsecret@example.invalid/disco',
      '--tenant-id',
      'default',
      '--expected-user-count',
      '1',
...
test('preflight treats references in the stable daemon home as review-only', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'disco-sandbox-preflight-'));
...
test('preflight fails closed when a credential file cannot be scanned', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'disco-sandbox-preflight-error-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const settingsDir = join(root, '.claude');
...
test('preflight checks the canonical target of a symlinked home', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'disco-sandbox-preflight-symlink-'));
  t.after(() => rm(root, { recursive: true, force: true }));
