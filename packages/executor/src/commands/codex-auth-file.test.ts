...
describe('codex.auth-file executor boundary', () => {
  it('atomically writes 0600, reads, and idempotently deletes in its own runtime home', async () => {
    process.env.CODEX_HOME = await mkdtemp(join(tmpdir(), 'disco-codex-auth-'));
    ...
    const writtenMode = (await stat(join(process.env.CODEX_HOME, 'auth.json'))).mode & 0o777;
    if (process.platform !== 'win32') expect(writtenMode).toBe(0o600);
