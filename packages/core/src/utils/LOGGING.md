...
the same `~/.disco/logs/daemon.log` file for both streams. Containers, systemd, and other process
...
`apps/disco-daemon/src/index.ts` and `packages/executor/src/index.ts`, respectively:
