...
import { AGENTIC_TOOL_DISPLAY_NAMES, AGENTIC_TOOL_KEY_CREATION_URL } from '@disco/agentic-tools';
import type { AgenticToolName, DiscoClient, AuthCheckResult } from '@disco-live/client';
...
function checkAuth(client: DiscoClient, tool: AgenticToolName): Promise<AuthCheckResult | null> {
      ...
      <SystemMessage
        content={
          <Space size="small" align="start">
            ...
            <Text type="secondary" style={{ fontSize: 13 }}>
              {tool === 'codex'
                ? '服务器共享的 Codex 登录已经可用，重新发送消息即可。'
                : `This run needed a connected ${displayName} account. Your account is already connected — sending a new message should work.`}
            </Text>
          </Space>
        }
      />
      ...
      <SystemMessage
        content={
          <Space orientation="vertical" size="small">
            <Text type="secondary" style={{ fontSize: 13 }}>
              Disco couldn't verify your {displayName} connection. Try sending a new message. If it
        ...
        content={
          <Space orientation="vertical" size="small" style={{ maxWidth: 480 }}>
            <Text style={{ fontSize: 13 }}>
              服务器共享的 Codex 登录当前不可用。请在“设置”中重新绑定；任意账号完成绑定后，所有账号都会立即可用。
            </Text>
            <Button
              size="small"
              onClick={() => onOpenAgenticToolSettings?.(tool)}
    ...
    <SystemMessage
      content={
        <div style={{ maxWidth: 480 }}>
          <Text style={{ fontSize: 13 }}>
            Disco doesn't have its own AI model — it drives {displayName} on your behalf, so it needs
