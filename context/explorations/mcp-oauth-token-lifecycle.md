...
> Audience: disco maintainers thinking about cron / schedule-driven agents.
...
1. **`apps/disco-daemon/src/register-hooks.ts:825-856`** — the
...
2. **`apps/disco-daemon/src/register-services.ts:1931-2018`** — the
...
3. **`apps/disco-daemon/src/register-services.ts:2034-2094`** —
...
**Initial-auth path** — `apps/disco-daemon/src/oauth-cache.ts:73-118`
...
| ------------------------- | ------------------------ | -------------------------- | ------------------------------------------------------------------------- |
| Right after first auth    | `now + 1h` (fake)        | `false` (until 1h elapses) | Disco _thinks_ token is good for 1h regardless of provider truth           |
...
`oauth-cache.ts:89` and **Disco may incorrectly mark the token as expired
...
- **Refresh response omits `expires_in`.** Returns `access_token`, `token_type`,
  ...
  check (Disco's current behavior) will never proactively refresh.
...
- **Implication**: Linear is the easy case. Current Disco code handles it
...
- Requires the **`offline_access`** scope on initial auth or no refresh
  token is issued — _easy to miss during DCR_. Worth a guardrail in Disco's
...
- **Implication**: Atlassian is well-specified and Disco handles it; the only
...
## Phase 3 — Gap analysis (Disco vs providers)

### Coverage matrix

| Capability needed                  | Disco implements? | Notes                                                     |
| ---------------------------------- | ---------------- | --------------------------------------------------------- |
...
5. **Slack rotation is one-way and easy to forget.** If an admin sets up an
   ...
   and Disco's code is correct (no refresh needed). But the user _thinks_
...
6. **Atlassian without `offline_access` leaves no refresh_token.** First
   tool call after 1h returns 401, Disco sees no refresh_token, falls to
...
The tooltip text already lives near `apps/disco-ui/src/components/MCPServerPill.tsx`
(token expiry display) and `apps/disco-ui/src/utils/mcpAuth.ts`
...
Status: Disco already supports per-tool credential storage (PR #1077) and
...
- 🔴 **The 1-hour default in `persistOAuthToken` (`oauth-cache.ts:89`) is the
  source of "Disco thinks this token expired after exactly 1 hour" reports.**
...
| JIT refresh hook (server reads)                           | `apps/disco-daemon/src/register-hooks.ts` (`injectPerUserOAuthTokens`)           |
| Auth-headers service (executor JIT)                       | `apps/disco-daemon/src/register-services.ts` (`/mcp-servers/oauth-auth-headers`) |
| Manual refresh endpoint                                   | `apps/disco-daemon/src/register-services.ts` (`/mcp-servers/oauth-refresh`)      |
| OAuth-status (UI green-pill)                              | `apps/disco-daemon/src/register-services.ts` (`/mcp-servers/oauth-status`)       |
