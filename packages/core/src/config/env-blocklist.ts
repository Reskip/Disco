 ...
 * - Disco encryption secret (DISCO_MASTER_SECRET)
 *
 * Special carve-out: names matching `PYTHON_DISCO_*` are allowed so Disco can
 ...
 * `PYTHON_DISCO_*` is carved out so Disco-specific Python config is allowed.
 */
...
export function getEnvVarBlockReason(varName: string): string | null {
  ...
  const reasons: Record<string, string> = {
    ...
    DISCO_MASTER_SECRET: 'DISCO_MASTER_SECRET is reserved for Disco encryption infrastructure',
