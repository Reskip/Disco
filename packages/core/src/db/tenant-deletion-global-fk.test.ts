...
describe('assertDeclaredGlobalRelation', () => {
  ...
  it.each([
    ...
    ['a USING expression that reads the tenant', { name: 'p', usingExpression: 'disco.tenant_id' }],
    ['a CHECK expression that reads the tenant', { name: 'p', checkExpression: 'disco.tenant_id' }],
