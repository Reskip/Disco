 ...
 *   ones fall through to Disco's UI; MCP tool calls are auto-approved in the
...
describe('getDefaultPermissionMode', () => {
  it('returns "allow-all" for codex (Disco MCP-heavy default)', () => {
    expect(getDefaultPermissionMode('codex')).toBe('allow-all');
  });

  it('returns "auto" for claude-code (model classifier; unresolved prompts fall through to Disco UI)', () => {
  ...
  describe('permission mode characteristics', () => {
    ...
    it('claude-code uses auto (model classifier; unresolved prompts fall through to Disco UI)', () => {
