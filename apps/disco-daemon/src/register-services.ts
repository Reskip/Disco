...
import { OPENCODE_DAEMON_CONTRIBUTION } from '@disco/agentic-tool-opencode/daemon';
...
import { Forbidden, NotAuthenticated } from '@disco/core/feathers';
import type {
  OAuthFlowContext,
  OAuthTokenResponse,
} from '@disco/core/tools/mcp/oauth-mcp-transport';
import { OAuthDCRFailure } from '@disco/core/tools/mcp/oauth-mcp-transport';
import type {
  AuthenticatedParams,
  HookContext,
  MCPAuth,
...
import type { UnixUserMode } from '@disco/core/unix';
import { safeOutboundFetch } from '@disco/core/utils/safe-outbound-fetch';
...
import { getRuntimeCapabilityCatalog } from './mcp/server.js';
...
import { createAgentCapabilitiesService } from './services/agent-capabilities.js';
import { createAgenticToolPresetsService } from './services/agentic-tool-presets.js';
import { AGENTS_SERVICE_TRANSPORT_METHODS, createAgentsService } from './services/agents.js';
...
import { createRuntimeCapabilitiesService } from './services/runtime-capabilities.js';
import {
...
import { createSessionSearchService } from './services/session-search.js';
...
export interface RegisterServicesContext {
  ...
  jwtSecret: string;
  daemonUrl: string;
  /** True when the daemon is serving the bundled UI itself at /ui (installed disco-live). */
...
export interface RegisteredServices {
  ...
  usersRepository: import('@disco/core/db').UsersRepository;
  sessionsRepository: import('@disco/core/db').SessionRepository;
...
export async function registerServices(ctx: RegisterServicesContext): Promise<RegisteredServices> {
  const { db, app, config, allowSuperadmin } = ctx;
  ...
  app.service('/runtime-capabilities').hooks({ before: { all: [ctx.requireAuth] } });
  ...
  app.use('/agents', createAgentsService(db), {
    methods: [...AGENTS_SERVICE_TRANSPORT_METHODS],
  });
  app.service('/agents').hooks({ before: { all: [ctx.requireAuth] } });
  app.use('/codex-skills', createCodexSkillsService(db), {
    methods: ['find', 'get', 'create', 'patch'],
  });
  app.service('/codex-skills').hooks({ before: { all: [ctx.requireAuth] } });
  app.use('/agent-capabilities', createAgentCapabilitiesService(db), {
    methods: ['find', 'get', 'create', 'patch', 'remove'],
  });
  app.service('/agent-capabilities').hooks({ before: { all: [ctx.requireAuth] } });
  ...
  const requireSharedCodexAdmin = async (context: HookContext): Promise<HookContext> => {
    if (!context.params.provider) return context;
    const user = (context.params as AuthenticatedParams).user;
    if (!user || !hasMinimumRole(user.role, ROLES.ADMIN)) {
      throw new Forbidden('Only admins can change the shared Codex login.');
    }
    return context;
  };

  // Imports a pasted Codex CLI auth.json into the server-wide credential home.
  // Only admins may replace this shared login; token material never leaves the
  // daemon after submission.
  app.use('/codex-auth/import', createCodexAuthImportService(app, db));
  app
    .service('/codex-auth/import')
    .hooks({ before: { create: [ctx.requireAuth, requireSharedCodexAdmin] } });

  // ChatGPT device-code sign-in: an admin starts an attempt (code + verification
  // URL back to the UI, daemon polls OpenAI for approval). Tokens stay
  // daemon-side end to end.
  app.use('/codex-auth/device', createCodexDeviceAuthService(app, db));
  app.service('/codex-auth/device').hooks({
    before: {
      create: [ctx.requireAuth, requireSharedCodexAdmin],
      find: [ctx.requireAuth, requireSharedCodexAdmin],
    },
  });

  // Removes the server-wide Codex login. Admin-only and server-local; this does
  // not revoke the ChatGPT grant on other machines.
  ...
  // and falls back to the static list at @disco/core/models/copilot if no
  ...
  app.use('/session-mcp-servers', {
    async find(params?: {
      ...
      // RBAC scoping is composed below via `_discoSqlSessionAccessUserId`.
      ...
      if (params?._discoSqlSessionAccessUserId) {
        conditions.push(
          visibleSessionReferenceAccessExists(
            db,
            params._discoSqlSessionAccessUserId,
...
function createExecuteHandler(
  ...
  return async (
    ...
    // The Session row is the execution authority. Its working directory is
    // stamped at creation time from either `standalone/<session-id>` or the
    // selected Agent workspace.
    const sandboxCfg = config.execution?.sandbox;
    const cwd = session.working_directory?.trim();
    if (!cwd) {
      throw new Error(`Session ${sessionId} has no working directory`);
    }
    const sandboxStorage =
      sandboxCfg?.enabled === true ? resolveSandboxStoragePaths(config, tenantId) : undefined;
    if (sandboxCfg?.enabled === true && !session.created_by) {
      throw new Error(
        `Sandboxed Session ${sessionId} has no owner; refusing to expose an unscoped workspace.`
      );
    }
    const sandboxUserWorkspaceRoot =
      sandboxStorage && session.created_by
        ? resolveDiscoUserWorkspaceDirectory(sandboxStorage.worktreesRoot, session.created_by)
        : undefined;
    ...
    const { resolveDelegatedHomeKey } = await import('@disco/core/unix');
    ...
    const { createUserProcessEnvironment } = await import('@disco/core/config');
    const executorEnv = await runWithTenantDatabaseScope(db, tenantId, async (tenantDb) => {
      ...
      return createUserProcessEnvironment(userId, tenantDb, undefined, sessionId as SessionID);
    });
    if (session.agentic_tool === 'codex') {
      executorEnv.DISCO_CODEX_SKILLS_CONFIG = JSON.stringify(
        await runWithTenantDatabaseScope(db, tenantId, (tenantDb) =>
          resolveCodexSkillRuntimeEntries(tenantDb, { workspacePath: cwd })
        )
      );
    }
...
async function registerMCPServices(
  ...
  function oauthResultPage(success: boolean, message: string): string {
    ...
    return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Disco OAuth</title>
...
<body><div class="card"><div class="icon">${icon}</div><p>${safeMessage}</p></div></body></html>`;
   ...
   *      different machine than the daemon (e.g. any remotely-deployed Disco).
   ...
   * `@disco/core/tools/mcp/oauth-mcp-transport.ts` so error messages don't
  ...
  type StartTwoPhaseOAuthOptions = {
     ...
     * provided.
     */
    prefetchedAuthServerMetadata?: import('@disco/core/tools/mcp/oauth-mcp-transport').AuthorizationServerMetadata;
    mcpServerId?: string;
    userId?: string;
    oauthMode?: 'per_user' | 'shared';
    tenantId?: string;
  ...
  async function startTwoPhaseMCPOAuthFlowInternal(
    ...
    const { startMCPOAuthFlow } = await import('@disco/core/tools/mcp/oauth-mcp-transport');
    ...
    let durableServer: import('@disco/core/types').MCPServer | undefined;
    let durableBinding:
      | {
          tenantId: string;
          userId: UserID;
      ...
      ? await runInOAuthTenantWriteScope(db, durableBinding.tenantId, async () => {
          ...
          return durableOAuthFlows!.create({
            ...
            configFingerprint: fingerprintMCPOAuthGrantConfiguration(
              process.env.DISCO_MASTER_SECRET!,
              currentServer,
              {
  ...
  const oauthCallbackHandler = async (req: express.Request, res: express.Response) => {
    ...
    try {
      ...
      try {
        ...
        const { completeMCPOAuthFlow } = await import('@disco/core/tools/mcp/oauth-mcp-transport');
    ...
    async create(
      ...
      try {
        ...
        let prefetchedAuthServerMetadata:
          | import('@disco/core/tools/mcp/oauth-mcp-transport').AuthorizationServerMetadata
          | null = null;
        let discoverySource: string | null = null;
        if (probeResponse.status === 401) {
          const { resolveMCPOAuthDiscovery } = await import(
            '@disco/core/tools/mcp/oauth-mcp-transport'
          );
        ...
        if (probeResponse.status === 401) {
          ...
          if (data.client_id && data.client_secret) {
            ...
            const { fetchOAuthToken, inferOAuthTokenUrl } = await import(
              '@disco/core/tools/mcp/oauth-auth'
            );
      ...
      try {
        ...
        const authenticatedServerIds = await resolveAuthenticatedServerIds({
          ...
          isGrantBoundToServer: (server, grant) =>
            isMCPOAuthGrantBoundToServer(process.env.DISCO_MASTER_SECRET!, server, grant),
        });
  ...
  app.use('/mcp-servers/oauth-auth-headers', {
    async create(
        ...
        const executorTokenService = (
          app as unknown as {
            sessionTokenService?: {
              validateToken: (
                token: string,
                expected?: { sessionId?: string; taskId?: string }
      ...
      const { needsRefresh, refreshAndPersistToken, InvalidGrantError } = await import(
        '@disco/core/tools/mcp/oauth-refresh'
      );
  ...
  app.use('/mcp-servers/oauth-refresh', {
    async create(
      ...
      const {
        ...
        FailedRefreshError,
      } = await import('@disco/core/tools/mcp/oauth-refresh');
      ...
      try {
        ...
        if (isPostgresDatabaseHandle(db)) {
          ...
          if (
            !currentGrant ||
            !isMCPOAuthGrantBoundToServer(process.env.DISCO_MASTER_SECRET!, server, currentGrant)
          ) {
            if (currentGrant) {
              await runInOAuthTenantWriteScope(db, tenantId, () =>
                new UserMCPOAuthTokenRepository(db).deleteGrantVersion(
  ...
  app.use('/mcp-servers/discover', {
    async create(
      ...
      try {
        ...
        const { restoreRedactedMCPAuthSecrets } = await import(
          '@disco/core/tools/mcp/auth-secrets'
        );
        const { resolveMCPAuthHeaders } = await import('@disco/core/tools/mcp/jwt-auth');
        const { mergeMCPRemoteHeaders, restoreRedactedMCPCustomHeaders } = await import(
          '@disco/core/tools/mcp/http-headers'
        );
        const tenantId = tenantIdFromParams(params);
        ...
        // process.env + DISCO_USER_ENV_KEYS, but the daemon's process.env
        ...
        const { resolveUserEnvironment } = await import('@disco/core/config');
        ...
        const probeAndAcquireOAuthToken = async (mcpUrl: string): Promise<string | undefined> => {
          try {
            ...
            // remote browsers can complete the redirect on a deployed Disco.
            const connection = params?.connection as { id?: string } | undefined;
            if (
        ...
        const createMCPConnection = (connHeaders: Record<string, string>) => {
          ...
          const mcpClient = new Client(
            { name: 'disco-discovery', version: '1.0.0' },
            { capabilities: {} }
          );
...
async function bootstrapSuperadminUsers(
  ...
  const { ROLES } = await import('@disco/core/types');
  ...
  for (const rawUserId of bootstrapUsers) {
    ...
    try {
      ...
      promotedCount++;
      console.log(
        `[RBAC] Bootstrap promoted user ${shortId(userId)} (${user.username}) to superadmin`
      );
    } catch (error) {
      console.warn(
