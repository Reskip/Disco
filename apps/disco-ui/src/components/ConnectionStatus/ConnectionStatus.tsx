...
export const ConnectionStatus: React.FC<ConnectionStatusProps> = ({
  ...
  // because useDiscoClient keeps `connected=true` for a 1.5s grace window
  // (DISCONNECT_GRACE_MS in apps/disco-ui/src/hooks/useDiscoClient.ts) to
  ...
  // useDiscoClient. Cheap (the navbar is the only consumer); only ticks when
  ...
  // cap-hit branch in useDiscoClient) to be mistaken for recovery and clear
  ...
  // via useDiscoClient's `retryConnection`.
