...
import { getDiscoHome } from '@disco/core/config';
...
function containmentFenceRoot(): string {
  ...
  return join(getDiscoHome(), 'runtime', 'executor-containment-fences');
}
...
type ProcessInspector = (pid: number, signal?: 0 | NodeJS.Signals) => GroupInspection;
...
interface ContainmentOptions {
  ...
  /** Deterministic seams for the Windows process-containment path. */
  inspectProcessForTest?: ProcessInspector;
...
function inspectProcess(pid: number, signal: 0 | NodeJS.Signals = 0): GroupInspection {
  try {
...
async function waitForLeaderAbsence(
  ...
  while (Date.now() < deadline) {
    if (tracked.leaderExited || inspect(tracked.pid, 0) === 'absent') return true;
    await new Promise<void>((resolve) => setTimeout(resolve, pollMs));
  }
  return tracked.leaderExited || inspect(tracked.pid, 0) === 'absent';
}
...
async function containExecutorIdentity(
  ...
  if (platform === 'win32') {
    const inspect = options.inspectProcessForTest ?? inspectProcess;
    let inspection = inspect(tracked.pid, 0);
    if (tracked.leaderExited || inspection === 'absent') return { status: 'verified_absent' };

    // Windows has no POSIX process group that can be probed with kill(-pgid, 0).
    // The daemon does, however, own the direct executor ChildProcess and marks
    // leaderExited from its exit callback. After the executor has reported SDK
    // quiescence, wait briefly for that authoritative callback before signaling.
    if (
      options.preSignalGraceMs &&
      (await waitForLeaderAbsence(tracked, options.preSignalGraceMs, options.pollMs ?? 50, inspect))
    ) {
      return { status: 'verified_absent' };
    }
    inspection = inspect(tracked.pid, 0);
    if (tracked.leaderExited || inspection === 'absent') return { status: 'verified_absent' };
    if (inspection === 'unverified') {
      return { status: 'unverified', reason: 'Executor process presence is unverified.' };
    }

    console.log(
      `🛑 [Executor] Sending SIGTERM to PID ${tracked.pid} (session ${shortId(tracked.sessionId)})`
    );
    const termResult = inspect(tracked.pid, 'SIGTERM');
    if (termResult === 'absent') return { status: 'verified_absent' };
    if (termResult === 'unverified') {
      return { status: 'unverified', reason: 'SIGTERM executor signal was not authorized.' };
    }
    if (
      await waitForLeaderAbsence(
        tracked,
    ...
    console.log(`🛑 [Executor] PID ${tracked.pid} still present; sending SIGKILL`);
    const killResult = inspect(tracked.pid, 'SIGKILL');
    if (killResult === 'absent') return { status: 'verified_absent' };
    if (killResult === 'unverified') {
      return { status: 'unverified', reason: 'SIGKILL executor signal was not authorized.' };
    }
    if (
      await waitForLeaderAbsence(
        tracked,
    ...
    return { status: 'unverified', reason: 'Executor process remained present after SIGKILL.' };
  }

  if (platform !== 'linux' && platform !== 'darwin') {
    return {
      status: 'unverified',
      reason: `Process-group verification is unsupported on ${platform}.`,
    };
  }
