...
**Base dependencies:** scheduler HA PR [#2174](https://github.com/preset-io/disco/pull/2174), merged as `f48e929e`, and Session queue HA PR [#2180](https://github.com/preset-io/disco/pull/2180), merged as `a1dc2c15`.
...
This change deliberately reuses the thin helpers in `@disco/core/coordination` for diagnostic identity and deterministic delay policy. It does **not** introduce a distributed-worker framework or a central controller. Task state remains owned by `TaskRepository` and Task-specific services.
...
PostgreSQL tests skip unless both `DISCO_TEST_POSTGRES_URL` and `DISCO_DB_DIALECT=postgresql` are set.
