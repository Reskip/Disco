...
description: 'Fully managed Disco: bring your own API keys, pen-tested and hardened for public-internet use (mobile included), same-day security patching, tenant-scoped execution isolation, private network access to your VPC, and an audit trail your security team will accept. Built and operated by the team behind Preset Cloud.'
...
Disco is fairly open under BSL 1.1. Its Additional Use Grant permits
production and commercial use, including internal self-hosting, consulting,
integration, and use in a broader product. It does not permit commercializing
Disco itself as the Disco Offering defined in the license. It has all the
building blocks to create next-gen, multi-agent, multiplayer servers that can
really change how you build software.
...
Here's what we learned productionizing Disco at Preset for our own use:

When you start running Disco for your team (multiple users, worktrees, dev environments per branch, AI sessions touching real code), you quickly realize the scale of what it takes to run this safely in production:
...
## What Disco Cloud Offers

Disco Cloud is a fully managed Disco instance. Up in minutes, reliable, always on the latest secured release. The pitch is simple: someone is going to operate this thing for your team. We'd like to make the case that it should be us.

### Bring Your Own Keys

We don't sell tokens.

You bring your own API keys for Claude, Codex, Gemini (whatever you use). Disco Cloud doesn't proxy them, doesn't mark them up, doesn't take a cut. Your AI spend is between you and the model providers.

Why this matters:

- **No incentive mismatch.** We make money running Disco well, not on how many tokens your team burns.
...
A self-hosted Disco is a real piece of infrastructure: a stateful database, a multi-tenant filesystem, isolated worktrees, Docker resource budgets, agent processes that can hang or run away. Running that well, full-time, is our job, not a side concern.
...
- **The latest/stable/secure balance.** New Disco releases land on Cloud after they've been validated against our own production traffic, not yours.
- **Same-day patching for zero-days.** When CVEs hit Node, Postgres, the kernel, or any of Disco's deps, the same SRE rotation that runs Preset Cloud ships the fix that day. You don't have to track Disco changelogs or rebuild images.
- **No on-call rotation for your team.** We're on-call for Disco; you're on-call for your product.
...
Cloud routes sessions and workspaces through tenant-scoped runtime, storage, home, and credential boundaries. The orchestration substrate owns that enforcement; Disco does not manufacture isolation by creating host Unix accounts.

### Hardened for the Public Internet

Disco is powerful software, and some of its capabilities (broad filesystem access, freeform shell, multi-user shared dev environments) are great inside a trusted network and risky on the open internet. If you self-host, you should think carefully about where you put it (and probably run it behind a VPN).

Disco Cloud is built the other way: hardened from day one to sit safely on the public internet.
...
The payoff is the obvious one: **use Disco from anywhere, mobile included, with no VPN gymnastics.** Same posture you'd want from any modern SaaS your team relies on.

### Managed Private Cloud

The default public Disco Cloud is a managed single-tenant instance per team, hosted on our infrastructure. For organizations whose code, data, or networking story rules that out, **Managed Private Cloud (MPC)** is a different deployment model:

- **Bring-your-own-cloud deployments.** We deploy and manage Disco inside your own AWS or GCP account. Data and compute stay in your tenancy.
...
- **VPN-fronted access.** Sit Disco behind your existing VPN if your security policy requires it.
- **Bring back the shared, fat dev server.** MPC unlocks the "team works on one beefy dev box" model, with tenant-scoped runtime, storage, home, and credential boundaries keeping users from stepping on each other. Not part of the default Cloud offering; ask about it if it fits your team.

Network topology is a conversation, not a checkbox. Tell us what your security team needs and we'll work out the shape.

### Auditing & Observability

If Disco is becoming critical to your PDLC, you need to see what's happening in it, both for ops and for the security review your team will eventually get pulled into.
...
Disco Cloud lets your CI agents live on the same platform as the rest of your team's work:

- **See CI-triggered sessions spawn** in your Disco environment in real-time
- **Watch agents work** with full visibility into tool use, file reads, and reasoning
- **Resume sessions** after CI completes, like "thanks for the review, now fix the issues you found"
...
API authentication, user isolation, secret management, network access controls. Disco Cloud handles the wiring. Your CI agents become first-class citizens, not ephemeral scripts.
...
## Who Disco Cloud Is For

- **Teams that want a managed multi-user Disco instance** without the ops burden
...
- **Anyone who wants Disco's capabilities** without owning the infrastructure underneath

## Signing Up for the Private Beta

We're scaling this aggressively but cautiously. Disco Cloud is quickly becoming critical to team productivity, so we're onboarding carefully to ensure every deployment is solid.
...
4. **Trial period** - Use Disco Cloud, see if it delivers value for your workflow
5. **Decide** - If it's solving real problems, stay on. If not, no hard feelings.

We're not trying to sell you something you don't need. We're looking for teams where Disco Cloud makes a measurable difference. Faster onboarding, better collaboration, cleaner development workflows.

## What You Get

- **Dedicated Disco instance** with your team's repos and worktrees
...
- **Tenant-scoped execution isolation** as defense-in-depth
...
We're building Disco Cloud because we're running Disco ourselves. The features we're adding are the ones we need. The operational experience we're developing is the same rigor we apply to Preset Cloud.
...
## Open + Managed Service

Disco remains source-available under BSL 1.1. Its Additional Use Grant permits
you to run it in production yourself if that fits your workflow better.

Disco Cloud is for teams that want the capabilities without the ops burden. No Docker config, no database migrations, no monitoring setup, no capacity planning. Just the platform, running reliably, with all the enterprise features baked in.

The open project benefits too: production learnings at Preset feed back into Disco's core. Better security, better performance, better defaults. Everyone wins.
...
Disco Cloud bridges that gap. Fully managed, secure, observable, built by the team using it every day.

If that sounds like what your team needs, we'd love to talk.

<JoinBetaCTA sourceCta="blog-disco-cloud" />

---

Questions? Reach out:

- [GitHub Discussions](https://github.com/preset-io/disco/discussions)
- [Email us](mailto:hello@disco.live)
- [Join our Discord](https://discord.gg/Qh4TrFQZpd)

---

_If you're running Disco yourself and want to share your experience, we'd love to hear about it. Open a discussion or drop us a note. Your feedback shapes where this goes next._
