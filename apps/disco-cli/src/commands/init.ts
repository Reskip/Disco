/**
 * `disco init` - Initialize Disco environment
 *
...
import { resolveAgenticToolSelectionPolicy } from '@disco/core/agentic-integrations';
import {
  type DiscoConfig,
  createInitialConfig,
  getConfigPath,
...
export function isFreshInitState(state: {
  ...
  return (
    !state.baseExists ||
    (!state.configExists && !state.databaseExists && !state.workspacesExist)
  );
}

export function createInstallTelemetryConfig(config: DiscoConfig, instanceId: string): DiscoConfig {
  return {
    ...config,
...
export function assertInitSupportsConfiguredDatabase(dialect = process.env.DISCO_DB_DIALECT): void {
  if (dialect === 'postgresql') {
    throw new Error(
      '`disco init` currently supports SQLite installations only. For PostgreSQL deployments, configure the deployment environment and run `disco db migrate --yes`; the daemon will bootstrap required database state.'
    );
  }
}
...
export default class Init extends Command {
  ...
  private initialDaemonConfig: NonNullable<DiscoConfig['daemon']> = {
    deployment_id: this.deploymentId,
  };
  private initialConfig: DiscoConfig = {
    ...getDefaultConfig(),
  ...
  static description =
    'Create the Disco config/database and install the agentic tools selected for first use';
  ...
  static flags = {
    ...
    force: Flags.boolean({
      char: 'f',
      description: 'Force re-initialization without prompts (deletes the entire .disco directory)',
      default: false,
    }),
    'skip-if-exists': Flags.boolean({
      description:
        'Skip initialization if .disco/ directory already exists (idempotent, safe for Docker)',
      default: false,
    }),
  ...
  private async persistDuringInitialCreation(config: DiscoConfig): Promise<void> {
    config.daemon = {
      ...config.daemon,
  ...
  private async assertDaemonStoppedBeforeReinit(): Promise<void> {
    ...
    // `disco daemon status` so re-init cannot drift from the CLI's canonical
    ...
    for (const daemonUrl of urls) {
      if ((await probeDiscoDaemon(daemonUrl)).running) {
        throw new Error(
          `The Disco daemon is running at ${daemonUrl}. Stop it with \`disco daemon stop\` (or Ctrl+C for a development daemon) before re-initializing.`
        );
      }
    }
  }

  /** List child directories without failing when the parent is absent. */
  ...
  /**
   * Detect if running in dev mode (from source) vs disco-live (npm package)
   *
   * Dev mode = running from disco monorepo source
   * Disco-live mode = running from npm package (globally installed or in node_modules)
   */
  private async isDevMode(): Promise<boolean> {
    ...
    // If running from node_modules/disco-live, it's definitely the npm package
    // (more specific than just checking for "node_modules" which could catch dev mode too)
    if (
      __dirname.includes('node_modules/disco-live') ||
      __dirname.includes('node_modules\\disco-live')
    ) {
      return false;
    }

    // Check if we're in the disco monorepo by looking for packages/core in cwd
    ...
    // Otherwise (could be anywhere when running disco-live), it's the npm package
    return isInMonorepo;
  }
  ...
  async run(): Promise<void> {
    ...
    if (flags.local) {
      this.error(
        '`disco init --local` is no longer supported because daemon configuration and data must share one canonical installation at ~/.disco. Run `disco init` without --local.'
      );
    }
    this.nonInteractive = flags['non-interactive'];
    const requestedTools = flags['agentic-tools'] ?? process.env.DISCO_AGENTIC_TOOLS;
    ...
    const baseDir = join(homedir(), '.disco');
    ...
    if (
      ...
      (await this.pathExists(join(baseDir, 'disco.db'))) &&
      (await this.pathExists(join(baseDir, 'config.yaml')))
    ) {
      this.log(chalk.green('✓ Disco already initialized at: ') + chalk.cyan(baseDir));
    ...
    try {
      ...
      this.error(
        `Failed to initialize Disco: ${error instanceof Error ? error.message : String(error)}`
      );
    }

    if (!flags.force && !this.nonInteractive && (!process.stdin.isTTY || !process.stdout.isTTY)) {
      this.error(
        'Interactive `disco init` requires a TTY. For headless setup, use `disco init --non-interactive --agentic-tools <comma-separated-list|all|none>`.'
      );
    }

    try {
      const dbPath = join(baseDir, 'disco.db');
      const workspacesDir = join(baseDir, 'worktrees');
      ...
      const workspacesExist = await this.pathExists(workspacesDir);
      ...
      if (freshState) {
        if (
          ...
          this.error(
            'Fresh noninteractive initialization requires an explicit agentic-tool policy. Pass `--agentic-tools <comma-separated-list|all|none>` (or set DISCO_AGENTIC_TOOLS).'
          );
        }
      ...
      this.log(chalk.yellow('⚠  Disco is already initialized at: ') + chalk.cyan(baseDir));
      await this.warnExistingInstallTelemetryUnconfigured();
      this.log('');
      ...
      const workspaces = workspacesExist ? await this.listDirs(workspacesDir) : [];
      ...
      this.log(
        chalk.dim(
          '    Backup moves it intact. Delete permanently removes everything in it, including config.yaml, database sidecars, logs, user workspaces, and installed tools.'
        )
      );
      this.log('');
      ...
      if (workspaces.length > 0) {
        this.log(`${chalk.cyan('  User workspaces:')} ${workspacesDir}`);
        for (const workspace of workspaces.slice(0, 5)) {
          this.log(chalk.dim(`    - ${workspace}`));
        }
        if (workspaces.length > 5) {
          this.log(chalk.dim(`    ... and ${workspaces.length - 5} more`));
        }
      }
  ...
  private async performInit(
    ...
    try {
      ...
      this.log(`${chalk.green('   ✓')} Database schema is ready`);
    } finally {
    ...
    if (!skipPrompts) {
      ...
      if (shouldDeferAdminSetup(this.nonInteractive)) {
        ...
        this.log(
          chalk.dim(
            '     Set DISCO_ADMIN_PASSWORD before daemon start, or use the generated admin-credentials file.'
          )
        );
      } else {
        try {
          ...
          try {
            await createUser(db, {
              username: DEVELOPMENT_DEFAULT_ADMIN_USER.username,
          ...
          this.log(chalk.dim(`     Username: ${DEVELOPMENT_DEFAULT_ADMIN_USER.username}`));
    ...
    } else if (process.env.DISCO_TELEMETRY === undefined) {
    ...
    if (process.env.DISCO_MANAGED_AGENTIC_TOOLS === '1') {
      try {
        ...
        throw new Error(
          `Agentic tool installation did not complete: ${error instanceof Error ? error.message : String(error)}. The new config was preserved. Recovery: disco install --sync`
        );
      }
    }
    ...
    // Success summary
    this.log('');
    this.log(chalk.green.bold('✅ Disco initialized successfully!'));
    this.log('');
    this.log(`   Database: ${chalk.cyan(dbPath)}`);
    this.log(`   User workspaces: ${chalk.cyan(join(baseDir, 'worktrees'))}`);
    ...
    if (missingTools.length > 0) {
      ...
      this.log(chalk.dim('   Repair the configured package set with: disco install --sync'));
      this.log(chalk.dim('   Recheck at any time with: disco doctor'));
    }
    this.log('');

    // Check if daemon is running
    const daemonUrl = await getDaemonUrl();
    const daemonRunning = (await probeDiscoDaemon(daemonUrl)).running;
    ...
    if (daemonRunning) {
      ...
      if (isDevMode) {
        this.log('   2. Restart: cd apps/disco-daemon && pnpm dev');
      } else {
        this.log('   2. Restart: disco daemon start');
      }
    } else {
      if (isDevMode) {
        this.log('   1. Start the daemon: cd apps/disco-daemon && pnpm dev');
        this.log('   2. In another terminal, start the UI: cd apps/disco-ui && pnpm dev');
        this.log('   3. Open the UI: http://localhost:5173');
      } else {
        this.log('   1. Start the daemon: disco daemon start');
        this.log('   2. Open the UI: disco open');
      }
    }
    this.log('');
  }
  ...
  private getInstallChannel(): 'npm' | 'docker' | 'source' | 'homebrew' | 'unknown' {
    if (process.env.KUBERNETES_SERVICE_HOST || process.env.DISCO_DOCKER) {
      return 'docker';
    }
  ...
  private getDeploymentKind(): 'local' | 'docker' | 'k8s' | 'unknown' {
    ...
    if (process.env.container || process.env.DISCO_DOCKER) return 'docker';
    return 'local';
  }
  ...
  private async promptForAgenticTools(
    ...
    this.log(
      chalk.dim(
        `Selected packages will be downloaded with npm, installed at ${discoVersion}, and recorded as this deployment's tool policy.`
      )
    );
    this.log(
      chalk.dim(
  ...
  private async warnExistingInstallTelemetryUnconfigured(): Promise<void> {
    try {
      ...
      this.log(
        chalk.gray(
          `   Disco will not send telemetry unless an admin enables it. Learn more: ${DISCO_TELEMETRY_DOCS_URL}`
        )
      );
      this.log(
        chalk.gray(
          '   Configure later with DISCO_TELEMETRY=1/0 or telemetry.enabled in config.yaml.'
        )
      );
    } catch {
  ...
  private async promptTelemetrySetup(): Promise<void> {
    if (isTelemetryFullyDisabledByEnv()) {
      ...
      this.log('');
      this.log(chalk.gray('Telemetry fully disabled by DISCO_TELEMETRY=0 or DO_NOT_TRACK=1.'));
      return;
    }
    ...
    this.log(
      chalk.gray(
        'Disco sends one anonymous install ping so we can count installs and whether ongoing ' +
          'telemetry was enabled.'
      )
    );
    this.log(
      chalk.gray(
        'If enabled, Disco also sends occasional anonymous install and aggregate usage summaries.'
      )
    );
    this.log(
      chalk.gray(
        'We never send prompts, messages, workspace names, file paths, user emails, ' +
          'session/task IDs, code, tool output, secrets, or raw custom model names.'
      )
    );
    this.log(chalk.gray(`Learn more: ${DISCO_TELEMETRY_DOCS_URL}`));
    this.log(
      chalk.gray(
        'To disable all telemetry, including the one-time install ping, set DISCO_TELEMETRY=0.'
      )
    );
    this.log('');
  ...
  private async promptAdminSetup(dbPath: string): Promise<void> {
    ...
    if (!setupNow) {
      this.log(
        chalk.gray(
          '   Skipped. The daemon will create the local user admin on first start; the generated password lands in ~/.disco/admin-credentials.'
        )
      );
      return;
    }
    ...
    this.log(`${chalk.green('   ✓')} Admin user created (${chalk.gray(username)})`);
  }
}
