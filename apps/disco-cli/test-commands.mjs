...
const tests = [
  { name: 'repo list', cmd: 'pnpm -w disco repo list' },
  { name: 'mcp list', cmd: 'pnpm -w disco mcp list' },
  { name: 'user list', cmd: 'pnpm -w disco user list' },
  { name: 'branch list', cmd: 'pnpm -w disco branch list' },
  { name: 'session list', cmd: 'pnpm -w disco session list' },
  { name: 'board list', cmd: 'pnpm -w disco board list' },
];
...
console.log(chalk.bold.blue('DISCO CLI COMMAND TEST SUITE'));
console.log(chalk.bold.blue('═'.repeat(50)));
console.log('');
...
let hangingCount = 0;
try {
  const result = execSync("pgrep -f 'tsx.*disco' | wc -l", { encoding: 'utf8' });
