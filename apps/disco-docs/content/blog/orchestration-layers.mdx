...
This is where tools like Disco come in. The realization: if you're managing multiple agents, you need multiplayer infrastructure.

I built Disco after hitting this wall on Apache Superset, first trying [Claudette](https://github.com/mistercrunch/claudette-cli) (project-specific tooling), then realizing this needed to be generalized. The key insights:

- **Git worktree management**: One worktree per feature, isolated environments
- **Spatial boards**: Inspired by Figma/Miro, leveraging human spatial memory instead of terminal archaeology
...
This is starting to happen through Disco's internal MCPs (Model Context Protocol servers). Agents in Disco can now:
...
This isn't science fiction. Disco's MCP integration enables exactly this pattern. Early on while building Disco, I realized that exposing an internal MCP service would let agents interact with the environment they're operating in, and with each other. That insight accelerated development dramatically. I was able to start using Disco to build Disco, and that's how it got so good so quick.
...
Disco has a built-in scheduler. You can run supervisor agents on a schedule that:
...
**All the primitives for this exist in Disco today.** The question is how much autonomy we're ready to hand over.
...
Disco (and tools like it) are breaking down those boundaries:
...
Call it **group vibe coding**. Call it **multi-conductor orchestration**. Whatever it is, Disco enables something completely new: synchronous, multiplayer, agent-assisted development with full visibility and structure.
...
If you're juggling agents across terminals and losing track, [try Disco](https://disco.live). If you're building orchestration tools or thinking about this space, I'd love to hear what you're seeing. The shape of this is still forming, and the best insights are coming from people in the thick of it.
