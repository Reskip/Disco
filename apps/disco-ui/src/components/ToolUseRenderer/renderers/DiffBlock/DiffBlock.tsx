...
const shortenPath = (filePath: string): string => {
  // Strip common managed-workspace and home prefixes.
  const markers = ['/worktrees/', '/standalone/', '/agents/', '/home/', '/Users/'];
  for (const marker of markers) {
    const idx = filePath.indexOf(marker);
    if (idx !== -1) {
      // Find the useful project tail after the marker.
      const afterMarker = filePath.slice(idx + marker.length);
      const parts = afterMarker.split('/');
      // Skip managed identity segments and show the useful file tail.
      if (parts.length > 3) {
