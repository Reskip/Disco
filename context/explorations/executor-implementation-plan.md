...
spawn('sudo', ['-u', 'disco_alice', '/usr/local/bin/disco-executor', '--socket', socketPath]);
```
...
packages/
  executor/
    src/index.ts      # Main DiscoExecutor class
    bin/disco-executor # Executable (#!/usr/bin/env node)
    package.json

# Install creates symlink:
/usr/local/bin/disco-executor → packages/executor/bin/disco-executor
```
...
```
Daemon (disco user)
  ↓ spawn('sudo', ['-u', 'disco_alice', '/usr/local/bin/disco-executor'])
Executor (disco_alice user)
...
```bash
# Create test users
sudo useradd -m -s /bin/bash disco_alice
sudo useradd -m -s /bin/bash disco_bob
sudo useradd -r -s /bin/bash -d /opt/disco disco

# Setup sudoers
sudo tee /etc/sudoers.d/disco > /dev/null <<'EOF'
disco ALL=(disco_*) NOPASSWD: /usr/local/bin/disco-executor
EOF
sudo chmod 440 /etc/sudoers.d/disco
```
...
packages/executor/
  src/
    index.ts                 # Main DiscoExecutor class
  ...
  bin/
    disco-executor           # Executable script
  test/
    ipc-server.test.ts
    executor.test.ts
  package.json
...
node packages/executor/bin/disco-executor --socket /tmp/test.sock
...
```
apps/disco-daemon/src/services/
...
```typescript
// apps/disco-daemon/src/services/executor-pool.ts
export class ExecutorPool {
  async spawn(options: { userId: UserID }): Promise<ExecutorInstance> {
    const user = await this.usersRepo.findById(options.userId);
    const unixUsername = user.unix_username || 'disco_executor';
    const socketPath = `/tmp/disco-executor-${randomUUID()}.sock`;
...
// apps/disco-daemon/src/services/executor-client.ts
...
```typescript
// apps/disco-daemon/test/executor-integration.test.ts
...
- [ ] Create `disco setup-executor-isolation` command
- [ ] Create `disco user setup-unix <email>` command
...
// apps/disco-daemon/test/executor-pool.test.ts
...
describe('Executor Integration', () => {
  ...
  test('executor runs as correct Unix user', async () => {
    ...
    expect(processInfo.user).toBe('disco_alice');
  });
...
```typescript
// apps/disco-daemon/test/e2e/executor-e2e.test.ts
describe('Executor E2E', () => {
  ...
  test('terminal spawns as correct user', async () => {
    ...
    expect(output).toContain('disco_alice');
  });
});
```
...
describe('Executor Security', () => {
  ...
  test('executor cannot access other users files', async () => {
    ...
    const result = await executor.client.request('execute_prompt', {
      prompt: 'cat /home/disco_bob/.ssh/id_ed25519',
      cwd: '/tmp',
    });
...
execution:
  ...
  # IPC settings
  ipc:
    socket_path_template: /tmp/disco-executor-{id}.sock
...
```bash
# Override config
export DISCO_EXECUTOR_ENABLED=true
export DISCO_EXECUTOR_UNIX_USER=disco_executor

# Development
export DISCO_EXECUTOR_DEBUG=true  # Verbose logging
```
