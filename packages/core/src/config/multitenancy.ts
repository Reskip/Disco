...
import type { DiscoConfig, DiscoMultiTenancySettings } from './types';
...
export function resolveMultiTenancyDatabaseDialect(
  ...
  if (process.env.DISCO_DB_DIALECT === 'postgresql' || process.env.DISCO_DB_DIALECT === 'sqlite') {
    return process.env.DISCO_DB_DIALECT;
  }
...
export function resolveMultiTenancyConfig(
  config: Pick<DiscoConfig, 'multi_tenancy'>
): ResolvedMultiTenancyConfig {
  const raw: DiscoMultiTenancySettings = config.multi_tenancy ?? {};
...
export function assertValidMultiTenancyConfig(
  config: Pick<DiscoConfig, 'multi_tenancy' | 'database'>
): void {
...
export function resolveTenantContext(
  config: Pick<DiscoConfig, 'multi_tenancy'> | ResolvedMultiTenancyConfig,
