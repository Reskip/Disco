...
const adminBootstrapMocks = vi.hoisted(() => ({
...
vi.mock('./first-run-admin.js', () => adminBootstrapMocks);
...
describe('initializeDatabase logging', () => {
  ...
  beforeEach(() => {
    ...
    adminBootstrapMocks.runFirstRunAdminBootstrap.mockResolvedValue({
      admin: null,
      createdAdmin: false,
  ...
  it.each([
    {
      backend: 'postgresql',
      url: 'postgresql://PG_USER_SENTINEL:PG_PASSWORD_SENTINEL@PG_HOST_SENTINEL:5432/disco?sslmode=PG_QUERY_SENTINEL',
      sentinels: [
        'PG_USER_SENTINEL',
    ...
    {
      backend: 'sqlite',
      url: 'libsql://LIBSQL_HOST_SENTINEL/disco?authToken=LIBSQL_QUERY_SENTINEL',
  ...
  it('uses the configured dialect fallback without logging the URL', async () => {
    vi.stubEnv('DISCO_DB_DIALECT', 'sqlite');
  ...
  it('forwards configured PostgreSQL pool settings to the database client', async () => {
    const url = 'postgresql://localhost/disco';
    const pool = { max: 25 };

    await initializeDatabase(url, { pool });
  ...
  it('does not log a hostile SQLite directory when creating it', async () => {
    const url = 'file:/tmp/SQLITE_DIRECTORY_SENTINEL/disco.db';
