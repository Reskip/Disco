...
Cursor's new TypeScript SDK is a credible fit for Disco's provider model, but it is still public beta and it adds a native-heavy package/runtime surface that deserves live smoke testing before non-beta support.
...
2. **Best Disco fit:** local runtime against the Disco branch worktree (`local: { cwd: branch.path }`) because Disco already owns branches, RBAC, Unix identity, git state capture, MCP session tokens, and process lifecycle. Cursor cloud runtime is valuable, but conflicts with Disco's branch-centric “worktree as card” model unless treated as a separate remote-execution mode.
3. **Largest blocker:** no obvious public permission callback equivalent to Claude/Copilot. Cursor has hooks and sandbox options, and the background/cloud agent docs warn cloud agents auto-run terminal commands, but the SDK public types do not expose an Disco-style interactive permission request channel. Treat v1 as “autonomous/auto-approved inside Disco's OS sandbox,” similar to OpenCode, until proven otherwise.
...
5. **Implementation shape:** add a `cursor` agentic tool, credentials (`CURSOR_API_KEY`), model discovery/cache, `CursorTool` adapter that maps `SDKMessage` events to Disco messages/tool widgets, session persistence via `sdk_session_id = Cursor agentId`, run tracking via task metadata, MCP injection via inline `mcpServers`, and cancellation via `Run.cancel()`.
...
### Disco code/docs reviewed

- SDK comparison guide source: `apps/disco-docs/pages/guide/sdk-comparison.mdx`.
...
| Area                   | Current Cursor SDK surface                                                                                                                                                                                                                       | Disco implication                                                                                                                                                                          |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Package                | `@cursor/sdk`, ESM/CJS exports, `Agent` and `Cursor` namespaces. `package.json` says Node `>=18`; Disco root already requires Node `>=22.12.0`.                                                                                                   | Compatible with Disco's Node floor. Add dependency to `packages/executor` only; avoid browser imports.                                                                                     |
...
| Runtime                | `AgentOptions.local.cwd` accepts a string or string array; local options include `settingSources` and `sandboxOptions`. `cloud` options include env, repos, current-branch work, auto-PRs, and env vars. Blog also mentions self-hosted workers. | v1 should use local runtime with Disco branch path. Cloud runtime is a later, separate mode because it creates remote branches/PRs outside Disco's worktree lifecycle.                      |
...
| Runs/cancellation      | `Run` has `id`, `agentId`, `stream()`, `conversation()`, `wait()`, `cancel()`, status callbacks, `result`, `model`, `durationMs`, `git`. Static `Agent.getRun` and `Agent.cancelRun`.                                                            | Maps cleanly to Disco task lifecycle and `AbortController`; implement `stopTask()` around current `Run.cancel()`.                                                                          |
| Streaming/events       | `run.stream()` yields `SDKMessage`: `system`, `user`, `assistant`, `tool_call`, `thinking`, `status`, `request`, `task`. `agent.send` also accepts `onStep` and `onDelta` callbacks.                                                             | Good fit for Disco `StreamingCallbacks`; use `assistant` text blocks for chunks, `thinking` for thinking callbacks, `tool_call` for tool widgets. Need tests for duplicate/partial events. |
| Tool calling/editing   | Exported tool-call types include shell/read/write/edit/delete/grep/glob/semantic search/MCP/todos/subagent/create-plan/read-lints. `SDKToolUseMessage` has `name`, `status`, `args`, `result`, truncation flags.                                 | Normalize to Disco `tool_use`/`tool_result` blocks. Cursor can produce rich widgets, likely closer to OpenCode/Copilot than Codex.                                                         |
| MCP                    | `mcpServers?: Record<string, McpServerConfig>` on agent and send options; configs support `stdio`, `http`, `sse`, env/headers/OAuth fields. Blog says MCP can come from `.cursor/mcp.json` or inline.                                            | Reuse Disco MCP scoping, inject built-in Disco MCP as Streamable HTTP with Authorization header, and convert user MCP rows to Cursor `McpServerConfig`.                                     |
| Skills/hooks/subagents | Blog says SDK agents pick up `.cursor/skills`, `.cursor/hooks.json`, and named subagents via `agents?: Record<string, AgentDefinition>`.                                                                                                         | Mostly leave native. Potential collision with Disco skills/AGENTS.md should be documented. Avoid writing `.cursor/*` in v1.                                                                |
| Permissions/sandbox    | Public types expose `sandboxOptions?: { enabled: boolean }`, but no Disco-style permission callback. Cursor MCP docs mention chat approval by default; background-agent docs say cloud agents auto-run terminal commands.                         | Treat as auto-approved/autonomous inside Disco's Unix/worktree sandbox. Do not promise interactive approvals until a Cursor hook/callback can block decisions safely.                      |
| Usage/cost             | Public types expose `RunResult.durationMs`, `model`, `git`, but no token/cost fields in inspected `.d.ts`. Changelog/blog say billing is token-based.                                                                                            | v1 may not populate Disco token/context pills unless hidden runtime events contain usage. Store raw responses and leave normalized usage blank initially.                                  |
...
| **Context window tracking** | ❌ Not exposed   | No context-window counters in inspected public types. Could estimate from Disco DB later.                   |
...
- **Worse than Claude/Codex/OpenCode** for Disco token/cost UI today, because no usage payload was visible in public types.
...
- Add `CURSOR_API_KEY` to `ApiKeyName`, `TOOL_API_KEY_NAMES`, `AgenticToolsConfig`, `AgenticToolConfigField`, and possibly `ConfigCredentialKey` / `DiscoCredentials`.
...
5. Create the Disco user message.
...
8. Iterate `for await (const event of run.stream())` and map to Disco streaming + final messages.
...
- `tool_call`: create/update `tool_use` and `tool_result` content blocks keyed by `call_id`; map `running`/`completed`/`error` to Disco tool lifecycle.
...
Disco already has a provider-neutral callback contract at `packages/executor/src/sdk-handlers/base/types.ts:36` and daemon broadcast implementation at `packages/executor/src/handlers/sdk/base-executor.ts:109`.

### 4. MCP handling

Reuse `getMcpServersForSession()` and template resolution (`packages/core/src/mcp/scoping.ts`). Convert Disco MCP rows to Cursor SDK configs:

- Built-in Disco MCP: `type: "http"`, `url: daemonUrl + "/mcp"`, and an `Authorization: Bearer <session.mcp_token>` header.
...
Open question: Cursor supports `.cursor/mcp.json`; Disco should prefer inline `mcpServers` so session-specific selection, OAuth token injection, and `mcp_token` scoping remain daemon-controlled.

### 5. Permissions/sandbox

V1 should be explicit: **Cursor SDK provider is autonomous.**

- Use Disco's existing branch Unix isolation/RBAC as the outer boundary.
...
- Do not wire Disco permission modals until Cursor exposes either:
  - a blocking SDK permission callback,
  - hook events that can deny/allow synchronously per operation, or
  - a reliable config policy language Disco can generate per task.

This is similar to OpenCode's current “auto-granted by Disco” behavior, but should be even more conservatively labeled because Cursor's harness includes rich native tools and context indexing.
...
- Estimate context from Disco message history for UI only, clearly labeled “estimated,” if Max wants parity.
...
| Token/cost absent                   | Medium     | Disco UI would lack pills/cost for Cursor unless Cursor adds usage events. Not a blocker for execution.                                         |
| Cursor local state location         | Medium     | Need empirical test where local agent stores state and whether `agentId` survives under Disco's Unix user/home isolation.                       |
...
| Cloud runtime model mismatch        | Medium     | Cursor cloud clones GitHub and creates branches/PRs externally; Disco branch cards would no longer be source of truth. Defer cloud mode.        |
...
- The SDK surface matches Disco's executor architecture better than the older Cursor CLI would.
- `agentId` + run-scoped streaming/cancel is a clean fit for `sdk_session_id` and task lifecycle.
- Cursor's harness (indexing, semantic search, MCP, skills, hooks, subagents) could be a strong differentiator for Disco users who already use Cursor.
...
- Cloud runtime semantics overlap/conflict with Disco's own branch/worktree orchestration.
...
- ✅ Surface Cursor as a beta provider in agent selection/settings.
...
- ✅ Initial mapping for `tool_call`, `thinking`, and terminal errors to Disco messages/tool callbacks.
...
- ✅ Convert Disco session/global MCP server configs to Cursor `McpServerConfig`.
- ✅ Inject built-in Disco MCP with session bearer token.
...
- Decide how remote branches/PR URLs become Disco branch cards or artifacts.
...
- [ ] Run a live local smoke test with a throwaway Cursor key in an Disco worktree:
...
- [ ] Update `apps/disco-docs/pages/guide/sdk-comparison.mdx` only after behavior is empirically verified.

## Open questions for Max

1. Is an **autonomous/no-permission-modal** provider acceptable if clearly labeled and constrained by Disco Unix isolation?
...
4. Do we want to expose Cursor-native `.cursor/skills`/hooks/subagent affordances in Disco UI, or leave them as repo-native files only?
