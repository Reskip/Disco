# Task Completion Chime Sounds

This directory contains the MP3 files for task completion notifications.

## Included Files

All files have been downloaded from free sources under CC0 or permissive licenses.

1. **gentle-chime.mp3** (66KB)
   - Source: Pixabay
   - License: Free to use (no attribution required)
   - Pleasant success chime

2. **notification-bell.mp3** (34KB)
   - Source: Mixkit
   - License: Mixkit Free License
   - Clear notification bell

3. **8bit-coin.mp3** (20KB)
   - Source: Freesound.org
   - License: CC0 (public domain)
   - Classic 8-bit coin pickup sound

4. **retro-coin.mp3** (12KB)
   - Source: Freesound.org
   - License: CC0 (public domain)
   - Alternative retro coin sound

5. **power-up.mp3** (45KB)
   - Source: Freesound.org
   - License: CC0 (public domain)
   - Satisfying retro power-up sound

6. **you-got-mail.mp3** (2.3KB)
   - Source: Freesound.org
   - License: CC0 (public domain)
   - Classic AOL "You've Got Mail" notification

7. **success-tone.mp3** (14KB)
   - Source: Mixkit
   - License: Mixkit Free License
   - Short professional beep

## License Info

**Mixkit License**: Free to use for commercial and non-commercial purposes. No attribution required.
**Pixabay License**: Free to use for commercial and non-commercial purposes. No attribution required.

All sounds are safe to use in Disco without legal concerns.

## Testing

Test the sounds in the Disco UI:

1. Open Settings → Audio tab
2. Enable task completion chimes
3. Select a chime from the dropdown
4. Click "Preview" to test the sound
5. Adjust volume and minimum duration settings
6. Run a task and hear it complete!
