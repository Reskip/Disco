// biome-ignore-all lint/plugin/noHardcodedColorLiteral: centralized categorical avatar-tile palette (muted mid-tones, tuned to Disco) — not expressible as semantic theme tokens
...
type IdentityUser = Pick<User, 'user_id' | 'avatar_url' | 'name' | 'username'>;
...
/** Up to two uppercase letters from the user's display name, then username. */
export function getUserInitials(user?: Pick<User, 'name' | 'username'> | null): string {
...
// Muted, modern avatar palette (Mixpanel-ish, tuned to Disco). A centralized
...
export function getUserAvatarColor(
  user: Pick<User, 'user_id' | 'username' | 'name'> | null | undefined
): string {
  const key = user?.user_id || user?.username || user?.name || '';
  let hash = 0;
