...
// in real time. Per frame it calls window.__discoDemo.setTime(ms), waits a
...
//
// Requires the disco-ui dev server running (pnpm --filter disco-ui dev) and
...
const captureScene = async (browser, name, config, args) => {
  ...
  await page.waitForFunction(() => window.__discoDemo?.isReady() === true, { timeout: 60_000 });
  console.log(`[${name}] fixture ready; waiting ${config.floorMs}ms for Sandpack to paint`);
  await page.waitForTimeout(config.floorMs);

  const durationMs = await page.evaluate(() => window.__discoDemo.getDuration());
  ...
  for (const i of frames) {
    const ms = (i / FPS) * 1000;
    await page.evaluate((time) => window.__discoDemo.setTime(time), ms);
    await settle(page);
