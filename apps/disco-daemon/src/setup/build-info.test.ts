...
beforeEach(() => {
  tmpRoot = mkdtempSync(join(tmpdir(), 'disco-build-info-test-'));
  originalEnv = process.env.DISCO_BUILD_SHA;
  delete process.env.DISCO_BUILD_SHA;
});
...
afterEach(() => {
  ...
  if (originalEnv === undefined) {
    ...
    process.env.DISCO_BUILD_SHA = originalEnv;
  }
});
...
describe('loadBuildInfo', () => {
  it('prefers DISCO_BUILD_SHA env over everything else', () => {
    process.env.DISCO_BUILD_SHA = 'env12345';
    process.env.DISCO_BUILT_AT = '2026-04-24T12:00:00Z';
    ...
    delete process.env.DISCO_BUILT_AT;
  });
