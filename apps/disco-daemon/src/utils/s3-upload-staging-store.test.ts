...
import type { AgentID, SessionID, TenantID, UploadOwner, UserID } from '@disco/core/types';
import {
  DeleteObjectCommand,
  GetObjectCommand,
  HeadObjectCommand,
...
const owner = {
  ...
  agentId: '00000000-0000-4000-8000-000000000002' as AgentID,
...
describe('S3UploadStagingStore', () => {
  ...
  it('streams bytes under a tenant prefix and supports true ranged reads', async () => {
    ...
    expect([...client.objects.keys()][0]).toMatch(
      /^uploads\/disco\/tenants\/tenant-a\/uploads\/objects\/[0-9a-f]{2}\/upl_/
    );
