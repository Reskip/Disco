const DEBUG_INITIAL_LOAD_STORAGE_KEY = 'disco.debug.initialLoad';
...
type DebugWindow = Window & {
  __DISCO_INITIAL_LOAD_TIMINGS__?: InitialLoadDebugTimings;
};
...
export function createInitialLoadDebugTimer(items: readonly InitialLoadDebugItem[]) {
  ...
  return {
    ...
    finish(status: 'success' | 'error', error?: unknown): InitialLoadDebugTimings {
      ...
      const win = getWindow();
      if (win) {
        win.__DISCO_INITIAL_LOAD_TIMINGS__ = timings;
      }

      if (typeof console !== 'undefined') {
        const group = console.groupCollapsed ?? console.group;
        group?.call(console, '[Disco initial load]', {
        ...
        if (timings.error) {
          console.warn('[Disco initial load] failed', timings.error);
        }
        console.log('Copy from window.__DISCO_INITIAL_LOAD_TIMINGS__', timings);
        console.groupEnd?.();
      }
