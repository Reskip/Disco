...
const targetManifest = 'packages/disco-live/package.json';
const sourceManifests = [
  'packages/agentic-tool-opencode/package.json',
  'packages/agentic-tools/package.json',
  'packages/core/package.json',
  'packages/git/package.json',
  'apps/disco-cli/package.json',
  'apps/disco-daemon/package.json',
  'packages/executor/package.json',
];

// Internal workspace packages are materialized inside the disco-live tarball.
...
const targetOnlyDependencies = new Set(['@disco-live/client']);
...
if (mode === 'check') {
  if (updates.length) {
    console.error('packages/disco-live/package.json is missing dependency updates:');
    for (const update of updates) {
      console.error(` - ${update.dep}: expected ${update.to ?? '∅'}, found ${update.from ?? '∅'}`);
    }
    console.error('Run pnpm sync:disco-live-deps to fix.');
    process.exit(1);
  }
  console.log('disco-live dependencies are in sync.');
  process.exit(0);
}

if (!updates.length) {
  console.log('disco-live dependencies already match workspace manifests.');
  process.exit(0);
}
