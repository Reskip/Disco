...
**Scope:** every `simple-git` call site, `git`-CLI shell-out, and `fs.rm`/`fs.read` against `~/.disco/repos/` or `~/.disco/worktrees/` inside `apps/disco-daemon/src/**`.
...
- **0 must-stay-in-daemon** git operations on managed dirs. Every read or write against `~/.disco/repos/` or `~/.disco/worktrees/` has a viable executor replacement; the "chicken-and-egg" cases (initial clone, branch lifecycle, archive/delete) **already** spawn the executor — they're not violations.
- **0 direct `simple-git` imports in the daemon.** The wrapper is in `packages/core/src/git/index.ts` (1689 lines, one canonical `import { simpleGit }`). Daemon reaches simple-git only transitively via `@disco/core/git` exports. **This reframes the experiment** (see §4) — the move is not "daemon → executor," it is "daemon stops importing the simple-git-backed exports of `@disco/core/git`," and we never had to touch a `package.json`.
- **Experiment confirms scope (§4).** Aliasing the daemon's simple-git-backed imports to non-existent names and running `pnpm typecheck` from `apps/disco-daemon/` produced **exactly 11 errors in exactly the 5 files** named in §2-B, on exactly the 9 symbol-import sites. No hidden transitive uses. No spillover to executor / CLI / core. Working tree was reset before commit.
...
The target end-state, per Max: **the daemon never touches `~/.disco/repos/` or `~/.disco/worktrees/`. The executor does.** The daemon's job is database / auth / API / WebSocket / orchestration. The executor's job is filesystem and git, inside the uid namespace appropriate for the branch.

Concretely:

- Daemon owns: `~/.disco/config.yaml`, `~/.disco/disco.db`, REST/WebSocket, FeathersJS services, MCP HTTP, executor spawns. Pure-string git helpers (URL parsing, slug derivation, ref validation, env-var construction) are fine in daemon — they don't touch disk.
- Executor owns: every read or write inside `~/.disco/repos/<slug>/` or `~/.disco/worktrees/<repo>/<branch>/`. Spawned per-task via `spawnExecutor` / `spawnExecutorFireAndForget` (`apps/disco-daemon/src/utils/spawn-executor.ts`), runs under the appropriate Unix uid (`asUser`-resolved by RBAC mode).
...
This audit applies the rule **call-site by call-site**, not import-by-import. A daemon file that imports `extractRepoName` from `@disco/core/git` is fine; one that imports `getGitState` is not, because the latter shells out under daemon uid.

---

## 2. Inventory — every daemon-side import of `@disco/core/git`

Static analysis (`git grep "from '@disco/core/git'" apps/disco-daemon/src/`):

| File:line                                                       | Imported symbol                                      | Reaches `simple-git`?                         | Touches `~/.disco/repos`/`worktrees`?                                           | Category                  |
| --------------------------------------------------------------- | ---------------------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------- |
| `apps/disco-daemon/src/index.ts:41`                              | `buildGitConfigParameters`                           | No — pure string assembly                     | No                                                                             | **C — fine**              |
| `apps/disco-daemon/src/services/users.ts:32`                     | `isLikelyGitToken`                                   | No — regex                                    | No                                                                             | **C — fine**              |
| `apps/disco-daemon/src/services/repos.ts:30-36`                  | `extractRepoName`, `getBranchPath`, `getReposDir`    | No — pure strings                             | No                                                                             | **C — fine**              |
| `apps/disco-daemon/src/services/repos.ts:32-36`                  | `getDefaultBranch`, `getRemoteUrl`, `isValidGitRepo` | Yes (via `createGit`)                         | Path is **user-supplied** (local repo registration only)                       | **D — ambiguous, see §3** |
| `apps/disco-daemon/src/services/repos.ts:1049` (dyn import)      | `deleteRepoDirectory`, `deleteBranchDirectory`       | No simple-git; uses `fs.rm`                   | **YES — rm -rf inside `~/.disco/repos/<slug>` and `~/.disco/worktrees/<r>/<b>`** | **B — move**              |
| `apps/disco-daemon/src/services/zone-trigger.ts:87` (dyn import) | `getGitState`, `getCurrentBranch`                    | Yes                                           | **YES — `branch.path`**                                                        | **B — move**              |
| `apps/disco-daemon/src/services/files.ts:10`                     | `createGit` → `git.raw(['ls-files', '-z'])`          | Yes                                           | **YES — `branch.path`**                                                        | **B — move (hot path)**   |
| `apps/disco-daemon/src/mcp/tools/sessions.ts:773` (dyn import)   | `getGitState`, `getCurrentBranch`                    | Yes                                           | **YES — `branch.path`**                                                        | **B — move**              |
| `apps/disco-daemon/src/utils/realign-repo-origin.ts:3,31`        | `ensureGitRemoteUrl`                                 | Yes (`git config --get-all`, `--replace-all`) | **YES — `repo.local_path = ~/.disco/repos/<slug>`, writes `.git/config`**       | **B — move**              |

Daemon shell-outs to `git` (`grep -E "spawn\('git'\|execFileSync\('git'\|execSync\('git\b\|exec\('git\b"`):

| File:line                                     | Command                      | Cwd                                              | Category                                                                                |
| --------------------------------------------- | ---------------------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------- |
| `apps/disco-daemon/src/setup/build-info.ts:73` | `git rev-parse --short HEAD` | The daemon's **own** source dir (cwd at startup) | **A — must stay**: this is the daemon stamping its own build SHA. Not the managed dirs. |

Daemon `child_process` usage that is **not** a git shell-out:

- `apps/disco-daemon/src/utils/spawn-executor.ts:24` — spawns the executor itself (correct).
- `apps/disco-daemon/src/services/terminals.ts:21,654` — Zellij PTY spawn for the web terminal.
- `apps/disco-daemon/src/utils/unix-group-init.ts:14` — `groupadd` / `chgrp` / `setfacl` via sudoers.
...
### 2.1 Filesystem path uses (`getReposDir`, `getBranchesDir`, `getBranchPath`, literal `'.disco/worktrees'`)

| File:line                                                 | Use                                                                                | Touches dir?                                              |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `apps/disco-daemon/src/services/repos.ts:237`              | `path.join(getReposDir(), expectedRepoName)` for the placeholder DB row            | No — just builds a string                                 |
| `apps/disco-daemon/src/services/repos.ts:661`              | `getBranchPath(repo.slug, data.name)` for the placeholder DB row                   | No — string                                               |
| `apps/disco-daemon/src/services/branches.ts:556, 586, 724` | Comments mentioning `getBranchesDir()` (explaining why daemon doesn't impersonate) | No — comments only                                        |
| `apps/disco-daemon/src/services/branches.ts:709`           | `existsSync(branch.path)` to gate unarchive                                        | **Yes — read-only probe**                                 |
| `apps/disco-daemon/src/services/terminals.ts:657`          | `fs.existsSync(symlinkPath)` to pick web-terminal cwd                              | Probe of the per-user symlink, not the managed dir itself |
| `apps/disco-daemon/src/mcp/tools/repos.ts:157`             | Literal `~/.disco/repos/<slug>` in a docstring                                      | No — docstring                                            |
...
| Call site                                                                                                                | Why                                                                                                                                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/disco-daemon/src/setup/build-info.ts:73` (`git rev-parse --short HEAD` against daemon source)                       | Daemon is stamping its own build identity. No executor exists yet, no managed dir involved. Already constrained to `currentDir` (the daemon's installed source).                                          |
| `apps/disco-daemon/src/index.ts:155` (`buildGitConfigParameters`)                                                         | Pure string assembly to set `GIT_CONFIG_PARAMETERS` env var for every subsequent spawn. Doesn't spawn git, doesn't read disk. Must happen daemon-side because it sets the env that the executor inherits. |
...
- Initial repo clone: `services/repos.ts:261` spawns `git.clone`. Daemon never invokes `cloneRepo()` from `@disco/core/git` directly — the import at line 30 is unused at runtime (`extractRepoName` from the same group is what's actually called); confirmed by `grep cloneRepo apps/disco-daemon/src` returning only comments.
...
1. **The wrapper is in `packages/core`, not `apps/disco-daemon`.** `simple-git` is declared as a dependency of `@disco/core` only (`packages/core/package.json`) and of the `disco-live` meta-package that bundles the published binary. The daemon never imports `simple-git` directly — confirmed by `git grep "from 'simple-git'"` returning four files, all under `packages/core/src/git/`. The proposed experiment "remove simple-git from daemon's package.json" is a no-op because it isn't there.
2. **Removing it from `@disco/core` would cascade-break the executor too,** which is exactly what we want to keep.

So instead I did the variant the brief explicitly authorized: **break exactly the daemon imports of simple-git-backed symbols, leave executor / CLI / core-internal alone, run typecheck, count errors.** Implementation: in each of the five daemon files identified in §2-B, I aliased the simple-git-backed imports to `<symbol>__MOVED` (a name `@disco/core/git` does not export), then ran `pnpm typecheck` from `apps/disco-daemon/`. The edits were reverted before committing — `git status --short` shows only the new doc.
...
### Phase 4 — `getGitState` + `getCurrentBranch` at session create — resolved

Session-level Git state was removed. The prompt executor now owns the authoritative
pre/post Git snapshots on each task, so session creation performs no checkout read.

### Phase 5 — Mechanical enforcement: split `@disco/core/git` into `…/git/pure` + `…/git/exec`
...
- **Sketch:** physical file split + re-export shim + ESLint `no-restricted-imports` rule on `apps/disco-daemon/src/**`.
...
1. **`addLocalRepository` git reads (category D).** `getRemoteUrl`, `isValidGitRepo`, `getDefaultBranch` against a user-supplied path that is **not** inside `~/.disco/`. The dir-scope rule doesn't trigger, but the daemon-uid concern does. Defer or move? My read: defer — the user-supplied path predates any Disco uid namespace, so there's no obvious "right" uid to run as. Confirm.
...
3. **Phase ordering: leapfrog Phase 5?** If the answer to "should the daemon `@disco/core/git` import surface be lint-enforced?" is yes, Phase 5 could come first and force the rest. I lean against — splitting before the call sites move just adds re-export shims that get deleted in the same week.
...
- [`context/explorations/executor-expansion.md`](../../context/explorations/executor-expansion.md) — target executor command surface.
...
- On-disk paths still use `worktrees/` (`~/.disco/worktrees/<repo>/<branch>`). `storage_mode='worktree'` is still the enum literal. Both refer to the git-worktree primitive and are correct; do not "fix" these in a follow-up.
- If you re-grep `@disco/core/git` in daemon and find new imports beyond the eight in §2, classify the new symbol first (pure vs. simple-git-backed) — adding a pure helper import is fine, adding a simple-git-backed one is the regression.
- The `cloneRepo` import at `apps/disco-daemon/src/services/repos.ts:30` looks like a violation but is a false positive — the symbol is never called (only `extractRepoName` from the same import group is). Worth deleting in a janitorial pass, but separate PR.
