...
export function isInstalledPackage(): boolean {
  ...
  // Dev path: /Users/max/code/disco/apps/disco-cli/dist/...
  // Prod path: /usr/local/lib/node_modules/disco-live/dist/cli/...
  const isInMonorepoSource =
    dirname.includes('/apps/disco-cli/') || dirname.includes('\\apps\\disco-cli\\');
...
export function getDaemonPath(): string | null {
  ...
  // Find the disco-live package root by looking for dist/cli in the path
...
/**
 * Check if Disco has been initialized
 *
 * A valid Disco initialization requires:
 * 1. ~/.disco/ directory exists
 * 2. ~/.disco/disco.db database file exists
 *
 * @returns true if Disco is initialized, false otherwise
 */
export async function isDiscoInitialized(): Promise<boolean> {
  try {
    const discoDir = path.join(homedir(), '.disco');
    const dbPath = path.join(discoDir, 'disco.db');

    // Check if both directory and database exist
    await access(discoDir, constants.F_OK);
