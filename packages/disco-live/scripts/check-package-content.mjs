...
const tarball = process.argv[2]
  ...
  : join(scriptRoot, 'release', `disco-live-${version}.tgz`);
try {
  const extractRoot = mkdtempSync(join(tmpdir(), 'disco-live-content-extract-'));
  try {
    ...
    if (failures.length) {
      throw new Error(`disco-live package-content budget exceeded:\n- ${failures.join('\n- ')}`);
    }
  } finally {
