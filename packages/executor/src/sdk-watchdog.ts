import { getAgenticToolIntegration } from '@disco/agentic-tools';
import type { ResolvedSdkWatchdogConfig } from '@disco/core/config';
import type { AgenticToolName, ExecutorPulseKind, SdkHealthFailureInput } from '@disco/core/types';
import { hasDiscoAbortCause, markDiscoAbortCause } from './termination-state.js';
...
export function markSdkHealthAbort(controller: AbortController): void {
  markDiscoAbortCause(controller, 'sdk_health_failure');
  controller.abort();
}

export function isSdkHealthAbort(controller: AbortController): boolean {
  return hasDiscoAbortCause(controller, 'sdk_health_failure');
}
