import type { Agent, DiscoClient, Schedule } from '@disco-live/client';
import {
...
vi.mock('../AgentSelectionGrid', () => ({
  ...
  AgentSelectionGrid: ({
    ...
    <div
      ...
      data-selected-agent={selectedAgentId ?? ''}
      data-fallback-enabled={fallbackToFirstVisibleAgent ? 'true' : 'false'}
    />
  ),
}));
...
describe('ScheduleModal agentic configuration payload', () => {
  ...
  it('defaults a new plan to Codex instead of an unconfigured Claude runtime', async () => {
    const client = {
      service: () => ({
        patch: vi.fn(),
        create: vi.fn(),
    ...
    expect(await screen.findByTestId('agent-grid')).toHaveAttribute('data-selected-agent', 'codex');
    expect(screen.getByTestId('agent-grid')).toHaveAttribute('data-fallback-enabled', 'true');
  });
  ...
  it('preserves a removed tool as historical configuration until explicitly replaced', async () => {
    ...
    expect(await screen.findByText('这个计划任务使用了已移除的执行工具')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /保\s*存/u })).toBeDisabled();
  ...
  it.each([
    ...
    expect(patch.mock.calls[0][1]).not.toHaveProperty('agent_id');
