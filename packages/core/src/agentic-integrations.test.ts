...
const originalEnv = {
  DISCO_AGENTIC_TOOLS_DIR: process.env.DISCO_AGENTIC_TOOLS_DIR,
  DISCO_MANAGED_AGENTIC_TOOLS: process.env.DISCO_MANAGED_AGENTIC_TOOLS,
  DISCO_VERSION: process.env.DISCO_VERSION,
};
...
async function createFakeManagedClaude(version: string): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'disco-agentic-tools-'));
  temporaryDirectories.push(root);
  process.env.DISCO_AGENTIC_TOOLS_DIR = root;
  process.env.DISCO_MANAGED_AGENTIC_TOOLS = '1';
  process.env.DISCO_VERSION = version;
  ...
  await writeFile(
    ...
    JSON.stringify({ name: '@disco-live/claude', type: 'module', exports: './index.js' })
  );
  await writeFile(
...
describe('managed agentic tool loading', () => {
  it('creates a minimal exact integration manifest with no lifecycle policy', () => {
    expect(createManagedAgenticToolInstallManifest('opencode', '1.2.3')).toEqual({
      name: 'disco-managed-opencode',
      version: '0.0.0',
      private: true,
      dependencies: { '@disco-live/opencode': '1.2.3' },
    });
  });
  ...
  it('loads only the version-aligned package from the managed directory', async () => {
    ...
    await writeFile(
      ...
      "export const DISCO_INTEGRATION_VERSION = '1.2.3'; export const sdk = { marker: 'managed' };"
    );
  ...
  it('rejects a package built for another Disco version', async () => {
    const packageDirectory = await createFakeManagedClaude('1.2.4');
    await writeFile(
      join(packageDirectory, 'index.js'),
      "export const DISCO_INTEGRATION_VERSION = '1.2.3'; export const sdk = {};"
    );

    await expect(loadManagedAgenticToolSdk('claude-code')).rejects.toThrow(
      'does not match Disco 1.2.4'
    );
  });
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await writeFile(
        join(packageDirectory, 'index.js'),
        "export const DISCO_INTEGRATION_VERSION = '1.2.6'; export const sdk = {};"
      );
      const vendorDirectory = join(
        packageDirectory,
        '..',
      ...
      const outside = await mkdtemp(join(tmpdir(), 'disco-agentic-vendor-'));
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await writeFile(
        join(packageDirectory, 'index.js'),
        "export const DISCO_INTEGRATION_VERSION = '1.2.7'; export const sdk = {};"
      );
      const outside = await mkdtemp(join(tmpdir(), 'disco-agentic-runtime-'));
      temporaryDirectories.push(outside);
  ...
  it('returns an actionable error when support is absent', async () => {
    ...
    process.env.DISCO_AGENTIC_TOOLS_DIR = root;
    process.env.DISCO_MANAGED_AGENTIC_TOOLS = '1';
    process.env.DISCO_VERSION = '1.2.5';
...
describe('agentic tool selection policy', () => {
  ...
  it('distinguishes a missing local manifest from an invalid one', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-agentic-policy-'));
    temporaryDirectories.push(root);
    process.env.DISCO_AGENTIC_TOOLS_DIR = root;
...
describe('configured agentic tool startup readiness', () => {
  ...
  it('requires an intentional policy instead of starting with an implicit empty selection', async () => {
    ...
    await expect(
      assertConfiguredAgenticToolsReady(
        {},
        { DISCO_MANAGED_AGENTIC_TOOLS: '1', DISCO_VERSION: '1.2.3' }
      )
    ).rejects.toThrow(/disco install[\s\S]*agentic_tools\.installed: \[\]/);
  });
  ...
  it('fails an upgraded selected tool with the exact repair command', async () => {
    ...
    ).rejects.toThrow(/Codex: missing or invalid[\s\S]*disco install --sync/);
  });
