# Disco Repo Health Audit — 2026-05-15
...
2. Drop `apps/disco-daemon/src/services/context.ts` (fully `@deprecated`, "use file service") + the three stacked deprecated aliases at `apps/disco-daemon/src/utils/spawn-executor.ts:571,576,590`.
3. Strip the `🔐 Authentication attempt:` payload-dump `console.log` at `apps/disco-daemon/src/register-routes.ts:353` — security + cleanup two-for-one (may be leaking auth payload contents to stdout).
...
2. **Remove `// @ts-nocheck` from `apps/disco-ui/src/hooks/useDiscoData.ts`** (1 244 LOC of WebSocket event handling currently un-typed). Worst hot-spot in the UI by blast-radius — replaces an `as any` bucket with proper event-payload types from `@disco-live/client`. Also split by entity domain.
...
3. **Onboarding consolidation** — three "quickstarts" (README's `npm i -g disco-live`, CONTRIBUTING's `docker compose up`, AGENTS.md's `pnpm dev` in two terminals) and the fastest dev loop is buried in agent-only docs. Pick one local-dev path, fold AGENTS.md's troubleshooting into CONTRIBUTING.md, and delete the rest.
...
| **Refactor**            | **B-** | Three god-files in the daemon (~8.2 k LOC combined). `useDiscoData.ts:1` carries `@ts-nocheck` over 1 244 LOC. `artifacts.ts` mixes five concerns in one 1 903-LOC service. `parseTruncationLength` copy/pasted byte-for-byte across two services.                                                                                                        |
...
| QW-2  | ⚪ partial | **Drop deprecated services & aliases** — delete `apps/disco-daemon/src/services/context.ts` (whole file `@deprecated`, use `file` service); remove 3 stacked deprecated aliases at `apps/disco-daemon/src/utils/spawn-executor.ts:571,576,590`; remove 5 deprecated `worktree.ts` types (`types/worktree.ts:592,779,807,824,840`). | daemon + core                               | Cleanup          |
| QW-3  | ✅         | **Strip auth-payload log** — `apps/disco-daemon/src/register-routes.ts:353` logs `🔐 Authentication attempt:` with the full payload. Security + cleanup.                                                                                                                                                                          | daemon                                      | Cleanup          |
| QW-4  | ✅         | **Drop unused npm deps** — `@webscopeio/react-textarea-autocomplete` (+ its `.d.ts`) in `apps/disco-ui`; `@iarna/toml` in `packages/{core,executor,disco-live}`; verify `@theguild/remark-mermaid` + `katex` in `apps/disco-docs`.                                                                                                  | various package.jsons                       | Dead code        |
| QW-5  | ✅         | **Delete duplicate type defs** — `apps/disco-cli/src/commands/{session,repo}/list.ts:15` both redefine `Paginated<T>`; replace with `PaginatedResult<T>` from `@disco/core`. Rename `SessionsService` interface in `packages/executor/src/sdk-handlers/claude/claude-tool.ts:146` so it stops shadowing the canonical type.        | cli + executor                              | Architecture     |
| QW-6  | ✅         | **Lift `parseTruncationLength`** — verbatim 22-line copy at `apps/disco-daemon/src/services/sessions.ts:102` and `apps/disco-daemon/src/services/worktrees.ts:58`. Move to `apps/disco-daemon/src/utils/parse-pagination.ts`.                                                                                                       | daemon                                      | Refactor         |
| QW-7  | ⚪         | **Enable executor + CLI tests in CI** — `.github/workflows/ci.yml` currently filters to `--filter=disco-ui --filter=@disco/daemon --filter=@disco/core`, leaving 25 executor tests + 2 CLI tests un-gated on PRs. Drop the filter.                                                                                                  | CI config                                   | DevEx            |
| QW-8  | ✅         | **Add basic Vite `manualChunks`** — `apps/disco-ui/vite.config.ts` has no chunking. Add a starter map (antd / reactflow / editor / syntax / emoji / particles / xterm) plus `chunkSizeWarningLimit: 1000`.                                                                                                                        | UI vite config                              | Build perf       |
| QW-9  | ✅         | **Fix CONTRIBUTING.md stale tooling reference** — line 62 says "ESLint + Prettier"; repo runs Biome. Plus `apps/disco-docs/pages/guide/development.mdx:108` mentions `eslint-linux`.                                                                                                                                              | docs                                        | DevEx            |
...
| QW-13 | ✅         | **Replace 2 stray `require()` in ESM** — `apps/disco-cli/src/commands/daemon/start.ts:194-195` and `packages/core/src/db/repositories/board-objects.test.ts:924`.                                                                                                                                                                 | cli + core                                  | Build perf       |
| QW-14 | ⚪         | **Split `Pill.tsx`** — `apps/disco-ui/src/components/Pill/Pill.tsx` is 1 107 LOC with 21 named pill exports. Mechanical split per pill into `Pill/<Name>Pill.tsx`.                                                                                                                                                                | UI                                          | Refactor         |
...
| M-2  | **Remove `@ts-nocheck` from `useDiscoData.ts`**                                                                                                  | 1 244 LOC of WebSocket handling; needs narrow event-payload types per channel from `@disco-live/client`. Split by entity domain (sessions/worktrees/boards) while doing it.                                                                                                                                                                  | Refactor        |
...
| M-8  | **Add `sideEffects: false` to `packages/core`** + audit UI's import graph for accidental root-barrel hits                                       | `packages/core/src/index.ts` is `export *` × 12; same for `db/index.ts` and `types/index.ts`. A small refactor of `db/index.ts:22-24` (the bcryptjs re-export) plus `sideEffects: false` would unblock real tree-shaking. Use `madge --circular` or `dpdm` to confirm no UI file imports `@disco/core` (root) instead of `@disco/core/types`. | Build perf      |
...
| D-2 | **`@disco/core` browser-safe surface**              | Root barrel hides Node-only modules (`bcryptjs`, `drizzle-orm`, `unix/*`, `simple-git`, `feathers`). Subpath exports already exist in `package.json`. Needs a written contract: "UI imports `@disco/core/types`, `@disco/core/client`, `@disco/core/config/browser`; never the root." Then a one-shot migration + lint rule.          |
...
| D-5 | **Onboarding canonical path**                      | Three competing quickstarts; the recurring AGENTS.md troubleshooting entries ("Method is not a function after editing @disco/core", `rm -rf node_modules/.tsx`) are symptoms of a tsup + tsx watch interaction that hasn't been root-caused. Solve that once = remove the troubleshooting section entirely.                         |
...
- **`apps/disco-docs` (Nextra/Next.js) bundle** — lower-priority surface, not deep-audited.
