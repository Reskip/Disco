...
describe('dateTruncUtc', () => {
  ...
  it('supports minute buckets for one-hour activity charts', () => {
    const fakePostgresDb = {} as Parameters<typeof dateTruncUtc>[0];
    const bucketExpr = dateTruncUtc(fakePostgresDb, tasks.created_at, 'minute');
    const rendered = new PgDialect().sqlToQuery(sql`select ${bucketExpr}`);

    expect(rendered.sql).toContain("date_trunc('minute'");
  });

  it('builds native thirty-second PostgreSQL buckets', () => {
    const fakePostgresDb = {} as Parameters<typeof dateTruncUtc>[0];
    const bucketExpr = dateTruncUtc(fakePostgresDb, tasks.created_at, '30s');
    const rendered = new PgDialect().sqlToQuery(sql`select ${bucketExpr}`);

    expect(rendered.sql).toContain('EXTRACT(EPOCH');
    expect(rendered.sql).toContain('/ 30');
  });
});
