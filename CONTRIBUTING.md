...
Thank you for your interest in contributing to Disco! 🚀

**📚 Complete contribution guide:** https://disco.live/guide/development
...
```bash
git clone https://github.com/preset-io/disco
cd disco
docker compose up
# Visit http://localhost:5173 → Login: admin@disco.live / admin
```
...
```bash
# Create a branch in Disco for your contribution
# Work on it using Disco sessions
...
| PR / main push that touches pnpm dependency configuration | `audit.yml`           | `pnpm audit` (CRITICAL production-tree advisories block; HIGH+ remains advisory)                                                                                                                          |
| PR push (publish-relevant paths only)                     | `disco-live-smoke.yml` | Pack the published tarball, install with `npm`, boot the daemon, curl `/ui/`                                                                                                                              |
| Nightly at 04:00 UTC                                      | `heavy-checks.yml`    | Node 22/24/25 install-compat matrix + full `disco-live-smoke` + `audit` (catches ambient CVEs against unchanged deps). On failure, auto-opens / comments on a tracking issue labelled `ci-nightly-failure` |
...
`disco-live-smoke.yml` if you only want to test the publish path on a
specific branch.
...
- **[GitHub Discussions](https://github.com/preset-io/disco/discussions)** - Ask questions, share ideas, get help
- **[GitHub Issues](https://github.com/preset-io/disco/issues)** - Report bugs, request features
...
By contributing to Disco, you agree that your contributions will be licensed under the [Business Source License 1.1](LICENSE).

---

**Thank you for considering contributing to Disco!**
...
**Remember: Use Disco to contribute to Disco!** Experience the tool, find the rough edges, and help us smooth them out.

Let's build the future of multiplayer AI development together. 🚀

---

## `.disco.yml` Configuration

Disco supports automatic environment configuration via `.disco.yml` in your repository root.

### Schema

Place a `.disco.yml` file in your repository root with the following structure:
...
1. Add `.disco.yml` to your repo root and commit it
2. Clone the repo in Disco → configuration auto-loaded ✨
...
4. Configuration loaded from `.disco.yml`

**Manual Export:**

1. Configure environment in the UI
2. Click "Export" button (⬆️ icon)
3. `.disco.yml` written to repository root
