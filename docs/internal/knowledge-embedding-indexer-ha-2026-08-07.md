...
These timings use the shared `@disco/core/coordination` helpers. No correctness
...
- **Mixed schema/app versions:** migration 0074 is deliberately non-rolling.
  ...
  `disco db migrate --offline-cutover`, then start only the new version. Normal
...
The PostgreSQL suite is environment-gated by `DISCO_TEST_POSTGRES_URL` and
`DISCO_DB_DIALECT=postgresql`; a missing disposable database must be reported as
