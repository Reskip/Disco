 ...
 *
 * Takes the raw `security` block from `~/.disco/config.yaml` and produces a
...
import type {
  ...
  DiscoSecuritySettings,
} from './types';

export { redactUrlUserinfo } from '../utils/url';
export type { DiscoGitConfigParametersSettings } from './types';
 ...
 *
 * These are designed to make every *bundled* Disco feature work out of the
 * box — notably authenticated Blob-backed attachments (`media-src`),
 * Sandpack-backed artifacts (`frame-src`, `worker-src`), and the FeathersJS
 * socket.io transport (`connect-src ws:/wss:`).
 *
...
function buildDefaultDirectives(opts: {
  ...
  return {
    ...
    'img-src': imgSrc,
    // Authenticated audio/video attachments are fetched as Blobs so the
    // browser never needs to put a bearer token in a media URL.
    'media-src': ["'self'", 'blob:'],
...
export interface ResolvedCsp {
   ...
   * choice; otherwise it's the built-in default (`disco-csp`). Undefined when
...
export interface ResolvedCors {
  /** Resolution strategy (see DiscoCorsMode). */
  mode: DiscoCorsMode;
...
function validateDirectives(
  directives: DiscoCspDirectives | undefined,
...
function mergeDirectives(
  defaults: DiscoCspDirectives,
  extras: DiscoCspDirectives | undefined,
  override: DiscoCspDirectives | undefined
): DiscoCspDirectives {
  const result: DiscoCspDirectives = {};
 ...
 */
function serializeCsp(directives: DiscoCspDirectives): string {
  return Object.entries(directives)
    .map(([name, sources]) => (sources.length === 0 ? name : `${name} ${sources.join(' ')}`))
    .join('; ');
}

function parseCorsEnv(value: string): { mode: DiscoCorsMode; origins: string[] } | null {
...
export function resolveSecurity(
  ...
  const security: DiscoSecuritySettings = config.security ?? {};
  ...
  if (csp.report_uri) {
    ...
    if (!merged['report-to']) {
      merged['report-to'] = ['disco-csp'];
      reportToGroup = 'disco-csp';
    } else {
  ...
  let mode: DiscoCorsMode = corsSettings.mode ?? 'list';
...
// ============================================================================
// Defaults + resolver semantics; the env-var encoding lives in @disco/core/git/pure.
...
export function resolveGitConfigParameters(
  configured: DiscoGitConfigParametersSettings | undefined
): readonly string[] {
