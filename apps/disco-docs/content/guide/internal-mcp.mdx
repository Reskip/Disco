---
title: Disco MCP Server
description: Agents are first-class users in Disco. The built-in MCP server gives them the same surface area you have in the UI (sessions, branches, boards, zones, users, environments) as structured JSON-RPC.
...
<div style={{ marginTop: '20px', marginBottom: '30px', display: 'flex', justifyContent: 'center' }}>
  <img
    ...
    alt="Claude Code discovering Disco MCP capabilities"
    style={{ width: '500px', maxWidth: '100%', borderRadius: '8px' }}
  />
</div>

**Anything a user can do in Disco, an agent can do too.** The same daemon that serves the UI's REST and WebSocket APIs also exposes a built-in **Model Context Protocol (MCP) server**, a structured automation surface that turns every session into a self-aware orchestrator.
...
> **The other doorway:** if you're driving Disco from outside an agent (TS/JS scripts, custom dashboards, internal services), reach for the official [`@disco-live/client`](/guide/typescript-client) instead. Same daemon, same types, REST + Socket.IO + a reactive session API.

## Key Ideas

- **Agents as first-class users:** Every MCP tool wraps a FeathersJS service. Sessions, branches, boards, zones, users, environments. Anything you can click or reach via the `disco` CLI is available as JSON-RPC.
...
- **Remote access:** External agents and dashboards can connect over HTTP to drive Disco (create boards, branches, or even invite new users).

> MCP is enabled by default and every session already has credentials injected. No setup required for in-Disco agents.
...
- `disco_search_tools` browses domains or searches concise tool summaries.
- `disco_get_tool_details` returns the exact schema for one selected tool.
- `disco_execute_tool` invokes the selected domain tool with validated arguments.

The MCP `2026-07-28` revision adds `server/discover` for protocol capabilities
and standard cache hints for tool lists. It does **not** replace Disco's semantic
catalog search, domain filters, annotation filters, or detail-on-demand flow.
Disco keeps those features while using the standard discovery response and
private 60-second cache hints for its deterministic visible catalog.

## Self-Aware Agents Inside Disco

When you launch a session in Disco, the SDK config includes the internal MCP server automatically:
...
`disco_environment_start`, `disco_environment_stop`, `disco_environment_logs`, and
`disco_environment_nuke` reject rendered shell commands and only invoke explicit
...
`https://<your-disco-host>/mcp`, or `http://<daemon-host>:3030/mcp` for a
default self-hosted daemon. In Disco, open **User Settings → Personal API Keys**,
create a key, copy it once, and store it in a password manager or your shell's
secret-loading mechanism:

```bash
export DISCO_API_KEY='…'
```

Disco authenticates personal keys with the `X-API-Key` header. Never paste a
...
```bash
claude mcp add -s user -t http disco-sandbox https://disco.sandbox.preset.zone/mcp \
  -H 'X-API-Key: ${DISCO_API_KEY}'
```

The single quotes are important: your shell passes the placeholder rather than
the secret. Claude Code stores `${DISCO_API_KEY}` in its user configuration and
...
```toml
[mcp_servers.disco]
url = "https://disco.sandbox.preset.zone/mcp"
env_http_headers = { "X-API-Key" = "DISCO_API_KEY" }
```
...
An external personal API key identifies you but does not imply an Disco session.
Most tools accept explicit IDs. To make current-session tools work, add
`X-Disco-Session-Id: <session-id>` as another header. The session must be visible
to your user. Do not copy Disco's short-lived internal session JWT into an
external client; those tokens are injected automatically into sessions that
Disco launches.

That is the key distinction: **inside an Disco session**, the endpoint, scoped
...
Disco's built-in endpoint uses the stable TypeScript MCP SDK v2 in a dual-era,
...
- Neither era receives `Mcp-Session-Id`. Disco does not retain a transport Map
...
- Authentication, trusted tenant identity, current user data, and optional
  `X-Disco-Session-Id` access are reconstructed and authorized on every request.

This contract applies only to Disco's built-in endpoint. MCP servers that users
configure under **Settings → MCP Servers** remain separate external services;
Disco still passes their configured stdio, Streamable HTTP, or legacy SSE
transports directly to the selected executor.

Version selection is protocol-driven rather than an Disco-specific client
...
Ask the client to call `disco_search_tools` with no arguments, then
`disco_boards_list` with `{ "limit": 1 }`. A successful response should show
tool domains and one accessible board page.

- **401:** confirm `DISCO_API_KEY` exists in the environment that launched the
...
- The built-in endpoint retains no transport context. Every POST resolves the
  tenant, authenticates and reloads the user, and authorizes any optional Disco
...
- **Keep permission policies tight.** MCP calls respect Disco’s permission system. Configure approvals so agents only do what the team expects.
...
MCP session tokens are short-lived JWTs (aud `disco:mcp:internal`) embedding
...
- [Teammates](/guide/teammates) (long-lived AI teammates that drive Disco through MCP)
- [Scheduler](/guide/scheduler) (cron-style triggers that fire prompts via the same surface)
- [TypeScript Client](/guide/typescript-client) (the non-agent doorway: drive Disco from JS/TS apps, scripts, and services)
- [Architecture: Disco as an MCP Server](/guide/architecture#disco-as-an-mcp-server)
