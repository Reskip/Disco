...
import type { Session, SessionID } from '@disco/core/types';
...
function createSessionData(overrides?: Partial<Session>): Partial<Session> {
  return {
    ...
    working_directory: `/tmp/session-env-${generateId()}`,
...
async function setup(db: any) {
  ...
  const otherSession = await sessionRepo.create(createSessionData());
