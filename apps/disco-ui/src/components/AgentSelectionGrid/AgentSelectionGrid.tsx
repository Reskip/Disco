...
import type { TenantAgenticToolName } from '@disco-live/client';
...
export const AgentSelectionGrid: React.FC<AgentSelectionGridProps> = ({
  ...
  const settings = useDiscoStore((state) => state.agenticToolSettingsByName);
  const settingsHydrated = useDiscoStore((state) => state.agenticToolSettingsHydrated);
        ...
        {visibleAgents.length === 0 && (
          <Alert
            ...
            title={
              isChinese
                ? '当前工作区未启用任何执行工具'
                : 'No agentic tools are enabled for this workspace'
            }
            description={
              isChinese
                ? '请联系工作区管理员启用执行工具。'
                : 'Contact a workspace administrator. Deployment package changes require a separate deployment operator.'
            }
...
const ComparisonLink: React.FC = () => (
  ...
  >
    Compare features:{' '}
    <a href="https://disco.live/guide/sdk-comparison" target="_blank" rel="noopener noreferrer">
