...
import { getDefaultConfig, loadConfigSync } from '@disco-live/client/config';
import react from '@vitejs/plugin-react';
import { defineConfig } from 'vite';
import viteCompression from 'vite-plugin-compression';

// Load Disco config to get daemon port
const discoConfig = (() => {
  try {
...
const daemonPort = process.env.VITE_DAEMON_PORT
  ...
  : discoConfig.daemon?.port || defaults.daemon?.port || 3030;
...
export default defineConfig({
  ...
  server: {
    ...
    watch: {
      // Watch the @disco-live/client package for changes
      ignored: ['!**/node_modules/@disco-live/client/**'],
    },
    fs: {
