...
const adminUrl = process.env.DISCO_TEST_POSTGRES_ADMIN_URL;
if (process.platform !== 'linux') {
  console.log('PostgreSQL runner interruption harness is Linux-only; skipping process inspection.');
  process.exit(0);
}
if (
  process.env.DISCO_DB_DIALECT !== 'postgresql' ||
  !process.env.DISCO_TEST_POSTGRES_URL ||
  !adminUrl
) {
  console.error(
    'The interruption harness requires DISCO_DB_DIALECT, DISCO_TEST_POSTGRES_URL, and DISCO_TEST_POSTGRES_ADMIN_URL.'
  );
  process.exit(1);
}

const runId = `interrupt_${Date.now().toString(36)}_${process.pid.toString(36)}`;
const marker = `disco-postgres-interruption-${runId}`;
const reportPrefix = `disco-postgres-tests-${runId}-`;
let output = '';
let signaled = false;
let triggerSignal;
...
const child = spawn(process.execPath, ['scripts/run-postgres-integration-tests.mjs'], {
  ...
  env: {
    ...process.env,
    DISCO_POSTGRES_INTERRUPTION_PROBE: marker,
    DISCO_POSTGRES_TEST_CONCURRENCY: '1',
    DISCO_POSTGRES_TEST_RUN_ID: runId,
  },
...
function markedProcessIds() {
  const needle = Buffer.from(`DISCO_POSTGRES_INTERRUPTION_PROBE=${marker}\0`);
  const matches = [];
...
try {
  databases = await admin`
    SELECT datname
    FROM pg_database
    WHERE datname LIKE ${`disco_test_${runId}_%`}
  `;
} finally {
  await admin.end();
}
