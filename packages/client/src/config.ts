...
import type { DiscoConfig } from '@disco/core/client';
...
export function loadConfigSync(): DiscoConfig {
  const configPath = path.join(homedir(), '.disco', 'config.yaml');
  let content: string;
  ...
  try {
    ...
    // Disco's executor sandbox masks the daemon config with a `/dev/null` bind
  ...
  try {
    const config = yaml.load(content) as DiscoConfig;
    return config || {};
  } catch (error) {
    throw new Error(
