...
Disco daemon HA is an **explicit, constrained deployment mode**. Independently started daemon replicas share PostgreSQL as the durable state and coordination plane and Redis as the Socket.IO/Feathers notification plane. Redis is not a task lock, tenant authority, or durable event log.

The accepted profile is `constrained-active-active`. Adding `REDIS_URL` to a standalone daemon does nothing: HA activates only with `deployment.mode: ha` (or `DISCO_DEPLOYMENT_MODE=ha`) and fails startup unless every required assertion passes.

> The checked-in runnable variant is static single-tenant. Realtime publication is tenant-qualified and has adversarial cross-tenant contract tests, but `required_from_auth` multi-tenant HA is not yet operationally certified.
...
- identical, explicitly supplied `DISCO_JWT_SECRET` and `DISCO_MASTER_SECRET` values (32+ characters);
...
- `execution.managed_envs_execution_mode: webhook-only`; and
- one explicit execution topology.

### Executor storage contract

`shared_filesystem` is **not a blanket requirement for every HA or Disco Cloud deployment**. It is an assertion tied to `execution_topology`:

- `shared-local`: either daemon may launch a local executor against a branch working copy, so the working-copy/repository paths must exist at the same path on both daemon containers. The Compose smoke stack mounts `disco-ha-user-home` at `/home/disco` for CLI-native credential files and a nested, stable `disco-ha-home` at `/home/disco/.disco` for registered repositories and branch workspaces. Keeping the historical Disco volume name also preserves the correspondence between PostgreSQL absolute paths and files across variant upgrades. These volumes preserve files, not the executor process: this topology does not by itself guarantee that an executor survives loss of its container. `shared_filesystem: true` is required.
...
volumes or mounts; they tell Disco what the launcher or Compose topology guarantees for every
executor invocation:
...
into the registered base repository's `.git/worktrees` metadata. Disco therefore requires
...
Before WebSocket upgrade, Engine.IO may use multiple HTTP polling requests with one session ID. All `/socket.io/` requests for that logical connection must therefore reach the same daemon. The example nginx configuration uses consistent client-IP hashing for `/socket.io/`, forwards upgrade headers, and sets a 120-second proxy timeout (above Disco's 85-second heartbeat window). Ordinary HTTP/REST uses a separate round-robin upstream and is not sticky.
...
For Disco Cloud, configure the platform load balancer or ingress so that:
...
`docker-compose.ha.yml` uses explicitly named `daemon-a` and `daemon-b`, PostgreSQL, fanout-only Redis (RDB and AOF disabled), a one-shot preparation service that reconciles the declared agentic tools and then runs migrations, and nginx. It does not use `deploy.replicas`. Because the example has exactly one nginx ingress, nginx also provides a coarse shared per-IP limiter for authentication/refresh/launch paths and logs the selected upstream. That limiter proves the fleet choke-point topology; it is not a credential-aware production quota and would not be shared across multiple nginx replicas.

The checked-in HA smoke configuration declares every first-party managed agentic-tool integration so the harness can exercise each one under HA. Before either daemon starts, the preparation job runs `disco install --sync` into the shared `/home/disco/.disco` volume. Production deployments should instead declare the exact `agentic_tools.installed` list they support; changing that list requires rerunning preparation with package-registry access. Agentic-tool installation and user credentials are separate: users still need valid credentials for the selected integration.

```bash
export DISCO_REDIS_KEY_PREFIX=my-company-production
export DISCO_JWT_SECRET="$(openssl rand -hex 32)"
export DISCO_MASTER_SECRET="$(openssl rand -hex 32)"
export DISCO_ADMIN_PASSWORD="$(openssl rand -base64 24)"
export HA_PORT=3030
export DISCO_HA_PUBLIC_ORIGIN=http://localhost:3030

docker compose -f docker-compose.ha.yml -p disco-ha up -d --build
docker compose -f docker-compose.ha.yml -p disco-ha ps
curl -fsS http://localhost:3030/livez
curl -fsS http://localhost:3030/readyz
docker compose -f docker-compose.ha.yml -p disco-ha logs --tail=150 daemon-a daemon-b redis ingress
```

`DISCO_HA_PUBLIC_ORIGIN` must be the exact scheme, host, and port that browsers use for the
ingress. The Compose file applies it as both the daemon public base URL and the credentialed CORS
origin on every replica. The checked-in `.disco.yml` `ha` variant renders this value from Disco's
host address and branch port automatically.

The Disco-managed `ha` variant is intentionally a disposable development environment and uses the
same `admin@disco.live` / `admin` login as the other development variants. The Compose file enables
...
`DISCO_ADMIN_PASSWORD` as shown above.

Compose volumes survive `up` and ordinary Disco environment stop/start operations. In particular,
`DISCO_ADMIN_PASSWORD` is bootstrap-only: changing it after the users table exists does not reset
...
HA startup requires an explicit `DISCO_ADMIN_PASSWORD` unless external-launch authentication owns
...
daemon:
  ...
  public_url: https://disco-daemon.internal.example
```
...
stack declares all three surfaces `shared`: `disco-ha-user-home` mounts `/home/disco`, while the
historical `disco-ha-home` is nested at `/home/disco/.disco` so PostgreSQL workspace paths remain
valid across upgrades. Short-lived auth-file helpers and Tasks on either daemon therefore see the
same CLI and Disco workspace state. This is one shared Unix execution identity, not per-user
isolation. Codex import/logout may use it with the same simple-mode credential-sharing caveat as
standalone; the process-local device-code poller remains gated.
...
`DISCO_DEPLOYMENT_MODE`, `DISCO_HA_SUPPORT_PROFILE`, `DISCO_HA_EXECUTION_TOPOLOGY`,
`DISCO_HA_SHARED_FILESYSTEM`, `DISCO_HA_INGRESS_AFFINITY`, `REDIS_URL`,
`DISCO_REDIS_KEY_PREFIX`, the `DISCO_REDIS_*_MS` timeout/backoff variables, and
`DISCO_HA_ENV_HEALTH_SCAN_INTERVAL_MS`, `DISCO_HA_ENV_HEALTH_MAX_IDLE_INTERVAL_MS`,
`DISCO_HA_ENV_HEALTH_STARTUP_OFFSET_MAX_MS`, `DISCO_HA_ENV_HEALTH_SCAN_BATCH_SIZE`,
`DISCO_HA_ENV_HEALTH_MAX_IN_FLIGHT`, `DISCO_HA_ENV_HEALTH_HTTP_TIMEOUT_MS`,
`DISCO_HA_ENV_HEALTH_CLAIM_LEASE_MS`, and
`DISCO_HA_ENV_HEALTH_SHUTDOWN_DRAIN_TIMEOUT_MS`. The health claim lease
...
- Feathers channel membership is process-local and is not replicated by the adapter. Disco sends a bounded JSON relay envelope (`tenantId`, service path/event metadata, dispatched payload) with `serverSideEmit`. Each receiving daemon reruns tenant and branch/user publication logic against its own connections before entering the Feathers dispatcher.
...
- Native cross-replica packets in this profile are `cursor-moved`, `cursor-left`, `presence-updated`, tenant-scoped `repo:cloneError`, and tenant/branch terminal cleanup metadata. `server-info` is socket-local. OAuth packets are behind unsupported HA flows. Terminal input/output is always socket-local and must never enter Redis.
...
- Board-presence joins call the authenticated Feathers `boards.get` path before joining a tenant-qualified native room. Authentication replacement/logout revokes prior tenant/user/board room membership.
...
| Task run/prompt                                                                                   | Supported for Claude, Copilot, and OpenCode, whose Disco-managed interactive callbacks route to the live executor through its private tenant/task room. Cursor is currently autonomous. Gemini requires `yolo`; Codex requires approval policy `never`. Other provider-native interactive modes return `503 HA_FEATURE_UNSUPPORTED`. Handoff additionally requires an executor substrate that survives owner death and a fleet-routed callback URL; the Compose smoke stack does not prove handoff.                                     |
...
| MCP OAuth/callback and OAuth-capable MCP discovery, Codex device auth, OpenCode OAuth/native auth | Blocked in the constrained profile. MCP OAuth has a durable PostgreSQL prerequisite but remains gated pending separate activation; Codex device attempts still live in one daemon's timer/Map.                                                                                                                                                                                                                                                                                                                                         |
...
2. Apply all migrations before admitting new-version daemons. Migration `0077_environment_health_ha` adds branch observation leases/generations and the narrow discovery policy. Migration `0078_mcp_oauth_pending_flows` replaces legacy MCP OAuth grant/callback authority, and migration `0082_github_install_state` moves one-time GitHub setup state into PostgreSQL. Both callback migrations require `disco db migrate --yes --offline-cutover` on an existing database. Use an all-at-once cohort replacement; old daemons do not understand the new capability contracts or versioned Feathers relay envelopes, and mixed relay revisions intentionally do not exchange events.
...
4. Distribute identical stable config and secrets. Set stable `DISCO_DAEMON_INSTANCE_ID` values for diagnostics only.
...
8. Confirm an authenticated Feathers `health.find` on every replica reports `mode: ha`, `supportProfile: constrained-active-active`, the expected fine-grained capability booleans, distinct instance/boot diagnostics, and ready Redis clients. In particular, do not interpret `completionCallbackDurableAdmission: true` as pre-admission recovery or `gatewayListeners: true` as outbound exactly-once; the corresponding negative capability flags remain visible. `codexCredentialFiles` reflects the declared home consistency while `codexDeviceAuth` remains false. `environmentHealthMonitor: true` confirms that the fenced observer is advertised, and `githubInstall: true` confirms that the durable setup callback is admitted. The public HTTP `GET /health` intentionally exposes only the configured instance label and dependency summary.
