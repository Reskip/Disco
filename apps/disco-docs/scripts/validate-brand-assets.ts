...
const uiLogoPath = path.join(repoDir, 'apps/disco-ui/public/logo.svg');
const uiMarkPath = path.join(repoDir, 'apps/disco-ui/public/logo-mark.svg');
...
function validateSvg(pathname: string, label: string): string | null {
  ...
  if (!svg.includes('<title>Disco</title>')) {
...
for (const [docsAsset, uiAsset, label] of [
  ...
  if (!existsSync(uiAsset)) {
    fail(`disco-ui deployment copy is missing: ${uiAsset}`);
  } else if (existsSync(docsAsset) && !readFileSync(uiAsset).equals(readFileSync(docsAsset))) {
    fail(`disco-ui public/${label} must be byte-identical to the docs ${label}`);
  }
}
...
const retiredAssets = [
  ...
  'apps/disco-docs/public/favicon.png',
  'apps/disco-docs/public/logo.png',
  'apps/disco-ui/public/favicon.png',
];
