...
import type { Message, MessageID } from '@disco/core/types';
...
describe('widget MCP/service boundary', () => {
  ...
  it('uses the same final-ID invariant for gateway-token widgets', async () => {
    const fixture = makeBoundaryApp();
    const result = await callThroughMcp(fixture.app, 'disco_widgets_request_gateway_token', {
  ...
  it('does not publish a failed create and retries with a new final ID', async () => {
    ...
    const failed = await callThroughMcp(fixture.app, 'disco_widgets_request_env_vars', {
      names: ['HUBSPOT_API_KEY'],
      reason: 'call hubspot',
      auto_resume: true,
    });
    ...
    const retry = await callThroughMcp(fixture.app, 'disco_widgets_request_env_vars', {
      names: ['HUBSPOT_API_KEY'],
      reason: 'call hubspot',
      auto_resume: true,
    });
  ...
  it('keeps concurrent widget creates bound to their final IDs', async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 3 }, () =>
        callThroughMcp(fixture.app, 'disco_widgets_request_env_vars', {
          names: ['HUBSPOT_API_KEY'],
          reason: 'call hubspot',
          auto_resume: true,
        })
