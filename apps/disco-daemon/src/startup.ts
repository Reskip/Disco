...
import type { Id, Paginated, Session, SessionID, Task, TenantContext } from '@disco/core/types';
import { isTerminalTaskStatus, SessionStatus } from '@disco/core/types';
...
const DEBUG_STARTUP =
  process.env.DISCO_DEBUG_STARTUP === '1' || process.env.DEBUG?.includes('startup');
...
async function writeCleanShutdownSentinel(signal: string): Promise<void> {
  try {
    ...
    await fs.writeFile(
      path.join(getDiscoHome(), SENTINEL_FILENAME),
    ...
    // read-only DISCO_HOME deployments (e.g. ConfigMap-mounted) can see
...
async function readAndClearSentinel(): Promise<boolean> {
  const sentinelPath = path.join(getDiscoHome(), SENTINEL_FILENAME);
  try {
...
function startupTenantParams(config: DiscoConfig): { tenant: TenantContext } {
...
async function cleanupOrphanStatusesInTenantScope(
  ...
  // reconciler pass; startup must stay non-blocking.
  ...
  // conversation patches it false, conversation rows highlight while it's true —
  // see SessionPromptState in @disco/core/types), so it is the normal resting
...
async function injectRestartNoticesInTenantScope(
  ...
  const messageText = wasGraceful
    ? 'The Disco daemon was restarted while this session was running.'
    : 'The Disco daemon restarted unexpectedly while this session was running.';
...
export async function startup(ctx: StartupContext): Promise<void> {
  const { app, db, config, DAEMON_PORT, DAEMON_HOST, getSocketServer } = ctx;
  ...
  // 2. Start server. Deployment secrets were resolved before service
  ...
  console.log(
    `🚀 Disco daemon running at http://${displayHost}:${DAEMON_PORT} (bound to ${DAEMON_HOST})`
  );
  console.log(
    `   health=/health auth=required services=/sessions,/tasks,/messages,/agents,/mcp-servers,/users`
  );
  ...
  runPostStartJob('host-ip-log', async () => {
    const { resolveHostIpAddress } = await import('@disco/core/utils/host-ip');
