...
- Services: `apps/disco-daemon/src/services/knowledge-documents.ts`, `knowledge-versions.ts`, `knowledge-search.ts`, `knowledge-graph.ts`
- MCP tools: `apps/disco-daemon/src/mcp/tools/knowledge.ts`
- UI: `apps/disco-ui/src/pages/KnowledgePage.tsx`
...
- `disco_kb_get`: reads a document by id/URI/namespace+path. It includes full markdown content by default.
- `disco_kb_put`: creates or updates a markdown document. The `content` input is the full markdown body for the new version.
- `disco_kb_history`: lists immutable versions; can include full historical content.
- `disco_kb_search`, namespace tools, and graph tools.

`disco_kb_put` has an `expectedVersion` optimistic concurrency argument, but the caller must still send the complete replacement content. For a large document where only one line changes, today an agent likely has to fetch the full document into context and post the full replacement back.
...
`disco://kb/document/<id>` or `disco://kb/<namespace>/<path>` create document
`references` edges on save. Links to other Disco object schemes such as
`disco://session/...` are not auto-extracted from markdown; agents should use
explicit `disco_kb_link` calls for session, board, artifact, external URL, or
...
- `disco_artifacts_publish(folderPath, ...)` reads a folder and stores a DB file map.
- `disco_artifacts_land(artifactId, branchId, subpath, overwrite)` writes stored files into a branch path, with branch permission checks and path containment.
- `ArtifactsService.land()` carefully prevents absolute paths, branch-root writes, destination escapes, and artifact file path escapes. It writes a sidecar (`disco.artifact.json`) so metadata survives round trips.
...
```text
agent
  -> disco_kb_outline(documentId)
     returns headings, line ranges, version id/number/hash

agent
  -> disco_kb_get_range(documentId, startLine, endLine, contextLines, version)
     returns only relevant lines + same version metadata

agent
  -> disco_kb_edit(documentId, expectedVersion, ops, dryRun=false, changeSummary)
...
```text
disco_kb_materialize(documentRef, target={kind:"branch", branchId, subpath})
agent edits file using normal file tools
disco_kb_publish_from_workspace(materializationId or sourceRef, expectedVersion)
```

The important product distinction: this is not "write this file to my local disk." It is "materialize this KB snapshot into an Disco-managed workspace resource." The workspace may happen to be a local worktree in self-hosted mode, or it may be an executor/env pod volume in hosted mode.

If a non-Disco local MCP client wants to edit a local file, it should use download/upload semantics or targeted remote edit tools. It should not pass arbitrary local paths to the daemon and expect them to exist.
...
**Shape:** Keep `disco_kb_get` and `disco_kb_put` as the only content API.
...
**Verdict:** Useful as optional materialization into explicit Disco workspace targets. Not clean as the primary API. If implemented, the MCP shape should be branch-relative (`branchId` + `subpath`) and permission-checked, matching the artifact hardening direction.
...
**Verdict:** Avoid many top-level tools. Prefer one `disco_kb_edit` tool with a small set of operation types.

### Option D — Unified diff/patch only

**Shape:** Expose `disco_kb_apply_patch(document, expectedVersion, unifiedDiff)`.
...
type KnowledgeDocumentRef = {
  ...
  uri?: string; // disco://kb/<namespace>/<path> or disco://kb/document/<id>
  namespace?: string;
  path?: string;
};
```

Prefer `documentId` or `disco://kb/document/<id>` for rename-proof edits.
...
For MCP, keep `expectedVersion` to match `disco_kb_put`, and add `ifMatch` only if REST clients need direct ETag ergonomics.

### 5.3 `disco_kb_outline`
...
disco_kb_get_range({
  ...
  headingPath?: string;
  occurrence?: number;
  sectionRef?: string; // preferred selector copied from disco_kb_outline
...
Hard caps should prevent accidental full-document reads via a range tool. If the caller asks for too many lines, return a helpful error suggesting `disco_kb_get` for intentional full reads.

### 5.5 `disco_kb_edit`

Mutating. Applies one or more operations atomically to a document version. **One successful `disco_kb_edit` call creates one new `kb_document_versions` row.** The tool description should tell agents to batch related micro-edits into a single call: if the agent calls this tool 12 times, it will create 12 full-content versions and re-run version side effects 12 times.
...
disco_kb_materialize({
  ...
  target: {
    kind: 'branch';
    branchId: string;
    subpath?: string; // branch-relative; default .disco/kb/<namespace>/<path>
  } | {
    kind: 'temp_workspace';
    ttlMinutes?: number;
  };
...
```ts
disco_kb_publish_from_workspace({
  materializationId?: string;
  source: {
    kind: 'branch';
    branchId: string;
  ...
  changeSummary?: string;
  dryRun?: boolean;
}) -> { ...same as disco_kb_edit-ish result... }
```
...
```json
{
  "$schema": "https://disco.live/schemas/kb-materialization/2026-06-06.json",
  "document_id": "...",
  "uri": "disco://kb/document/...",
  "namespace": "global",
  "path": "foo.md",
  "version_id": "...",
  "version_number": 12,
...
`disco_artifacts_land` is already close because it requires `branchId`; `disco_artifacts_publish` and `disco_artifacts_check_build` are the bigger alignment targets because they currently accept absolute folders.
...
- If it does not match, return `409 Conflict` (or MCP error payload) with:
  ...
  - a hint to re-run `disco_kb_outline`/`disco_kb_get_range`,
...
Avoid MCP tools that accept daemon-local absolute paths unless the path is explicitly scoped by an Disco resource id.
...
```ts
publish_from_workspace({ source: { kind: 'branch', branchId, path: '.disco/kb/foo.md' } });
```

The second shape lets Disco validate branch RBAC, path containment, executor locality, and audit provenance.
...
- Temp directories: if supported, use Disco-created temp workspaces or per-call upload/materialization IDs, not arbitrary `/tmp` reads.

### 7.4 MCP local/remote model

MCP calls are authenticated HTTP requests to Disco. They may come from:

- an Disco-launched agent running in a branch worktree,
...
Tool names and schemas should not imply a shared local filesystem. A returned `path` should be described as an Disco workspace path, not necessarily a path the MCP client can open locally.
...
- MCP/task transcripts when the edit happened inside an Disco session.
...
- Add branch-relative variants to `disco_artifacts_publish` and `disco_artifacts_check_build`:
  - `branchId`
  - `subpath`
...
- Consider replacing arbitrary temp-dir publishing with Disco-managed temp workspace IDs.
...
3. What is the maximum range size for `disco_kb_get_range` before the caller should use `disco_kb_get` intentionally?
...
Build the KB editing API around **small remote reads + atomic version-checked edit operations**. It is the cleanest fit for Disco's DB-backed Knowledge model and works regardless of where the MCP client runs.

Then add filesystem materialization as an explicit Disco workspace feature, not as a generic local path feature. That keeps the excellent agent file-edit workflow available without compromising RBAC, auditability, or the hosted/remote architecture.
...
- The MCP tool `disco_kb_edit` surfaces the same guarantee and encourages batching to avoid version spam.
...
- `disco_kb_outline` and `disco_kb_get_range` provide bounded remote reads for large documents, returning line ranges, compact outline size hints, exact-read content hashes, and the current version token.
- `disco_kb_materialize` writes a KB markdown snapshot plus a `.disco-kb.json` sidecar into a branch worktree after branch `session` permission and containment checks. `disco_kb_publish_from_worktree` reads the branch-relative file back, uses the sidecar for document/version context when present, and updates existing documents through the targeted edit service so one publish creates one KB version.
- Automatic markdown link extraction is KB-document-link oriented. Links to sessions, boards, branches, external URLs, etc. should be represented with explicit `disco_kb_link` calls until/unless broader auto-link extraction is intentionally added.
...
- Worktree materialize/publish validates the derived `.disco-kb.json` sidecar path through the same branch-workspace canonical containment helper as the markdown file before reading or writing it. Worktree publish updates the sidecar after successful non-dry publishes so subsequent publishes carry a fresh expected version.
