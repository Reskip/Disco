 ...
 * reads process.env.DISCO_MASTER_SECRET in its constructor. A deployment that
...
describe('resolveMasterSecretIntoEnv', () => {
  ...
  beforeEach(() => {
    original = process.env.DISCO_MASTER_SECRET;
    delete process.env.DISCO_MASTER_SECRET;
  });

  afterEach(() => {
    if (original === undefined) delete process.env.DISCO_MASTER_SECRET;
    else process.env.DISCO_MASTER_SECRET = original;
  });

  it('promotes a config-only master secret onto process.env (the previously-broken Postgres path)', async () => {
    const config = { daemon: { masterSecret: SECRET_64 } } as DiscoConfig;

    const source = await resolveMasterSecretIntoEnv(config);

    expect(source).toBe('config');
    expect(process.env.DISCO_MASTER_SECRET).toBe(SECRET_64);
  });

  it('prefers the env var over the config value when both are present', async () => {
    process.env.DISCO_MASTER_SECRET = 'b'.repeat(64);
    const config = { daemon: { masterSecret: SECRET_64 } } as DiscoConfig;

    const source = await resolveMasterSecretIntoEnv(config);

    expect(source).toBe('env');
    expect(process.env.DISCO_MASTER_SECRET).toBe('b'.repeat(64));
  });

  it('fail-fasts when neither the env var nor daemon.masterSecret is configured', async () => {
    const config = { daemon: {} } as DiscoConfig;

    await expect(resolveMasterSecretIntoEnv(config)).rejects.toThrow(/DISCO_MASTER_SECRET/);
    expect(process.env.DISCO_MASTER_SECRET).toBeUndefined();
  });
});
