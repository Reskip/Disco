...
import type { OpenCodeProviderSettings as Settings } from '@disco/core/types';
...
function renderSettings(
  ...
  const client = { service: vi.fn(() => service) } as unknown as DiscoClient;
  render(
    <AntApp>
...
describe('OpenCodeProviderSettings', () => {
  ...
  it('clears provider form secrets when the authenticated client scope changes', async () => {
    ...
    const clientA = { service: vi.fn(() => serviceA) } as unknown as DiscoClient;
    const clientB = { service: vi.fn(() => serviceB) } as unknown as DiscoClient;
