...
1. The error string is `error.message` from a failed Feathers service call,
   ...
   `<Alert>` at `apps/disco-ui/src/components/ConversationView/ConversationView.tsx:318-322`.
...
3. There are **two unrelated** `client.io.on('connect', …)` listeners:
   `useDiscoClient.ts:216` (re-authenticates the socket) and
   ...
   (`useDiscoClient.ts:145-207`) is the safety net that catches 401s and
...
4. The banner sticks when the safety net itself fails:
   - The around-hook catches `isDefiniteAuthFailure` only
     ...
     (`useDiscoClient.ts:169-174`) — `resync()` records that as `error`.
...
`apps/disco-ui/src/components/ConversationView/ConversationView.tsx:200-322`:
...
`apps/disco-ui/src/hooks/useSharedReactiveSession.ts:21-60` retains a shared
...
`apps/disco-daemon/src/register-routes.ts:247-254`:
...
Tokens live in `localStorage` under `disco-access-token` and
`disco-refresh-token` (`apps/disco-ui/src/utils/tokenRefresh.ts:10-11`).

There are **four** distinct refresh paths, layered for resilience:

| # | Trigger | File | Notes |
|---|---|---|---|
| 1 | Proactive timer | `apps/disco-ui/src/hooks/useAuth.ts:241-286` | Decodes JWT `exp` claim, schedules a refresh `60s` before expiry |
| 2 | Tab regains focus | `apps/disco-ui/src/hooks/useAuth.ts:169-214` | Calls `reAuthenticate()` if unauthenticated; refreshes if `exp` within 60s |
| 3 | Socket reconnect | `apps/disco-ui/src/hooks/useDiscoClient.ts:216-303` | Tries `authenticate({jwt})` first, falls back to `refreshAndReauthenticate` |
| 4 | Per-call 401 retry | `apps/disco-ui/src/hooks/useDiscoClient.ts:145-207` | `client.hooks({ around: { all: [...] } })` — catches `isDefiniteAuthFailure`, refreshes, retries the original call once via `_refreshRetried` guard |
...
- Path #4 catches `NotAuthenticated` only. **Transient** failures of the
  ...
  `useDiscoClient.ts:169-174`, which re-throws the *original* 401:
...
- `useDiscoClient.ts:216` registers `client.io.on('connect', async () => {…})`
...
2. `useDiscoClient` listener runs synchronously, hits `await client.authenticate(...)`, suspends.
...
| Auth | `client.authenticate({ jwt })` from `useDiscoClient` initial connect | Same, in connect handler |
...
**Sketch.** The around-hook in `useDiscoClient.ts:145-207` already does
this. To close the gaps:

1. In the around-hook's inner `catch` at `useDiscoClient.ts:169-174`,
...
- **Cross-tab single-flight.** `singleFlightRefresh.ts` is per-tab. On
  ...
  token. A `BroadcastChannel('disco-auth')` to coordinate refreshes
  cross-tab would close that gap. Non-trivial; track separately.
- **REST client around-hook.** The 401-retry hook lives only on the
  socket client (`useDiscoClient.ts:145-207`). Any REST consumer (CLI,
...
*global* byId state (the `useDiscoData` hub), which had the same kind of
...
- `packages/client/src/reactive-session.ts` —
  ...
  - New `state.terminal: boolean` discriminates non-recoverable errors
    ...
    taking a UI cross-package dependency on `apps/disco-ui`'s
...
- `apps/disco-ui/src/hooks/useSharedReactiveSession.ts` — added a second
...
- `apps/disco-ui/src/components/ConversationView/ConversationView.tsx` —
...
`useDiscoData` was the obvious "global state" surface the user identified —
...
- `apps/disco-ui/src/hooks/useDiscoData.ts` —
  ...
  - `fetchData()` now accepts `{ silent }`; the reconnect path uses
    ...
    in `useDiscoClient`, hitting a 401 once before the around-hook refresh
...
- Cross-tab refresh-token rotation race in `singleFlightRefresh.ts`
  ...
  `BroadcastChannel('disco-auth')` if multi-tab users start hitting it.
- The transient-error retry inside `resync()` was *not* added — doing so
  would have required moving `isTransientConnectionError` from
  `apps/disco-ui` into `packages/client`, a cross-layer dependency we'd
...
- REST-client around-hook is still socket-only. Acceptable; the
  conversation panel and `useDiscoData` both use the socket client.
...
| What | File | Lines |
|---|---|---|
| Banner render | `apps/disco-ui/src/components/ConversationView/ConversationView.tsx` | 318-322 |
| Hook → handle wire-up | `apps/disco-ui/src/hooks/useSharedReactiveSession.ts` | 21-60 |
...
| `connect` listener (re-auth) | `apps/disco-ui/src/hooks/useDiscoClient.ts` | 216-303 |
| 401 around-hook | `apps/disco-ui/src/hooks/useDiscoClient.ts` | 145-207 |
| `isDefiniteAuthFailure` | `apps/disco-ui/src/utils/authErrors.ts` | 43-51 |
| `isTransientConnectionError` | `apps/disco-ui/src/utils/authErrors.ts` | 59-85 |
| Single-flight refresh + unrecoverable latch | `apps/disco-ui/src/utils/singleFlightRefresh.ts` | 52-154 |
| `refreshAndReauthenticate` | `apps/disco-ui/src/utils/singleFlightRefresh.ts` | 165-175 |
| `TOKENS_REFRESHED_EVENT` | `apps/disco-ui/src/utils/singleFlightRefresh.ts` | 43, 121-125 |
| Proactive refresh timer | `apps/disco-ui/src/hooks/useAuth.ts` | 241-286 |
| Visibility-change recovery | `apps/disco-ui/src/hooks/useAuth.ts` | 169-214 |
| Token storage (localStorage keys) | `apps/disco-ui/src/utils/tokenRefresh.ts` | 10-11 |
| Server JWT TTLs | `apps/disco-daemon/src/register-routes.ts` | 247-254 |
