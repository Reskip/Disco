...
describe('path expansion', () => {
  ...
  it('should expand ~ in file paths using HOME', () => {
    ...
    createDatabase({ url: 'file:~/.disco/test.db' });

    expect(createClient).toHaveBeenCalledWith(
      expect.objectContaining({
        url: 'file:/home/testuser/.disco/test.db',
      })
    );
  });
...
describe('createLocalDatabase', () => {
  it('should use DEFAULT_DB_PATH when no path provided', () => {
    ...
    expect(createClient).toHaveBeenCalledWith(
      expect.objectContaining({
        url: expect.stringContaining('.disco/disco.db'),
      })
    );
  });
...
describe('resolveDatabaseUrl', () => {
    ...
    expect(
      resolveDatabaseUrl({
        ...
        config: {
          database: {
            dialect: 'postgresql',
            postgresql: { url: 'postgresql://db.example/disco' },
          },
        },
      })
    ).toBe('postgresql://db.example/disco');
  });
  ...
  it('does not fall back to SQLite when config selects PostgreSQL without a URL', () => {
    expect(resolveDatabaseUrl({ env: {}, config: { database: { dialect: 'postgresql' } } })).toBe(
      'postgresql://localhost:5432/disco'
    );
  });
  ...
  it('honors and expands the configured SQLite path using the explicit operator home', () => {
    expect(
      resolveDatabaseUrl({
        ...
        config: { database: { dialect: 'sqlite', sqlite: { path: '~/.disco/custom.db' } } },
      })
    ).toBe('file:/home/operator/.disco/custom.db');
  });

  it('keeps environment database overrides ahead of config', () => {
    expect(
      resolveDatabaseUrl({
        env: { DISCO_DB_PATH: 'file:~/.disco/override.db' },
        homeDir: '/home/operator',
        config: { database: { dialect: 'postgresql' } },
      })
    ).toBe('file:/home/operator/.disco/override.db');
  });
});

describe('constants', () => {
  it('should export DEFAULT_DB_PATH', () => {
    expect(DEFAULT_DB_PATH).toBe('file:~/.disco/disco.db');
  });
});
