...
describe('formatExecutorFailure', () => {
  ...
  it('does not misclassify Windows process errors as SQLSTATE codes', () => {
    const error = Object.assign(new Error('spawn codex EPERM'), { code: 'EPERM' });
    expect(formatExecutorFailure(error)).toBe('spawn codex EPERM');
  });
});
