...
describe('login command', () => {
  it('uses an explicit remote URL without loading invalid local config', async () => {
    const home = await mkdtemp(join(tmpdir(), 'disco-login-remote-'));
    ...
    try {
      await mkdir(join(home, '.disco'), { recursive: true });
      await writeFile(join(home, '.disco', 'config.yaml'), 'daemon: [invalid yaml');
