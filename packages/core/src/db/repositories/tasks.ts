...
import { visibleSessionReferenceAccessExists } from './session-access';
...
export class TaskRepository implements BaseRepository<Task, Partial<Task>> {
  ...
  private taskToInsert(task: Partial<Task>): TaskInsert {
    ...
    return {
      ...
      data: {
        ...
        metadata: task.metadata, // Generic metadata bag (e.g., is_disco_callback, source)
  ...
  async findAll(filter?: {
    ...
    try {
      ...
      if (filter?.ownerSessionUserId) {
        conditions.push(sql`exists (
          select 1 from ${sessions}
          where ${sessions.session_id} = ${tasks.session_id}
            and ${sessions.created_by} = ${filter.ownerSessionUserId}
        )`);
      }
  ...
  async findPage(
    ...
    if (opts.ownerSessionUserId) {
      conditions.push(sql`exists (
        select 1 from ${sessions}
        where ${sessions.session_id} = ${tasks.session_id}
          and ${sessions.created_by} = ${opts.ownerSessionUserId}
      )`);
    }
