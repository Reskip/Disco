...
describe('useUserLocalStorage', () => {
  ...
  it('persists values under a per-user key', () => {
    ...
    expect(localStorage.getItem('disco:user:user-a:panel:left:size')).toBe('33');
  });

  it('loads a different value when the user changes', () => {
    localStorage.setItem('disco:user:user-a:panel:right:size', '42');
    localStorage.setItem('disco:user:user-b:panel:right:size', '55');
