...
import {
  DISCO_USER_ENV_KEYS_VAR,
  type ApiKeyName,
  stripProviderCredentialEnvironment,
} from '@disco/core/config';
import { generateId, sanitizeDbError, shortId } from '@disco/core/db';
import type {
  AgenticToolName,
...
import { MessageRole, PROVIDER_CREDENTIAL_FIELDS } from '@disco/core/types';
...
const DEBUG_SDK_EXECUTOR =
  process.env.DISCO_DEBUG_SDK_EXECUTOR === '1' || process.env.DEBUG?.includes('sdk-executor');
...
export interface BaseTool {
  ...
  /** Apply user guidance to the provider's currently running turn. */
  steerTask?(sessionId: SessionID, hint: TaskSteeringHint): Promise<void>;
 ...
 *
 * The daemon should not run git inside a Session working directory just to
 * stamp task bookkeeping. The executor already has that exact directory.
 */
async function captureGitStateForSession(
  ...
  try {
    ...
    if (!workingDirectory) {
      console.warn(`[Git SHA Capture] Session has no working directory at task ${phase}`);
      return undefined;
    }

    const sha = await getGitState(workingDirectory);
    let ref = 'unknown';
    try {
      ref = await getCurrentBranch(workingDirectory);
    } catch (error) {
      console.warn(
...
export async function resolveApiKeyForTask(
...
): Promise<import('@disco/core/config').KeyResolutionResult> {
  ...
  const executorSessionToken = (client as DiscoClient & { executorSessionToken?: string })
  ...
  })) as import('@disco/core/config').KeyResolutionResult;
...
export function installProviderConnection(
  ...
  // Keep DISCO_USER_ENV_KEYS truthful: drop names that no longer exist so MCP
  // template resolution doesn't advertise vars this strip just removed.
  const advertisedKeys = process.env[DISCO_USER_ENV_KEYS_VAR];
  if (advertisedKeys) {
    const remaining = advertisedKeys.split(',').filter((key) => process.env[key] !== undefined);
    if (remaining.length > 0) {
      process.env[DISCO_USER_ENV_KEYS_VAR] = remaining.join(',');
    } else {
      delete process.env[DISCO_USER_ENV_KEYS_VAR];
    }
  }
}
...
export async function executeToolTask(params: {
  ...
  let detachSteeringListener: (() => void) | undefined;
  let steeringDrain: Promise<void> = Promise.resolve();
  ...
  try {
    ...
    // authoritative working-directory git read belongs here with the rest of
    ...
    const executorSessionToken = (
      client as typeof client & { executorSessionToken?: string }
    ).executorSessionToken;
    ...
    // Steering hints are admitted by the daemon under the per-session turn
    // lock and appended to the running Task. Consume the durable list here so
    // a hint cannot be lost in the short gap between executor launch and the
    // provider turn becoming ready.
    if (tool.steerTask) {
      const tasksService = client.service('tasks');
      const seenHintIds = new Set<string>();
      const consumeSteeringHints = (candidate: unknown) => {
        if (!candidate || typeof candidate !== 'object') return;
        const task = candidate as Task;
        if (task.task_id !== taskId) return;

        for (const hint of task.metadata?.steering_hints ?? []) {
          if (seenHintIds.has(hint.hint_id)) continue;
          seenHintIds.add(hint.hint_id);
          steeringDrain = steeringDrain
            .then(() => tool.steerTask!(sessionId, hint))
            .catch((error) => {
              // A supplemental hint must never fail the task already in
              // progress. The Codex tool persists the failed hint for audit.
              console.error(
                `[${toolName}] Failed to apply supplemental guidance ${shortId(hint.hint_id)}:`,
                error
              );
            });
        }
      };
      const onSteeringRequested = (task: Task) => consumeSteeringHints(task);
      tasksService.on('steering_requested', onSteeringRequested);
      detachSteeringListener = () =>
        tasksService.off('steering_requested', onSteeringRequested);
      consumeSteeringHints(await tasksService.get(taskId));
    }
    ...
    // Store raw for debugging and normalized for UI/analytics. Codex's public
    // turn.completed event reports whole-thread lifetime usage, while the tool
    // result has already converted rollout checkpoints into this task's delta.
    // Never let the lifetime value overwrite the live, task-local accounting.
    ...
    if (toolName === 'codex' && result.tokenUsage) {
      const taskLocalNormalized = normalizeRawSdkResponse(
        'codex',
        {
          type: 'turn.completed',
          usage: {
            input_tokens: result.tokenUsage.input_tokens ?? 0,
            cached_input_tokens: result.tokenUsage.cache_read_tokens ?? 0,
            output_tokens: result.tokenUsage.output_tokens ?? 0,
          },
        },
        { modelHint: result.model }
      );
      if (taskLocalNormalized) {
        if (result.tokenUsage.total_tokens !== undefined) {
          taskLocalNormalized.tokenUsage.totalTokens = result.tokenUsage.total_tokens;
        }
        if (result.tokenUsage.cache_creation_tokens !== undefined) {
          taskLocalNormalized.tokenUsage.cacheCreationTokens =
            result.tokenUsage.cache_creation_tokens;
        }

        patchData.normalized_sdk_response = patchData.normalized_sdk_response
          ? {
              ...patchData.normalized_sdk_response,
              tokenUsage: taskLocalNormalized.tokenUsage,
              costUsd: taskLocalNormalized.costUsd,
            }
          : taskLocalNormalized;
      }
    }
