/**
 * `disco config` - Show the effective deployment configuration.
 */
...
import { getDatabaseUrl } from '@disco/core/db';
import { Command, Flags } from '@oclif/core';
import chalk from 'chalk';

const REDACTED = '<redacted>';

function redactSecrets(config: DiscoConfig): DiscoConfig {
...
function materializeDatabaseEnvironment(config: DiscoConfig): DiscoConfig {
  const dialect = process.env.DISCO_DB_DIALECT;
  if (!dialect && !process.env.DATABASE_URL && !process.env.DISCO_DB_PATH) return config;
