...
export function normalizeOpenApiDocument(document: OpenApiDocument): void {
  ...
  for (const name of referencedSchemas) {
    document.components.schemas[name] ??= {
      ...
      description: `Schema for ${name}. See @disco-live/client for the authoritative TypeScript type.`,
    };
  }
}
 ...
 * - A stable redirect from /docs to the maintained reference at disco.live
...
export function configureSwagger(app: Application, options: SwaggerOptions): void {
  ...
  app.configure(
    swagger({
      ...
      specs: {
        info: {
          title: 'Disco API',
          description: 'REST and WebSocket API for Disco agent orchestration platform',
          version,
        },
  ...
  (app as unknown as Express).get(['/docs', '/docs/'], (_request: Request, response: Response) => {
    response.redirect(302, 'https://disco.live/api-reference');
  });
}
