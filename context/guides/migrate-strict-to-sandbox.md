...
Operator runbook for moving a self-hosted, multi-user Disco deployment off the
...
> User-facing setup: `apps/disco-docs/content/guide/multiplayer-unix-isolation.mdx` → "Sandbox mode".
...
| Host plumbing    | sudoers, host accounts, `disco_wt_*` groups, password sync | none                                                    |
...
1. Linux daemon host with `bubblewrap` installed **and unprivileged user
   ...
   ```bash
   disco doctor          # look for a green "unprivileged userns" row
   # or directly:
...
2. The daemon must run **as the user you will chown everything to** (e.g.
   `discopg`). Note it — you'll pass it as `--daemon-user`.
...
5. Install the 0.25.1 software without starting its daemon. Run
   `disco db migrate --offline-cutover` from 0.25.1 and verify
...
```bash
sudo scripts/sandbox-home-migration-preflight.sh '/home/*' --stable-home /home/discopg
```
...
- `cat ~/.disco/config.yaml` → **Permission denied / No such file** (secrets hidden)
...
- **Repo-level git ops** (clone / realign-origin) run unwrapped as the daemon
  user — Disco's own trusted code, no branch cwd.

## Failure modes / gotchas

| Symptom                                | Cause                                                    | Fix                                                                 |
| -------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------- |
| All sessions fail to start immediately | unprivileged userns blocked on host                      | `disco doctor`; enable userns, or fall back to `simple`              |
