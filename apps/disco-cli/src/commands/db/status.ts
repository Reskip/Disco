/**
 * `disco db status` - Show applied database migrations
 */
...
export default class DbStatus extends Command {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      // Priority: If DISCO_DB_DIALECT=postgresql, use DATABASE_URL; otherwise DISCO_DB_PATH
      if (flags.json) {
      ...
      if (status.applied.length === 0 && status.pending.length === 0) {
        this.log(
          `${chalk.yellow('⚠')} No migrations table found. Run ${chalk.cyan('disco db migrate')} to initialize.`
        );
        process.exit(0);
      }
      ...
      if (status.hasPending) {
        ...
        this.log('');
        this.log(chalk.dim(`Run ${chalk.cyan('disco db migrate')} to apply pending migrations.`));
      }
