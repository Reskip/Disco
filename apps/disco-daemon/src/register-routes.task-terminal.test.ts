...
import { type Task, TaskStatus } from '@disco/core/types';
...
describe('task complete/fail route authorization', () => {
  ...
  it("does not let an admin settle another user's task", async () => {
    await expect(authorizeTaskTerminalRoute(harness('other-user', 'admin'))).rejects.toBeInstanceOf(
      Forbidden
    );
  });
});
...
describe('force-fail route authorization and request fencing', () => {
  ...
  it('authorizes the Session owner and returns the exact Task epoch', async () => {
    await expect(
      authorizeForceFailRoute({
        session: { session_id: 'session-a' as never, created_by: 'user-a' as never },
  ...
  it('rejects a stale Task epoch instead of selecting a later unverified Task', async () => {
    ...
    await expect(
      authorizeForceFailRoute({
        session: { session_id: 'session-a' as never, created_by: 'admin-a' as never },
