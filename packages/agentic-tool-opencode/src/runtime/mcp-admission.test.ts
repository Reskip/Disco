...
import type { MCPServer, MessageID, SessionID, TaskID } from '@disco/core/types';
...
describe('MCP servers reaching OpenCode invocation config', () => {
  ...
  it('keeps the built-in Disco server addressable', async () => {
    const { config, withheld } = await buildConfig([server(PLAIN)]);

    expect(Object.keys(config.mcp).some((name) => name.startsWith('disco_'))).toBe(true);
...
describe('MCP servers reaching the OpenCode process environment', () => {
  it('never names a gated server in OPENCODE_CONFIG_CONTENT', async () => {
    ...
    await expect(
      tool.runTurn({
        discoSessionId: 'session-1' as SessionID,
        taskId: 'task-1' as TaskID,
        prompt: 'hi',
        discoAssistantMessageId: 'message-1' as MessageID,
        title: 'turn',
        directory: tmpdir(),
        provider: 'anthropic',
