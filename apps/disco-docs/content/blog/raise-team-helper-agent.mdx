...
description: 'A practical recipe for turning an Disco Teammate into a PM-style teammate that remembers context, coordinates work, reports progress, and keeps a team aligned.'
...
That is the shape of a good team-helper agent in Disco: a 24/7 PM-style teammate with memory, access, cadence, and enough initiative to turn messy team activity into clear next actions.

Not an omniscient robot. Not a manager replacement. A practical coordination layer.

This post is a fast recipe for raising one with [Disco Teammates](/guide/teammates), [Knowledge](/guide/knowledge), [Boards](/guide/boards), [branches](/guide/branches), [sessions](/guide/sessions), the [scheduler](/guide/scheduler), and MCP-connected tools where useful.
...
1. **Identity**, meaning who the teammate serves and how it should behave.
...
7. **Correction** (humans review, correct, and improve the teammate over time).

That loop is faster than a formal requirements workshop. You can get useful output the same day, then keep tightening the teammate as the team uses it.

## 1. Give it a job, not just a name

An [Teammate](/guide/teammates) is a persistent agent with identity, memory, skills, and scheduled heartbeats. Treat the first hour like onboarding.
...
Create a canonical [Knowledge](/guide/knowledge) space for the team or project. Use it for the context the teammate should preserve across sessions:
...
Keep the structure boring. The teammate can reorganize later, but it needs somewhere obvious to write today.
...
The fastest way to raise a team teammate is not a polished prompt. It is a messy context dump followed by organization.
...
Then ask the teammate to file the dump into useful pages:
...
You are not trying to make the first version perfect. You are creating the substrate the teammate can keep improving.
...
In Disco, the built-in [MCP server](/guide/internal-mcp) gives agents self-awareness about boards, branches, sessions, schedules, and Knowledge. You can also attach external MCP servers or integrations for systems like Slack, GitHub, Linear, Jira, calendars, meeting notes, support tools, or internal APIs.
...
If you connect Slack or GitHub through a [Message Gateway](/guide/message-gateway) or MCP server, be deliberate about permission mode, credentials, and who can reach the teammate. A team helper that can post or comment should usually draft first and act after approval until its runbooks are proven.
...
Because Disco boards are branch-centric, each card can hold sessions, environment status, PR links, notes, and spatial meaning. The teammate can use that structure to answer questions like:
...
Zones can also trigger templated prompts when a branch is dropped into them. For example, a “Needs coordination” zone can prompt the teammate to identify owners, summarize the blocker, and draft a coordination note.
...
Once the teammate has enough context, give it recurring jobs through the [Branch Scheduler](/guide/scheduler).
...
Keep recurring work idempotent. The teammate should write where it wrote, link what it used, and avoid double-posting.

## 8. Teach it how to slice work

A PM-like teammate should not only summarize. It should help convert ambiguity into tractable work.
...
Ask the teammate to propose workstreams:
...
- dependencies,
- risks,
- recommended branch/session shape in Disco,
- and what should be tracked outside Disco.
```

This is where the teammate starts feeling PM-like: it notices the coordination cost, not just the tasks.
...
## 10. Keep improving the teammate like a product
...
Also let the teammate capture its own friction:
...
That feedback is valuable whether you are improving the teammate, the team process, or Disco itself.

## A practical starter checklist

If you want to do this quickly, start here:

1. Create an [Teammate](/guide/teammates) for the team.
...
5. Dump messy project context and ask the teammate to structure it.
...
12. Revisit memory and runbooks weekly until the teammate feels boringly useful.
...
That is enough to change the texture of team coordination: fewer status scavenger hunts, fewer repeated questions, better handoffs, clearer risks, and a shared teammate that helps everyone get exactly what they need to be effective.
