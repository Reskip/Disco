import type { DiscoClient, Message, Session, Task } from '@disco/core/client';
import { TaskStatus } from '@disco/core/client';
...
describe('shared reactive session idle cache', () => {
  it('reuses a recently viewed transcript and disposes it after the idle TTL', async () => {
    ...
    const first = retainReactiveSession(mock.client, SESSION_ID, { taskHydration: 'lazy' });
    await first.ready();
    expect(mock.order.filter((event) => event === 'hydrate')).toHaveLength(1);
    expect(mock.messageFindAll).toHaveBeenCalledTimes(1);

    vi.useFakeTimers();
    try {
      releaseReactiveSession(mock.client, SESSION_ID, { taskHydration: 'lazy' });
      const second = retainReactiveSession(mock.client, SESSION_ID, { taskHydration: 'lazy' });
      ...
      releaseReactiveSession(mock.client, SESSION_ID, { taskHydration: 'lazy' });
      await vi.advanceTimersByTimeAsync(SHARED_REACTIVE_SESSION_IDLE_TTL_MS + 1);
      await Promise.resolve();
      await Promise.resolve();
      expect(mock.sessionStreams.remove).toHaveBeenCalledTimes(1);
    } finally {
