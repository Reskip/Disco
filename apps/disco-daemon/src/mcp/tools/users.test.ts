...
describe('user MCP tools in sessionless context', () => {
  it('disco_users_get_current works without current session context', async () => {
    ...
    const fakeServer = {
      registerTool: (name: string, _cfg: unknown, cb: ToolHandler) => {
        if (name === 'disco_users_get_current') handler = cb;
      },
    ...
    if (!handler) throw new Error('disco_users_get_current was not registered');
  ...
  it('disco_users_list paginates, searches, and returns compact rows by default', async () => {
    ...
    const fakeServer = {
      registerTool: (name: string, _cfg: unknown, cb: ToolHandler) => {
        if (name === 'disco_users_list') handler = cb;
      },
    } as unknown as McpServer;
    ...
    if (!handler) throw new Error('disco_users_list was not registered');
  ...
  it('disco_users_list supports detailed and field-selected output modes', async () => {
    const detailedUser = {
      user_id: 'user-2',
      email: 'reed@preset.io',
      name: 'Reed',
  ...
  it('disco_users_find returns compact matches using the search query', async () => {
    const findUsers = vi.fn(async () => ({
      total: 1,
      limit: 10,
      skip: 0,
    ...
    const fakeServer = {
      registerTool: (name: string, _cfg: unknown, cb: ToolHandler) => {
        if (name === 'disco_users_find') handler = cb;
      },
    } as unknown as McpServer;
    ...
    if (!handler) throw new Error('disco_users_find was not registered');
  ...
  it('disco_users_find applies field-specific filters after broad service search', async () => {
    const findUsers = vi.fn(async () => ({
      total: 2,
      limit: 10000,
      skip: 0,
...
describe('user MCP input schemas', () => {
  it('rejects aliases and malformed required fields with caller-oriented messages', () => {
    ...
    const listWithAlias = configs.get('disco_users_list')?.inputSchema.safeParse({ query: 'reed' });
    ...
    const emptyUserId = configs.get('disco_users_get')?.inputSchema.safeParse({ userId: '' });
