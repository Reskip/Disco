...
describe('entity URL builders — fullUrl double-prefix regression', () => {
  it('produces correct session URL when baseUrl has no /ui suffix', () => {
    const url = getSessionUrl(SESSION_ID, 'https://disco.example.com');
    expect(url).toMatch(/^https:\/\/disco\.example\.com\/ui\/s\//);
    expect(url).not.toContain('/ui/ui/');
  });

  it('strips /ui suffix from baseUrl to prevent double /ui/ui/ prefix in session URLs', () => {
    const url = getSessionUrl(SESSION_ID, 'https://disco.example.com/ui');
    expect(url).toMatch(/^https:\/\/disco\.example\.com\/ui\/s\//);
    expect(url).not.toContain('/ui/ui/');
  });
  ...
  it('does not strip /ui from a path-prefixed base (e.g. https://host/myapp)', () => {
    const url = getSessionUrl(SESSION_ID, 'https://disco.example.com/myapp');
    expect(url).toMatch(/^https:\/\/disco\.example\.com\/myapp\/ui\/s\//);
  });
});

describe('normalizeHttpBaseUrl', () => {
  it('canonicalizes origins and preserves a path prefix without a trailing slash', () => {
    expect(normalizeHttpBaseUrl(' HTTPS://Example.com:443/disco/// ', 'daemon URL')).toBe(
      'https://example.com/disco'
    );
  });
