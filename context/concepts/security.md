...
How Disco's daemon handles Content-Security-Policy (CSP) and Cross-Origin
Resource Sharing (CORS), and how operators tune both from `~/.disco/config.yaml`.

> This document is scoped to **web-layer hardening** (CSP / CORS / response
> headers). For authentication, RBAC, and Unix isolation see
> [`apps/disco-docs/pages/guide/multiplayer-unix-isolation.mdx`](../../apps/disco-docs/pages/guide/multiplayer-unix-isolation.mdx)
...
1. **Hardcoded defaults hid a bug for Sandpack artifacts.** CSP lacked
   ...
   which blocks the hosted CodeSandbox bundler iframes Disco uses for
...
bundled Disco feature work, and operators can append or replace per-directive
...
Disco ships, just let me add `https://plausible.io` to `script-src`." That's
...
security:
  cors:
    ...
    origins: ['https://disco.mydomain.com'] # used when mode=list
...
### Sandpack, local/private Disco URLs, and Chrome Private Network Access

Artifacts commonly render inside the hosted Sandpack iframe
(`https://*.codesandbox.io`) while the Disco API URL may point at a loopback,
...
`https://disco.sandbox.preset.zone` if DNS routes them to a private network
...
1. Try the same artifact in Firefox. If Firefox works and Chrome/Edge fail, it
   is likely Chrome local-network enforcement, not CSP or an Disco token bug.
2. In Chrome, allow local network access for the Disco/Sandpack page if that site
   permission is shown.
...
Disco still keeps Sandpack on the non-credentialed side of CORS: browser cookies
...
service (`apps/disco-daemon/src/services/templates.ts`). The browser bundle
ships no Handlebars and never calls `new Function` / `eval`.

`fonts.bunny.net` hosts the Inter font CSS imported by `apps/disco-ui/src/index.css`.
...
4. Add the required source to `security.csp.extras.<directive>` in
   `~/.disco/config.yaml`, restart the daemon.
...
```
DiscoConfig.security          →  resolveSecurity()  →  ResolvedSecurity
...
- `apps/disco-daemon/src/setup/security-headers.ts` — consumes `ResolvedCsp`
- `apps/disco-daemon/src/setup/cors.ts` — consumes `ResolvedCors`
- `apps/disco-daemon/src/register-routes.ts` — surfaces it on `/health`
- `apps/disco-ui/src/components/SettingsModal/AboutTab.tsx` — renders it
...
- `apps/disco-daemon/src/setup/security-headers.ts` — CSP middleware
- `apps/disco-daemon/src/setup/cors.ts` — CORS policy builder
- `packages/core/src/config/security-resolver.ts` — config resolver
- [`apps/disco-docs/pages/guide/multiplayer-unix-isolation.mdx`](../../apps/disco-docs/pages/guide/multiplayer-unix-isolation.mdx) — daemon auth, RBAC, OS-level isolation tiers
