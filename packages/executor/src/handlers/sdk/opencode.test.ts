...
vi.mock('@disco/core/mcp', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@disco/core/mcp')>()),
  getMcpServersForSession: mocks.getMcpServersForSession,
}));

vi.mock('@disco/agentic-tool-opencode/runtime', () => ({
  ...
  const services = {
    sessions: {
      get: vi.fn(async () => ({
        ...
        working_directory: 'E:\\Disco\\data\\worktrees\\user-test\\standalone\\session-test',
