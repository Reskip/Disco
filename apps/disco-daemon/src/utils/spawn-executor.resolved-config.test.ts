 ...
 * The contract: the daemon resolves only a small subset of DiscoConfig and
 * embeds it in the payload; the executor never reads ~/.disco/config.yaml
...
import { __resetConfigCacheForTests, loadConfigSync } from '@disco/core/config';
...
describe('buildResolvedConfigSlice', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'disco-rcs-'));
  ...
  async function writeConfigYaml(yamlBody: string): Promise<void> {
    ...
    await fs.writeFile(path.join(discoDir, 'config.yaml'), yamlBody, 'utf-8');
  }
  ...
  it('survives the executor-side payload schema as-is', async () => {
    const { ResolvedConfigSliceSchema } = await import('@disco/core/config');
    await writeConfigYaml(
      [
        'execution:',
        '  permission_timeout_ms: 60000',
...
describe('withResolvedConfig', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'disco-with-rc-'));
    vi.spyOn(os, 'homedir').mockReturnValue(tempDir);
    __resetConfigCacheForTests();
    configureResolvedConfigSlice(loadConfigSync());
  });
