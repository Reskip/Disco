 ...
 *   pnpm --filter @disco/core exec vitest run src/db/tenant-portability.postgres.test.ts
 */
...
async function seedTenant(db: Database, tenantId: string): Promise<void> {
  await runWithTenantDatabaseScope(db, tenantId, async (scoped) => {
    await new SessionRepository(scoped).create({
      ...
      working_directory: `/tmp/tenant-portability/${tenantId}/${generateId()}`,
    });
  });
}
    ...
    await new UserMCPOAuthTokenRepository(scoped, masterSecret).saveToken(
      ...
      serverId as MCPServerID,
      {
        ...
        grantBinding: {
          ...
          redirectUri: 'https://disco.example.test/oauth/callback',
        },
      }
    );
  });
...
async function seedFilesystem(root: string): Promise<void> {
  await mkdir(join(root, 'worktrees', 'session-a'), { recursive: true });
  await writeFile(join(root, 'worktrees', 'session-a', 'file-a.txt'), 'alpha');
...
describe.skipIf(!postgresUrl || !usesPostgresSchema)('tenant portability (PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    ...
    scratch = await mkdtemp(join(tmpdir(), 'disco-tenant-portability-'));
  ...
  it('round-trips a tenant through export → delete → import → verify', async () => {
    ...
    expect(await readFile(join(fsRoot, 'worktrees', 'session-a', 'file-a.txt'), 'utf8')).toBe(
      'alpha'
    );
  ...
  it('completes the filesystem on retry after a re-home database commit then FS-tail failure', async () => {
    ...
    const archivedPayloadPath = join(
      archive,
      'files',
      'worktrees',
      'session-a',
    ...
    expect(await readFile(join(goodFsRoot, 'worktrees', 'session-a', 'file-a.txt'), 'utf8')).toBe(
      'alpha'
    );
