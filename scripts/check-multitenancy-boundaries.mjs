...
function filesUnder(prefix) {
  return walk(join(ROOT, prefix)).map((file) => file.slice(ROOT.length + 1).replace(/\\/g, '/'));
}

const checks = [
  {
    name: 'raw realtime/socket primitives',
    roots: ['apps/disco-daemon/src'],
    patterns: [
      /\bapp\.io\.(?:emit|to)\s*\(/g,
      /\bio\.(?:emit|to)\s*\(/g,
    ...
    baseline: {
      ...
      'apps/disco-daemon/src/register-services.ts': 5,
      // HA fork/spawn now use the tenant-aware Feathers event helper instead
      // of raw global Socket.IO broadcasts.
      'apps/disco-daemon/src/register-routes.ts': 4,
      'apps/disco-daemon/src/startup.ts': 1,
      // Facade tests inspect the tenant-aware channels directly to prove that
      // publish/leave operations do not materialize or leak session rooms.
      'apps/disco-daemon/src/utils/realtime-publish.test.ts': 3,
      ...
      'apps/disco-daemon/src/utils/realtime-publish.ts': 10,
      ...
      'apps/disco-daemon/src/setup/socketio.ts': 10,
    },
  },
...

  {
    ...
    roots: ['apps/disco-daemon/src/mcp/tools'],
    excludeTests: true,
...

  {
    ...
    baseline: {
      ...
      'apps/disco-daemon/src/register-routes.ts': 1,
    },
  },
  {
    ...
    roots: ['apps/disco-daemon/src'],
    excludeTests: true,
    patterns: [/import\s*{[^}]*\btenantDatabaseScope\b[^}]*}\s*from\s*['"]@disco\/core\/db['"]/gs],
    baseline: {},
  },
  {
    name: 'raw tenant database scope exits',
    roots: ['packages/core/src', 'apps/disco-daemon/src'],
  ...
  },
  {
    ...
    baseline: {
      ...
      'apps/disco-daemon/src/utils/tenant-db-scope.test.ts': 1,
      ...
      'apps/disco-daemon/src/utils/tenant-db-scope.ts': 2,
    },
  },
  {
    ...
    excludeTests: true,
    patterns: [
      /import\s+(?:type\s+)?{[^}]*(?:\bDatabase\b|\bRawDatabase\b)[^}]*}\s*from\s*['"]@disco\/core\/db(?:\/client)?['"]/gs,
      /import\s+(?:type\s+)?\*\s+as\s+\w+\s+from\s*['"]@disco\/core\/db(?:\/client)?['"]/gs,
    ],
    baseline: {
      ...
      'apps/disco-daemon/src/health/db-probe.ts': 1,
      'apps/disco-daemon/src/health/routes.ts': 1,
      ...
      'apps/disco-daemon/src/uploads-maintenance.ts': 1,
      // Widget renderer accepts both tenant-aware and repository-compatible database shapes.
      'apps/disco-daemon/src/widgets/env-vars/index.ts': 1,
    },
  },
  {
    ...
    // Disco store/tenant transaction wrapper once introduced.
    baseline: {
      // Tenant scopes use one transaction for disco.tenant_id. Narrow system
      ...
      // the Disco store/tenant transaction wrapper covers both patterns.
