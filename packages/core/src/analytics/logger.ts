...
import type { DiscoAnalyticsSettings, DiscoConfig } from '../config/types.js';
...
function mergeAnalyticsConfig(config?: DiscoAnalyticsSettings): DiscoAnalyticsSettings {
...
export function resolveAnalyticsConfig(
  config: DiscoConfig | DiscoAnalyticsSettings
): DiscoAnalyticsSettings {
  if ('analytics' in config) {
    return mergeAnalyticsConfig(config.analytics);
  }
  return mergeAnalyticsConfig(config as DiscoAnalyticsSettings);
}
