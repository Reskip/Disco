...
describe('Codex token_count accounting usage', () => {
  it('extracts cumulative and last-call usage, then derives the current task delta', () => {
    const snapshot = extractCodexTokenCountUsageFromEvent({
      type: 'event_msg',
      payload: {
        type: 'token_count',
        info: {
          total_token_usage: {
            input_tokens: 75_086,
            cached_input_tokens: 46_592,
            output_tokens: 1_091,
            total_tokens: 76_177,
          },
          last_token_usage: {
            input_tokens: 26_983,
            cached_input_tokens: 23_296,
            output_tokens: 503,
            total_tokens: 27_486,
          },
        },
    ...
    const baseline = subtractCodexTokenUsage(snapshot!.cumulative, snapshot!.last!);
    expect(subtractCodexTokenUsage(snapshot!.cumulative, baseline)).toEqual({
      input_tokens: 26_983,
      output_tokens: 503,
      cache_read_tokens: 23_296,
      total_tokens: 27_486,
    });
  });

  it('rejects non-token events and clamps reset deltas to zero', () => {
    expect(extractCodexTokenCountUsageFromEvent({ type: 'turn.completed' })).toBeUndefined();
    expect(
      subtractCodexTokenUsage(
