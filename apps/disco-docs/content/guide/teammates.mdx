...
<div style={{ padding: '12px 16px', borderLeft: '4px solid #3b82f6', borderRadius: '8px', background: 'rgba(59, 130, 246, 0.12)', margin: '16px 0' }}>
  <strong>Rename note:</strong> In July 2026, Disco renamed <strong>Assistants</strong> to <strong>Teammates</strong>. If you arrived from <code>/guide/assistants</code>, this is the canonical guide. Older references to <code>assistant</code> may still appear where they describe LLM message roles or provider protocol terms.
</div>

An **AI teammate** is a persistent AI companion that lives in your Disco workspace. While a normal branch is _where_ code gets written, an AI teammate is a _who_, a persistent entity with memory, skills, and the ability to orchestrate work across your entire workspace.

<img
  src="/screenshots/teammates-list.png"
  alt="A chat response listing twelve teammates in an Disco instance, a mix of coding orchestrators (DiscoClaw, GitHub Handler), ops bots (patch-bot, OpEx), and business helpers (Saul, Blake, Peggy, Hippa), each with an emoji, name, and one-line function"
  style={{ maxWidth: '600px', borderRadius: '8px' }}
/>

_A real Disco instance with a curated mix of coding, ops, and business teammates, each callable from any session via the `disco_teammates_list` MCP tool._

The teammate framework is heavily inspired by [OpenClaw](https://github.com/ricklamers/openclaw), a proven pattern for building persistent AI agents with file-based memory and identity. Disco's implementation adapts the core ideas (markdown-based memory, identity files, and scheduled loops) while making the teammate deeply aware of its environment through Disco's [MCP tools](/guide/internal-mcp). An Disco teammate can create branches, spawn sessions, manage boards, and coordinate multi-agent workflows, capabilities that go well beyond the original pattern.

**Framework repository:** [github.com/preset-io/disco-teammate](https://github.com/preset-io/disco-teammate)

## How it works

An AI teammate is a special branch backed by the **framework repository**, a template repo containing memory files, identity configuration, skills, and task templates. When you create a teammate, Disco:
...
Configure heartbeat frequency through Disco's [scheduler](/guide/scheduler), from every few minutes to once a day.
...
Through Disco's [MCP tools](/guide/internal-mcp), teammates can:
...
The [first-run wizard](/guide/getting-started#follow-the-onboarding-wizard) (**You → AI → Workspace → Tools**) ends by seeding your first teammate: it clones the [teammate framework repository](https://github.com/preset-io/disco-teammate), creates the teammate's branch on the board you named, and launches its first session (which follows `ONBOARDING.md`) with the agent you connected — briefed with your persona and the tools you flagged.
...
During `disco init`, you'll be prompted to set up a teammate. This queues the setup for the UI wizard on your next login.

## Teammate vs. Agent

These terms mean different things in Disco:
...
For real-world use cases and the story behind Disco Teammates, see our blog post: [Introducing Disco Teammates](/blog/disco-assistants).
