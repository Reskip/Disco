/**
 * `disco local create-admin` - Create bootstrap admin user
 */

import { join } from 'node:path';
import { getConfigPath, loadConfig, resolveMultiTenancyConfig } from '@disco/core/config';
import {
...
export default class LocalCreateAdmin extends Command {
  ...
  static flags = {
    username: Flags.string({
      ...
      default: DEVELOPMENT_DEFAULT_ADMIN_USER.username,
    }),
    ...
    'dev-default': Flags.boolean({
      description:
        'Development/test only: use admin / admin. Refused when NODE_ENV=production.',
      default: false,
    }),
  };
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (!databaseUrl) {
        ...
        const discoHome = join(configPath, '..');
        const dbPath = join(discoHome, 'disco.db');
      ...
      await runWithTenantDatabaseScope(db, tenantId, async () => {
        ...
        const existingAdmin = await getUserByUsername(db, flags.username);

        if (existingAdmin) {
          this.log(chalk.yellow('⚠ Admin user already exists'));
          this.log('');
          this.log(`  Username: ${chalk.cyan(flags.username)}`);
          ...
          this.log(
            chalk.gray(
              `To reset password, use: disco user update ${flags.username} --password newpassword`
            )
          );
          process.exit(0);
        }
