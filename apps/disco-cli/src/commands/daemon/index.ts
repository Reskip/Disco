/**
 * `disco daemon` - Daemon lifecycle management overview
 */
...
export default class DaemonIndex extends Command {
  static description = 'Manage the Disco daemon lifecycle';
  ...
  async run(): Promise<void> {
    ...
    if (!isInstalledPackage()) {
      ...
      this.log('    cd apps/disco-daemon && pnpm dev');
      this.log('');
    ...
    this.log('');
    this.log(`  ${chalk.cyan('disco daemon start')}    Start daemon in background`);
    this.log(`  ${chalk.cyan('disco daemon stop')}     Stop daemon gracefully`);
    this.log(`  ${chalk.cyan('disco daemon restart')}  Restart daemon`);
    this.log(`  ${chalk.cyan('disco daemon status')}   Check daemon status`);
    this.log(`  ${chalk.cyan('disco daemon logs')}     View daemon logs`);
    this.log('');
    this.log(chalk.bold('Examples:'));
    this.log('');
    this.log('  # Start daemon');
    this.log('  $ disco daemon start');
    this.log('');
    this.log('  # Check status');
    this.log('  $ disco daemon status');
    this.log('');
    this.log('  # View recent logs');
    this.log('  $ disco daemon logs');
    this.log('');
    this.log('  # View last 100 log lines');
    this.log('  $ disco daemon logs --lines 100');
    this.log('');
  }
}
