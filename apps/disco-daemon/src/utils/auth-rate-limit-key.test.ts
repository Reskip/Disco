...
describe('buildAuthRateLimitKey', () => {
  it('keys POST /authentication by lowercased ip + username', () => {
    const key = buildAuthRateLimitKey(make({ path: '/', body: { username: 'FamilyUser' } }));
    expect(key).toBe('1.2.3.4|familyuser');
  });

  it('keys POST /authentication/refresh by ip only (no username in body)', () => {
  ...
  it('treats missing / non-string username as empty (still IP-bucketed)', () => {
    expect(buildAuthRateLimitKey(make({ path: '/', body: undefined }))).toBe('1.2.3.4|');
    expect(buildAuthRateLimitKey(make({ path: '/', body: { username: 42 } }))).toBe('1.2.3.4|');
  });
  ...
  it('falls back to "unknown" when req.ip is missing', () => {
    ...
    const key = buildAuthRateLimitKey(
      make({ ip: undefined, path: '/', body: { username: 'localuser' } })
    );
    expect(key).toBe('unknown|localuser');
  });
  ...
  it('lowercases the IP component (defence against case-variant spoofing of IPv6)', () => {
    ...
    const key = buildAuthRateLimitKey(
      make({ ip: '::FFFF:1.2.3.4', path: '/', body: { username: 'MixedCase' } })
    );
    expect(key).toBe('::ffff:1.2.3.4|mixedcase');
  });
});
