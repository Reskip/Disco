import { type Session, type Task, TaskStatus } from '@disco/core/types';
...
const DERIVED_TITLE = 'Add a JWT-based authentication system…';
...
describe('TasksService auto-title', () => {
  ...
  it('derives a title from the current authenticated-upload preamble', async () => {
    const { service, sessionsPatch } = makeService({ session: { title: undefined } });
    ...
    expect(titlePatchCall(sessionsPatch)?.[1]).toEqual({ title: '解释图中的异常' });
  });
