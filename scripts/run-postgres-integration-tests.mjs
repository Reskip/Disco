...
const workspaces = ['packages/core', 'apps/disco-daemon'];
...
if (process.env.DISCO_DB_DIALECT !== 'postgresql') {
  console.error('DISCO_DB_DIALECT must be set to postgresql for the PostgreSQL integration lane.');
  process.exit(1);
}
if (!process.env.DISCO_TEST_POSTGRES_URL) {
  console.error('DISCO_TEST_POSTGRES_URL must point at the PostgreSQL application database.');
  process.exit(1);
}
if (!process.env.DISCO_TEST_POSTGRES_ADMIN_URL) {
  console.error(
    'DISCO_TEST_POSTGRES_ADMIN_URL is required to create isolated databases and install test extensions.'
  );
  process.exit(1);
}
...
function resolveConcurrency() {
  const configured = process.env.DISCO_POSTGRES_TEST_CONCURRENCY?.trim();
  if (!configured) return defaultConcurrency;
  const parsed = Number(configured);
  if (!Number.isSafeInteger(parsed) || parsed < 1 || parsed > maxConcurrency) {
    throw new Error(
      `DISCO_POSTGRES_TEST_CONCURRENCY must be an integer from 1 to ${maxConcurrency}`
    );
  }
  return parsed;
}
...
function resolveRunId() {
  const configured = process.env.DISCO_POSTGRES_TEST_RUN_ID?.trim();
  if (!configured) return `${Date.now().toString(36)}_${process.pid.toString(36)}`;
  if (!/^[a-z0-9_]{1,48}$/.test(configured)) {
    throw new Error(
      'DISCO_POSTGRES_TEST_RUN_ID must use 1-48 lowercase letters, numbers, or underscores'
    );
  }
  return configured;
}
    ...
    const child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          DISCO_TEST_POSTGRES_URL: appUrl,
          DISCO_TEST_POSTGRES_ADMIN_URL: adminUrl,
        },
        stdio: 'inherit',
      }
    );
...
const workspaceTests = workspaces.map((workspace) => {
  ...
  const misnamedGatedTests = tests.filter(
    (file) =>
      ...
      readFileSync(file, 'utf8').includes('DISCO_TEST_POSTGRES_URL')
  );
...
const appBaseUrl = process.env.DISCO_TEST_POSTGRES_URL;
const adminBaseUrl = process.env.DISCO_TEST_POSTGRES_ADMIN_URL;
const appRole = decodeURIComponent(new URL(appBaseUrl).username);
if (!appRole) throw new Error('DISCO_TEST_POSTGRES_URL must include the application role');
const runId = resolveRunId();
const reportDirectory = mkdtempSync(path.join(tmpdir(), `disco-postgres-tests-${runId}-`));
...
async function runIsolatedTest(entry, index) {
  const databaseName = `disco_test_${runId}_${index}`;
