import { isTenantAgenticToolEnabled, resolveApiKey } from '@disco/core/config';
...
const usesServerSharedCodexAuthMock = vi.hoisted(() => vi.fn(() => false));
...
vi.mock('@disco/core/agentic-integrations', async () => {
  const actual = await vi.importActual<typeof import('@disco/core/agentic-integrations')>(
    '@disco/core/agentic-integrations'
  );
  return {
    ...actual,
...
vi.mock('./codex-auth-shared.js', () => ({
  ...
  usesServerSharedCodexAuth: usesServerSharedCodexAuthMock,
}));
...
beforeEach(() => {
  ...
  usesServerSharedCodexAuthMock.mockReturnValue(false);
});
...
describe('check-auth tri-state', () => {
  ...
  it('probes the server-wide Codex login even when a new user has no auth method', async () => {
    usesServerSharedCodexAuthMock.mockReturnValue(true);
    ...
    await expect(
      service().create({ tool: 'codex', validateNative: true }, params)
    ).resolves.toMatchObject({ status: 'authenticated', method: 'oauth' });
  });
