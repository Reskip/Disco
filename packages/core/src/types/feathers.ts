 ...
 *
 * Centralized type definitions for FeathersJS patterns used across Disco.
 ...
 *
 * Daemon should import these from @disco/core/types instead of @feathersjs/feathers
 * This provides:
...
/** Request-local entity cache used by authenticated service hooks. */
export interface RBACParams extends AuthenticatedParams {
  /** Cached Session after server-side owner validation. */
  session?: import('./session').Session;
  /** Cached session ID from resolveSessionContext hook */
  sessionId?: string;
  /**
   * Cached Schedule from direct owner validation (used by the
 ...
 *
 * All Disco services implement this interface for consistency.
