/**
 * DISCO_MASTER_SECRET resolution.
 *
 ...
 * registration — can read it off `process.env.DISCO_MASTER_SECRET`.
 *
 ...
 * reads `process.env.DISCO_MASTER_SECRET` in its constructor (fail-fast). It was
 ...
 * "PostgreSQL MCP OAuth pending flows require the deployment DISCO_MASTER_SECRET"
...
/**
 * Resolve DISCO_MASTER_SECRET and set it on `process.env`.
 *
 ...
 */
export async function resolveMasterSecretIntoEnv(config: DiscoConfig): Promise<'env' | 'config'> {
  const resolution = await resolvePersistedSecret({
    name: 'DISCO_MASTER_SECRET (API key encryption)',
    envVar: 'DISCO_MASTER_SECRET',
  ...
  process.env.DISCO_MASTER_SECRET = resolution.value;
