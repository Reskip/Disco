---
title: Branches
description: Branches are the primary unit of work in Disco, isolated git working directories where AI sessions live, organized spatially on boards.
---

# Branches

**A branch is the primary unit of work in Disco.** It's an isolated git working directory (a checkout of your repo at a specific branch) paired with a metadata record (issue/PR links, owners, environment, sessions) that gives it identity on the board.

> **Best practice:** 1 branch = 1 issue = 1 PR = 1 feature.

Disco uses [git branches](https://git-scm.com/docs/git-branch) under the hood, so multiple branches of the same repo can be checked out simultaneously without stashing or switching. Disco manages all of that for you. Just create a branch in the UI or CLI and it handles the git plumbing.

```
~/.disco/worktrees/<repo>/<name>
...
A git branch is a ref. An Disco branch is a _workspace_.
...
When you create a branch, Disco can materialize its working directory in two ways:
...
If `branch_storage` is omitted, Disco keeps the backwards-compatible default: `worktree` is selected by default and both `worktree` and `clone` are available. When an operator restricts `allowed_modes`, the create-branch UI disables unavailable options and the daemon rejects disallowed API/MCP requests.
