...
const DEBUG_MCP_SCOPING =
  process.env.DISCO_DEBUG_MCP_SCOPING === '1' || process.env.DEBUG?.includes('mcp-scoping');
...
/**
 * Config key the built-in Disco MCP server is installed under. Reserved: it
 ...
 */
export const DISCO_MCP_SERVER_NAME = 'disco';
...
export async function getMcpServersForSession(
  ...
  try {
    ...
    // SECURITY: Only user-defined vars are exposed (via DISCO_USER_ENV_KEYS)
