...
— it still fires every **6 days** (`apps/disco-ui/src/hooks/useAuth.ts:241`) on the assumption
...
| Aspect | Value | Source |
|---|---|---|
| Issuer | Daemon (FeathersJS `AuthenticationService` + manual `jwt.sign` in refresh route) | `apps/disco-daemon/src/register-routes.ts:262`, `:412` |
| Library | `jsonwebtoken` v9.0.3 (direct); `@feathersjs/authentication` v5.0.44 (framework) | `apps/disco-daemon/package.json:37`, `packages/core` |
| Algorithm | HS256, `iss: "disco"`, `aud: "https://disco.dev"`, `typ: "access"` | `apps/disco-daemon/src/register-routes.ts:248-254` |
| **Access TTL** | **`15m`** (hardcoded constant `ACCESS_TOKEN_TTL`) | `apps/disco-daemon/src/register-routes.ts:239` |
| Refresh TTL | `30d` (hardcoded constant `REFRESH_TOKEN_TTL`) | `apps/disco-daemon/src/register-routes.ts:240` |
| Secret | `jwtSecret` resolved at daemon boot; not user-configurable via `~/.disco/config.yaml` | `apps/disco-daemon/src/register-routes.ts:243` |
| Refresh endpoint | `POST /authentication/refresh` — accepts `{refreshToken}`, returns fresh access + new refresh tokens | `apps/disco-daemon/src/register-routes.ts:392-442` |

> Note: `session_token_expiration_ms` (default 24h) and `mcp_token_expiration_ms` (default 24h)
> in `~/.disco/config.yaml` are **separate** token systems (executor tokens and the daemon↔MCP
...
| Aspect | Value | Source |
|---|---|---|
| Access token storage | `localStorage['disco-access-token']` | `apps/disco-ui/src/utils/tokenRefresh.ts:10` |
| Refresh token storage | `localStorage['disco-refresh-token']` | `apps/disco-ui/src/utils/tokenRefresh.ts:11` |
| HTTP transport | `Authorization: Bearer <token>` (via Feathers auth-client) | `packages/core/src/api/index.ts:708-781` |
| Socket transport | Socket.io handshake auth after connect (`client.authenticate({strategy:"jwt", accessToken})`) | `apps/disco-ui/src/hooks/useDiscoClient.ts:81-84` |
...
1. **Proactive timer — BROKEN.** `apps/disco-ui/src/hooks/useAuth.ts:236-281`:
   ```ts
...
3. **Socket reconnect fallback.** `apps/disco-ui/src/hooks/useDiscoClient.ts:72-138`: on
...
No commit since `43dfe6b4` has touched `apps/disco-ui/src/hooks/useAuth.ts`'s refresh interval.
...
**Evidence:**
- `apps/disco-ui/src/hooks/useAuth.ts:241` — `REFRESH_INTERVAL = 6 * 24 * 60 * 60 * 1000` (6 days).
...
**Change:** In `apps/disco-ui/src/hooks/useAuth.ts:241`, replace the 6-day interval with a
...
- **Cons:** Requires CSRF protection (double-submit or SameSite + custom header check),
  ...
  with Disco's multi-process topology (daemon, executors, MCP server) where Bearer is
...
3. **Do you want the TTL to be configurable via `~/.disco/config.yaml`?** Right now it's
...
- Server TTL: `apps/disco-daemon/src/register-routes.ts:239-240` (`ACCESS_TOKEN_TTL = '15m'`,
  `REFRESH_TOKEN_TTL = '30d'`).
- Auth config block: `apps/disco-daemon/src/register-routes.ts:242-259`.
- Refresh endpoint: `apps/disco-daemon/src/register-routes.ts:392-442`.
- UI auto-refresh timer (broken): `apps/disco-ui/src/hooks/useAuth.ts:236-281`; interval
  defined on line 241.
- UI tab-visibility re-auth poll: `apps/disco-ui/src/hooks/useAuth.ts:208-234`.
- Socket reconnect → refresh-token fallback: `apps/disco-ui/src/hooks/useDiscoClient.ts:72-138`
  (fallback at `:92-114`).
- Token storage helpers: `apps/disco-ui/src/utils/tokenRefresh.ts:10-11, 49-80`.
