...
export function createStdoutAnalyticsPlugin(
  settings: DiscoAnalyticsStdoutPluginSettings
): ResolvedAnalyticsPlugin {
  const pretty = settings.options?.pretty === true;
  return {
    name: 'disco-stdout-analytics',
    loaded: () => true,
    track: (input: unknown) => {
      try {
...
export function createHttpBatchAnalyticsPlugin(
  settings: DiscoAnalyticsHttpBatchPluginSettings
  ...
  return {
    name: 'disco-http-batch-analytics',
    loaded: () => true,
    track: (input: unknown) => {
      try {
