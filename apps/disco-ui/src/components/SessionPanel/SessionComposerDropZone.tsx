...
export const SessionComposerDropZone: React.FC<SessionComposerDropZoneProps> = ({
  ...
  useEffect(() => {
    ...
    const clearWhenLeavingDocument = (event: DragEvent) => {
      // Native file drags that leave the browser do not reliably emit
      // `dragend` or blur. At the document boundary relatedTarget is null; the
      // next dragover will immediately restore the affordance if the file
      // re-enters the page.
    ...
    document.addEventListener('dragleave', clearWhenLeavingDocument);
    return () => {
      window.removeEventListener('drop', clearDragState);
      window.removeEventListener('dragend', clearDragState);
      window.removeEventListener('blur', clearDragState);
      window.removeEventListener('pagehide', clearDragState);
      document.removeEventListener('dragleave', clearWhenLeavingDocument);
    };
  }, [isDragOver]);
