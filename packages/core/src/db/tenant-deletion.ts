...
const CANONICAL_TENANT_POLICY_EXPRESSION =
  "tenant_id=coalesce(nullif(current_setting('disco.tenant_id',true),''),'default')";
...
const MCP_OAUTH_PENDING_TENANT_POLICY_EXPRESSION =
  "(coalesce(current_setting('disco.system_scope',true),'')='')and(tenant_id=coalesce(nullif(current_setting('disco.tenant_id',true),''),'default'))";
...
export function assertDeclaredGlobalRelation(
  ...
  for (const policy of relation.policies) {
    const referencesTenant =
      ...
      (policy.usingExpression ?? '').includes('disco.tenant_id') ||
      (policy.checkExpression ?? '').includes('disco.tenant_id');
    if (referencesTenant) {
      throw new TenantDeletionCatalogError(
