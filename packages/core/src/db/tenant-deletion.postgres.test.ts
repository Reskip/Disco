 ...
 *   pnpm --filter @disco/core exec vitest run src/db/tenant-deletion.postgres.test.ts
 */
...
const postgresAdminUrl = process.env.DISCO_TEST_POSTGRES_ADMIN_URL;
...
async function seedTenant(db: Database, tenantId: string): Promise<void> {
  await runWithTenantDatabaseScope(db, tenantId, async (scoped) => {
    await new SessionRepository(scoped).create({
      ...
      working_directory: `/tmp/tenant-deletion/${tenantId}/${generateId()}`,
    });
  });
}
  ...
  await executeRaw(
    ...
    sql`
      CREATE POLICY ${policy} ON ${table}
        ...
        USING (
          tenant_id = COALESCE(
            NULLIF(current_setting('disco.tenant_id', true), ''),
            'default'
          )
        )
        WITH CHECK (
...
describe.skipIf(!postgresUrl || !usesPostgresSchema)('deleteTenantData (PostgreSQL)', () => {
  ...
  it.skipIf(!postgresAdminUrl)('fails closed when live-catalog discovery is empty', async () => {
    ...
    try {
      ...
      await executeRaw(
        adminDb,
        sql`CREATE DATABASE ${sql.identifier(databaseName)} OWNER disco_app`
      );
