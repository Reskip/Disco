...
interface MarkdownRendererProps {
   ...
   * Intended for non-streaming document views such as artifact pages.
   */
...
const MarkdownRendererInner: React.FC<MarkdownRendererProps> = ({
  ...
  // publishes the same file through Disco's authenticated upload endpoint.
  // Browsers cannot navigate Windows/file:// paths, and Streamdown otherwise
  // renders a noisy "Blocked URL" placeholder. Keep the filename readable;
  // the adjacent authenticated upload block remains the actual download action.
  text = text.replace(LOCAL_FILESYSTEM_MARKDOWN_LINK, (_, label: string) => {
    return `\`${label.replaceAll('`', '\\`')}\``;
  });

  // Pre-process text to highlight @ mentions
  text = highlightMentionsInMarkdown(text);
  // Convert Disco's authenticated virtual upload links into a closed custom
  // element before Streamdown's external-link hardener runs.
  text = text.replace(
    INTERNAL_UPLOAD_MARKDOWN_LINK,
    (_, filename: string, uploadRef: string) =>
      `<disco-upload upload_ref="${uploadRef}" filename="${escapeHtmlAttribute(filename)}"></disco-upload>`
  );
  ...
  const components = useMemo(
    () => ({
      ...
      a: MarkdownAnchor,
      'disco-upload': UploadAttachmentTag,
    }),
    []
  );
...
function UploadAttachmentTag(props: Record<string, unknown>) {
  ...
  if (/\.(?:avif|bmp|gif|jpe?g|png|svg|webp)$/i.test(filename)) {
    return <InlineUploadImage uploadRef={uploadRef} filename={filename} />;
  }
