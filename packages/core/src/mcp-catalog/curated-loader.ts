...
import type { MCPCatalogEntry, MCPCatalogTransport } from '@disco/core/types';
import {
...
const catalogFileSchema = z
  .object({
     ...
     * registry, so `name` is Disco's reverse-DNS guess at the identity.
     */
 ...
 * a server under exactly that name, or Disco inferred it from the vendor's
