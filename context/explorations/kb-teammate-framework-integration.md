...
**Date:** 2026-06-07
**Scope:** How Disco Knowledge Base should support the `preset-io/disco-teammate` file-based teammate framework. This is an options/design note, not an implementation PR.

## Current state inspected

Disco KB today is DB-backed markdown with namespaces, immutable document versions, internal search units, optional semantic embeddings, graph links, MCP tools, and a UI page:
...
- Services: `apps/disco-daemon/src/services/knowledge-*.ts`
- MCP tools: `apps/disco-daemon/src/mcp/tools/knowledge.ts`
- UI: `apps/disco-ui/src/pages/KnowledgePage.tsx`, `components/KnowledgeGraph/`, `AutocompleteTextarea/kbMentions.ts`
- Existing agent-editing design: `context/explorations/kb-agent-targeted-edits.md`

The public `preset-io/disco-teammate` framework is a markdown file manifold. Its core files are `AGENTS.md`, `BOOT.md`, `BOOTSTRAP.md`, `IDENTITY.md`, `SOUL.md`, `USER.md`, `MEMORY.md`, `BOARD.md`, `HEARTBEAT.md`, `TOOLS.md`, `skills/`, and daily logs under `memory/YYYY-MM-DD.md`. It currently tells agents that filesystem files are state/memory, and git is backup.

## Recommendation summary

1. **Give every Disco Teammate branch an assigned KB namespace.** Store the namespace slug/id in `branch.custom_context.teammate.kb` and mirror it on the KB namespace as `kind: "branch"`, `branch_id`, and metadata identifying it as teammate-owned.
2. **Make context-aware teammate tools the default write path.** Generic `disco_kb_put`, `disco_kb_edit`, and `disco_kb_search` should remain, but teammates should mostly call higher-level tools such as `disco_teammate_memory_append` and `disco_teammate_kb_search` that infer the teammate namespace from the current MCP session's branch.
...
1. **Context-aware MCP:** `disco_teammate_context` returns teammate branch config, board, namespace, privacy policy, and canonical roots.
2. **Session/branch reads:** existing `disco_sessions_get_current` / branch tools can expose `branch.custom_context.teammate.kb` once populated.
...
The teammate KB policy should be evaluated against an **effective operating user**. Disco already has this concept in practice:
...
- Gateway sessions resolve an effective user from platform-user alignment when enabled; otherwise they use the configured channel `disco_user_id` ("Post messages as"). Alignment failures are rejected rather than silently falling back.
...
- `disco_teammate_context` — read-only, current session required.
- `disco_teammate_memory_append` — append one memory entry to the current teammate namespace.
- `disco_teammate_memory_search` — search current teammate memories, defaulting to `kind: "memory"` and `pathPrefix: "memory/"`.
- `disco_teammate_kb_search` — search current teammate namespace plus optional explicitly named shared namespaces.
- Later: `disco_teammate_memory_promote` to curate daily notes into `MEMORY.md`/`memory/curated.md` style docs.
...
- `disco_kb_put`: create/update whole documents. Good for docs, guides, skills, and imports.
- `disco_kb_edit`: deterministic targeted edits. Good for curated memory or docs.
- `disco_teammate_memory_append`: append-only semantic memory primitive. It should internally call repository/service helpers, not ask the model for `KnowledgeEditOp[]`.
- `disco_kb_search`: cross-KB explicit search.
- `disco_teammate_*_search`: opinionated search with namespace and privacy defaults.
...
The teammate framework accumulates `USER.md`, preferences, operational logs, repo names, external-system notes, PR context, and sometimes personal data. The public `disco-teammate` repo itself warns that public backup of teammate state is unsafe. KB should not recreate that footgun with public default visibility.
...
```text
disco://kb/teammate-<branch-short-id>/
...
Teammate tools should use this policy to choose defaults and reject out-of-scope writes. Generic `disco_kb_*` tools can remain powerful/admin-like, but teammates should be instructed and UI-configured to prefer the teammate tools when operating on memory or teammate-owned docs.
...
<!-- disco-memory:v1 date=2026-06-07 -->

<!-- disco-memory-entry id=01J... ordinal=000001 sha256=... -->

- 2026-06-07T14:23:11Z [decision] User prefers review-first PR workflow. (source: disco://session/...)

<!-- /disco-memory-entry -->

<!-- disco-memory-entry id=01J... ordinal=000002 sha256=... -->

- 2026-06-07T15:01:04Z [learning] For repo preset-io/disco, do not run `pnpm build` unless asked.
...
unit.metadata = {
  chunker_version: 'disco-memory-v1',
  entry_id,
  date,
  ordinal,
  category,
...
1. Add a memory-specific chunker that recognizes `disco-memory-entry` blocks and produces one unit per entry or per small batch of adjacent entries.
...
Keep the teammate repo useful without Disco KB:
...
- Minimal `IDENTITY.md`, `SOUL.md`, `USER.md`, `BOARD.md`, `TOOLS.md`: local bootstrap/cache/export for agent platforms that do not have Disco KB.
...
- Daily raw memory (`memory/YYYY-MM-DD.md`) via `disco_teammate_memory_append`.
- Curated long-term memory (`MEMORY.md`) as KB docs, edited with `disco_kb_edit` or future curate/promote tools.
...
1. **Dual-read, KB-preferred:** BOOT says: call `disco_teammate_context`; if KB is configured, read/search KB memory; otherwise read files.
...
3. **Filesystem export:** Provide `disco_teammate_kb_export` or reuse `disco_kb_materialize` to write KB snapshots into the teammate worktree for offline/local agents.
...
5. **No breaking local workflows:** If Disco KB is off or unavailable, the public framework continues exactly as file-mode today.
...
| Targeted KB edit append     | Uses `disco_kb_edit`; version-checked.                   | Still exposes markdown mechanics to agent; concurrency retries awkward.    | Useful intermediate.        |
...
4. Add `disco_teammate_context` MCP tool that resolves current session → branch → teammate config → namespace and returns policy/default roots.
5. Update teammate bootstrap prompt/framework docs to prefer `disco_teammate_context` and KB when present, while preserving file fallback.
...
- Add `disco_teammate_memory_append` and `disco_teammate_memory_search` MCP tools.
...
7. How much filesystem materialization is needed for non-Disco agents and local IDE workflows?
8. Should memory append accept only one bullet by design, or also allow batch append for heartbeat summaries?
9. Should graph links to sessions/tasks/branches be explicit metadata only in V1, or rendered into markdown as `disco://` URIs?
10. How should teammate namespace backup/export interact with git backup of the teammate branch?
11. Should generic `disco_kb_*` tools be policy-aware when called from an teammate session, or should enforcement live only in `disco_teammate_*` tools?

## Bottom line

Make KB the teammate's managed memory substrate, not just a place where agents manually edit markdown. Assign each teammate a private operational namespace, expose context-aware teammate MCP tools, keep generic KB tools for deliberate cross-namespace work, and migrate the file framework gradually so local/file workflows continue to work. Seed KB permissions from branch/teammate ownership, but let KB have its own ACLs and search semantics. The first useful slice is namespace provisioning plus `disco_teammate_context`; the highest-leverage follow-up is an Teammate Knowledge settings tab and deterministic `disco_teammate_memory_append`.
