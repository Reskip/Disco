/**
 * `disco login` - Authenticate with daemon
 *
 * Prompts for username/password and stores a JWT token for future CLI commands.
 */
...
import { normalizeHttpBaseUrl } from '@disco/core/utils/url';
import { createRestClient } from '@disco-live/client';
import { Command, Flags } from '@oclif/core';
import chalk from 'chalk';
import inquirer from 'inquirer';
import { saveToken } from '../lib/auth';
import { probeDiscoDaemon } from '../lib/daemon-probe';
...
export default class Login extends Command {
  ...
  static examples = [
    ...
    '<%= config.bin %> <%= command.id %> --username admin',
  ];
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      this.log('');
      this.log(chalk.dim('User:'), chalk.cyan(authResult.user.username));
      // biome-ignore lint/suspicious/noExplicitAny: AuthenticatedUser type doesn't include name, but it's returned
      ...
      this.log('');
      this.log(chalk.dim('Token saved to ~/.disco/cli-token'));
      ...
      if (errorMessage.includes('Invalid login') || errorMessage.includes('NotFound')) {
        this.error(chalk.red('✗ Invalid username or password'));
      }
