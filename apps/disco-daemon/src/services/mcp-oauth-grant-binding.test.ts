import type { UserMCPOAuthToken } from '@disco/core/db';
import type { MCPServer, MCPServerID } from '@disco/core/types';
...
const resolved = {
  ...
  redirectUri: 'https://disco.example/oauth/callback',
...
describe('MCP OAuth grant configuration binding', () => {
  ...
  it('binds every provider, client, callback, server, mode, and version input', () => {
    ...
    const variants: Array<[string, ServerBinding, MCPOAuthResolvedGrantBinding]> = [
      ...
      ['redirect contract', server, { ...resolved, redirectUri: 'https://other-disco.example/cb' }],
