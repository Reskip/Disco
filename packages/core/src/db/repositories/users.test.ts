...
beforeAll(() => {
  if (!process.env.DISCO_MASTER_SECRET) {
    process.env.DISCO_MASTER_SECRET = 'test-master-secret-users-repo';
  }
});

async function makeUser(repo: UsersRepository): Promise<UserID> {
  const u = await repo.create({
    username: `users-test-${Date.now()}-${Math.random()}@example.com`,
    name: 'Users Test',
  });
...
describe('UsersRepository execution home key validation', () => {
  dbTest('rejects invalid keys on create and update', async ({ db }) => {
    ...
    await expect(
      repo.create({ username: 'invalid-home-create@example.com', unix_username: '1alice' })
