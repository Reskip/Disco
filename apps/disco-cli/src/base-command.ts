/**
 * Base Command - Shared logic for all Disco CLI commands
 *
...
import { createRestClient, getApiKeyFromEnv } from '@disco-live/client';
...
export abstract class BaseCommand extends Command {
   ...
   */
  protected async connectToDaemon(): Promise<DiscoClient> {
    ...
    if (!target) {
      this.error(
        chalk.red('✗ Not authenticated') +
          ...
          chalk.dim('Run:') +
          '\n  ' +
          chalk.cyan('disco login --url <daemon-url>')
      );
    }
      ...
      this.log(
        chalk.red('✗ Connected deployment is not reachable') +
          ...
          (target.source === 'environment'
            ? '\n\nCheck DISCO_API_KEY, DAEMON_URL, and DISCO_DEPLOYMENT_ID.'
    ...
    if (probe.deploymentId !== target.deploymentId) {
      this.error(
        `The daemon identity at ${daemonUrl} changed. Run disco login --url ${daemonUrl} again.`
      );
    }
    if (apiKey) {
  ...
  protected async connectToLocalDaemon(): Promise<DiscoClient> {
   ...
   */
  protected async cleanupClient(client: DiscoClient): Promise<void> {
