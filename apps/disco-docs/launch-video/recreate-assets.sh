...
cp "$PUB"/screenshots/marketing/{disco-marketing-facepile-tooltip,disco-marketing-cursor-indicator,disco-marketing-social-comment-context,disco-marketing-slack-thread}.png "$A/screenshots/marketing/"
...
# 0-40s window, tiny fade-in + 1.2s fade-out — must match assets/music/cues/disco-epic-40s.*
ffmpeg -y -loglevel error -t 40 -i "$SRC" \
  -af "afade=t=in:st=0:d=0.08,afade=t=out:st=38.8:d=1.2" \
  -c:a libmp3lame -q:a 2 "$A/music/disco-epic-40s.mp3"
