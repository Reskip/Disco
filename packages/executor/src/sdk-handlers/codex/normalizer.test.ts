...
function buildUsage(overrides: Record<string, number | undefined> = {}): CodexSdkResponse['usage'] {
  // Mirror the real @openai/codex-sdk Usage shape (input/cached_input/cache_write/output/
...
describe('CodexNormalizer', () => {
  ...
  it('maps and prices GPT-6 Astra prompt-cache writes', () => {
    const normalizer = new CodexNormalizer();
    const event = buildTurnCompletedEvent({
      usage: buildUsage({
        input_tokens: 10_000,
    ...
    const result = normalizer.normalize(event, { modelHint: 'gpt-6-astra' });

    expect(result.tokenUsage.cacheCreationTokens).toBe(2_000);
    expect(result.costUsd).toBeCloseTo(0.119, 8);
  });
