...
describe.skipIf(!postgresUrl || !usesPostgresSchema)(
  ...
  () => {
    ...
    const originalMasterSecret = process.env.DISCO_MASTER_SECRET;
    const servers: http.Server[] = [];

    beforeAll(async () => {
      process.env.DISCO_MASTER_SECRET = masterSecret;
    ...
    afterAll(async () => {
      ...
      if (originalMasterSecret === undefined) delete process.env.DISCO_MASTER_SECRET;
      else process.env.DISCO_MASTER_SECRET = originalMasterSecret;
    });
    ...
    async function seed(label: string, tokenEndpoint: string): Promise<Seed> {
      ...
      return runWithTenantDatabaseScope(dbA, tenantId, async (scoped) => {
        const user = await new UsersRepository(scoped).create({
          username: `${crypto.randomUUID()}@example.test`,
          name: `Refresh ${label}`,
        });
    ...
    it('allows one daemon to rotate while a peer observes the committed result', async () => {
      ...
      expect(
        Object.values(rawStored ?? {}).every((value) =>
          String(value).startsWith('disco-mcp-oauth:v1:')
        )
      ).toBe(true);
    });
