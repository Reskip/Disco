...
import type { DiscoConfig } from '../config/types.js';
import type {
...
export const DISCO_TELEMETRY_DOCS_URL = 'https://disco.live/faq#open-source-telemetry';
...
export function pruneDefaultOpenSourceTelemetryDestination(config: DiscoConfig): DiscoConfig {
...
export function resolveOpenSourceTelemetryConfig(
  ...
  const forced = envFlag(env.DISCO_TELEMETRY);
  ...
  return {
    ...
    endpoint:
      env.DISCO_TELEMETRY_ENDPOINT ?? telemetry?.endpoint ?? DEFAULT_OPEN_SOURCE_TELEMETRY_ENDPOINT,
    writeKey:
      env.DISCO_TELEMETRY_WRITE_KEY ??
      telemetry?.write_key ??
      DEFAULT_OPEN_SOURCE_TELEMETRY_WRITE_KEY,
    debug: envFlag(env.DISCO_TELEMETRY_DEBUG) ?? telemetry?.debug === true,
  ...
  track(input: OpenSourceTelemetryEvent): void {
    ...
    try {
      const payload: SegmentTrackPayload = {
        ...
        context: { app: 'disco', telemetry: 'open-source' },
      };
...
export function createOpenSourceTelemetryLogger(
  configInput: DiscoConfig | { telemetry?: DiscoConfig['telemetry'] },
