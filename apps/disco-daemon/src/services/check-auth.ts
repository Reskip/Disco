...
import { getAgenticToolIntegration, TOOL_API_KEY_NAMES } from '@disco/agentic-tools';
import { loadManagedAgenticToolSdk } from '@disco/core/agentic-integrations';
import { type DiscoConfig, isTenantAgenticToolEnabled, resolveApiKey } from '@disco/core/config';
import {
  getCurrentTenantId,
...
import type { SDKUserMessage } from '@disco/core/sdk';
import type {
  AuthCheckResult,
  AuthCheckStatus,
  AuthenticatedParams,
...
import { isAgenticToolName } from '@disco/core/types';
...
export function createCheckAuthService(
  ...
  return {
    async create(
      ...
      if (
        tool === 'codex' &&
        (useNativeAuth || (!apiKey && usesServerSharedCodexAuth(config)))
      ) {
        ...
        return data.validateNative
          ...
          : unknown('The server-wide ChatGPT login is configured but has not been validated.');
      }
