...
The functional gap that anonymous mode fills today — _"user runs `disco init`, then opens the UI without typing credentials"_ — can be replaced by **auto-creating a default admin during `disco init` and persisting a one-time login token to disk** so the UI authenticates silently on first open. Single-user installs stay frictionless; multi-user installs gain the consistency they should have always had.
...
The `disco init` command flips both atomically — `init.ts:447-448` and `init.ts:346-347` — so the only user-facing knob is "Enable authentication?". But config files on disk can drift, and `apps/disco-cli/src/commands/config/index.ts:94-128` exposes both as separately-settable.

No env-var override, no `soloMode` / `singleUser` / `devMode` flag exists. Public-deployment detection (`apps/disco-daemon/src/index.ts:213-217`) refuses to boot if `allowAnonymous && (non-loopback bind || public base URL || RAILWAY/RENDER env)`, which is a guard against the most obvious misconfiguration. **That guard exists only because anonymous mode exists.** Remove anonymous mode → remove the guard.

### 1.2 The synthetic anonymous user

`apps/disco-daemon/src/strategies/anonymous.ts:21-49` mints a fake user object on every anonymous request:
...
- **`apps/disco-daemon/src/utils/inject-created-by.ts:46`** defaults internal-call attribution to `'anonymous'` if the caller user is missing
...
| Site                                                                         | Count | Where                                                                                                              |
| ---------------------------------------------------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------ |
| `getReadAuthHooks()` spread (`...allowAnonymous ? [] : [requireAuth]`)       | 16    | `apps/disco-daemon/src/register-hooks.ts` (lines 344, 433, 442, 480, 566, 577, 591, 896, 1476, 1867, 1945, +5 more) |
...
| Auth strategy array branch                                                   | 1     | `apps/disco-daemon/src/index.ts:147`                                                                                |
| Public-deployment security check                                             | 1     | `index.ts:219-239`                                                                                                 |
| Anonymous-strategy class                                                     | 1     | `apps/disco-daemon/src/strategies/anonymous.ts` (whole file)                                                        |
| Anonymous fallback in `inject-created-by`                                    | 1     | `utils/inject-created-by.ts:46`                                                                                    |
| WebSocket "anonymous OK" branch                                              | 1     | `apps/disco-daemon/src/setup/socketio.ts:288`                                                                       |
| Health endpoint exposes flag to client                                       | 2     | `register-routes.ts:3099-3100`                                                                                     |
| CLI base-command anonymous fallback auth                                     | 1     | `apps/disco-cli/src/base-command.ts:84-116`                                                                         |
...
| UI client connect branches                                                   | 3     | `useDiscoClient.ts:110, 281, 427`                                                                                   |
...
`disco init` (`apps/disco-cli/src/commands/init.ts:425-505`) prompts:
...
If **no**: prints a "you can enable later" hint with `disco config set daemon.requireAuth true` + `disco local create-admin`. Leaves both flags unset (defaults take over → anonymous wins).

`--force` mode (`init.ts:345-372`) auto-enables auth and creates the default `admin@disco.live` / `admin` user. **Today, `disco init` already has a complete auth-required path that produces a working solo install.** What anonymous mode buys is "skip the email/password prompts during init."

There is **no daemon-side first-run bootstrap.** If the database has zero users and `requireAuth: true`, the daemon will start and the UI's LoginPage will accept no credentials. The user must run `disco local create-admin` (or `disco init`) out of band. This is an upgrade-path gap we'll have to close as part of removal.

### 1.6 Documentation surface

Anonymous mode is mentioned in:

- `apps/disco-docs/pages/api-reference/index.mdx:94` — "Anonymous Mode: For local development, anonymous authentication is enabled by default"
- `apps/disco-docs/pages/guide/typescript-client.mdx:80, 144` — code comment + auth-strategy table row
- `apps/disco-docs/pages/guide/architecture.mdx:177` — strategy list mentions `anonymous`
- `apps/disco-docs/pages/security.mdx:171-172` — "Anonymous mode (if enabled) is read-only viewer"
- `apps/disco-docs/pages/guide/getting-started.mdx`, `README.md` — Quick Start implicitly relies on no-auth-by-default
...
- `apps/disco-daemon/src/auth-jwt-integration.test.ts` (46 tests) — unchanged
- `apps/disco-daemon/src/register-hooks.test.ts` (19 tests) — unchanged
- `apps/disco-daemon/src/setup/socketio.test.ts` — 1-2 anonymous-specific tests removed
- `apps/disco-daemon/src/utils/inject-created-by.test.ts:105` — 1 test for the `'anonymous'` fallback removed
...
**The bug:** `disco_repos_create_remote` returns `{status: "pending"}` for private GitHub repos but the clone never completes; daemon process has no `GITHUB_TOKEN`.
...
```bash
$ pnpm install -g disco
$ disco init
✔ Created database at ~/.disco/disco.db
✔ Created admin user (admin@disco.live)
  Generated password: ko4n-5xqj-b8wm-pa11   ← shown ONCE, also written to ~/.disco/admin-credentials with mode 0600
✔ Wrote ~/.disco/auth-token (browser auto-login token, mode 0600, single-use, 7-day expiry)
...
1. **Generated password** printed to stdout once + saved to `~/.disco/admin-credentials` (mode 0600). User can recover it any time with `disco user show-credentials`.
2. **One-time auth token** in `~/.disco/auth-token` consumed by the UI on first `disco open`. Deleted after use; regenerable via `disco user issue-login-link`.
...
- The "I want to add a second user" path is just `disco user create` — no config-file edits, no daemon restart
...
- `apps/disco-daemon/src/strategies/anonymous.ts` (entire file)
...
- The UI client's anonymous-mode branches in `useDiscoClient.ts` (3 sites)

### 3.4 What we add

- **First-run admin bootstrap on daemon start.** New module `apps/disco-daemon/src/setup/first-run-bootstrap.ts`: if `users` table has zero rows, create the default admin (with a generated password, not the hardcoded `admin/admin`), write credentials to `~/.disco/admin-credentials` (mode 0600), and write a one-time auth token to `~/.disco/auth-token`. Log to stderr that this happened. **This is the safety net for upgrades** — daemons that started anonymous and have data with `created_by='anonymous'` will get an admin user automatically; the operator finds credentials in `~/.disco`.
- **`disco open` consumes `~/.disco/auth-token`** when present, appends `?t=<token>` to the URL, deletes the file.
...
- **`disco user issue-login-link`** CLI to regenerate a one-time token (covers password loss, lets sysadmins email a self-serve link).
...
- `disco init` interactive flow continues to work. The only change: the "Enable auth?" question is removed (it's always enabled).
- `disco local create-admin` continues to work for sysadmins who want to scripted-bootstrap.
- The default `admin@disco.live` / `admin` constant stays for `--force` mode but emits a "MUST change password" warning that's already enforced by `must_change_password`.
...
2. **Anonymous-mode installs being upgraded** — daemon starts, sees zero users, runs first-run bootstrap, attributes all `created_by='anonymous'` rows to the new admin. Operator finds credentials in `~/.disco/admin-credentials`. **No data loss.** Print a clear notice on daemon startup: _"Migrated N rows from anonymous attribution. Admin credentials at ~/.disco/admin-credentials."_
...
- Add `disco user issue-login-link` CLI
- Add UI `/auth/redeem` route handler
- Modify `disco open` to consume `~/.disco/auth-token`
...
- Update `disco init` to no longer ask the auth question (always enabled)
...
- Delete `apps/disco-daemon/src/strategies/anonymous.ts`
...
- `apps/disco-daemon/src/index.ts` — ~65 (auth strategy array, security check)
- `register-hooks.ts` — ~40 (helper definition + 16 spreads collapse + 3 ternaries)
- `apps/disco-cli/src/commands/init.ts` — ~120 (`promptAuthSetup` simplifies dramatically)
- `useDiscoClient.ts` — ~25 (3 connection branches collapse)
...
2. **The `~/.disco/auth-token` UX** — is writing a file to disk and consuming it via URL the right pattern, or do we want a different bootstrap (e.g., printed magic link, env var, browser-side prompt)? **This is the single biggest UX decision in the proposal.** I picked file-on-disk because it makes `disco open` Just Work and matches the SSH-like ergonomic of "if you have shell access, you have admin."
...
- The `~/.disco/auth-token` mechanism (alternatives: magic link printed to stdout that you paste, or always show LoginPage and let user paste credentials from `~/.disco/admin-credentials`).
...
- `apps/disco-daemon/src/strategies/anonymous.ts` (entire file)
- `apps/disco-daemon/src/index.ts:130-240` (auth config, security check)
- `apps/disco-daemon/src/register-hooks.ts` (16 spread sites + 3 conditional gates)
- `apps/disco-daemon/src/register-routes.ts:280, 1114, 1710, 3099-3100`
- `apps/disco-daemon/src/utils/inject-created-by.ts:32-46`
- `apps/disco-daemon/src/setup/socketio.ts:288`
- `apps/disco-cli/src/base-command.ts:84-116`
- `apps/disco-cli/src/commands/init.ts:340-505`
- `apps/disco-cli/src/commands/config/index.ts:94-128`
- `apps/disco-cli/src/commands/local/create-admin.ts` (whole file)
- `apps/disco-ui/src/hooks/useDiscoClient.ts:41, 110, 281, 427`
- `apps/disco-ui/src/hooks/useAuthConfig.ts:14, 91`
- `apps/disco-ui/src/App.tsx:326-376`
