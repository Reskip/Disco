...
import { getCurrentTenantId, type TenantScopedDatabase, UsersRepository } from '@disco/core/db';
...
import { resolveDelegatedHomeKey, type UnixUserMode } from '@disco/core/unix';
...
/**
 * Disco's trusted local `simple` deployment intentionally has one Codex CLI
 * home for the whole server. In that narrow mode auth.json is server state,
 * not a per-account preference. Hosted/auth-resolved tenancy must never take
 * this path because it requires an independently routed credential home.
 */
export function usesServerSharedCodexAuth(
  config: DeepReadonly<DiscoConfig> | undefined
): boolean {
