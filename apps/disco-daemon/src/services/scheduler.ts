 ...
 *
 * **Template Rendering:**
 * - Uses Handlebars to render prompt templates with optional Agent and
 *   Schedule context.
 */
...
/**
 * Render a Handlebars schedule-prompt template against the optional Agent
 * target and Schedule metadata.
 *
...
async function resolveScheduleAgenticToolConfig(
  ...
  const materialized = await materializeAgenticToolConfiguration(db, {
    ...
    executionOwnerId: schedule.created_by as import('@disco/core/types').UserID,
  });
  return {
...
export interface SchedulerConfig {
  ...
  /** Tenant-aware root containing `user-*` workspace directories. */
  worktreesRoot?: (tenantId?: string) => string;
}
...
export class SchedulerService {
  ...
  constructor(db: TenantScopeAwareDatabase, app: Application, config: SchedulerConfig = {}) {
    ...
    this.config = {
      ...
      worktreesRoot: config.worktreesRoot ?? getWorktreesRoot,
    };
   ...
   * keyed directly off `schedules.created_by`.
   *
  ...
  private async resolveCreatorUnixUsername(
    ...
    if (!unixUsername && unixUserModeRequiresExecutionHomeKey(this.config.unixUserMode)) {
      throw new Error(
        `Schedule creator ${creator.username} has no unix_username set. Cannot spawn scheduled session in ${this.config.unixUserMode} execution mode.`
      );
    }
   ...
   *
   * Steps:
   * 1. Resolve the optional Agent target and the per-run Session directory.
   ...
   *    by the SAME SCHEDULE. Other schedules do not block one another.
  ...
  private async spawnScheduledSession(
    ...
    const agent = schedule.agent_id
      ? await this.withTenantDatabase(() =>
          this.agentRepo.findOwnedById(schedule.agent_id!, schedule.created_by)
        )
      : null;
    if (schedule.agent_id && (!agent || agent.archived || agent.state !== 'ready')) {
      throw new ScheduleNotReadyError(
        'schedule_incomplete',
        `Schedule ${schedule.schedule_id} references an unavailable Agent`
      );
    }
    ...
    const renderedPrompt = renderSchedulePrompt(schedule.prompt, schedule, scheduledRunAt, agent);
    ...
    const effectiveMcpIds = schedule.mcp_server_ids ?? [];
    ...
    const userRoot = resolveDiscoUserWorkspaceDirectory(
      this.config.worktreesRoot(getCurrentTenantId()),
      schedule.created_by
    );
    const candidateWorkingDirectory = agent
      ? resolveDiscoAgentSessionWorkingDirectory(agent.workspace_path, candidateSessionId)
      : resolveDiscoStandaloneSessionWorkingDirectory(userRoot, candidateSessionId);
    ...
    try {
      admission = await this.withTenantDatabase(async () => {
        ...
        const session: Partial<Session> = {
          ...
          working_directory: candidateWorkingDirectory,
    ...
    const workingDirectory = admission.session.working_directory;
    if (!workingDirectory) {
      throw new Error(`Scheduled Session ${admission.session.session_id} has no working directory`);
    }
  ...
  private async recoverIncompleteSession(
    ...
    if (!session?.is_scheduled || session.scheduled_run_at !== scheduledRunAt) {
      return;
    }
    if (
      await this.withTenantDatabase(() =>
    ...
    if ((fallbackPrompt === undefined || fallbackMcpIds === undefined) && schedule) {
      const agent = schedule.agent_id
        ...
        : null;
      if (schedule.agent_id && !agent) return;
      fallbackPrompt ??= renderSchedulePrompt(schedule.prompt, schedule, scheduledRunAt, agent);
      fallbackMcpIds ??= schedule.mcp_server_ids ?? [];
    }
  ...
  private async initializeScheduledSession(input: {
    ...
    const task = (await this.app.service('/sessions/:id/prompt').create(
      ...
      } as import('@disco/core/types').AuthenticatedParams & { route: { id: string } }
    )) as Task;
