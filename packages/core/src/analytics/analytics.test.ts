...
import type { DiscoAnalyticsSettings } from '../config/types.js';
...
const enabledBase: DiscoAnalyticsSettings = {
  enabled: true,
...
describe('analytics logger', () => {
  ...
  it('resolves enabled stdout plugins', async () => {
    ...
    expect(plugins[0].name).toBe('disco-stdout-analytics');
  });
...
describe('analytics plugins', () => {
  ...
  it('rejects unsupported plugins passed by an untyped caller', async () => {
    await expect(
      ...
      } as unknown as DiscoAnalyticsSettings)
