 ...
 */
export type DateBucket = '30s' | 'minute' | 'hour' | 'day' | 'week' | 'month';
  ...
  if (isSQLiteDatabase(db)) {
    ...
    switch (bucket) {
      case '30s':
        return sql`strftime(
          '%Y-%m-%dT%H:%M:%S.000Z',
          CAST(${unixSeconds} / 30 AS INTEGER) * 30,
          'unixepoch'
        )`;
      case 'minute':
        return sql`strftime('%Y-%m-%dT%H:%M:00.000Z', ${unixSeconds}, 'unixepoch')`;
      case 'hour':
    ...
    // PostgreSQL date_trunc accepts minute/hour/day/week/month; week is already ISO (Monday).
    ...
    if (bucket === '30s') {
      return sql`to_char(
        to_timestamp(FLOOR(EXTRACT(EPOCH FROM ${column}) / 30) * 30) AT TIME ZONE 'UTC',
