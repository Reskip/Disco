 ...
 */
import { Conflict, NotFound } from '@disco/core/feathers';
import type { Params, Task, TaskID } from '@disco/core/types';
...
describe('normalizeMessageSource', () => {
  it('derives disco provenance for every external request', () => {
    expect(normalizeMessageSource('disco', { provider: 'rest' })).toBe('disco');
    expect(normalizeMessageSource('gateway', { provider: 'rest' })).toBe('disco');
    expect(normalizeMessageSource(undefined, { provider: 'socketio' })).toBe('disco');
    expect(normalizeMessageSource('bogus' as unknown as 'disco', { provider: 'rest' })).toBe('disco');
  });

  it('preserves valid provenance only for trusted internal requests', () => {
    expect(normalizeMessageSource('disco', {})).toBe('disco');
  ...
  it('omits absent or invalid provenance for internal requests', () => {
    ...
    expect(normalizeMessageSource('bogus' as unknown as 'disco', {})).toBeUndefined();
  });
});
