...
describe('waitForMCPOAuthAttempt', () => {
  ...
  it('replaces optimistic UI auth state with durable status and server reads', async () => {
    ...
    expect(discoStore.getState().userAuthenticatedMcpServerIds).toEqual(new Set(['server-1']));
    expect(discoStore.getState().mcpServerById.get('server-1')).toMatchObject({
      name: 'durable',
  ...
  it('refetches deleted durable grant state before returning needs_reauth', async () => {
    ...
    expect(discoStore.getState().userAuthenticatedMcpServerIds).toEqual(new Set());
    expect(discoStore.getState().mcpServerById.get('server-1')).toMatchObject({
      auth: { type: 'oauth' },
    });
  });
