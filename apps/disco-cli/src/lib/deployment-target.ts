...
import { getApiKeyFromEnv } from '@disco-live/client';
...
export async function resolveConnectedDeploymentTarget(): Promise<DeploymentTarget | null> {
  ...
  if (apiKey) {
    if (!process.env.DAEMON_URL || !process.env.DISCO_DEPLOYMENT_ID) {
      throw new Error(
        'Environment API-key authentication requires DAEMON_URL and DISCO_DEPLOYMENT_ID.'
      );
    }
    return {
      url: normalizeHttpBaseUrl(process.env.DAEMON_URL, 'DAEMON_URL'),
      deploymentId: process.env.DISCO_DEPLOYMENT_ID,
      source: 'environment',
    };
  }
