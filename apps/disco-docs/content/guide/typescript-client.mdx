...
description: Drive Disco from TypeScript or JavaScript with @disco-live/client, the same fully-typed client Disco's UI uses, including REST, real-time Socket.IO, and a reactive session API.
---

# TypeScript Client

If you're scripting Disco from Node, building a custom dashboard, wiring it into a CI workflow, or embedding live session views in your own app, **`@disco-live/client` is the way**. It's the same fully-typed client the Disco UI consumes internally, so it stays in lockstep with the daemon API and ships first-class types for every service, model, and event.

**Two doorways into Disco (same daemon, two audiences):**

- **Inside agents** (Claude Code, Codex, Gemini), the [Disco MCP Server](/guide/internal-mcp) gives sessions self-aware orchestration.
- **Outside agents** (TS/JS apps, scripts, services), `@disco-live/client` gives humans and services the same API.
...
```bash
npm install @disco-live/client
# or
pnpm add @disco-live/client
```
...
const apiKey = getApiKeyFromEnv() ?? 'disco_sk_…'; // from User Settings → Personal API Keys
...
The headline feature: a **reactive session handle** that mirrors the entire UI state surface for one session (tasks, messages, queued prompts, streaming chunks, tool executions) and notifies you on every change. Same primitive that powers the Disco UI's session panel.

```ts
import { createClient, retainReactiveSession } from '@disco-live/client';
...
- Mirror Disco sessions into a Slack or terminal UI
- Pipe live tool calls into your own observability stack
- Embed an Disco session widget in an internal portal

## Authentication

| Strategy | When to use | How |
|---|---|---|
| **API Key** | Scripts, services, CI | Pass to `createRestClient(url, apiKey)`, or set `DISCO_API_KEY=disco_sk_…` and use `getApiKeyFromEnv()` |
...
Service types are exported and fully typed, including `client.service('sessions')`, `client.service('branches')`, `client.service('boards')`, `client.service('messages')`, `client.service('tasks')`, `client.service('repos')`, `client.service('users')`, and the rest. Every model lives in `@disco/core/types` (re-exported from `@disco-live/client`), so `Session`, `Branch`, `Task`, `Message`, `Board`, etc. are available without round-tripping through `any`.

Helpers worth knowing:

- `isDaemonRunning(url)` runs a health check before connecting
- `getApiKeyFromEnv()` pulls `DISCO_API_KEY` from the environment
- `shortId(id)` renders the canonical short form Disco uses everywhere (URLs, pills, logs, notifications)

## When to reach for which client

| You are… | Use… |
|---|---|
| An agent running inside a session | [Disco MCP Server](/guide/internal-mcp) (already injected) |
...
- [Disco MCP Server](/guide/internal-mcp) (the agent-facing companion to this client)
