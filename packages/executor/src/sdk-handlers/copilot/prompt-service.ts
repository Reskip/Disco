...
export class CopilotPromptService {
   ...
   *
   * Converts Disco's MCP server config to Copilot SDK format.
  ...
  private async buildMcpServers(
    ...
    const serversWithSource = await getMcpServersForSession(
      ...
      // Disco cannot construct, and a miss here reads as "allow". The CLI does
      // document a `<mcp-server-name>(tool-name?)` permission pattern built
      // from names Disco holds verbatim, but only for its `--deny-tool` flag,
    ...
    // Include Disco self-access MCP server
    if (mcpToken) {
      const daemonUrl = await getDaemonUrl();
      copilotMcpServers.disco = {
        type: 'http',
        url: `${daemonUrl}/mcp`,
        headers: {
      ...
      console.log(`   📝 [Copilot MCP] Configured Disco MCP server (HTTP)`);
    }
  ...
  /**
   * Create static Disco system prompt for Copilot orientation
   */
  private async buildSystemMessage(_sessionId: SessionID): Promise<string> {
    return renderDiscoSystemPrompt();
  }
