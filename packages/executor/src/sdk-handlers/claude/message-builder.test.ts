import { generateId } from '@disco/core';
  ...
  it('preserves the provider failure marker without allowing reserved metadata overrides', async () => {
    ...
    const result = await createSystemMessage(
      ...
      messagesService,
      {
        ...
        is_meta: false,
        model: 'spoofed-model',
      } as NonNullable<import('@disco/core/types').Message['metadata']>
    );
...
describe('createUserMessage', () => {
  ...
  describe('skip-if-exists guard (Alt D — never-lose-prompt)', () => {
    ...
    it('should skip create for callback-shaped row (type:system, role:user) — blocker regression', async () => {
      // Daemon writes Disco callback prompts as `type:'system', role:'user'`
      ...
      const existingMessages = [
        {
          ...
          task_id: taskId,
          metadata: { is_disco_callback: true },
        },
      ];
