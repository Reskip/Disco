import type { EffortLevel } from '@disco/core/types';
...
async function submittedPrompt(effort?: EffortLevel) {
  ...
  await expect(
    executeTask(
      client,
      {
        discoSessionId: 'session-1',
        taskId: 'task-1',
        prompt: 'Continue',
        discoAssistantMessageId: 'message-1',
        effort,
...
describe('OpenCodeTool prompt variants', () => {
  it('submits the configured Disco effort as the native prompt variant alongside the Disco system prompt', async () => {
    const request = await submittedPrompt('max');

    expect(request?.body).toMatchObject({ variant: 'max' });
    expect(request?.body?.system).toContain('Disco Session Context');
    expect(request?.body?.system).toContain('disco_sessions_get_current_context');
  });
  ...
  it('omits the native prompt variant but still submits the Disco system prompt when effort is unset', async () => {
