...
description: 'Disco rich GUI sits atop a fully-typed REST API, powerful CLI, and TypeScript client enabling git worktree management, agent orchestration from CI/CD, and custom workflows from any app.'
author: 'Maxime Beauchemin'
date: 2025-11-16
image: '/images/blog/disco-platform.png'
---

At first glance, Disco looks like a GUI for managing AI agents. A multiplayer canvas where you can see Claude Code sessions, track progress, and coordinate work across worktrees.
...
**Here's the insight:** Disco isn't a GUI with an API. It's a platform that happens to have a really good GUI.

## The Architecture: Layers All the Way Down

Every agent session in Disco is persistent, observable, and resumable. And it's built in layers:

```
┌─────────────────────────────────────────┐
│  Layer 4: Interfaces                    │
│  • Disco GUI (React + Canvas)            │
│  • Your Custom GUI                      │
│  • Terminal UI                          │
└─────────────────┬───────────────────────┘
                  │
...
┌─────────────────▼───────────────────────┐
│  Layer 2: CLI                           │
│  disco session list, disco worktree create│
│  • Shell composable                     │
│  • CI/CD ready                          │
└─────────────────┬───────────────────────┘
                  │
...
Every operation Disco supports is exposed via a clean REST API with real-time WebSocket events:
...
The same client that powers the Disco GUI and CLI is available as `@disco/core/api`:
...
Fully typed, real-time, same capabilities as the GUI. Build automation scripts, custom tooling, or integrate Disco into your existing apps.
...
Since the REST API is OpenAPI/Swagger compliant, generating clients for other languages should be straightforward. If you're interested in building or maintaining a client for your language of choice, reach out! Open a [GitHub Discussion](https://github.com/preset-io/disco/discussions) and we can coordinate.
...
disco worktree create myapp feat-auth --branch main

# Start a session in that worktree
disco session create \
  --worktree feat-auth \
  --agent claude-code \
  --prompt "Implement JWT authentication"

# List all running sessions
disco session list --status running

# Watch a session's progress
disco session watch abc123
```
...
Disco takes a different approach. Here's what it looks like:

```yaml
# .github/workflows/disco-review.yml
on:
  pull_request:
...
jobs:
  disco-review:
    ...
    steps:
      - uses: disco-live/disco-action@v1
        with:
          agent: claude-code
...
1. GitHub Action creates an Disco worktree for the PR branch
2. Spawns a Claude Code / Codex / Gemini session on your "CI Reviews" board
3. The agent reviews the code (**you can watch it live in the Disco GUI**)
4. Session persists after the review completes
5. Results get posted back to the PR
6. **You get a direct link to the session in Disco**
...
Because Disco is **stateful**, you're not starting from scratch every time. The PR creation session? Still there. The review session? Available for follow-up. The bug fix attempt? Its context can inform the next iteration.

**Example follow-up workflow:**

```bash
# In PR comments or subsequent GHA runs:
@disco find the session that opened this PR and ask it to fix the failing unit tests

# Or programmatically:
disco session list --worktree pr-1234
disco session prompt abc123 "The tests are failing on line 45, can you fix it?"
```

The GitHub Action gives you a link that goes **straight to the live session** in your Disco instance. Click it and you're watching the agent work in real-time, on your board, with full history.
...
Forget the agents for a moment. Disco is a solid git worktree manager on its own:

```bash
# Create parallel worktrees for different features
disco worktree create myapp feat-auth
disco worktree create myapp feat-payments
disco worktree create myapp fix-cors
...
Even if you never spawn an agent through Disco, Disco gives you:
...
Your app can spawn and coordinate Disco sessions:

```typescript
// From your VS Code extension, Slack bot, internal tool, etc.
import { createClient } from '@disco/core/api';

const client = createClient(process.env.DISCO_DAEMON_URL);
...
Given it happens in Disco, you're always able to use the GUI for observability and interactions.

Use cases:

- **VS Code extension** that delegates complex refactors to Disco
...
- Mobile app (why not?) - Disco's got one, but maybe something more fitted to your app or workflow.

The platform doesn't care. It's just HTTP + WebSocket. The Disco GUI is one opinion about how to visualize sessions. Yours might be different.
...
Connect your issue tracker directly to Disco:
...
app.post('/webhooks/github', async req => {
  ...
  if (issue.labels.includes('agent-fix')) {
    // Auto-spawn Disco session for labeled issues
...
Linear tickets → Disco sessions. Jira issues → worktrees. Your workflow → programmatic orchestration.
...
async function reviewPR(prNumber: number, prBranch: string) {
  ...
  return `https://disco.live/sessions/${session.session_id}`;
}
```
...
We built Disco with a great visual interface because spatial canvases are powerful for coordination. But we also knew that locking people into one interface would be a mistake.
...
This isn't theoretical. These are the primitives Disco provides today.

## What's Next

We're working on:

- **Official `@disco` GitHub Action** - One-line integration for PRs and issues
...
Read about [how Disco is built](/architecture) - FeathersJS, Drizzle ORM, LibSQL, the whole stack.

**Join the discussion:**

Building something with Disco's API? Running into limitations? Have ideas for the GitHub Action? We'd love to hear about it.

- [GitHub Issues](https://github.com/preset-io/disco/issues)
- [GitHub Discussions](https://github.com/preset-io/disco/discussions)

---

**The takeaway:** Disco is a platform for persistent, observable, resumable agent orchestration. The GUI is one way in. The API, CLI, and TypeScript client give you the rest.
...
_If you're juggling agents across terminals or dealing with ephemeral CI bots that vanish, [try Disco](https://disco.live). If you're building on top of the platform, we'd love to know what you're making._
...
- [CLI Reference](/cli)
- [GitHub Repository](https://github.com/preset-io/disco)
