...
import { EMPTY_MAPS } from './discoMaps';
import { discoStore } from './discoStore';
...
describe('discoStore scaffold', () => {
  ...
  it('reset() restores empty maps and initial meta after mutation', () => {
    ...
    discoStore.getState().setMap('sessionById', populated);
    discoStore.getState().setLoading(false);
    discoStore.getState().setError('boom');
  ...
  it('setLoading / setMap update their fields', () => {
    discoStore.getState().setLoading(false);
    expect(discoStore.getState().loading).toBe(false);

    const next = new Map<string, Session>([['s1', { session_id: 's1' } as Session]]);
    discoStore.getState().setMap('sessionById', next);
    expect(discoStore.getState().sessionById).toBe(next);

    // Functional-updater form mirrors setMapSlice's signature.
    discoStore.getState().setMap('sessionById', (prev) => {
      const copy = new Map(prev);
      copy.set('s2', { session_id: 's2' } as Session);
      return copy;
    });
    expect(discoStore.getState().sessionById.size).toBe(2);
  });

  it('no-op setMap (same reference) preserves the outer state reference', () => {
    const before = discoStore.getState();
    ...
    discoStore.getState().setMap('sessionById', before.sessionById);
    expect(discoStore.getState()).toBe(before);

    // A genuine change DOES allocate a new state object.
    discoStore.getState().setMap('sessionById', new Map());
    expect(discoStore.getState()).not.toBe(before);
  });
  ...
  it('replaceMaps writes changed slices and skips unchanged ones', () => {
    ...
    discoStore.getState().replaceMaps({
      sessionById: sessions,
      boardById: before.boardById,
    });

    expect(discoStore.getState().sessionById).toBe(sessions);
    expect(discoStore.getState().boardById).toBe(before.boardById);

    // An all-no-op replaceMaps preserves the outer state reference.
    const stable = discoStore.getState();
    discoStore.getState().replaceMaps({ sessionById: sessions });
    expect(discoStore.getState()).toBe(stable);
  });
});

describe('discoStore branch hard-delete cascade', () => {
  it('mirrors branch/session cascades and SET NULL updates across normalized slices', () => {
    ...
    discoStore.getState().applyBranchHardDeleteCascade(branch.branch_id);
    const state = discoStore.getState();
  ...
  it('removes a deleted remote target surrogate from the surviving source branch', () => {
    ...
    discoStore.getState().applyBranchHardDeleteCascade(targetBranch.branch_id);
    const state = discoStore.getState();
  ...
  it('clears a surviving remote target relationship when the source branch is deleted', () => {
    ...
    discoStore.getState().applyBranchHardDeleteCascade(sourceBranch.branch_id);
    const state = discoStore.getState();
...
describe('discoStore agentic tool-settings hydration gate', () => {
  ...
  it('does not mark hydrated when a single-row upsert lands before the full fetch', () => {
    ...
    discoStore.getState().upsertAgenticToolSetting(setting('codex', false));

    expect(discoStore.getState().agenticToolSettingsHydrated).toBe(false);
    expect(discoStore.getState().agenticToolSettingsByName.get('codex')).toEqual(
      setting('codex', false)
    );

    // The complete snapshot lands: now the collection is authoritative.
    discoStore.getState().setAgenticToolSettings([setting('codex', false), setting('gemini', true)]);

    expect(discoStore.getState().agenticToolSettingsHydrated).toBe(true);
    expect(discoStore.getState().agenticToolSettingsByName.size).toBe(2);
  });

  it('upsert merges one row without disturbing the hydrated flag', () => {
    discoStore.getState().setAgenticToolSettings([setting('codex', true)]);
    expect(discoStore.getState().agenticToolSettingsHydrated).toBe(true);

    discoStore.getState().upsertAgenticToolSetting(setting('gemini', false));

    expect(discoStore.getState().agenticToolSettingsHydrated).toBe(true);
    expect(discoStore.getState().agenticToolSettingsByName.get('codex')).toEqual(
      setting('codex', true)
    );
    expect(discoStore.getState().agenticToolSettingsByName.get('gemini')).toEqual(
      setting('gemini', false)
    );
  });

  it('resetMaps clears the tool-settings map AND the hydration flag (tenant switch)', () => {
    discoStore.getState().setAgenticToolSettings([setting('codex', false)]);
    expect(discoStore.getState().agenticToolSettingsHydrated).toBe(true);
    expect(discoStore.getState().agenticToolSettingsByName.size).toBe(1);

    discoStore.getState().resetMaps();

    // The next tenant must start un-hydrated with an empty map — no cross-tenant
    // fail-open or stale rows.
    expect(discoStore.getState().agenticToolSettingsHydrated).toBe(false);
    expect(discoStore.getState().agenticToolSettingsByName.size).toBe(0);
  });
});
...
// resetMaps.
describe('discoStore agentic tool-settings fetch lifecycle races', () => {
  const setting = (tool: string, enabled: boolean) =>
    ({ tool, enabled }) as unknown as TenantAgenticToolSettings;
  const apply = (settings: TenantAgenticToolSettings[]) =>
    discoStore.getState().setAgenticToolSettings(settings);
