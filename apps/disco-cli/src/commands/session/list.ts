/**
 * `disco session list` - List all sessions
 *
 * Displays sessions in a beautiful table with filters.
 */

import type { PaginatedResult, Session } from '@disco-live/client';
import { PAGINATION, SessionStatus, shortId } from '@disco-live/client';
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (!Array.isArray(sessions) || sessions.length === 0) {
        ...
        this.log('');
        this.log(`Create one with: ${chalk.cyan('disco session create')}`);
        return;
      }
