...
function collectMdxPaths(dir = contentDir): string[][] {
  return readdirSync(dir).flatMap((entry) => {
    ...
    const withoutExtension = relative(contentDir, fullPath)
      .replaceAll('\\', '/')
      .replace(/\.mdx$/, '');
...
export async function generateMetadata(props: PageProps) {
  ...
  const pageTitle = metadata.title ?? 'disco';
  const title = pageTitle === 'disco' ? DEFAULT_TITLE : `${pageTitle} – disco`;
...
export default async function Page(props: PageProps) {
  ...
  // A/B variants and the Disco Cloud landing page) skip the docs Wrapper
