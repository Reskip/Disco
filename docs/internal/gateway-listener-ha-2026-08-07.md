...
completed event is a successful no-op. An incomplete event can be reclaimed
after that expiry, but only by the then-current channel listener. A retry
delivered to the same still-current owner may immediately reconcile its own
stable IDs. A different owner fails the callback until expiry: Slack does not
acknowledge it, and pollers restore their prior checkpoint instead of marking
the payload processed.
...
| Provider event before occurrence admission                                     | No accepted-event Disco/provider acknowledgement side effect is made. Ignored Slack envelopes are transport-acknowledged; accepted failures are left for provider retry. Poll overlap recovers GitHub/Shortcut.                                                                                                                                                                                                                                  |
| After occurrence admission, before acknowledgement                             | The event remains processing. A retry to the same current owner immediately reconciles its stable IDs; a different owner rejects the callback without acknowledgement/checkpoint advancement until the two-minute expiry permits reclaim.                                                                                                                                                                                                       |
...
- Connector unit coverage: polling, checkpoint-safe awaitable callbacks, stable
  ...
  environment-gated with `DISCO_TEST_POSTGRES_URL`.
