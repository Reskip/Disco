...
import type { PermissionMode, SDKResultMessage } from '@disco/core/sdk';
import type {
...
export class ClaudePromptService {
  ...
  private buildCLICommandHelpMessage(command: string): string {
    if (command === 'login') {
      return `**[Disco system message]**

\`/login\` is a CLI-only command that doesn't work in Disco.

To configure your Anthropic API key:

1. **System Settings → Agentic Tools → Claude Code**
2. **User Settings → Claude Code**
3. **For Claude Max Pro plan (OAuth):** You must start a \`claude\` CLI session while logged in as the Disco user

If you continue to see authentication errors, please contact your Disco administrator.`;
    }

    return `**[Disco system message]**

\`/${command}\` is a CLI-only command that only works in the standalone Claude Code terminal, not through Disco's SDK integration.`;
  }
  ...
  async *promptSessionStreaming(
    ...
    const discoCommandMatch = trimmedPrompt.match(/^\/(\w+)(?:\s|$)/);
    if (discoCommandMatch) {
      const command = discoCommandMatch[1];
      const discoInterceptedCommands = ['login', 'clear', 'help', 'usage'];

      if (discoInterceptedCommands.includes(command)) {
