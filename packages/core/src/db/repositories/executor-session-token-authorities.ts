      ...
      const result = await executeRaw(
        ...
        sql`
          WITH consumed AS (
            ...
            RETURNING session_id, task_id, user_id
          ), unlimited AS (
            SELECT session_id, task_id, user_id
          ...
          SELECT session_id, task_id, user_id FROM consumed
          UNION ALL
          SELECT session_id, task_id, user_id FROM unlimited
          LIMIT 1
        `
      );
