...
function retiredTenantTables(): Set<string> {
  const migration = [
    ...
    readRepoFile('packages/core/drizzle/postgres/0099_drop_retired_agor_architecture.sql'),
  ].join('\n');
  return new Set(
...
function migrationTenantTables(): string[] {
  ...
  const agentsMigration = readRepoFile(
    'packages/core/drizzle/postgres/0094_agents_and_direct_session_runtime.sql'
  );
  const usageLedgerMigration = readRepoFile(
    'packages/core/drizzle/postgres/0100_task_usage_ledger.sql'
  );
  ...
  return [
    ...new Set(
      [
        ...
        ...agentsMigration.matchAll(/CREATE TABLE "([^"]+)" \([\s\S]*?"tenant_id"/g),
        ...usageLedgerMigration.matchAll(/CREATE TABLE "([^"]+)" \([\s\S]*?"tenant_id"/g),
      ]
        .map((m) => m[1])
...
function rlsPolicyTables(): string[] {
  const migration = [
    ...
    readRepoFile('packages/core/drizzle/postgres/0094_agents_and_direct_session_runtime.sql'),
    readRepoFile('packages/core/drizzle/postgres/0100_task_usage_ledger.sql'),
  ].join('\n');
  const retiredTables = retiredTenantTables();
  return [
    ...new Set(
...
describe('Postgres multitenancy schema coverage', () => {
  ...
  it('limits cross-tenant gateway discovery to enabled rows and an explicit capability', () => {
    ...
    expect(migration).toContain("current_setting('disco.system_scope', true)");
  ...
  it('limits scheduler discovery to enabled rows and an explicit capability', () => {
    ...
    const directScheduleMigration = readRepoFile(
      'packages/core/drizzle/postgres/0098_schedule_direct_agent.sql'
    );
    ...
    expect(directScheduleMigration).toContain(
      'RENAME COLUMN "scheduled_from_branch" TO "is_scheduled"'
    );
    expect(migration).toContain("current_setting('disco.system_scope', true)");
  ...
  it('keeps OAuth capabilities and token material out of Redis paths', () => {
    const sources = [
      ...
      readRepoFile('apps/disco-daemon/src/services/mcp-oauth-pending-flow-authority.ts'),
      readRepoFile('apps/disco-daemon/src/oauth-cache.ts'),
    ].join('\n');
