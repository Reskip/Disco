...
function createTempGitRepo(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'disco-diff-enrichment-'));
  tempDirs.push(dir);
...
describe('diff enrichment', () => {
  ...
  it('enriches absolute edit_files paths when tool path and git root use different symlink prefixes', () => {
    const repoDir = createTempGitRepo();
    const aliasParent = fs.mkdtempSync(path.join(os.tmpdir(), 'disco-diff-enrichment-alias-'));
    tempDirs.push(aliasParent);
    const aliasRepo = path.join(aliasParent, 'repo-link');
    fs.symlinkSync(repoDir, aliasRepo, process.platform === 'win32' ? 'junction' : 'dir');
  ...
  symlinkIt('skips tracked symlinks during Codex turn baseline capture', async () => {
    const repoDir = createTempGitRepo();
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'disco-diff-enrichment-outside-'));
  ...
  symlinkIt('skips symlinks in edit_files add fallback when no baseline is available', () => {
    const repoDir = createTempGitRepo();
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'disco-diff-enrichment-add-outside-'));
    tempDirs.push(outsideDir);
    const outsideFile = path.join(outsideDir, 'secret.txt');
