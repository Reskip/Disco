...
CREATE POLICY "tenant_isolation_mcp_oauth_pending_flows" ON "mcp_oauth_pending_flows"
	USING (
		COALESCE(current_setting('disco.system_scope', true), '') = ''
		AND "tenant_id" = COALESCE(NULLIF(current_setting('disco.tenant_id', true), ''), 'default')
	)
	WITH CHECK (
		COALESCE(current_setting('disco.system_scope', true), '') = ''
		AND "tenant_id" = COALESCE(NULLIF(current_setting('disco.tenant_id', true), ''), 'default')
...
CREATE POLICY "mcp_oauth_callback_select" ON "mcp_oauth_pending_flows"
	FOR SELECT
	USING (
		current_setting('disco.system_scope', true) = 'mcp_oauth_callback'
		AND "state_hash" = current_setting('disco.oauth_state_hash', true)
	);
--> statement-breakpoint
...
CREATE POLICY "mcp_oauth_maintenance_select" ON "mcp_oauth_pending_flows"
	FOR SELECT
	USING (
		current_setting('disco.system_scope', true) = 'mcp_oauth_maintenance'
		AND (
