...
/**
 * Inside Disco's outer sandbox, bwrap's `--chdir` is the authoritative runtime
 * session directory. It may intentionally use a canonical home alias for
 * continuity with path-keyed SDK state. Outside that boundary, preserve the
 * working directory supplied by the daemon session record.
 */
export function resolveExecutorWorkingDirectory(
  workingDirectory: string,
  outerSandbox = process.env.DISCO_OUTER_SANDBOX === '1',
  getCwd: () => string = process.cwd
): string {
  return outerSandbox ? getCwd() : workingDirectory;
}
