 ...
 * Schedule hook tests
 *
 * Covers the direct user-owned Schedule model:
 *
 * 1. Patching `cron_expression` without a preloaded row — the
 *    `ensureScheduleLoaded` lazy-load fetches the row, then
...
import type { HookContext, Schedule, ScheduleID } from '@disco/core/types';
...
function makeContext(opts: {
  method: 'find' | 'create' | 'get' | 'patch' | 'remove';
  id?: string;
...
function makeStubRepo(row: Schedule | null = null) {
  ...
  return { findById } as unknown as import('@disco/core/db').ScheduleRepository & {
    findById: ReturnType<typeof vi.fn>;
  };
}

describe('scopeSchedulesToCaller', () => {
  it('forces external list queries to the authenticated owner', () => {
    const ctx = {
      method: 'find',
      params: {
        provider: 'rest',
        user: { user_id: 'user-alice' },
        query: { enabled: true, created_by: 'user-bob' },
      },
    } as unknown as HookContext;

    scopeSchedulesToCaller()(ctx);

    expect(ctx.params.query).toEqual({ enabled: true, created_by: 'user-alice' });
  });

  it('leaves internal scheduler queries untouched', () => {
    const query = { enabled: true };
    const ctx = { method: 'find', params: { query } } as unknown as HookContext;

    scopeSchedulesToCaller()(ctx);

    expect(ctx.params.query).toBe(query);
  });
});

describe('ensureScheduleLoaded', () => {
  it('no-ops on create (no current row to load)', async () => {
    ...
    await ensureScheduleLoaded(repo)(ctx);
    expect(repo.findById).not.toHaveBeenCalled();
    expect(ctx.params.schedule).toBeUndefined();
  });

  it('no-ops when the schedule was already loaded', async () => {
  ...
  it('loads an item operation when no cached schedule exists', async () => {
...
describe('ensureScheduleRunsAsCaller', () => {
  ...
  it('rejects superadmins patching schedules created by another user', () => {
    const current = makeSchedule({ created_by: 'user-alice' as Schedule['created_by'] });
    const ctx = makeContext({
      method: 'patch',
      id: 'sched-test-0001',
...
describe('recomputeNextRunAt', () => {
  ...
  it('H1: recomputes on patch after ensureScheduleLoaded cached the current schedule', async () => {
    const current = makeSchedule({ cron_expression: '0 0 * * 0', next_run_at: 1_000_000_000_000 });
    const ctx = makeContext({
      method: 'patch',
      id: 'sched-test-0001',
      data: { cron_expression: '*/5 * * * *' },
      cachedSchedule: current, // simulates ensureScheduleLoaded having run
    });
    await recomputeNextRunAt()(ctx);
  ...
  it('bails gracefully on patch with no current row cached', async () => {
    // Defensive: shouldn't happen post-ensureScheduleLoaded, but
