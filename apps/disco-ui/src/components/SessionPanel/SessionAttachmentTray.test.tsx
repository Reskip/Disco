...
describe('SessionAttachmentTray', () => {
  it('renders thumbnails with remove controls and top-level batch settings', () => {
    ...
    expect(screen.getByRole('button', { name: '预览 chart.png' })).toBeInTheDocument();

    fireEvent.click(screen.getByLabelText('移除 chart.png'));
  ...
  it('opens and closes a larger image preview from the thumbnail', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '预览 chart.png' }));

    expect(screen.getByRole('dialog', { name: '预览 chart.png' })).toBeInTheDocument();
    expect(screen.getByAltText('chart.png 的预览')).toHaveAttribute('src', 'blob:chart');
  ...
  it('locks remove and batch settings while composer files are uploading', () => {
    ...
    expect(screen.getByText('文件正在上传，完成前不能修改附件。')).toBeInTheDocument();
    expect(screen.getByLabelText('移除 chart.png')).toBeDisabled();
