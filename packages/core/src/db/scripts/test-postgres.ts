 ...
 *
 * The role in DISCO_TEST_POSTGRES_URL must have CREATEDB. PostgreSQL tests use a
...
async function main(): Promise<void> {
  const baseUrl = process.env.DISCO_TEST_POSTGRES_URL?.trim();
  if (!baseUrl) {
    throw new Error('DISCO_TEST_POSTGRES_URL is required');
  }
  const postgresUrl = baseUrl;
  const parsedUrl = new URL(postgresUrl);
  if (parsedUrl.protocol !== 'postgres:' && parsedUrl.protocol !== 'postgresql:') {
    throw new Error('DISCO_TEST_POSTGRES_URL must be a PostgreSQL URL');
  }
