...
export class GeminiPromptService {
   ...
   * @param permissionMode - Disco permission mode ('ask' | 'auto' | 'allow-all')
    ...
    try {
      ...
      while (loopCount < MAX_LOOPS) {
        ...
        const scheduler = new Gemini.Scheduler({
          // Gemini 0.40 replaced the standalone config option with its
          // execution-scoped AgentLoopContext. Config implements that contract.
          context: config,
          messageBus,
        ...
        const toolCallRequests: GeminiTypes.ToolCallRequestInfo[] = pendingToolCalls.map(
          (toolCall) => ({
  ...
  private async getOrCreateClient(
    ...
    // Map Disco permission mode to Gemini ApprovalMode
    ...
    if (!workingDirectory) throw new Error(`Session ${sessionId} has no working directory`);
    console.log(`✅ Using Session working directory: ${workingDirectory}`);
    ...
    // Inject static Disco orientation via temp file (no race conditions!)
    ...
    const tempSessionContextPath = path.join(os.tmpdir(), `disco-gemini-${sessionId}.md`);
    await fs.writeFile(tempSessionContextPath, discoSystemPrompt, {
      encoding: 'utf-8',
      mode: 0o600,
    });
    console.log(`✅ Created Disco session context at ${tempSessionContextPath}`);
    ...
    // Configure Disco MCP server (self-access to daemon) - only if MCP is enabled
    if (this.mcpEnabled !== false) {
      ...
      if (mcpToken) {
        ...
        console.log(`🔌 Configuring Disco MCP server at ${daemonUrl}/mcp`);
        ...
        mcpServersConfig.disco = new Gemini.MCPServerConfig(
          undefined, // command
          undefined, // args
          {}, // env
          undefined, // cwd
      ...
      console.log(`🔒 Disco MCP server disabled - skipping MCP configuration`);
    }
    ...
    if (this.sessionMCPRepo && this.mcpServerRepo) {
      try {
        ...
        for (const { server } of serversWithSource) {
          ...
          // Convert Disco's MCP server format to Gemini SDK's MCPServerConfig
    ...
    const config = new Gemini.Config({
      sessionId, // Use Disco session ID
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  'Authentication timeout. If using OAuth, please set GEMINI_API_KEY instead or run `gemini login` outside of Disco to authenticate.'
                )
              ),
            AUTH_TIMEOUT_MS
          )
