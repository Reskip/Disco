/** `disco version` - Print the selected daemon's build identity. */
...
export default class Version extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (!target) {
      this.error('Not connected. Run disco login --url <daemon-url>.');
    }
    ...
    if (info.deploymentId !== target.deploymentId) {
      this.error(
        flags.local
          ...
          : `The daemon identity at ${target.url} changed. Run disco login --url ${target.url} again.`
      );
    }
...
async function fetchHealth(daemonUrl: string): Promise<HealthBuildInfo | null> {
  try {
    ...
    return body.service === 'disco-daemon' ? body : null;
  } catch {
    return null;
  }
}
