...
import { dump as dumpYaml, load as loadYaml } from '@disco/core/yaml';
...
const isolatedDaemonUrl = 'http://127.0.0.1:65432';
...
const match = /^(@disco-live\\/[^@]+)@(.+)$/.exec(spec);
if (!prefix || !match) process.exit(64);
const packageName = match[1];
const version = match[2];
const slug = packageName.slice('@disco-live/'.length);
const vendors = {
...
writePackage(packageName, \`export const DISCO_INTEGRATION_VERSION = \${JSON.stringify(version)}; export const sdk = { fixture: true };\`);
writePackage(vendors[slug], 'export const fixture = true;');
console.log('fixture npm installed ' + packageName + '@' + version);
`;
  const fixturePath = join(binDirectory, 'npm-fixture.cjs');
  await writeFile(fixturePath, fixture, 'utf8');
  if (process.platform === 'win32') {
    await writeFile(
      join(binDirectory, 'npm.cmd'),
      `@echo off\r\n"${process.execPath}" "%~dp0npm-fixture.cjs" %*\r\n`,
      'utf8'
    );
  } else {
...
function createInitEnvironment(
  ...
  const env: Record<string, string | undefined> = {
    ...
    HOME: home,
    USERPROFILE: home,
    DISCO_AGENTIC_TOOLS_DIR: join(home, '.disco', 'agentic-tools'),
    DISCO_MANAGED_AGENTIC_TOOLS: '1',
    DISCO_TELEMETRY: '0',
    DISCO_VERSION: '9.8.7-test',
    // Never probe a developer's live default daemon during isolated CLI tests.
    DAEMON_URL: isolatedDaemonUrl,
    ...overrides,
  };
  // The parent Disco session points at its own daemon. A clean-install fixture
...
async function runInteractiveInit(
  ...
  if (!managedTools) delete env.DISCO_MANAGED_AGENTIC_TOOLS;
  ...
  return await new Promise((resolve, reject) => {
    ...
    terminal.onData((data) => {
      ...
      if (
        ...
        plain.includes('@disco-live/codex@9.8.7-test')
      ) {
...
async function runInteractiveReinit(
  home: string,
  fixtureBin?: string,
  daemonUrl: string = isolatedDaemonUrl
): Promise<{
  exitCode: number;
  output: string;
}> {
...
describe('safe init state detection', () => {
  it('treats a pre-created empty .disco mount as fresh', () => {
    expect(
      isFreshInitState({
        baseExists: true,
        configExists: false,
...
describe('initial agentic tool selection', () => {
  ...
  it('rejects --local instead of mixing per-directory data with global config', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-init-local-'));
    ...
    expect(result.output).toContain('disco init --local` is no longer supported');
    await expect(access(join(home, '.disco'))).rejects.toMatchObject({ code: 'ENOENT' });
  }, 30_000);

  it('rejects a non-TTY interactive invocation before creating partial state', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-init-nontty-'));
    temporaryDirectories.push(root);
    const home = join(root, 'home');
    await mkdir(home);
    ...
    expect(result.exitCode).toBe(2);
    expect(result.output).toContain('Interactive `disco init` requires a TTY');
    await expect(access(join(home, '.disco'))).rejects.toMatchObject({ code: 'ENOENT' });
  }, 30_000);

  it('requires an explicit fresh headless policy before creating partial state', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-init-headless-'));
    temporaryDirectories.push(root);
    const home = join(root, 'home');
    await mkdir(home);
  ...
  it('refuses to unlink a live daemon database, then safely re-initializes after it stops', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-reinit-pty-'));
    temporaryDirectories.push(root);
    const home = join(root, 'home');
    ...
    const server = createServer((_request, response) => {
      ...
      response.end('{"service":"disco-daemon","status":"ok"}');
    });
    ...
    try {
      ...
      const originalConfig = loadYaml(
        await readFile(join(home, '.disco', 'config.yaml'), 'utf8')
      ) as { daemon?: { deployment_id?: string; jwtSecret?: string } };

      const daemonUrl = `http://127.0.0.1:${address.port}`;
      const refused = await runInteractiveReinit(home, fixtureBin, daemonUrl);
      expect(refused.exitCode, refused.output).toBe(2);
      expect(refused.output).toContain('The Disco daemon is running');
      expect(refused.output).toContain('disco daemon stop');
      await expect(access(join(home, '.disco', 'disco.db'))).resolves.toBeUndefined();

      await closeServer(server);
      await writeFile(join(home, '.disco', 'disco.db-wal'), 'stale WAL fixture');
      await writeFile(join(home, '.disco', 'disco.db-shm'), 'stale SHM fixture');

      const reinitialized = await runInteractiveReinit(home, fixtureBin, daemonUrl);
      expect(reinitialized.exitCode, reinitialized.output).toBe(0);
      expect(reinitialized.output).toContain('Migrations complete');
      expect(reinitialized.output).toContain('Disco initialized successfully');
      expect(reinitialized.output).toContain('Backing up existing installation');
      expect(reinitialized.output).toContain(`Moved ${join(home, '.disco')} to`);
      const backupDir = (await readdir(home)).find((entry) => entry.startsWith('.disco.bkp.'));
      expect(backupDir).toBeDefined();
      await expect(access(join(home, backupDir!, 'disco.db-wal'))).resolves.toBeUndefined();
      await expect(access(join(home, backupDir!, 'disco.db-shm'))).resolves.toBeUndefined();
      const config = loadYaml(await readFile(join(home, '.disco', 'config.yaml'), 'utf8')) as {
  ...
  it('drives the real CLI through a pseudo-TTY, persists declarative policy, and proves managed readiness', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-init-pty-'));
    temporaryDirectories.push(root);
    const home = join(root, 'home');
    const fixtureBin = join(root, 'bin');
    await mkdir(home, { recursive: true });
    ...
    const previousToolsDirectory = process.env.DISCO_AGENTIC_TOOLS_DIR;
    process.env.DISCO_AGENTIC_TOOLS_DIR = join(home, '.disco', 'agentic-tools');
    try {
      ...
      if (previousToolsDirectory === undefined) delete process.env.DISCO_AGENTIC_TOOLS_DIR;
      else process.env.DISCO_AGENTIC_TOOLS_DIR = previousToolsDirectory;
    }
  }, 35_000);

  it('always shows the tool selector for an interactive source/development init', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-init-source-tools-'));
    temporaryDirectories.push(root);
    const home = join(root, 'home');
    const fixtureBin = join(root, 'bin');
    await mkdir(home, { recursive: true });
  ...
  it('backs up a config-only install so interactive tool changes persist in a fresh config', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-init-config-only-'));
    temporaryDirectories.push(root);
    const home = join(root, 'home');
    const fixtureBin = join(root, 'bin');
    await mkdir(home, { recursive: true });
    ...
    expect(firstInit.exitCode, firstInit.output).toBe(0);
    await rm(join(home, '.disco', 'disco.db'), { force: true });
    await rm(join(home, '.disco', 'worktrees'), { recursive: true, force: true });
  ...
  it('prompts an upgraded local install for tools before destructive re-initialization', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-reinit-tools-'));
    temporaryDirectories.push(root);
    const home = join(root, 'home');
    const fixtureBin = join(root, 'bin');
    await mkdir(home, { recursive: true });
    ...
    const configPath = join(home, '.disco', 'config.yaml');
  ...
  it('preserves the declarative choice and prints one recovery command when install fails', async () => {
    const root = await mkdtemp(join(tmpdir(), 'disco-init-pty-failure-'));
    temporaryDirectories.push(root);
    const home = join(root, 'home');
    const fixtureBin = join(root, 'bin');
    await mkdir(home, { recursive: true });
    ...
    expect(config.agentic_tools?.installed).toEqual(['codex']);
    expect(result.output.match(/disco install --sync/g)).toHaveLength(1);
