...
`packages/core/src/db/schema-factory.ts:63-94` resolves the dialect from `DISCO_DB_DIALECT`, `DATABASE_URL`, or `database.dialect` in config, defaulting to SQLite. The shared schema then delegates: `packages/core/src/db/schema.ts:19-20`:
...
```ts
// apps/disco-daemon/src/services/worktrees.ts:42-53
...
```ts
// apps/disco-daemon/src/services/sessions.ts:74-85
...
`apps/disco-daemon/src/utils/worktree-authorization.ts:1516-1570` (`determineSpawnIdentity`) stamps the child's `created_by` to **the caller** by default. Cross-user inheritance only happens if `worktree.dangerously_allow_session_sharing === true`. So "sessions I created" = `sessions.created_by = me`, full stop, no genealogy walk.
...
`apps/disco-ui/src/components/AppHeader/AppHeader.tsx:241-265` already inserts BoardSwitcher + RecentBoardPills between a vertical divider and the session-drawer button. A 320px search input slots in after `RecentBoardPills`. No layout rework.
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│  [logo] Disco  [boards▾] [recents] │  [🔍 Search…  ⌘K]  │   [users] [⚙]
└──────────────────────────────────────┬───────────────────────────────────────┘
                                       │
        ┌──────────────────────────────┴────────────────────────────────────┐
        │  payments refactor                                         [×]    │
...
```tsx
// apps/disco-ui/src/components/GlobalSearch/SearchResult.tsx
...
The discriminated union lets the component switch on `entity.type` to render the right icon, tag, secondary line. Parent entities are passed in pre-resolved (from `useDiscoData` Maps) to avoid per-row fetches.
...
| MCP        | Open SettingsModal on the MCP-servers tab, scroll the matched server into view. Reuses the modal pattern at `apps/disco-ui/src/components/MCPServer/MCPServerEditModal.tsx`. |
...
When the unified endpoint becomes worth building: as soon as V2 message search lands and we need ranking — *or* when we want a single MCP tool `disco_search` that an agent can call (V3).
...
- Agent-callable `disco_search` MCP tool — agents in a session can search the workspace.
...
- Mobile-specific UX (Disco UI is desktop-first).
...
- `apps/disco-daemon/src/services/{worktrees,sessions,artifacts,boards,mcp-servers}.ts` — extend each `*Params` with `search?: string`, `created_by_me?: boolean` (and `assistant_only?: boolean` on worktrees).
- `apps/disco-ui/src/components/GlobalSearch/` — new dir:
...
- `apps/disco-ui/src/hooks/useGlobalSearch.ts` — new orchestrator with debounce + AbortController.
- `apps/disco-ui/src/hooks/useRecents.ts` — new; backend-driven recents query (no localStorage).
- `apps/disco-ui/src/components/AppHeader/AppHeader.tsx` — insert `<GlobalSearch />` after `RecentBoardPills`.
