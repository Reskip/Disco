...
if (process.env.DISCO_HA_INTEGRATION !== '1') {
  console.log('SKIP: set DISCO_HA_INTEGRATION=1 to exercise the branch HA Compose stack');
  process.exit(0);
}

for (const name of [
  'DISCO_REDIS_KEY_PREFIX',
  'DISCO_JWT_SECRET',
  'DISCO_MASTER_SECRET',
...
const publicOrigin = process.env.DISCO_HA_PUBLIC_ORIGIN ?? ingress;
...
const project = process.env.COMPOSE_PROJECT_NAME ?? 'disco-ha-integration';
...
try {
  if (process.env.DISCO_HA_INTEGRATION_START === '1') docker('up', '-d', '--build');
  ...
  for (const service of ['daemon-a', 'daemon-b']) {
    ...
    assert(destinations.has('/home/disco'), `${service} is missing the shared executor home mount`);
    assert(
      destinations.has('/home/disco/.disco'),
      `${service} is missing the stable Disco workspace/state mount`
    );
  }
  ...
  try {
    dockerOutput(
      ...
      'sh',
      '-c',
      `printf '%s' '${mountProbe}' > /home/disco/.ha-user-home-probe && printf '%s' '${mountProbe}' > /home/disco/.disco/.ha-workspace-probe`
    );
    assert.equal(
      dockerOutput('exec', '-T', 'daemon-b', 'cat', '/home/disco/.ha-user-home-probe').trim(),
      mountProbe
    );
    assert.equal(
      dockerOutput('exec', '-T', 'daemon-b', 'cat', '/home/disco/.disco/.ha-workspace-probe').trim(),
      mountProbe
    );
  } finally {
    dockerOutput(
      ...
      'rm',
      '-f',
      '/home/disco/.ha-user-home-probe',
      '/home/disco/.disco/.ha-workspace-probe'
    );
  }
  console.log('ok - executor home and stable Disco workspace mounts are shared across replicas');

  for (const service of ['daemon-a', 'daemon-b']) {
    const doctor = JSON.parse(dockerOutput('exec', '-T', service, 'disco', 'doctor', '--json'));
  ...
  async function loginAdmin() {
    const response = await fetch(`${daemonA}/authentication`, {
      ...
      body: JSON.stringify({
        strategy: 'local',
        email: 'admin@disco.live',
        password: process.env.DISCO_ADMIN_PASSWORD,
      }),
    });
  ...
  const setupPage = await fetch(
    `${ingress}/api/github/setup/new?name=Disco&state=${encodeURIComponent(ingressState)}`
  );
  ...
  if (process.env.DISCO_HA_INTEGRATION_FAILURES === '1') {
  ...
  for (const health of [healthA, healthB]) {
    ...
    assert.equal(health.deployment.capabilities.discoManagedInteractivePermissions, true);
  ...
  for (const response of edgeRateLimitProbes) {
    assert.equal(response.headers.get('x-disco-edge-rate-limit'), 'compose-per-ip');
    await response.text();
  }
    ...
    const initialize = await fetch(`${ingress}/mcp`, {
      ...
      body: JSON.stringify({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: 'disco-ha-harness', version: '1.0.0' },
        },
      }),
    });
