...
**Created:** 2026-05-16
**Companion exploration docs:** [`executor-expansion.md`](./executor-expansion.md), [`executor-isolation.md`](./executor-isolation.md), and the user-facing [`containerized-execution`](../../apps/disco-docs/pages/guide/containerized-execution.mdx) guide.
...
offline `disco local` maintenance. Startup DB URL sanitation remains daemon-owned,
...
The daemon process (only `apps/disco-daemon/src/**`) directly touches the FS in the following places. **Touchpoint count: ~35 distinct call sites, in ~13 files.**

#### Config + secrets

| Touchpoint                                       | R/W | Frequency                | File:line                                                              | Decoupling path                                                                                                                                            |
| ------------------------------------------------ | --- | ------------------------ | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `~/.disco/config.yaml` (load)                     | R   | startup-once             | `index.ts:135-139` via `packages/core/src/config/config-manager.ts:60` | Already mostly env-overrideable (`DISCO_HOME`, `DISCO_DATA_HOME`, `PORT`, etc.). For hosted: load from env + ConfigMap / DB-backed Config service. **Easy.** |
| `~/.disco/config.yaml` (write JWT secret)         | W   | startup-once if absent   | `index.ts:502-506`                                                     | Persist in DB (`auth_state` row) or require operator-provided secret in hosted.                                                                            |
| `~/.disco/config.yaml` (write DISCO_MASTER_SECRET) | W   | startup-once if absent   | `startup.ts:329`                                                       | Same.                                                                                                                                                      |
| `~/.disco/admin-credentials`                      | W   | first-run-once           | `setup/first-run-admin.ts:67`                                          | Stdout/log in hosted; no file.                                                                                                                             |
| `~/.disco/daemon-shutdown-clean.flag` (sentinel)  | R/W | shutdown + startup       | `startup.ts:57-83`                                                     | DB row (`daemon_runs`) or drop the feature in containers (k8s restart counts are already known).                                                           |
...
| Touchpoint        | R/W | Frequency     | File:line                                                           | Decoupling path                                                                                                |
| ----------------- | --- | ------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `~/.disco/disco.db` | R/W | every request | `setup/database.ts:31-127` via `packages/core/src/db/client.ts:286` | **Already solved.** Postgres path exists today (`packages/core/src/db/client.ts:293`). Hosted = Postgres-only. |
...
| Touchpoint                                                       | R/W | Frequency                       | File:line                                                                                                 | Decoupling path                                                                                                                                                                                                                   |
| ---------------------------------------------------------------- | --- | ------------------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `~/.disco/repos/<slug>/` existsSync, isValidGitRepo, listBranches | R   | per-request                     | `services/repos.ts:238, 489, 641, 701, 713`                                                               | These reads exist to **orient before delegating to executor**. In a daemon-FS-free world, the orient step needs to become an executor probe (`git.repo.inspect`) or come from cached DB state.                                    |
| `~/.disco/repos/<slug>/.git/config` (realign origin)              | W   | per-request                     | `utils/realign-repo-origin.ts:31`                                                                         | Already small — move to executor (`git.repo.realign-origin`). One day.                                                                                                                                                            |
| `~/.disco/worktrees/<r>/<wt>/` existsSync                         | R   | per-request                     | `services/branches.ts:706`, `services/repos.ts:701`, `services/zone-trigger.ts:87-89`                     | Replace with `branch.filesystem_status` cached in DB, refreshed by executor probe. Today this is already partly done — PR #1109 added "FS-drift detection" but it's still daemon-side.                                            |
| `~/.disco/worktrees/<r>/<wt>/` git state read                     | R   | per-task creation               | `mcp/tools/sessions.ts:775-777`, `services/zone-trigger.ts:87-89` (`getGitState()`, `getCurrentBranch()`) | Snapshot SHA/ref. Move to executor probe or cache in DB at create/refresh time.                                                                                                                                                   |
...
| Touchpoint                                                                     | R/W | Frequency     | File:line                         | Decoupling path                                                                                                 |
| ------------------------------------------------------------------------------ | --- | ------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `<branch>/.disco/artifacts/<name>/disco.artifact.json` sidecar read              | R   | per-request   | `services/artifacts.ts:172-200`   | Cache sidecar metadata in DB; if missing, executor probe.                                                       |
| `<branch>/.disco/artifacts/<name>/` recursive land (mkdir + writeFile per file) | W   | per-land      | `services/artifacts.ts:717-812`   | **Pure FS write done in-daemon-process.** Move to executor as `artifact.land(artifactId, branchPath, subpath)`. |
| `<branch>/.disco/artifacts/<name>/` rm                                          | W   | per-overwrite | `services/artifacts.ts:775`       | Same — executor.                                                                                                |
...
| Touchpoint             | R/W | Frequency  | File:line                  | Decoupling path                                                                                                                                                                                                                                                                                                                                   |
| ---------------------- | --- | ---------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Canonical upload bytes | W   | per-upload | `utils/upload-staging*.ts` | **Implemented:** local self-hosted staging is intentional boundary B; hosted deployments may use S3. Branch materialization streams through a scoped capability and is written only by the executor under `.disco/session-staging/`. No daemon branch-path read/write is required. Storage quotas and materialized-copy cleanup remain follow-ups. |

#### Terminals

| Touchpoint                                             | R/W  | Frequency    | File:line                       | Decoupling path                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------ | ---- | ------------ | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/tmp/disco-env-<userId>.sh` writeFileSync + sudo chown | W    | per-attach   | `services/terminals.ts:104-119` | Move env-file creation **into the impersonating boundary** (executor / `zellij.attach` payload). Today the env file is written by the daemon (as `discopg`) and chowned to the target user via sudo — a privileged daemon-side operation that wouldn't survive a multi-pod split. |
...
The existing exploration doc (Max, 2025-12-17) **already articulates this exact picture** under "Target Architecture": daemon never touches `DISCO_DATA_HOME`, executor as single isolation boundary, JSON-over-stdin, `kubectl run` for remote. The `getDiscoDataHome()` / `DISCO_DATA_HOME` / `paths.data_home` plumbing is **already implemented** in `packages/core/src/config/config-manager.ts:623-759`. The shell exists. The execution is partial.
...
The daemon FS touchpoint table treats `~/.disco/config.yaml` as one row, but the actual situation across the topology has texture worth pulling out, because **the config story is the cleanest, smallest, most landable slice of work** and is what this branch intends to take on first.

#### Position

1. **The daemon is the only authority on config.** It loads `~/.disco/config.yaml` on first use, caches in memory, and stat-validates the file on every subsequent read (cheap; nanoseconds). `saveConfig()` invalidates the cache so writes are visible immediately.
2. **`~/.disco/config.yaml` should be readable only by the daemon user.** Other processes (executor, UI, CLI) MUST NOT read it directly.
...
4. **CLI gets its own config**, separate from the daemon's. CLI config holds only what the CLI needs to find and authenticate against a daemon (URL + auth state from `disco login`). If no default daemon is reachable, the CLI prompts the user for the URL on first use and stores the answer.
...
| **H1** | **`resolvedConfig` payload slice — executor stops reading `config.yaml`**                                                                                                                                                                                         | ✅ Shipped | `packages/core/src/config/resolved-config-slice.ts`, `apps/disco-daemon/src/utils/build-resolved-config-slice.ts`, executor handlers + `config.ts` (env-only `getDaemonUrl`) (+ 8 contract tests in executor and daemon)                      | Daemon resolves a strict subset of `DiscoConfig` (`execution.permission_timeout_ms`, `opencode.serverUrl`, `daemon.host_ip_address`) — shape lives in `@disco/core` so both producer and consumer type-check the same schema. Embeds in `BasePayloadSchema.resolvedConfig`. The executor's `getDaemonUrl()` is now env-only (no config.yaml fallback); source-level tests pin the contract. Daemon-side credential env routing in `spawn-executor.ts:270-289` is unchanged — `resolvedConfig` complements that channel, it does not replace it.                                                                                                               |
| **H4** | **Shutdown sentinel logs once and degrades silently**                                                                                                                                                                                                             | ✅ Shipped | `apps/disco-daemon/src/startup.ts`                                                                                                                                                                                                            | The write path already swallowed errors; this commit adds a single log line so operators debugging crash classification on read-only DISCO_HOME deployments have signal. Read path keeps its silent catch (correct semantics, noisy if logged).                                                                                                                                                                                                                                                                                                                                                                                                              |
| **H3** | **Capability-driven secret resolution** for `DISCO_JWT_SECRET`, `DISCO_MASTER_SECRET`, `DISCO_ADMIN_PASSWORD`                                                                                                                                                        | ✅ Shipped | `apps/disco-daemon/src/setup/persisted-secret.ts` (shared JWT+MASTER helper), `apps/disco-daemon/src/index.ts`, `apps/disco-daemon/src/startup.ts`, `apps/disco-daemon/src/setup/first-run-admin.ts` (+ 9 tests across helper + admin-bootstrap) | Order: (1) env var → use it, don't touch FS; (2) persisted value → use it; (3) path writable → generate + persist; (4) fail-fast with concrete remediation naming both the env-var and writable-config escape hatches. No mode flag. JWT secret gained env-var precedence (previously ignored). MASTER_SECRET gained fail-fast message. JWT + MASTER share `resolvePersistedSecret()`. Admin bootstrap is structurally different (factory-with-rollback inside `bootstrapFirstRunAdmin`) so it doesn't use the helper; it gained an `DISCO_ADMIN_PASSWORD` env-var path with the stderr banner pointing at the env var when no credentials file was written. |

| **H5 (split)** | **CLI gets its own config file** — `~/.disco/cli.yaml` from `disco login`; CLI stops reading the daemon's `config.yaml`; first-use prompts for URL when no default is reachable. | 📋 Follow-up branch | `apps/disco-cli/src/`, possible refactor in `packages/core/src/config/config-manager.ts` | Most substantive item. Migration shim for existing single-host installs. Risk: CLI UX regression. Worth its own PR. |

**Phase 1A actual cost:** roughly 1.5 eng-days across the four items, including the Codex-review-driven follow-up pass that closed the `getDaemonUrl()` fallback, fixed two cache regressions (validation-bypass on the sync path and mutable shared state), and extracted the resolved-slice schema into `@disco/core` plus the persisted-secret helper into its own daemon-side module. Significantly faster than the ~5.25-day estimate because the existing code was closer to capability-driven than the analysis assumed — H4 was nearly a no-op and H1's surface was smaller than first thought.
...
- **Daemon** — runs as Unix user `discopg` (or whatever `daemon.unix_user` is set to). Long-lived process.
- **Executor / agent** — spawned per-task, may impersonate user `alice` (per `strict` mode) or shared `disco_executor` (per `insulated` mode). Short-lived.
...
All three need read+write to the same branch dir at `$DISCO_DATA_HOME/branches/<repo>/<wt>/`.

**Today's scheme** (`packages/core/src/unix/group-manager.ts:185-240`):

- Branch dir is owned by a per-branch group `disco_wt_<id>`.
- DEFAULT POSIX ACLs (`setfacl -d -m g:disco_wt_<id>:rwX`) ensure new files inherit group write.
...
3. **uid/gid locality.**
   uid 1001 inside pod A is not the same human as uid 1001 inside pod B unless an NSS source (LDAP/sssd) bridges them. `apps/disco-docs/pages/guide/containerized-execution.mdx:90-180` already mandates this for the executor model — it's a real, hard prereq, not a footnote.
...
The single hardest question in hosted-Disco topology is: **where do `--watch`-mode env processes run, and who owns their FS view?**
...
- **What changes:** Postgres-only path enforced; `DISCO_HOME` becomes env-only (no `~/.disco/config.yaml` writes); log centralization; `executor:artifact.land`, `executor:upload.stage`, `executor:branch.probe`; UI bundle out of daemon image.
...
- **Risk:** Low.
- **Scale ceiling:** One pod per customer; vertical only. Good for "early hosted" but not for "disco at scale."
...
- **Risk:** High. Some of these are open problems for the entire industry, not just for Disco.
...
- **Loses:**
  - **Local-dev story.** A user running `disco` against their own laptop can't get the env-pod model. (Mitigation: keep Option A as the self-hosted shape.)
...
- **Not C alone.** Self-hosted users want vite-with-inotify on their laptop, agent edits live-reloading, etc. Forcing them to spin up an env-pod model on their MacBook is a regression. The local-dev experience is part of why Disco is interesting and we shouldn't break it for a hosted scale win.
...
| Env-pod image: includes git, the relevant runtimes (node/python/etc — pick a baseline + customer extends), the disco-executor CLI for in-env ops.                                                                                                                       | 1.0                                               |
...
| --- | ---------------------------------------- | ----------------- | ---------------- | ------------- | ------------------------------------------------- | ----- |
| 1   | `~/.disco/config.yaml` (load)             | R                 | startup-once     | daemon        | env+ConfigMap                                     | 1     |
| 2   | `~/.disco/config.yaml` (write secrets)    | W                 | startup-once     | daemon        | DB row or operator-provided                       | 1     |
| 3   | `~/.disco/admin-credentials`              | W                 | first-run        | daemon        | stdout in hosted                                  | 1     |
| 4   | `daemon-shutdown-clean.flag`             | R/W               | shutdown+startup | daemon        | DB or drop in containers                          | 1     |
| 5   | `~/.disco/disco.db` (SQLite file)          | R/W               | every-req        | daemon        | Postgres-only for hosted                          | 1     |
...
| 8   | `~/.disco/repos/<slug>/` probe reads      | R                 | per-req          | daemon        | executor probe + DB cache                         | 2/3   |
| 9   | `.git/config` realign-origin             | W                 | per-req          | daemon        | executor                                          | 1     |
| 10  | `~/.disco/worktrees/<r>/<wt>/` existsSync | R                 | per-req          | daemon        | DB cache + executor probe                         | 2/3   |
...
| 19  | `/tmp/disco-env-<uid>.sh` write + chown   | W                 | per-attach       | daemon        | fold into `zellij.attach`                         | 2     |
