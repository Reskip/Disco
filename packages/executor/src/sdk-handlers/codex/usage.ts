...
/**
 * Normalize Codex SDK usage payload into Disco's TokenUsage shape.
 *
...
export interface CodexTokenCountUsage {
  /** Cumulative usage counter reported for the active Codex turn. */
  cumulative: TokenUsage;
  /** Usage of the most recent internal model call, when the protocol supplies it. */
  last?: TokenUsage;
}

/**
 * Read accounting usage from a Codex `event_msg/token_count` event.
 *
 * `total_token_usage` is useful for billing/accounting. Observed CLI versions
 * reset it at a turn boundary, while compatible transports may carry a prior
 * counter; consumers should derive the first-row baseline from
 * `last_token_usage` rather than assume either behavior. Meanwhile,
 * `last_token_usage` is useful for deriving the previous-turn baseline. They
 * must not be confused with the context-window snapshot below.
 */
export function extractCodexTokenCountUsageFromEvent(
  raw: unknown
): CodexTokenCountUsage | undefined {
  ...
  const cumulative = extractCodexTokenUsage(
    info.total_token_usage ?? info.totalTokenUsage
  );
  if (!cumulative) return undefined;
  const last = extractCodexTokenUsage(info.last_token_usage ?? info.lastTokenUsage);
  return { cumulative, ...(last ? { last } : {}) };
}

/** Return the non-negative usage delta between two cumulative snapshots. */
export function subtractCodexTokenUsage(current: TokenUsage, baseline: TokenUsage): TokenUsage {
  const delta = (value: number | undefined, previous: number | undefined) =>
    Math.max(0, (value ?? 0) - (previous ?? 0));
  const inputTokens = delta(current.input_tokens, baseline.input_tokens);
  const outputTokens = delta(current.output_tokens, baseline.output_tokens);
  const explicitTotal = delta(current.total_tokens, baseline.total_tokens);

  return {
    input_tokens: inputTokens,
    output_tokens: outputTokens,
    cache_read_tokens: delta(current.cache_read_tokens, baseline.cache_read_tokens),
    // Preserve the protocol's explicit total when present; older events that
    // omit it still get the standard input + output derivation.
    total_tokens:
      current.total_tokens !== undefined || baseline.total_tokens !== undefined
        ? explicitTotal
 ...
 *
 * The only Codex context-window source Disco currently trusts is
 ...
 *                              reasoning_output_tokens, total_tokens },
   *         total_token_usage: { ...same fields, cumulative for the active turn },
