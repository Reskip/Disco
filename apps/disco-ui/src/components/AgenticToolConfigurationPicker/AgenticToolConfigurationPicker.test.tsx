import type { AgenticToolName, DiscoClient, User } from '@disco-live/client';
import {
...
vi.mock('../../store/discoStore', () => ({
  useDiscoStore: (selector: (state: unknown) => unknown) =>
    selector({
...
describe('persistUserDefaultFromForm', () => {
  it('writes the config + inline selection under the selected tool key', async () => {
    ...
    const client = { service: () => ({ patch }) } as unknown as DiscoClient;
    const user = {
      user_id: 'u9',
      default_agentic_config: {
...
function renderSchedulePicker(
  ...
  client: DiscoClient | null = makeClient()
) {
  return render(
...
describe('AgenticToolConfigurationPicker schedule-run resolution copy', () => {
  ...
  it('describes live resolution for a named schedule preset', async () => {
    ...
    const client = { service: () => service } as unknown as DiscoClient;
