 ...
 */
import { loadConfigSync, resolveEffectiveConfig } from '@disco/core/config';
import { type TenantScopedDatabase, UsersRepository } from '@disco/core/db';
import type { UserID } from '@disco/core/types';
import { join, resolve } from 'node:path';
import { beforeEach, describe, expect, it, vi } from 'vitest';

vi.mock('@disco/core/config', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@disco/core/config')>();
  return {
    ...actual,
...
vi.mock('@disco/core/db', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@disco/core/db')>();
  return {
    ...actual,
...
describe('usesServerSharedCodexAuth', () => {
  it('is limited to trusted simple deployments', () => {
    expect(
      usesServerSharedCodexAuth({
...
describe('resolveCodexCredentialRoute — delegated mode', () => {
  ...
  it('returns unsupported-mode for delegated + templated execution without any user lookup', async () => {
    loadConfigSyncMock.mockReturnValue({
      execution: {
        ...
        executor_command_template: 'launcher --user {unix_user} -- disco-executor --stdin',
      },
    } as never);
  ...
  it('keeps simple mode untouched even with a template configured', async () => {
    loadConfigSyncMock.mockReturnValue({
      execution: {
        unix_user_mode: 'simple',
        executor_command_template: 'launcher -- disco-executor --stdin',
      },
    } as never);
  ...
  it('routes delegated templated auth through a persistent per-user home', async () => {
    loadConfigSyncMock.mockReturnValue({
      execution: {
        unix_user_mode: 'delegated',
        executor_command_template: 'launcher --user-id {user_id} -- disco-executor --stdin',
        executor_storage: {
...
describe('resolveCodexCredentialRoute — sandbox mode', () => {
  it('writes Codex auth into the user filesystem home mounted by sandbox sessions', async () => {
    ...
    findById.mockResolvedValue({
      user_id: USER_ID,
      unix_username: null,
      filesystem_home: '/srv/disco-homes/alice',
    });
    ...
    await expect(
      ...
      delegatedHomeKey: null,
      userId: USER_ID,
      codexHome: join(resolve('/srv/disco-homes/alice'), '.codex'),
    });
