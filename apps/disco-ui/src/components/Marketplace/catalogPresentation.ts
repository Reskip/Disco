 ...
 * what a user reads. Kept beside the marketplace rather than in `@disco/core`
...
import { catalogDisplayName, MCP_CATALOG_CATEGORIES } from '@disco/core/types';
 ...
 * `@disco/core/types`. A flat 30-entry dropdown is unscannable; the groups are
 ...
 *
 * The rule lives in `@disco/core` because connect needs it too — it names the
...
const CONNECT_STATUSES = {
  ...
  unchecked: {
    ...
    detail:
      'Disco has not checked this endpoint, so it may ask for an account. Connecting checks it — and stops there if it does.',
  },
  ready: {
    readiness: 'ready',
