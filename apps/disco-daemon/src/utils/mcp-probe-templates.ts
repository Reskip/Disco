 ...
 * tagged with DISCO_USER_ENV_KEYS" path verbatim. Instead, callers load the
 ...
 * DISCO_USER_ENV_KEYS tag so `buildMCPTemplateContextFromEnv` exposes the
...
import { DISCO_USER_ENV_KEYS_VAR } from '@disco/core/config';
import { buildMCPTemplateContextFromEnv, resolveMcpServerTemplates } from '@disco/core/mcp';
import type { MCPAuth, MCPServer, MCPServerID, MCPTransport } from '@disco/core/types';
...
export function resolveProbeServerTemplates(
  ...
  // Tag the synthetic env with DISCO_USER_ENV_KEYS so
  ...
  const templateContext = buildMCPTemplateContextFromEnv({
    ...userEnv,
    [DISCO_USER_ENV_KEYS_VAR]: Object.keys(userEnv).join(','),
  });
