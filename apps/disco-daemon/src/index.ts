...
import { configureAnalyticsLogger } from '@disco/core/analytics';
import {
  configureOpenSourceTelemetryLogger,
  loadOpenSourceTelemetryDiscoVersion,
  openSourceTelemetryLogger,
} from '@disco/core/telemetry';
import { patchConsole } from '@disco/core/utils/logger';
import { extractDbFilePath } from '@disco/core/utils/path';
import { UI_MOUNT_PATH } from '@disco/core/utils/url';
...
import type { DiscoConfig, ResolvedSecurity } from '@disco/core/config';
import {
...
import { generateId, resolveDatabaseUrl } from '@disco/core/db';
import {
  authenticate,
  Forbidden,
  feathers,
...
import { buildGitConfigParameters } from '@disco/core/git/pure';
import { registerHandlebarsHelpers } from '@disco/core/templates/handlebars-helpers';
import type { HookContext, User } from '@disco/core/types';
...
import {
  isBundledUiDeepLink,
  setBundledUiFallbackHeaders,
  setBundledUiStaticHeaders,
} from './setup/static-assets.js';
import { configureSwagger } from './setup/swagger.js';
import { loadDaemonVersion } from './setup/version.js';
import { serveVisualizationFrame, VISUALIZATION_FRAME_PATH } from './setup/visualization-frame.js';
...
const DISCO_VERSION = await loadOpenSourceTelemetryDiscoVersion(DAEMON_VERSION, import.meta.url);
...
/**
 * Options for programmatic daemon startup (used by `disco daemon start` CLI command).
 */
...
/**
 * Start the Disco daemon programmatically.
 *
 * Called by `disco daemon start` CLI command with a pre-loaded config,
...
export async function startDaemon(options?: DaemonStartOptions): Promise<void> {
  ...
  // the `disco_widgets_request_*` MCP tools.
  registerAllWidgets();
  ...
  let config: DiscoConfig = options?.config
    ? options.config
    : options?.configPath
  ...
  // invocation under Disco's control inherits it. See @disco/core/config
  ...
  if (gitConfigParams.length > 0) {
    ...
    // override: [] in config — Disco defaults disabled; any inherited env var preserved.
    console.log(
      '🔒 Disco git hardening disabled (override: []); inherited GIT_CONFIG_PARAMETERS preserved'
    );
  }
  ...
  if (effectiveConfig.telemetry?.enabled === undefined) {
    console.warn(
      'ℹ  Community telemetry is not configured; no telemetry will be sent. ' +
        'Set DISCO_TELEMETRY=1/0 or edit telemetry.enabled in operator-managed config.yaml.'
    );
  }
  ...
  const configuredPath = resolve(
    options?.configPath ?? process.env.DISCO_CONFIG_PATH ?? getConfigPath()
  );
  ...
  configureExecutor(effectiveConfig.execution, {
    ...
    sandboxRuntimePaths: {
      ...
      discoConfigPath: configuredPath,
      discoDbPath: localDatabasePath,
    },
  });
  ...
  const distributedWorkIdentity = initializeDistributedWorkIdentity(app, {
    environment: {
      DISCO_DAEMON_INSTANCE_ID: process.env.DISCO_DAEMON_INSTANCE_ID,
  ...
  // A srcdoc iframe inherits the app's strict CSP and therefore cannot run the
  // approved CDN scripts used by Codex visualizations. Serve an empty document
  // with a dedicated CSP instead; the sandboxed frame receives attachment HTML
  // from the authenticated parent via postMessage.
  (app as unknown as express.Application).get(VISUALIZATION_FRAME_PATH, serveVisualizationFrame);
  ...
  let bundledUiAvailable = false;
  {
    ...
    if (existsSync(uiPath)) {
      ...
      app.use('/', ((req: express.Request, res: express.Response, next: express.NextFunction) => {
        if ((req.method === 'GET' || req.method === 'HEAD') && isBundledUiDeepLink(req.path)) {
          setBundledUiFallbackHeaders(res);
          res.sendFile(path.join(uiPath, 'index.html'));
          return;
        }
  ...
  switch (jwtResolution.source) {
    case 'env':
      console.log(
        `🔑 Loaded JWT secret from DISCO_JWT_SECRET env var (length=${jwtSecret.length})`
      );
      break;
    case 'config':
  ...
  // DISCO_MASTER_SECRET: resolve BEFORE Phase 1 (registerServices). Several
  ...
  console.log(
    masterSecretSource === 'env'
      ? '🔐 API key encryption enabled (DISCO_MASTER_SECRET set)'
      : '🔐 Using saved DISCO_MASTER_SECRET from config'
  );
  ...
  if (openSourceTelemetryLogger.isEnabled()) {
    const startupTelemetryProperties = {
      ...
      deployment_kind: process.env.KUBERNETES_SERVICE_HOST
        ? 'k8s'
        : process.env.container || process.env.DISCO_DOCKER
          ? 'docker'
          : 'local',
      db_backend: process.env.DISCO_DB_DIALECT === 'postgresql' ? 'postgresql' : 'sqlite',
      os_family: platform(),
    ...
    if (
      ...
      effectiveConfig.telemetry.last_reported_version !== DISCO_VERSION
    ) {
      openSourceTelemetryLogger.track({
        event: 'daemon.upgraded',
        properties: {
          from_version: effectiveConfig.telemetry.last_reported_version,
          to_version: DISCO_VERSION,
        },
      });
    }
  ...
  await startup({
    ...
    // runtime workers replica-independent. Disco-managed permission callbacks
