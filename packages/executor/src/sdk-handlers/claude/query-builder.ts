...
import { validateDirectory } from '@disco/core/lib/validation';
...
export interface InterruptibleQuery {
  ...
  getContextUsage(): Promise<import('@disco/core/sdk').SDKControlGetContextUsageResponse>;
  /**
...
export async function setupQuery(
  ...
  const storedWorkingDirectory = session.working_directory?.trim();
  if (!storedWorkingDirectory) throw new Error(`Session ${sessionId} has no working directory`);
  const cwd = resolveExecutorWorkingDirectory(storedWorkingDirectory);
  ...
  try {
    ...
    try {
      ...
      if (fileCount === 0) {
        console.warn(`⚠️  Working directory is empty`);
      } else if (!hasGit) {
        console.debug(`Working directory is not a Git checkout`);
      }
  ...
  // Append static Disco orientation. Dynamic context is available through Disco MCP.
  ...
  if (deps.sessionMCPRepo && deps.mcpServerRepo) {
    try {
      ...
      // The built-in Disco server carries this session's daemon bearer token and
      ...
      const attachableServers = serversWithSource.filter(({ server }) => {
        if (server.name !== DISCO_MCP_SERVER_NAME) return true;
        console.warn(
          `   ⚠️  Skipping MCP server "${server.name}": reserved for the built-in Disco MCP server`
        );
        return false;
      });
      ...
      if (attachableServers.length > 0) {
        ...
        for (const { server } of attachableServers) {
          ...
          let canAlwaysLoad = transport !== 'stdio' && server.auth?.type === 'oauth';
        ...
        // Merge with existing MCP servers (preserve Disco MCP server)
  ...
  // This enables Disco's custom permission UI (WebSocket-based) when the SDK would
