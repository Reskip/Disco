export interface DiscoDaemonProbe {
  running: boolean;
  deploymentId?: string;
  managedInstanceId?: string;
}

/** A successful HTTP response alone is not proof that the configured port belongs to Disco. */
export async function probeDiscoDaemon(url: string): Promise<DiscoDaemonProbe> {
  try {
...
export async function isExpectedManagedDaemon(url: string, instanceId?: string): Promise<boolean> {
  ...
  const probe = await probeDiscoDaemon(url);
