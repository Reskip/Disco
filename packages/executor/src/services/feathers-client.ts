...
const DEBUG_FEATHERS_CLIENT =
  process.env.DISCO_DEBUG_FEATHERS_CLIENT === '1' || process.env.DEBUG?.includes('feathers-client');
...
export interface ExecutorClientHooks {
   ...
   * executor service account. Long-running task execution uses this to
   * re-establish socket-scoped state after transport replacement. Never fired
   * for the initial connect; only for reconnects.
   */
...
export async function createExecutorClient(
  ...
  (client as DiscoClient & { executorSessionToken?: string }).executorSessionToken = sessionToken;
  ...
  client.hooks({ around: { all: [authRetryHook] } } as Parameters<DiscoClient['hooks']>[0]);
