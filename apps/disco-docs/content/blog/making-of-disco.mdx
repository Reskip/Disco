...
description: Behind the scenes of building disco - from solving session context loss to creating a multiplayer platform for AI agent orchestration. Developer notes on session trees and shared environments.
...
![Using Disco to build Disco](/screenshots/disco_for_disco.png)

_Building Disco with Disco: multiple parallel sessions across different zones (Analyze/plan, Code!, Done!), each working on different parts of the system simultaneously._
...
- MCP self‑awareness: sessions that know where they are (board, worktree) and can nudge Disco (spawn, log, attach artifacts).
