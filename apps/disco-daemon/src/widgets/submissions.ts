 ...
 *   2. Authorize: caller owns the Session.
...
import { Conflict, Forbidden, NotAuthenticated, NotFound } from '@disco/core/feathers';
import type {
  Message,
  MessageID,
  Session,
...
/**
 * Check whether the caller owns the Session that requested the widget.
 */
export function canResolveWidget(
  caller: AuthenticatedCaller,
  session: Pick<Session, 'created_by'>
): boolean {
  return session.created_by === caller.user_id;
}
...
async function doResolveWidget(
  ...
  const { message, session, widget } = await deps.runInTenantDatabaseScope(async () => {
    let message: Message;
    try {
    ...
    return { message, session, widget };
  });

  // 2. Authorize using the context loaded above.
  if (!canResolveWidget(caller, session)) {
    throw new Forbidden('Only the Session owner may resolve this widget.');
  }
  ...
  if (action.kind === 'submit') {
    ...
    autoResumePrompt = entry
      ...
      : `[Disco] User dismissed a widget request. Do not re-request immediately — ask whether to proceed without, or move on to other work.`;
  }
