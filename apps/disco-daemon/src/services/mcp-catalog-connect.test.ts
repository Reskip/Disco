...
import type { AuthenticatedParams, MCPCatalogEntry, UserID } from '@disco/core/types';
...
vi.mock('@disco/core/mcp-catalog', () => ({ probeRemoteAuthType }));
  ...
  const services: Record<string, unknown> = {
    ...
    sessions: {
      get: vi.fn(async (id: string) => {
        created.sessions.push({ session_id: id });
        return { session_id: id, created_by: ALICE, status: 'idle', title: 'Existing session' };
      }),
    },
...
describe('mcp-catalog/connect', () => {
  ...
  it('attaches the server to the selected existing session', async () => {
    const { app, created } = buildApp(CURATED);

    const result = await createMCPCatalogConnectService(app).create(request, params);

    expect(created.sessions[0]).toEqual({ session_id: 'session-1' });
  ...
  it('does not reuse an install carrying custom headers', async () => {
    // Disco redacts every custom header value on read and documents them as
  ...
  it('takes back the server it created when the session cannot be made', async () => {
    ...
    (services.sessions as { get: ReturnType<typeof vi.fn> }).get.mockRejectedValue(
