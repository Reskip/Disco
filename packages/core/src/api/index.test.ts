...
import type { AuthenticationResult, Session } from '@disco/core/types';
import authClient from '@feathersjs/authentication-client';
import type { Socket } from 'socket.io-client';
import io from 'socket.io-client';
import { beforeEach, describe, expect, it, type MockedFunction, vi } from 'vitest';
import type { DiscoService, UpdatePayload } from './index';
...
describe('createClient', () => {
  ...
  describe('return value type', () => {
    it('should return DiscoClient with socket exposed', () => {
      const client = createClient();
...
describe('type-level API ergonomics', () => {
  it('accepts plain string IDs for create/patch/update payloads', () => {
    type SessionCreateInput = Parameters<DiscoService<Session>['create']>[0];
    type SessionPatchInput = Exclude<Parameters<DiscoService<Session>['patch']>[1], null>;
    type SessionIdUpdateInput = UpdatePayload<Session>['session_id'];

    const createPayload: SessionCreateInput = {
      agent_id: '01933e4a-7b89-7c35-a8f3-9d2e1c4b5a6f',
    };
    const patchPayload: SessionPatchInput = {
      agent_id: '01933e4a-7b89-7c35-a8f3-9d2e1c4b5a6f',
    };
    const updateId: SessionIdUpdateInput = '01933e4a-7b89-7c35-a8f3-9d2e1c4b5a6f';

    expect(createPayload.agent_id).toBeDefined();
    expect(patchPayload.agent_id).toBeDefined();
  ...
  it('uses concrete user typing for AuthenticationResult.user', () => {
    ...
    const getEmail = (user: AuthUser): string => user.username;
