...
const DISCO_PERMISSION_INTERCEPTION = {
  '*': 'ask',
  read: 'ask',
  edit: 'ask',
  glob: 'ask',
...
const DISCO_MANAGED_AGENT = 'disco-managed';
...
export type RunOpenCodeTurnInput = {
  ...
  taskId: TaskID;
  prompt: string;
  discoAssistantMessageId: MessageID;
...
/**
 * Session context for an Disco session mapped to OpenCode
 */
      ...
      for (const effect of effects) {
        switch (effect.type) {
          case 'text-delta':
            ...
            if (!textStarted) {
              textStarted = true;
              await input.streamingCallbacks.onStreamStart(input.turn.discoAssistantMessageId, {
                session_id: input.turn.discoSessionId,
            ...
            await input.streamingCallbacks.onStreamChunk(
              input.turn.discoAssistantMessageId,
              effect.delta,
              sequence++
            );
            break;
...
export class OpenCodeTool {
  ...
  constructor(dependencies: OpenCodeToolDependencies) {
    this.dependencies = {
      ...
      resolveInvocationConfig:
        ...
        ((input) =>
          this.buildInvocationConfig(input.discoSessionId, input.mcpToken, input.directory)),
      resolveMcpServers:
  ...
  private protectedInvocationConfig(resolved: OpenCodeInvocationConfig): OpenCodeInvocationConfig {
    const configuredAgents =
      typeof resolved.agent === 'object' && resolved.agent !== null
        ...
        : {};
    const configuredManagedAgent = configuredAgents[DISCO_MANAGED_AGENT];
    const managedAgent =
    ...
    return {
      ...resolved,
      permission: DISCO_PERMISSION_INTERCEPTION,
  ...
  async runTurn(
    ...
    // configuration. Force every interceptable permission through Disco even if
    ...
    try {
      managedServer = await startManagedOpenCodeServer(
        {
          ...
          environment: {
            ...
            // permissive project/agent rules cannot bypass Disco interception.
            OPENCODE_PERMISSION: JSON.stringify(DISCO_PERMISSION_INTERCEPTION),
          },
      ...
      this.dependencies.cancelPendingPermissions?.(input.discoSessionId);
      throw preliminarySanitizer.error(error);
    }
  ...
  private async buildInvocationConfig(
    ...
    if (!mcpToken) {
      throw new Error('OpenCode requires the built-in Disco MCP token');
    }
    const mcp: Record<string, unknown> = {};
    mcp[`disco_${shortId(sessionId)}`] = {
      type: 'remote',
    ...
    for (const { server } of servers) {
      ...
      const name = `disco_${shortId(sessionId)}_${shortId(server.mcp_server_id)}_${sanitizedName}`;
    ...
    return { mcp, permission: DISCO_PERMISSION_INTERCEPTION };
  }
  ...
  private async createSession(
    ...
    const response = await client.session.create({
      body: { title: title || 'Disco Session' },
      query: { directory },
    });
  ...
  private async executeTask(
    ...
    // Carry the shared Disco orientation on every turn. `system` appends to
    // OpenCode's provider baseline; the managed agent's own prompt would
    // replace it.
    const discoSystemPrompt = await renderDiscoSystemPrompt();
    const request = {
      path: { id: context.opencodeSessionId },
      signal: input.signal,
      body: {
        agent: DISCO_MANAGED_AGENT,
        system: discoSystemPrompt,
    ...
    const canUseTool = this.createPermissionCallback(input.discoSessionId, input.taskId);
    ...
    try {
      ...
      if (effects.thinkingStarted) {
        await streamingCallbacks?.onThinkingEnd?.(input.discoAssistantMessageId);
      }
      if (effects.textStarted) await streamingCallbacks?.onStreamEnd(input.discoAssistantMessageId);
      return finalMessage;
    } catch (error) {
      const failure = sanitizer.error(error);
      if (effects.textStarted) {
        await streamingCallbacks?.onStreamError(input.discoAssistantMessageId, failure);
      }
      throw failure;
    } finally {
