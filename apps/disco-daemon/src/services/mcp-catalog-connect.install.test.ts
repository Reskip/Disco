 ...
 * they decide nothing about MCP-server ownership.
 */
...
import { feathers } from '@disco/core/feathers';
import type {
  AuthenticatedParams,
  MCPCatalogEntry,
  MCPMemberPolicy,
...
async function buildDaemon(policy: MCPMemberPolicy, role: UserRole = 'member') {
  ...
  app.use('sessions', {
    async get(id: string, params: AuthenticatedParams) {
      return { session_id: id, created_by: params.user?.user_id, status: 'idle' };
    },
  } as never);
  ...
  const addUser = (email: string, addedRole: UserRole) =>
    users.create({
      email,
      username: email.replace(/[^a-z0-9]+/gi, '_'),
      name: email,
      role: addedRole,
    }) as Promise<User>;
...
describe('marketplace install, as it lands in the database', () => {
  ...
  it('does not hand one user’s install to the next caller through reuse', async () => {
    ...
    const member = await addUser('bob@member.disco.live', 'member');
  ...
  it('gives each user their own install of the same entry', async () => {
    ...
    const second = await addUser('second@disco.live', 'member');
