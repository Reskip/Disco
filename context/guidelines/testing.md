...
`docker/postgres-init-app-user.sql`, verifies that `disco_app` is both
...
DISCO_TEST_POSTGRES_URL='postgresql://disco_app:<password>@127.0.0.1:5432/disco' \
DISCO_TEST_POSTGRES_ADMIN_URL='postgresql://<bootstrap-user>:<password>@127.0.0.1:5432/disco' \
...
non-superuser `DISCO_TEST_POSTGRES_URL`; the isolated database is owned by that
application role. Set `DISCO_POSTGRES_TEST_CONCURRENCY=1..8` to override the
default file concurrency of three.

PostgreSQL-only suites must use the `*.postgres.test.ts` suffix. The runner
fails if it finds `DISCO_TEST_POSTGRES_URL` in a differently named test, finds no
