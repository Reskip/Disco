...
const secrets = {
  DISCO_JWT_SECRET: 'j'.repeat(32),
  DISCO_MASTER_SECRET: 'm'.repeat(32),
  DISCO_ADMIN_PASSWORD: 'bootstrap-admin-password',
};

const haConfig: DiscoConfig = {
  deployment: {
    mode: 'ha',
...
describe('resolveDeploymentConfig', () => {
  ...
  it('accepts the constrained support profile from the environment override', () => {
    ...
    expect(
      resolveDeploymentConfig(config, {
        ...secrets,
        DISCO_HA_SUPPORT_PROFILE: 'constrained-active-active',
      })
  ...
  it.each([
    ...
    [haConfig, { ...secrets, DISCO_JWT_SECRET: undefined }, 'DISCO_JWT_SECRET'],
    [haConfig, { ...secrets, DISCO_MASTER_SECRET: undefined }, 'DISCO_MASTER_SECRET'],
    [haConfig, { ...secrets, DISCO_ADMIN_PASSWORD: undefined }, 'DISCO_ADMIN_PASSWORD'],
  ...
  it('fails closed when requested PostgreSQL does not match the daemon runtime URL', () => {
    expect(() => resolveDeploymentConfig(haConfig, secrets, 'file:/tmp/disco.db')).toThrow(
  ...
  it('rejects an environment health discovery page above the repository limit', () => {
    expect(() =>
      resolveDeploymentConfig(haConfig, {
        ...secrets,
        DISCO_HA_ENV_HEALTH_SCAN_BATCH_SIZE: '1001',
      })
  ...
  it('allows disabling the distributed monitor startup offset through the environment', () => {
    const deployment = resolveDeploymentConfig(haConfig, {
      ...secrets,
      DISCO_HA_ENV_HEALTH_STARTUP_OFFSET_MAX_MS: '0',
    });
  ...
  it('does not tie a one-observation lease to the polling interval', () => {
    expect(() =>
      resolveDeploymentConfig(haConfig, {
        ...secrets,
        DISCO_HA_ENV_HEALTH_SCAN_INTERVAL_MS: '300000',
        DISCO_HA_ENV_HEALTH_MAX_IDLE_INTERVAL_MS: '300000',
        DISCO_HA_ENV_HEALTH_HTTP_TIMEOUT_MS: '1000',
        DISCO_HA_ENV_HEALTH_CLAIM_LEASE_MS: '6000',
      })
    ).not.toThrow();
  });
  ...
  it('supports an external executor topology without a daemon workspace mount', () => {
    const external = {
      ...
      execution: {
        ...
        executor_storage: {
          ...
          session_workspace: 'persistent-per-session',
        },
      },
      daemon: { public_url: 'https://disco.internal.example' },
    } as DiscoConfig;
    ...
    expect(resolveDeploymentConfig(external, secrets)).toMatchObject({
      ...
      executorStorage: {
        ...
        sessionWorkspace: 'persistent-per-session',
      },
    });
  });
  ...
  it('allows explicit false environment assertions to override file booleans', () => {
    ...
    expect(
      resolveDeploymentConfig(external, {
        ...secrets,
        DISCO_HA_EXECUTION_TOPOLOGY: 'external',
        DISCO_HA_SHARED_FILESYSTEM: 'false',
      })
  ...
  it.each([
    'https://user:secret@disco.internal.example',
    'https://disco.internal.example?token=secret',
    'https://disco.internal.example#fragment',
  ...
  it('rejects non-boolean topology environment values', () => {
    expect(() =>
      resolveDeploymentConfig(haConfig, {
        ...secrets,
        DISCO_HA_INGRESS_AFFINITY: 'yes',
      })
    ).toThrow('DISCO_HA_INGRESS_AFFINITY must be true or false');
  });
});
