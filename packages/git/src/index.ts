/**
 * Operational Git Utilities for Disco
 *
 ...
 * Disco "branch" entity it materialises — the carve-out keeps `worktree` in
...
export function createGit(
  ...
  // pull whose existing `.git/config` carries one). Disco's daemon-issued
  ...
  const git = simpleGit({
    ...
    unsafe: {
      ...
      // On Windows the trusted Git binary commonly lives under
      // `C:\\Program Files`. simple-git rejects spaces in a custom binary
      // path unless this opt-in is explicit. `gitBinary` is resolved locally
      // from executable PATH entries above, never from request input.
...
export interface CloneOptions {
   ...
   * the repo's effective base to be a non-default branch — e.g. so `.disco.yml`
...
export async function cloneRepo(options: CloneOptions): Promise<CloneResult> {
  ...
  // (for example ~/.disco/repos/org/repo), not just direct children of reposDir.
  ...
  if (existsSync(targetPath)) {
    ...
    if (isValid) {
      ...
      // record, so `.disco.yml` parsed at the cached `path` would come from
  ...
  // `.disco.yml` lives on a non-default branch would clone with the file
 ...
 * git-worktree primitive — not the Disco Branch entity (which carries env
...
/**
 * Preflight a clone-mode base ref before Disco persists any branch/session
 ...
 * not touch the per-repo base clone at `~/.disco/repos/<slug>/`. The
 ...
 * with a real `.git/worktrees/<name>/` entry in the base repo). Disco "branches"
 * in clone storage mode are not git worktrees of the base repo and won't appear
 * here — that's intentional; callers wanting the canonical list of Disco
 ...
 * Disco "Branch" entity. Branches in clone storage mode aren't registered as
 ...
 *
 * Removes the repository directory and all its contents from ~/.disco/repos/.
 * This is typically used when deleting a remote repository that was cloned by Disco.
 *
...
export async function deleteRepoDirectory(
  ...
  // Safety check: ensure we're only deleting from ~/.disco/repos/
