 ...
 *
 * Maps Disco's permission modes to Copilot SDK's onPermissionRequest callback behavior.
 * Integrates with Disco's PermissionService for interactive permission UI via WebSocket.
 *
...
import type { Message, MessageID, SessionID, TaskID } from '@disco/core/types';
import { MessageRole, PermissionStatus, SessionStatus, TaskStatus } from '@disco/core/types';
import type {
  PermissionHandler,
  PermissionRequest,
...
async function isAttachedMcpServer(
  ...
  // Built-in "disco" server is always auto-approved
  if (serverName === 'disco') return true;
...
/**
 * Create a permission handler based on Disco's permission mode
 *
 ...
 * @param permissionMode - Disco permission mode from session config
