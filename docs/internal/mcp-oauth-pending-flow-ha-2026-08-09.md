...
This design covers browser-based MCP OAuth authorization-code flows when Disco
...
| PKCE verifier, DCR client ID/secret, endpoints    | Secret-derived pending material                | AES-GCM sealed envelope using `DISCO_MASTER_SECRET`          | Never stored                                                            |
...
`DISCO_MASTER_SECRET`; plaintext encryption fallback is refused.
...
`disco.oauth_state_hash` transaction-locally. RLS requires both the system
...
consume authorization codes once, and Disco cannot prove whether an interrupted
...
Per-user grants remain self-service. Disconnect removes Disco's local row only;
...
1. Confirm every daemon uses the same stable `DISCO_MASTER_SECRET`. There is no
...
2. Stop every daemon and take a database backup. Migration `0078` is an
   ...
   `disco db migrate --offline-cutover`. It deletes legacy OAuth grant rows
