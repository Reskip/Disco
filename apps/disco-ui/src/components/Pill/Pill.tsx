...
import type { SessionStatus, TaskStatus } from '@disco-live/client';
...
export const SessionIdPill: React.FC<SessionIdPillProps> = ({
  ...
  const { token } = theme.useToken();
  // Prefer SDK session ID (more useful for CLI/logs) over Disco internal ID
...
export const UserPill: React.FC<UserPillProps> = ({
  ...
  const label = user.name || user.username || 'Someone';
