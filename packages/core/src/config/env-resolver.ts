...
export const ALLOWED_ENV_VARS = new Set([
  ...
  // Disco session context (safe for executor/sessions)
  'DAEMON_URL',

  // Managed agentic tool runtime. These locate the version-aligned integration
  // tree (~/.disco/agentic-tools/<version>/<tool>) that loadManagedAgenticToolSdk
  ...
  // the executor sees DISCO_MANAGED_AGENTIC_TOOLS unset, falls back to importing
  // the vendor SDK by bare specifier, and every session fails with
  // "Cannot find package '@openai/codex-sdk'".
  'DISCO_VERSION',
  'DISCO_AGENTIC_TOOLS_DIR',
  'DISCO_MANAGED_AGENTIC_TOOLS',
  // Path-only bridge to the daemon account's shared Codex configuration.
  // Windows executors run with an Disco-owned HOME/USERPROFILE, while Codex can
  // still discover the real Windows profile through the OS.  Forwarding this
  // explicit path lets the headless launcher disable desktop-only skills from
  // that same profile before app-server initialization.  It carries no token
  // or credential contents; auth.json remains read by Codex itself.
  'DISCO_HOST_CODEX_HOME',
  // Disco may use an isolated Codex home so its durable app-server threads do
  // not appear in the desktop Codex task list. This is a path only; credential
  // contents are still read by Codex itself from auth.json.
  'CODEX_HOME',
]);
...
export interface ResolveUserEnvOptions {
   ...
   * terminals and other contexts that don't run an SDK.
   */
...
export async function resolveUserEnvironment(
  ...
  try {
    ...
    if (row) {
      ...
      // Normalize optional fields in canonical StoredEnvVar records.
 ...
 */
export const DISCO_USER_ENV_KEYS_VAR = 'DISCO_USER_ENV_KEYS';
...
/**
 * Create a clean environment for user-owned processes.
 *
 ...
 * secrets (DATABASE_URL, DISCO_MASTER_SECRET, etc.) to agent sessions.
 *
 * This function:
 ...
 * 4. Sets DISCO_USER_ENV_KEYS with comma-separated list of user-defined var keys
 *
 ...
 *
 * @example
 * // For a user-owned Session runtime
 * const env = await createUserProcessEnvironment(session.created_by, db);
 * spawn(command, { cwd, shell: true, env });
 *
 * @example
 * // For a user-owned background process with custom NODE_ENV
...
export async function createUserProcessEnvironment(
   ...
   * credentials are NEVER merged. Omit for non-SDK contexts such as generic
   * background jobs.
   */
  tool?: AgenticToolName
): Promise<Record<string, string>> {
  ...
  // 1. Resolve and merge user environment variables (if userId provided).
  ...
  // 2. Merge additional environment variables (highest priority)
  ...
  // Set DISCO_USER_ENV_KEYS to communicate user-defined var keys to child processes
  // This is used by MCP template resolver to restrict context to user-scoped vars only
  if (userEnvKeys.length > 0) {
    env[DISCO_USER_ENV_KEYS_VAR] = userEnvKeys.join(',');
  }
