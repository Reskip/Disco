 ...
 *      links — session paths (`/s/<sessionShort>/`).
 ...
 * `apps/disco-daemon/src/index.ts` static-serving block). React Router
 ...
 * one path-leading discriminator. Keep this in lockstep with the route table in
 * `apps/disco-ui/src/App.tsx`.
 */
...
/** `/s/<sessionShort>/` — session deep link. */
 ...
 *  base URL values (e.g. `https://disco.example.com/`) don't
 *  produce double-slashed URLs like `https://disco.example.com//ui/...`.
 *  Also strips a trailing `/ui` suffix so operators who set
 *  a base URL to the full UI address (e.g. `https://disco.example.com/ui`)
...
// reasons. Used by external URL normalization and the health-check allowlist.
// ---------------------------------------------------------------------------
