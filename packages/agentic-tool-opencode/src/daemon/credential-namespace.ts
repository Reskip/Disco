...
import type { DiscoConfig, UnixUserMode } from '@disco/core/config';
...
const SUBJECT_KEY_VERSION = 'disco-opencode-v1';
...
export function resolveOpenCodeCredentialNamespace(input: {
  ...
  const namespaceKey = createHash('sha256')
    ...
    .digest('hex');
  const root = join(homeDir, '.local', 'share', 'disco', 'opencode');
