/**
 * Tests for the disco_messages_list MCP tool.
 *
 * Focus: the tool bypasses the Feathers hook pipeline by running a raw Drizzle
 * query against the messages table. Every query must therefore apply the
 * direct Session-owner predicate, regardless of the caller's global role.
 */
...
vi.mock('@disco/core/db', async () => {
  const actual = await vi.importActual<Record<string, unknown>>('@disco/core/db');
  return {
    ...actual,
    visibleSessionReferenceAccessExists: (...args: unknown[]) => {
      mockVisibleSessionReferenceAccessExists(...args);
      return actual.sql`owned-session-access`;
    },
    select: () => ({
      from: () => ({
        where: (cond: unknown) => {
...
async function registerAndGetTool(ctx: { userId: string; role?: string }): Promise<CapturedTool> {
  ...
  registerMessageTools(fakeServer, {
    app: { get: () => ({}) } as any,
    db: {} as any,
    userId: ctx.userId as import('@disco/core/types').UserID,
    sessionId: 'sess-0001' as import('@disco/core/types').SessionID,
...
describe('disco_messages_list MCP tool', () => {
  beforeEach(() => {
  ...
  it('fails fast for broad cross-session keyword search with an over-wide window', async () => {
    ...
    ).rejects.toThrow(/window of 32 days or less/);
    expect(mockAllSpy).not.toHaveBeenCalled();
  });

  it('accepts a natural full-month window including the following day boundary', async () => {
    const handler = await registerAndGetHandler({ userId: 'user-1' });

    await handler({
      search: '跑步 | 训练 | 健康 | Apple | 半马 | 运动 | 跑量',
  ...
  it('enforces direct Session ownership for cross-session search', async () => {
    const handler = await registerAndGetHandler({ userId: 'user-1' });
  ...
  it('does not let a superadmin bypass another user\'s Session boundary', async () => {
