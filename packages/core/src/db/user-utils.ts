...
function userUsernamePredicate(db: Database, username: string) {
  const tenantPredicate = currentUserTenantPredicate(db);
  const usernamePredicate = eq(users.username, username);
  return tenantPredicate ? and(usernamePredicate, tenantPredicate) : usernamePredicate;
}
...
export async function createUser(db: Database, data: CreateUserData): Promise<User> {
  ...
  const existing = await select(db)
    .from(users)
    .where(userUsernamePredicate(db, data.username))
    .one();
 ...
 */
export async function userExists(db: Database, username: string): Promise<boolean> {
  const existing = await select(db).from(users).where(userUsernamePredicate(db, username)).one();
  return !!existing;
}
...
export async function getUserByUsername(db: Database, username: string): Promise<User | null> {
  const row = await select(db).from(users).where(userUsernamePredicate(db, username)).one();
...
export interface CreateDefaultAdminUserOptions {
  ...
  unix_username?: string;
  /**
   * Explicitly opt into the local admin/admin credential for
 ...
 * admin/admin credential is available only behind an explicit
...
export async function createDefaultAdminUser(
  ...
  const existing = await getUserByUsername(db, adminData.username);

  if (existing) {
    throw new Error(`Admin user already exists (username: ${adminData.username})`);
  }
