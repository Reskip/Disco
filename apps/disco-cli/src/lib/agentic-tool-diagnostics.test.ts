...
describe('agentic tool diagnostics', () => {
  it('reports integrations that are not installed for this Disco version', async () => {
  ...
  it('reports a corrupt managed integration as unusable', async () => {
    ...
    await writeFile(
      join(install, 'disco-integration.json'),
      JSON.stringify({
        discoVersion: '0.24.0',
    ...
    expect(codex?.detail).toContain('disco install');
  });
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      expect(codex?.detail).toContain('disco install --sync');
    }
  );
});
