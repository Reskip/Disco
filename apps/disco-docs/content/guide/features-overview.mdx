---
title: Feature Map
description: A grouped map of Disco's core workflow, agents and context, team workflow, product experience, and technical reference surfaces.
---

# Feature Map

Disco is the **command center for AI enablement**: branches, sessions, boards, teammates, shared context, and runtime surfaces in one workspace.
...
These are the concepts that deserve first-read visibility because they explain how Disco organizes work.

### 🌳 [Branches](/guide/branches)

**The unit of work in Disco.** A git branch pinned to a board, with its isolated environment, sessions, and conversations all attached. Multiple branches coexist with their own ports, dependencies, and database state, so parallel work doesn't collide.

**Use when:** Always. Branches are how Disco organizes each issue, PR, or experiment.
...
These features explain how Disco becomes a durable team workspace, not just a place to run one-off prompts.

### 🤖 [Teammates](/guide/teammates)
...
**Use when:** Context should be easy for the whole Disco workspace to find, update, link, and reuse across branches, sessions, teammates, and people.

### 🔌 [Disco MCP Server](/guide/internal-mcp)

**Anything a user can do in Disco, an agent can do too.** The daemon exposes MCP tools for sessions, branches, boards, zones, users, environments, and more. Every session is auto-issued an MCP token, so agents are self-aware and can act on their own context.

**Use when:** You want agents to operate Disco itself: spawn peers, reorganize boards, create branches, prompt other sessions, or publish reports.
...
**Slack and GitHub as portals to Disco sessions.** DM a Slack bot or `@disco` mention on a PR. Disco spins up a session on the right branch with the right agent, then routes responses back.

**Use when:** You want to drive Disco from where conversations already happen (chat and PR threads) without switching contexts.

<img
  src="/screenshots/marketing/disco-marketing-slack-thread.png"
  alt="Slack thread showing an Disco gateway bot creating a session and replying in place"
...
**Use when:** Always. This is what makes Disco's conversation surface richer than running a coding agent in a terminal.
