 ...
 * Used by daemon services (sessions, terminals, tasks) to delegate
...
import type { DiscoExecutionSettings } from '@disco/core/config';
import { getCurrentTenantId } from '@disco/core/db';
import {
  EXECUTOR_RESULT_PREFIX,
  INTERACTIVE_EXECUTOR_EVENT_PREFIX,
} from '@disco/core/executor-protocol';
import { isValidExecutionHomeKey } from '@disco/core/unix';
import { getCurrentLogLevel } from '@disco/core/utils/logger';
...
export interface ExecutorTemplateVariables {
  ...
  /** Trusted Disco user UUID used by external launchers for identity-scoped storage. */
  user_id?: string;
  log_level?: string;
...
export function substituteTemplateVariables(
  ...
  if (
    ...
    throw new Error(
      'executor_command_template {user_id} value is not a valid Disco user UUID; refusing to execute'
    );
  }
  ...
  const substitutions: Record<string, string | number | undefined> = {
    ...
    workspace_path: variables.workspace_path,
  ...
  for (const [key, value] of Object.entries(substitutions)) {
    if (value !== undefined) {
      ...
      const renderedValue =
        key === 'tenant_id' || key === 'workspace_path'
...
export function findExecutorPath(): string {
  const configuredPath = process.env.DISCO_EXECUTOR_PATH;
  if (configuredPath) {
    if (!existsSync(configuredPath)) {
      throw new Error(`Configured DISCO_EXECUTOR_PATH does not exist: ${configuredPath}`);
    }
  ...
  const possiblePaths = [
    path.join(dirname, '../executor/cli.js'), // Bundled in disco-live
    path.join(dirname, '../../executor/cli.js'), // Bundled one level up
    path.join(dirname, '../../../packages/executor/bin/disco-executor'), // Development - bin script
    path.join(dirname, '../../../packages/executor/dist/cli.js'), // Development - built dist
    path.join(dirname, '../../../../packages/executor/bin/disco-executor'), // Development from deeper nesting
...
function spawnExecutorLocal(payload: Record<string, unknown>, options: SpawnExecutorOptions): void {
  ...
  // OS-level sandbox wrap — covers ALL tools at this one chokepoint.
  let spawnCmd = cmd;
  let spawnArgs = args;
  let spawnEnv = envWithDaemonUrl;
  // Sandbox around the Session working directory: it is
  ...
  // Authoritative mount inputs the daemon resolved from its own Session state and
  ...
  const sandboxParams = payload.params as
    | {
        ...
        sandboxUserWorkspaceRoot?: unknown;
      }
    | undefined;
  ...
  const sandboxUserWorkspaceRoot =
    typeof sandboxParams?.sandboxUserWorkspaceRoot === 'string'
      ? sandboxParams.sandboxUserWorkspaceRoot
      : undefined;
  if (sandboxWorkdir) {
    try {
      const wrap = buildSandboxWrap({
        sandbox: configuredExecutorDefaults.sandbox,
        workspacePath: sandboxWorkdir,
        userWorkspaceRoot: sandboxUserWorkspaceRoot,
        cmd,
        args,
...
function logChunkedOutput(prefix: string, stream: 'stdout' | 'stderr', chunk: Buffer): void {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (stream === 'stdout') {
      if (process.env.DISCO_EXECUTOR_DEBUG_STDOUT === '1') {
 ...
 * Loaded from ~/.disco/config.yaml execution section.
 */
export type ExecutorConfig = Pick<DiscoExecutionSettings, 'executor_command_template' | 'sandbox'>;
...
interface ExecutorSpawnDefaults {
  ...
  sandbox?: DiscoExecutionSettings['sandbox'];
