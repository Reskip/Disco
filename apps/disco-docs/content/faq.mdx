---
title: FAQ
description: Frequently asked questions about branches, multi-repo workflows, session forking, spawning, and Disco's multiplayer workflow
---
...
Disco uses **git branches** under the hood (same concept as `git branch` command), but manages them for you automatically. When you create a branch in Disco, it:

- Creates a git branch in `~/.disco/worktrees/<repo>/<name>`
...
**Disco manages the git branches for you** - no need to run `git worktree add` manually. Just create a branch in the UI or CLI, and Disco handles the git operations.

</FaqSection>

<FaqSection>

## Can Disco work on multiple repos or branches?

Yes. Disco can support workflows that span multiple repositories and branches. The right pattern depends on how those repos relate to each other.

### Pattern 1: Use git submodules for clear dependencies

If one repo has a clear dependency on another, use a [git submodule](https://git-scm.com/book/en/v2/Git-Tools-Submodules) to formalize that relationship. The agent then gets a repo-in-a-repo inside the current Disco branch/workspace, so it can inspect and modify both codebases from one working directory.
...
For coordinated work across independent repos, create Disco branches for the same feature or workstream in each repo, then ask an orchestrating agent to coordinate across them. For example:
...
Disco's strength is the control plane: multiple isolated branches, visible on boards, with agents that can coordinate across them. A single OS working directory is still one branch/workspace, but Disco makes multi-repo work natural through orchestration, branch alignment, and multi-agent visibility.
...
**Sessions in Disco can fork and spawn, creating genealogy trees** - this is what makes Disco fundamentally different from linear CLI tools.
...
**Disco**: Branch your conversation like git branches code. Every fork/spawn is:
...
**Disco adds**: Session-level branching and composition. You can:
...
Disco uses a **2D spatial canvas** instead. Here's why that matters:
...
**Disco does the same for AI coding:**
...
### How Disco differs from Claude Code's Task tool
...
**Note**: You can still use Claude Code's Task tool and subagent workflows in Disco exactly like the CLI - those follow the normal Claude Code flow with waiting and callbacks.
...
**Best practice**: Think of a branch as a project or unit of work. Ideally 1 branch = 1 issue = 1 PR. Disco lets you attach issue URLs and PR URLs to branches for this reason.
...
**Rule of thumb**: If they're working on the same issue/feature and won't conflict on disk, fork the session. If they're different features or would conflict, use a different Disco branch.
...
## What does Disco's community telemetry collect?

<a id="open-source-telemetry" />

During `disco init`, Disco explains its community telemetry and asks whether to enable ongoing anonymous telemetry. Disco sends one anonymous install result so the maintainers can count installs and whether ongoing telemetry was enabled. To disable all telemetry, including that one-time install result, set `DISCO_TELEMETRY=0` before running Disco.

When ongoing telemetry is enabled, Disco sends lightweight install and aggregate usage summaries that help prioritize product work. These can include:

- Disco version and install channel
- OS family, Node major version, deployment kind, and database backend
- broad configuration modes such as RBAC and execution mode
- daemon start events and daily active-install heartbeats
- aggregate usage counts such as prompt count, session count, branch count, active user count, and counts by agentic tool / provider / normalized model family

Disco does **not** send prompts, messages, repo names or remotes, file paths, user emails or names, branch/session/task IDs, code, tool output, secrets, tokens, or raw custom model names.

You can inspect the setting with `disco telemetry`. Change it in deployment-owned
`config.yaml`, or use an environment override:

```bash
disco telemetry
DISCO_TELEMETRY=1 disco daemon start
```

`DO_NOT_TRACK=1` and `DISCO_TELEMETRY=0` disable telemetry regardless of config.
`DISCO_TELEMETRY=1` explicitly enables telemetry and creates an anonymous install ID if one has not been configured yet.
