...
const faqItems: Array<{ question: string; answer: string }> = [
  {
    ...
    answer:
      'A branch is an isolated working directory for a git repository. Disco manages git branches for you automatically (in ~/.disco/worktrees/<repo>/<name>), tracks metadata like issue and PR URLs, and associates all AI sessions with the branch. Best practice: 1 branch = 1 issue = 1 PR = 1 feature. Each branch is completely isolated, so you can work on multiple features simultaneously without switching branches or stashing changes.',
  },
  {
    question: 'Can Disco work on multiple repos or branches?',
    answer:
      'Yes. If one repo clearly depends on another, use git submodules so the agent can inspect and modify both codebases from one working directory. For coordinated work across independent repos, create Disco branches for the same feature in each repo and ask an orchestrating agent to coordinate across them — Disco provides the control plane: multiple isolated branches, visible on boards, with agents that can coordinate across them.',
  },
  {
    question: 'Session trees? WTF?',
    answer:
      'Sessions in Disco can fork and spawn, creating genealogy trees. Fork creates a sibling session with a copy of the conversation context; spawn creates a child session with a fresh, curated context window. Every fork and spawn keeps its full conversation history and can be inspected, re-prompted, or branched again — so you can branch your conversations the way git branches code.',
  },
  {
  ...
  {
    question: "What does Disco's community telemetry collect?",
    answer:
      'When enabled, Disco sends lightweight anonymous install and aggregate usage summaries: version, install channel, OS family, deployment kind, broad configuration modes, and aggregate counts like sessions and prompts. It never sends prompts, messages, repo names, file paths, code, emails, secrets, or tokens. Disable it with DISCO_TELEMETRY=0, DO_NOT_TRACK=1, or deployment-owned config.yaml.',
  },
];
