...
describe('daemon-manager logs', () => {
  ...
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'disco-cli-logs-test-'));
  ...
  it('reads structured identity records while retaining legacy compatibility', () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        configPath: '/tmp/custom-disco.yaml',
      })
    );
  ...
  it('clears both ownership records when the PID is stale', () => {
    fs.mkdirSync(path.dirname(getDaemonIdentityFilePath()), { recursive: true });
    fs.writeFileSync(path.join(tempHome, '.disco', 'daemon.pid'), '424242');
    ...
    expect(fs.existsSync(path.join(tempHome, '.disco', 'daemon.pid'))).toBe(false);
