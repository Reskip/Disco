...
<img
  ...
  alt="Disco board with live artifacts: interactive Sandpack applications built by AI agents directly on the canvas"
  style={{
    width: '100%',
    borderRadius: '12px',
    marginBottom: '24px',
...
2. **Agent calls `disco_artifacts_publish`** with the folder path, the board, the Sandpack template, and (optionally) declarative metadata: `requiredEnvVars`, `discoGrants`, `sandpackConfig`. The daemon serializes the file map into the database.
...
Each artifact is a stored file map plus a small set of declarative metadata columns. The folder on disk is just the staging area an agent writes before calling `publish`. No Disco-specific sidecars required.
...
- `disco_grants`: Declarative daemon capabilities (`disco_api_url`, viewer email, and metadata grants).
- `disco_runtime`: Controls injection of the daemon-side `disco-runtime.js` (which powers agent DOM introspection via `disco_artifacts_query_dom`). Enabled by default; opt out per-artifact with `{ enabled: false }`. The runtime ships as an iframe-level `<script>` tag via Sandpack's `externalResources`, so it works regardless of which user files exist (no entry-file detection, no file mutation, no template-specific gotchas).
...
- **Legacy banner**: Old-format artifacts (those still carrying `sandpack.json` / `disco.config.js`) render in safe-degraded mode with an inline upgrade prompt you can hand to an agent.
...
Agents manage artifacts through Disco's [MCP tools](/guide/internal-mcp):

| Tool                                | Purpose                                                                                                                                                                                                               |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `disco_artifacts_publish`            | Publish a folder as a new artifact, OR re-publish an existing one with `artifactId`. Optional `waitForStatus: true` waits for this user's browser-reported Sandpack status/logs.                                      |
| `disco_artifacts_get`                | Read an artifact's full file map + declarative metadata                                                                                                                                                               |
| `disco_artifacts_land`               | Materialize an artifact's stored files back into a branch (round-trip with publish)                                                                                                                                   |
| `disco_artifacts_update`             | Patch metadata only (board move, name, `requiredEnvVars`, `discoGrants`, `sandpackConfig`)                                                                                                                             |
| `disco_artifacts_check_build`        | Browserless folder validation: source presence, package.json syntax, configured entry, missing local imports, env/template warnings. Does not run Sandpack.                                                           |
| `disco_artifacts_validate_folder`    | Clearer alias for `disco_artifacts_check_build`; prefer this name for new agent workflows.                                                                                                                             |
| `disco_artifacts_status`             | Get build status + Sandpack errors + console logs + compact diagnostic suggestions for debugging                                                                                                                      |
| `disco_artifacts_list`               | List visible artifacts (optionally filtered by board)                                                                                                                                                                 |
| `disco_artifacts_delete`             | Remove an artifact (DB record + board placement)                                                                                                                                                                      |
| `disco_artifacts_export_codesandbox` | Eject the artifact to CodeSandbox (returns a sandbox URL). Daemon-supplied capabilities won't work there, so set required env vars via CodeSandbox Secret Keys                                                        |
| `disco_artifacts_query_dom`          | Query the rendered DOM via CSS selector. Round-trips through the viewer's open tab (no headless browser). Requires `disco_runtime.enabled !== false` and a browser tab logged in as you currently viewing the artifact |
...
`disco_artifacts_publish({ waitForStatus: true })` is synchronous-ish, not a server-side build. Sandpack boots in the viewer's browser, and runtime logs are scoped per viewer because rendered code may include secret-derived values. If the publishing user has the artifact open on the board or fullscreen page, publish waits for that browser to report Sandpack status, bundler/runtime errors, and early `console.error` output. If no browser render reports before `waitTimeoutMs`, the tool returns `publish_validation.observed: false` with guidance instead of claiming the artifact rendered successfully. `disco_artifacts_validate_folder` / `disco_artifacts_check_build` are browserless smoke checks only: they catch obvious file/import/config mistakes, but they do not run Sandpack.
...
- `discoGrants`: capabilities the daemon will inject (daemon URL, viewer email, and artifact metadata).
...
disco_artifacts_publish({
  folderPath: '.disco/artifacts/staging/openai-chat',
  boardId: '01924f3b-...',
  name: 'OpenAI Chat',
  template: 'react-ts',
  requiredEnvVars: ['OPENAI_KEY'],
  discoGrants: { disco_api_url: true },
});
```
...
const discoApiUrl = process.env.REACT_APP_DISCO_API_URL;
...
### Available `disco_grants`

| Grant                    | Env var name       | Behavior                   |
| ------------------------ | ------------------ | -------------------------- |
| `disco_api_url: true`     | `DISCO_API_URL`     | Daemon base URL.           |
| `disco_user_email: true`  | `DISCO_USER_EMAIL`  | Viewer's email.            |
| `disco_artifact_id: true` | `DISCO_ARTIFACT_ID` | Pure metadata, no consent. |
| `disco_board_id: true`    | `DISCO_BOARD_ID`    | Pure metadata, no consent. |

#### Local Chrome note: Private Network Access

When an artifact uses the hosted Sandpack preview iframe
(`https://*.codesandbox.io`) and calls an Disco URL that Chrome classifies as
local/private (for example `http://localhost:3030`, or an internal/sandbox
hostname such as `https://disco.sandbox.preset.zone` when it resolves to a
...
network protection rather than an Disco token, CSP, or app-code bug.

For local development in Chrome, allow local network access for the Disco/Sandpack
...
- Build/run with Disco's self-hosted Sandpack bundler so the preview is served
  locally instead of from `https://*.codesandbox.io`.
- Use a public HTTPS Disco URL, as hosted Disco does.
...
- **Raw secret values never enter agent prompts.** Agents see env var **names**; values are resolved at view time on the daemon and injected into the served file map. They're not in any payload an agent reads via the artifacts MCP tools. **Caveat:** `disco_artifacts_query_dom` and `disco_artifacts_query_document_html` return whatever the iframe rendered, so an artifact that prints (or otherwise reflects) a secret-derived value into the DOM will surface that into the calling agent's context. Opt out with `disco_runtime: { enabled: false }` for artifacts that intentionally render sensitive output.
...
>
  **Trust boundary:** Once you grant trust, secrets are injected into the artifact's JavaScript and
  ...
  `fetch()`, or render them into the DOM where `disco_artifacts_query_dom` would surface them to a
...
### Legacy format (`disco.config.js`)

Older artifacts used a `/disco.config.js` Handlebars sidecar. That format is no longer rendered. Artifacts still carrying it render in safe-degraded mode (no env vars or grants injected) and the board node displays a warning banner with an interpolated upgrade prompt. Copy it into a new conversation with an agent that has `disco_artifacts_get` and `disco_artifacts_publish` tools, and the agent can do the migration mechanically.
...
1. **The code is already on disk**: artifact source files live at `.disco/artifacts/<id>/` inside your branch. They're standard source files, not a proprietary format
...
You can also one-click eject to CodeSandbox (`disco_artifacts_export_codesandbox`) if you want a quick share link. Note that daemon-supplied capabilities won't work there.
...
[Sandpack](https://sandpack.codesandbox.io/) is the open-source in-browser bundler that powers artifact rendering. It's built and maintained by the team at [CodeSandbox](https://codesandbox.io/), who also offer a full cloud development platform with collaborative VMs, instant environments, and team features. Sandpack itself is free and open-source (MIT licensed). Disco uses the [`@codesandbox/sandpack-react`](https://www.npmjs.com/package/@codesandbox/sandpack-react) package.
...
- [Disco MCP Server](/guide/internal-mcp): The MCP tools agents use to create and manage artifacts
