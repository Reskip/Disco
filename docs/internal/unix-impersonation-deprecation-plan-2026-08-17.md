...
**Authoritative brief:** `disco://kb/disco-cloud-team/architecture/unix-impersonation-deprecation-planning-brief.md`
...
`apps/disco-daemon/src/utils/spawn-executor.ts` is the common substrate seam for prompt executors,
...
- legacy session sharing may deliberately inherit the parent identity; the safe path attributes a
  child to the caller. This behavior must continue to use immutable Disco principals, not a mutable
  external mapping.
...
privileged lifecycle commands through executor service tokens. `docker/sudoers/disco-daemon.sudoers`
...
| Unix management UI/API                         | Remove Unix Username controls/search terms and password-sync copy when no compatibility writer remains. Do not remove Disco Groups UI.                                                                                 |

### Retain unchanged

| Component                                                               | Reason                                                                                |
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| Disco groups, memberships, branch/board grants, owners, permission tiers | Product authorization concepts; not POSIX groups.                                     |
...
| `disco local scrub-git-remotes`                                          | Credential remediation, unrelated.                                                    |
| `disco local uploads cleanup`                                            | Tenant-aware upload lifecycle, unrelated.                                             |
...
### `disco local`, command by command
...
| ----------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `simple`    | local daemon-user process; trusted, no Disco filesystem isolation | explicit; warn for multiplayer/web terminal                                                     |
...
may depend on it. Tighten its documentation: it means Disco delegates isolation to an operator-
...
Publish a prominent advisory: back up, drain/stop Disco, migrate ownership/home mappings, verify
...
- startup prints the resolved substrate/isolation, sandbox functional probe result, external
  ...
  safely obtainable;
- `disco doctor` distinguishes “simple (trusted; no isolation)” from “sandbox available and enforced”;
...
| RBAC                       | owners, direct grants, Disco groups, board-aligned grants, and permission changes affect app access and sandbox mounts without POSIX side effects                            |
...
- Update architecture, security, development, Docker/HA docs, config examples, `disco init`, doctor,
...
- Retain the current username field as a home key but remove claims that it corresponds to a host
  account; update UI/API/help copy accordingly. Do not remove Disco Groups screens.
- Apply the command-by-command `disco local` table above; unrelated maintenance commands remain.
...
7. Keep application RBAC and Disco groups untouched; delete only their POSIX projection.
