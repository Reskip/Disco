...
import type { User } from '@disco/core/types';
 ...
 * before embedding a user-controlled field (display name, username) into the
 ...
 *
 * Sanitizes display name and username separately, then falls back name → username →
 * `'unknown user'`. A whitespace-only name no longer leaks through as a
 * blank display (the old fallback ran before sanitization, so
 * `'   '` was truthy).
 */
export function formatPrompterPrefix(prompter: Pick<User, 'name' | 'username'>): string {
  const username = sanitizeUserField(prompter.username);
  const name = sanitizeUserField(prompter.name) || username || 'unknown user';
  return username ? `[Prompted by: ${name} (${username})]` : `[Prompted by: ${name}]`;
}
...
export interface PrompterLookup {
  findById(id: string): Promise<Pick<User, 'name' | 'username'> | null>;
}
...
export async function buildPrompterPrefixedPrompt(
  ...
  let prompter: Pick<User, 'name' | 'username'> | null;
  try {
