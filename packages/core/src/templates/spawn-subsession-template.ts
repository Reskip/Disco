 ...
 * *parent* session's LLM to call the `disco_sessions_spawn` MCP tool with an
...
const SPAWN_SUBSESSION_TEMPLATE = `INSTRUCTION: The user is requesting a subsession to handle a delegated task. You MUST use the Disco
MCP tool 'disco_sessions_spawn' to create a child session that will handle this request. USER
REQUEST: """
{{userPrompt}}
"""
...
YOUR TASK: You must call the disco_sessions_spawn MCP tool with a comprehensive prompt. Do NOT
...
disco_sessions_spawn with your enriched prompt and the configuration specified above EXAMPLE: User:
"add tests" Correct response: Call disco_sessions_spawn with: - prompt: "Write Jest unit tests for
...
session will do YOUR EXACT TOOL CALL MUST BE: disco_sessions_spawn({ "prompt": "{{your_carefully_prepared_enriched_prompt_with_full_context}}",{{#if
  agenticTool
}}
...
}) Proceed now by calling disco_sessions_spawn with the exact parameters shown above.`;
