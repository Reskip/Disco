...
function run(files, registry = [], options = {}) {
  const root = mkdtempSync(join(tmpdir(), 'disco-fs-boundary-'));
  try {
    ...
    return checkDaemonFilesystemBoundaries({
      root,
      entry: 'apps/disco-daemon/src/index.ts',
...
const base = {
  id: 'TEST-A-001',
  class: 'A',
  file: 'apps/disco-daemon/src/index.ts',
  import: 'node:fs',
  symbol: 'readFileSync',
  local: 'read',
...
test('detects exact calls through named aliases, namespace/default imports, require, and dynamic import', () => {
  const errors = run({
    'apps/disco-daemon/src/index.ts': `import {readFileSync as read} from 'node:fs'; import fs from 'node:fs'; const p=require('node:fs/promises'); async function load(){read('a');fs.statSync('a');p.rm('a');const {writeFile:w}=await import('node:fs/promises');w('a','x');}`,
  });
  for (const name of [
    'call:read@load#1',
...
test('non-literal import and require require review', () => {
  const errors = run({
    'apps/disco-daemon/src/index.ts': `const name='node:fs'; async function load(){await import(name);require(name)}`,
  });
...
test('tracks simple assignments and destructuring from privileged bindings', () => {
  const errors = run({
    'apps/disco-daemon/src/index.ts': `import fs from 'node:fs'; async function load(){const read=fs.readFileSync;read('a');const {rm}=fs.promises;await rm('a')}`,
  });
...
test('numbers repeated non-literal imports and requires independently', () => {
  const errors = run({
    'apps/disco-daemon/src/index.ts': `async function load(a,b){await import(a);await import(b);require(a);require(b)}`,
  }).join('\n');
...
test('follows a workspace package and barrel into shared filesystem code', () => {
  const errors = run({
    'apps/disco-daemon/src/index.ts': `import {load} from '@fixture/shared'; load();`,
...
test('adapter placement is not trusted and path.posix remains pure', () => {
  const errors = run({
    'apps/disco-daemon/src/index.ts': `import './host/local/config.js'; import path from 'node:path'; path.posix.join('a','b');`,
    'apps/disco-daemon/src/host/local/config.ts': `import {readFileSync as read} from 'node:fs'; export function load(){return read('x')}`,
  });
...
test('one exact declaration does not permit a second call', () => {
  const files = {
    'apps/disco-daemon/src/index.ts': `import {readFileSync as read} from 'node:fs'; function load(){read('a');read('b')}`,
  };
...
test('capability manifest covers indirect APIs and child process callsites', () => {
  const errors = run({
    'apps/disco-daemon/src/index.ts': `import {execFile as run} from 'node:child_process'; import multer from 'multer'; function boot(){run('rm',['x']);multer.diskStorage({})}`,
  });
  assert.match(errors.join('\n'), /call:run@boot/);
  assert.match(errors.join('\n'), /multer#diskStorage/);
});
test('validates schema, dates, paths, IDs, exact tuples, stale entries, and class null policy', () => {
  ...
  const errors = run({ 'apps/disco-daemon/src/index.ts': 'export {}' }, [
...
test('rejects expired transitional entries but permits indefinite exact A capabilities', () => {
  assert.deepEqual(
    run(
      {
        'apps/disco-daemon/src/index.ts': `import {readFileSync as read} from 'node:fs'; function load(){read('a')}`,
      },
      [base]
    ),
    []
