...
import type { TenantID } from '@disco/core/types';
 ...
 *
 * @param dbPath - Database connection string (file:~/.disco/disco.db or postgresql://...)
 */
...
async function checkAndReportMigrations(
  ...
  if (migrationStatus.hasPending) {
    // Use the shared formatter from @disco/core/db so this message stays
    // in lockstep with the CLI pre-flight check (disco daemon start).
