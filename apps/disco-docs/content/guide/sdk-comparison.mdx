...
description: Compare AI coding agent capabilities in disco. Feature matrix for Claude Code, Codex, Gemini, GitHub Copilot, and OpenCode integrations including streaming, tool approval, and SDK differences.
---
...
Disco integrates with multiple AI coding agents. Each SDK has different capabilities based on their native features and API design.
...
| **MCP integration**         | ✅ Native       | ✅ Native          | ✅ Native        | ✅ Native       | ✅ Native          | All agents support MCP with Disco's built-in self-access server                                |
| **Permission requests**     | ✅ Rich         | ⚠️ Basic           | ⚠️ Basic         | ✅ Interactive  | ✅ Interactive     | OpenCode routes permission decisions through Disco and honors session permission mode          |
...
| **Context window tracking** | 🟡 Estimated    | 🟡 Estimated       | ❌ No            | 🟡 Estimated    | 🟡 Estimated       | Disco derives cumulative conversation usage from SDK metrics                                   |
...
- 🟡 **Emulated** - Disco implements the feature
...
- Disco displays text immediately when the complete event arrives, but there is no typewriter effect
- This is an upstream SDK limitation, not an Disco limitation
...
**Disco Implementation:** StreamingCallbacks interface provides unified streaming for all agents. Claude, Gemini, Copilot, and OpenCode get the full typewriter experience; Codex text appears instantly once the model finishes generating.
...
- On the fork's first prompt, Disco sets `resume: <parent_sdk_session_id>` and `forkSession: true` on the Agent SDK query
...
- On the fork's first prompt, Disco spawns `codex app-server` as a short-lived sidecar, calls `thread/fork` with the parent's thread id, and persists the returned thread id on the child session
...
In every case Disco tracks `genealogy.forked_from_session_id` and `fork_point_message_index` in its own database so the UI can render the parent ↔ child relationship. The "Ask a side question" (`btw`) action is built on the same fork primitive and therefore lights up wherever fork is supported.
...
- Disco UI for managing MCP server configs per session
...
- Disco's built-in MCP server auto-configured for self-awareness (sessions, branches, boards, etc.)
- Session-level MCP selection via Disco UI, same as Claude and Gemini
...
- Disco provides the same session-level MCP selection UI for Gemini sessions
...
- Disco's built-in MCP server auto-configured with the session token in the
  `Authorization: Bearer` header
- Session-level MCP selection via Disco UI, same as other agents
...
- Disco injects its built-in MCP server with session token authentication
- User-defined MCP servers injected per-session from Disco UI
...
**See [Architecture Guide: Disco as an MCP Server](/guide/architecture#disco-as-an-mcp-server) for details on using MCP with all agents.**
...
- Disco shows rich permission modals with previews
...
- Disco shows interactive permission UI with approve/deny buttons
...
- MCP tools from attached servers (including Disco's built-in server) are auto-approved

**OpenCode:** ✅ **Interactive permission system**

- OpenCode emits `permission.asked` events when tool execution requires approval
- Disco routes decisions through the same task-scoped permission service used by other agents
- Auto modes approve only their documented scope; `question` and nested `task` remain denied
- Permissions include `external_directory` access when working outside the default project

**Disco Implementation:** Permission modals adapt based on agent type. Claude shows persistent scope options (Project/User/Local); Copilot and OpenCode use task-scoped approve/deny decisions.
...
- Disco sessions inherit project instructions automatically

**Codex:** ✅ **Native instructions injection**

- Codex automatically loads `AGENTS.md` from the working directory and the user's `~/.codex/`
- Disco renders per-session project context to a temp file and passes it via the SDK's `model_instructions_file` config, never mutating the user's `~/.codex/` directory
- No manual injection needed

**Gemini:** ⚠️ **Manual injection**

- No standard instruction file format
- Disco can inject instructions via system prompt
- Requires explicit configuration

**Copilot:** ⚠️ **Manual injection**

- No standard instruction file convention
- Disco injects context via the system prompt and initial message
...
- Disco passes the branch path to scope OpenCode to the correct project

**Recommendation:** Use Claude Code or Codex for projects with rich context requirements. OpenCode is ideal for teams using multiple LLM providers. For Gemini and Copilot, manage instructions in Disco session config.
...
- Disco can build sophisticated tool visualizations

**Codex:** ⚠️ **Basic function calling**

- OpenAI function calling format
- Less structured than Claude's tools
- Disco normalizes to common format
...
- Disco normalizes Copilot events to the common message format

**Disco's Tool Widgets:**
...
- Disco normalizes OpenCode tool parts to common `tool_use` / `tool_result` content blocks
...
- Disco captures and stores `sdk_session_id`
...
- Disco passes message history array to each API call
...
- Disco captures and stores `sdk_session_id` from CopilotSession
...
- Disco captures and stores `sdk_session_id` from OpenCode session
...
- Real-time usage display in Disco UI
- Historical usage analytics per session/task

**Codex:** ✅ **Full tracking (input/output/cache)**

- `turn.completed` events now emit usage payloads that Disco maps to `TokenUsage`
...
- Not yet fully wired into Disco's token tracking pipeline
...
- Disco stores these in message metadata under `opencode` key
- Supports cost tracking across all 75+ providers

**Disco Implementation:** Token data stored in `messages` table, aggregated for session-level analytics. Claude, Codex, and OpenCode sessions display token/context pills today; Gemini and Copilot support is next.

---

### 10. Context Window Tracking
...
Accurately tracking context window usage is surprisingly difficult because **Anthropic doesn't document which tokens count toward the 200K limit**. Here's what Disco does:
...
Claude Code's Agent SDK exposes several advanced features that Disco fully supports. These are **Claude-exclusive** and not available in Codex, Gemini, or Copilot integrations.
...
- Disco shows a simple Approve/Deny UI without scope options
- Future: When the SDK adds permission persistence support, Disco will wire it up
...
### Other Claude-Exclusive Features in Disco
...
- Session-level MCP server selection via Disco UI
- Disco's built-in MCP server for self-awareness (sessions, branches, boards)
...
- Token is configured per-session in Disco's session settings
...
- Disco's integration will track SDK updates as they release
...
- Model selection configured in Disco session settings
...
- You need MCP integration (full STDIO + HTTP support, including Disco self-access)
...
- You need MCP integration with Google's SDK (full support via Disco UI)
...
- **Server:** Disco starts and contains a task-scoped `opencode serve` process
- **SDK:** `@opencode-ai/sdk` provides a REST client that communicates with the server
- **Directory Scoping:** Disco passes the branch path via the `x-opencode-directory` header and `directory` query parameter to scope all operations

### Provider Configuration

OpenCode supports 75+ providers. Connect them in Settings → OpenCode Providers; Disco keeps native credentials in a tenant-and-user-scoped home and does not copy provider secrets into the executor environment.
...
- **Local (STDIO):** Disco injects `command` + `args` configurations
- **Remote (Streamable HTTP):** Disco injects its built-in MCP server URL with
  the session token in an `Authorization: Bearer` header
...
- **Execution is local-only.** Disco must be able to contain the native writer process safely.
- **Only the session owner can prompt.** Shared prompting is disabled until credential identity is end-to-end consistent.

---

## Future Improvements

As agent SDKs evolve, Disco will gain new capabilities.

Agent integration priorities:

- **Codex text streaming** - The Codex SDK (`@openai/codex-sdk`) does not support token-level text streaming as of v0.99.0. When OpenAI adds `item.updated` support for `agent_message` items (or a new text delta event), Disco's code is already prepared to handle it
...
- **Copilot permission persistence** - The Copilot SDK (v0.2.0) doesn't expose a way to persist permission rules. When the SDK adds an `updatedPermissions`-equivalent, Disco will support persistent scopes (session, project, user) matching Claude Code's UX
...
- [Disco as an MCP Server](/guide/architecture#disco-as-an-mcp-server)
