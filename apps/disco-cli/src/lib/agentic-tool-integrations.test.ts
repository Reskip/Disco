...
const originalRoot = process.env.DISCO_AGENTIC_TOOLS_DIR;
const originalPath = process.env.PATH;
const temporaryDirectories: string[] = [];

afterEach(async () => {
  if (originalRoot === undefined) delete process.env.DISCO_AGENTIC_TOOLS_DIR;
  else process.env.DISCO_AGENTIC_TOOLS_DIR = originalRoot;
...
async function createRoot(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'disco-agentic-tools-'));
  temporaryDirectories.push(directory);
  process.env.DISCO_AGENTIC_TOOLS_DIR = directory;
  return directory;
}
...
async function installFixture(
  ...
  await writeFile(
    join(directory, 'disco-integration.json'),
    JSON.stringify(
      manifest ?? {
...
describe('listManagedDiscoVersions', () => {
  it('returns an empty list when the root does not exist', async () => {
    const root = await createRoot();
    await rm(root, { recursive: true, force: true });

    expect(await listManagedDiscoVersions()).toEqual([]);
  });
  ...
  it('orders versions numerically, not lexically', async () => {
    ...
    expect(await listManagedDiscoVersions()).toEqual(['0.9.0', '0.10.0', '0.24.0']);
  });
  ...
  it('ignores in-flight staging and backup directories', async () => {
    ...
    expect(await listManagedDiscoVersions()).toEqual(['0.24.0']);
  });
...
describe('listInstalledAgenticTools', () => {
  it('lists tools that have a verified manifest', async () => {
    const root = await createRoot();
    await installFixture(root, '0.24.0', 'claude-code', '@disco-live/claude');
    await installFixture(root, '0.24.0', 'codex', '@disco-live/codex');
  ...
  it('skips directories whose manifest is missing or misaligned', async () => {
    ...
    await installFixture(root, '0.24.0', 'gemini', '@disco-live/gemini', {
      discoVersion: '0.23.0',
...
if (project.private !== true || project.allowScripts !== undefined || project.dependencies['@disco-live/codex'] !== '0.24.0') process.exit(65);
...
writePackage('@disco-live/codex', "export const DISCO_INTEGRATION_VERSION = '0.24.0'; export const sdk = {};");
writePackage('@openai/codex-sdk', 'export const fixture = true;');
const privatePackage = path.join(prefix, 'node_modules', '@disco-live', 'codex');
...
`
      ...
      const installedPackage = join(root, '0.24.0', 'codex', 'node_modules', '@disco-live', 'codex');
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = await createRoot();
      const destination = await installFixture(root, '0.24.0', 'codex', '@disco-live/codex');
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      const destination = await installFixture(root, '0.24.0', 'codex', '@disco-live/codex');
      const privateDirectory = join(destination, 'node_modules', '@disco-live', 'codex');
...
describe('findRestorableAgenticTools', () => {
  it('returns the newest older version that still has tools', async () => {
    const root = await createRoot();
    await installFixture(root, '0.22.0', 'gemini', '@disco-live/gemini');
    await installFixture(root, '0.23.0', 'claude-code', '@disco-live/claude');
    await installFixture(root, '0.23.0', 'codex', '@disco-live/codex');
  ...
  it('never restores from the current or a newer version', async () => {
    const root = await createRoot();
    await installFixture(root, '0.24.0', 'claude-code', '@disco-live/claude');
    await installFixture(root, '0.25.0', 'codex', '@disco-live/codex');
...
describe('removeManagedDiscoVersion', () => {
  it('removes a version directory and leaves the others intact', async () => {
    const root = await createRoot();
    await installFixture(root, '0.23.0', 'claude-code', '@disco-live/claude');
    await installFixture(root, '0.24.0', 'claude-code', '@disco-live/claude');

    await removeManagedDiscoVersion('0.23.0');
  ...
  it('is a no-op for a version that is not installed', async () => {
    ...
    await expect(removeManagedDiscoVersion('0.19.0')).resolves.toBeUndefined();
    expect(await listManagedDiscoVersions()).toEqual(['0.24.0']);
  });
});
...
describe('local selection persistence', () => {
  it('writes an atomic private manifest and rejects a concurrent install', async () => {
    ...
    await expect(acquireAgenticToolInstallLock()).rejects.toThrow(
      'Another `disco install` is already updating agentic tools'
    );
    await release();
