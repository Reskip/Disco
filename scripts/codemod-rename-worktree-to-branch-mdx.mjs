 ...
 * docs in `apps/disco-docs/pages/`. Walks each .mdx file and rewrites
 ...
 *   - The string `git worktree` (CLI primitive, not Disco's concept)
 *   - Substrings containing machine markers: `worktree_id`, `worktreeId`,
 *     `WorktreeID`, `/worktrees/`, `worktrees/`, `disco_worktrees_`,
 ...
 *
 * Usage:
 *   node scripts/codemod-rename-worktree-to-branch-mdx.mjs              # apply to apps/disco-docs/pages
...
const rootDir = rootArgIdx >= 0 ? args[rootArgIdx + 1] : 'apps/disco-docs/pages';

// Word-boundary rewrite for `Worktree(s)` / `worktree(s)`. Boundaries:
//
//   - Lookbehind blocks `git ` (CLI primitive) and the identifier
//     characters `[A-Za-z0-9_{]` (catches `disco_worktrees_`, `{{worktree`,
...
//
// URL paths (`/guide/worktrees`, `~/.disco/worktrees/`, `href="…"`,
...
const URL_PLACEHOLDER_PREFIX = '__DISCO_URL_PH_';
const URL_PLACEHOLDER_SUFFIX = '__';
const URL_PLACEHOLDER_RE = /__DISCO_URL_PH_(\d+)__/g;
