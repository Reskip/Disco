...
describe('distributed work diagnostic identity', () => {
  it('uses daemon-env, hostname, then daemon precedence', () => {
    ...
    expect(
      createDistributedWorkIdentity({
        environment: { DISCO_DAEMON_INSTANCE_ID: ' env ', HOSTNAME: ' host ' },
        generateBootId,
      }).instanceId
    ).toBe('env');
    expect(
  ...
  it('trims values and skips invalid empty precedence candidates', () => {
    expect(
      createDistributedWorkIdentity({
        environment: { DISCO_DAEMON_INSTANCE_ID: '\t', HOSTNAME: '  stable-host  ' },
    ...
    expect(
      createDistributedWorkIdentity({
        environment: { DISCO_DAEMON_INSTANCE_ID: '', HOSTNAME: '' },
