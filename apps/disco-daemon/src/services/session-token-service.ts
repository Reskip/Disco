...
const DEBUG_SESSION_TOKENS =
  process.env.DISCO_DEBUG_SESSION_TOKENS === '1' || process.env.DEBUG?.includes('session-token');
...
export class SessionTokenService {
  ...
  constructor(
    ...
    if (dependencies.authorityStore) {
      ...
      if (!dependencies.db && process.env.DISCO_DB_DIALECT === 'postgresql') {
  ...
  async generateToken(
    sessionId: string,
    userId: string,
    scope: { taskId?: string; maxUses?: number } = {}
  ): Promise<string> {
    if (!this.jwtSecret) {
