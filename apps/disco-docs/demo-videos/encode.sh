...
#          out/disco-hero.mp4         (scenes stitched with fade-through-dark joins)
#          out/disco-hero-poster.jpg
...
ffmpeg -y -loglevel error -f concat -safe 0 -i out/concat.txt -c copy \
  -movflags +faststart out/disco-hero.mp4

ffmpeg -y -loglevel error -i out/disco-hero.mp4 -vframes 1 -q:v 3 out/disco-hero-poster.jpg
...
ffmpeg -y -loglevel error -i out/disco-hero.mp4 \
  -vf "scale=1280:720:flags=lanczos" \
  -c:v libx264 -profile:v high -crf 22 -preset slow -movflags +faststart -an \
  out/disco-hero-720.mp4
ffmpeg -y -loglevel error -i out/disco-hero.mp4 \
  -vf "scale=960:540:flags=lanczos" \
  -c:v libx264 -profile:v main -crf 23 -preset slow -movflags +faststart -an \
  out/disco-hero-540.mp4

hero_bytes=$(stat -f%z out/disco-hero.mp4 2>/dev/null || stat -c%s out/disco-hero.mp4)
hero_mb=$(echo "scale=1; $hero_bytes / 1048576" | bc)
echo "disco-hero.mp4: ${hero_mb} MB"
if ((hero_bytes > 6 * 1024 * 1024)); then
  echo "⚠️  disco-hero.mp4 exceeds 6 MB — consider raising CRF or trimming scenes." >&2
fi

if $PUBLISH; then
  echo "── publishing to ../public/videos/"
  mkdir -p ../public/videos
  cp out/disco-hero.mp4 out/disco-hero-720.mp4 out/disco-hero-540.mp4 \
    out/disco-hero-poster.jpg out/multiplayer.mp4 ../public/videos/
fi
