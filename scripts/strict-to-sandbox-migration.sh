#!/usr/bin/env bash
#
# Migrate a self-hosted Disco deployment from unix_user_mode: strict/insulated
...
# SQLite through <data-home>/disco.db. PostgreSQL callers must name the tenant
...
TEARDOWN=0
SKIP_MANIFEST=0
DATA_HOME="${DISCO_DATA_HOME:-$HOME/.disco}"
DAEMON_USER="$(id -un)"
CONFIG=""
DATABASE_URL_VALUE="${DATABASE_URL:-}"
PGDATABASE_URL_SAFE=""
PGPASSWORD_VALUE=""
TENANT_ID="${DISCO_TENANT_ID:-default}"
EXPECTED_USER_COUNT=""
MANIFEST=""
SERVICE_NAME="disco-daemon"
DB_SQL_TMP=""
CONFIG_TMP=""
MANIFEST_TMP=""
OWNERSHIP_PLAN_TMP=""
...
if [[ "$DATABASE_URL_VALUE" =~ ^postgres(ql)?:// ]]; then
  ...
  pg_parts_output="$(
    DISCO_MIGRATION_DATABASE_URL="$DATABASE_URL_VALUE" node <<'NODE'
const url = new URL(process.env.DISCO_MIGRATION_DATABASE_URL);
...
if [ "$DB_KIND" = "postgres" ]; then
  ...
  user_rows_output="$(
    postgres_psql -v disco_tenant="$TENANT_ID" -At -F '|' <<'SQL'
SELECT set_config('disco.tenant_id', :'disco_tenant', false) \gset
...
for row in "${USER_ROWS[@]}"; do
  ...
  if [ "$home_canonical" = "$DATA_HOME_CANONICAL" ] || \
    ...
    die "host home overlaps the Disco data root for $unix_username: $home"
  fi
...
if [ "$APPLY" = 1 ]; then
  ...
  if [ "$DB_KIND" = "postgres" ]; then
    {
      echo 'BEGIN;'
      printf 'SELECT set_config(%s, %s, true);\n' "$(sql_quote 'disco.tenant_id')" "$(sql_quote "$TENANT_ID")"
      echo 'CREATE TEMP TABLE disco_sandbox_home_map (tenant_id text, user_id text, filesystem_home text) ON COMMIT DROP;'
      echo 'INSERT INTO disco_sandbox_home_map (tenant_id, user_id, filesystem_home) VALUES'
      ...
      echo 'UPDATE users AS u SET filesystem_home = m.filesystem_home FROM disco_sandbox_home_map AS m WHERE u.tenant_id::text = m.tenant_id AND u.user_id::text = m.user_id;'
      cat <<SQL
DO \$disco\$
BEGIN
  IF (SELECT count(*) FROM users AS u JOIN disco_sandbox_home_map AS m ON u.tenant_id::text = m.tenant_id AND u.user_id::text = m.user_id AND u.filesystem_home = m.filesystem_home) <> ${#HOME_ROWS[@]} THEN
...
SQL
  ...
  else
    {
      ...
      echo 'CREATE TEMP TABLE disco_sandbox_home_map (tenant_id text, user_id text, filesystem_home text);'
      echo 'INSERT INTO disco_sandbox_home_map (tenant_id, user_id, filesystem_home) VALUES'
      for i in "${!HOME_ROWS[@]}"; do
        row="${HOME_ROWS[$i]}"
        IFS='|' read -r tenant_id user_id unix_username home home_canonical <<< "$row"
      ...
      echo 'UPDATE users SET filesystem_home = (SELECT m.filesystem_home FROM disco_sandbox_home_map AS m WHERE m.tenant_id = users.tenant_id AND m.user_id = users.user_id) WHERE EXISTS (SELECT 1 FROM disco_sandbox_home_map AS m WHERE m.tenant_id = users.tenant_id AND m.user_id = users.user_id);'
      printf 'CREATE TEMP TABLE disco_sandbox_home_verification (matched integer CHECK (matched = %s));\n' "${#HOME_ROWS[@]}"
      echo 'INSERT INTO disco_sandbox_home_verification SELECT count(*) FROM users AS u JOIN disco_sandbox_home_map AS m ON u.tenant_id = m.tenant_id AND u.user_id = m.user_id AND u.filesystem_home = m.filesystem_home;'
      echo 'COMMIT;'
    } > "$DB_SQL_TMP"
...
if [ "$TEARDOWN" = 1 ]; then
  ...
  if [ "$APPLY" = 1 ]; then
    getent group | awk -F: '/^disco_wt_/{print $1}' | xargs -r -n1 groupdel
    rm -f -- /etc/sudoers.d/disco-daemon
    say "removed per-branch groups and daemon sudoers; Unix accounts retained"
  else
    say "would remove disco_wt_* groups and /etc/sudoers.d/disco-daemon"
  fi
  echo
fi
