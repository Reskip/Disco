...
describe('executeToolTask provider-failure settlement', () => {
  ...
  it('keeps Codex task-local usage instead of the raw whole-thread lifetime total', async () => {
    const taskPatch = vi.fn().mockResolvedValue(undefined);
    const rawLifetimeResponse = {
      type: 'turn.completed',
      usage: {
        input_tokens: 9_115_667,
        cached_input_tokens: 8_000_000,
        output_tokens: 48_965,
      },
    };
    const client = {
      service(name: string) {
    ...
    const createTool = vi.fn(() => ({
      executePromptWithStreaming: vi.fn().mockResolvedValue({
        ...
        rawSdkResponse: rawLifetimeResponse,
        model: 'gpt-5.6-sol',
      }),
    }));
    ...
    const patch = taskPatch.mock.calls.at(-1)?.[1];
    expect(patch).toMatchObject({
      status: 'completed',
      raw_sdk_response: rawLifetimeResponse,
      normalized_sdk_response: {
        tokenUsage: {
          inputTokens: 1_200,
          outputTokens: 80,
    ...
    expect(patch.normalized_sdk_response.tokenUsage.totalTokens).not.toBe(9_164_632);
  });
});
...
describe('installProviderConnection', () => {
  ...
  const SEEDED = {
    ...
    DISCO_USER_ENV_KEYS: 'GITHUB_TOKEN,MY_CUSTOM_VAR,ANTHROPIC_API_KEY',
  } as const;
  ...
  it('rewrites DISCO_USER_ENV_KEYS to only advertise surviving vars', () => {
    installProviderConnection('copilot', { COPILOT_GITHUB_TOKEN: 'resolved-copilot' });

    const advertised = (process.env.DISCO_USER_ENV_KEYS ?? '').split(',');
