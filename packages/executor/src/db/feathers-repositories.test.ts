...
describe('FeathersAgentsRepository', () => {
  it('maps an explicit 404 to a missing Agent', async () => {
    const get = vi.fn().mockRejectedValue({ code: 404, name: 'NotFound' });
    const repo = new FeathersAgentsRepository({
      service: () => ({ get }),
    } as unknown as DiscoClient);

    await expect(repo.findById('agent-1')).resolves.toBeNull();
  });

  it('preserves tenant, authentication, and transport failures for diagnosis', async () => {
    const error = Object.assign(new Error('tenant scope missing'), { code: 500 });
    const get = vi.fn().mockRejectedValue(error);
    const repo = new FeathersAgentsRepository({
      service: () => ({ get }),
    } as unknown as DiscoClient);

    await expect(repo.findById('agent-1')).rejects.toBe(error);
  });
});
