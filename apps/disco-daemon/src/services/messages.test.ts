...
import { MESSAGE_PAGINATION } from '@disco/core/config';
import {
...
import type { Message, MessageCreate, SessionID, TaskID, UUID } from '@disco/core/types';
...
} from '../utils/session-authorization';
...
async function createTestSession(
  ...
  const session = await new SessionRepository(db).create({
    ...
    working_directory: `/tmp/messages-service-${generateId()}`,
  });
...
dbTest(
  'parses authenticated REST role filters and scopes messages to directly owned sessions',
  async ({ db }) => {
    const usersService = createUsersService(db);
    const bearer = await usersService.create({
      username: `messages-rest-${generateId()}`,
      name: 'Messages REST bearer',
      email: 'messages-rest-bearer@example.test',
      password: 'password-123',
      role: ROLES.MEMBER,
    });
    const ownerId = bearer.user_id as UUID;
    const accessibleSessionId = await createTestSession(db, {
      createdBy: ownerId,
    });
    const inaccessibleSessionId = await createTestSession(db, {
      createdBy: generateId() as UUID,
    });
    const repository = new MessagesRepository(db);
