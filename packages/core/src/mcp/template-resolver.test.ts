...
describe('buildMCPTemplateContextFromEnv', () => {
  it('should only include user-defined env vars (from DISCO_USER_ENV_KEYS)', () => {
    const env = {
      DISCO_USER_ENV_KEYS: 'GITHUB_TOKEN,API_KEY',
      GITHUB_TOKEN: 'gh_secret123',
      API_KEY: 'api_secret456',
      PATH: '/usr/bin:/bin',
      HOME: '/home/user',
      DISCO_MASTER_SECRET: 'should_not_be_exposed',
    };
    ...
    expect(context.user.env.DISCO_MASTER_SECRET).toBeUndefined();
  });

  it('should return empty env when DISCO_USER_ENV_KEYS is not set', () => {
    const env = {
      GITHUB_TOKEN: 'gh_secret123',
      PATH: '/usr/bin:/bin',
    };
  ...
  it('should handle missing env vars gracefully', () => {
    const env = {
      DISCO_USER_ENV_KEYS: 'GITHUB_TOKEN,MISSING_VAR',
      GITHUB_TOKEN: 'gh_secret123',
