 ...
 *
 * Imperative tenant tables created lazily outside the Drizzle schema are leaf
 * tables; the deletion engine places them first
 ...
 *
 * Multi-tenancy is PostgreSQL-only in Disco, so this ordering is derived from the
 ...
 *
 * Only compiled manifest tables carry movable rows. Any future imperative tenant
 * tables are reported in inventory but are not exported or imported as rows.
 * See {@link IMPERATIVE_TENANT_TABLES}.
 */
 ...
 * export/import. Reported in inventory for completeness.
 */
