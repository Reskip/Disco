...
description: 'I recreated the OpenClaw agent framework to run entirely within Disco (persistent agents with full visibility, introspection, and multi-agent coordination on a spatial canvas). The results are crazy amazing.'
author: 'Maxime Beauchemin'
date: 2026-02-04
image: '/images/blog/disco-openclaw.png'
---

> **Update (March 2026):** This experiment has evolved into a first-class Disco feature. See [Introducing Disco Teammates](/blog/disco-assistants) for the latest, including real-world use cases and Slack integration via Gateway channels.
...
Like everyone out there, I've been super inspired and anxious while watching the rise of ClawdBot → MoltBook → OpenClaw recently. Crazy stuff and it's been hard to keep up with all the developments. But I've been [thinking deeply](/blog/openclaw) about the implications (what it means for where AI's going next, agent orchestration, and how any of that might relate to the work I've been doing on [Disco](https://disco.live)).

So I decided to experiment: **disco-openclaw**, an adaptation of the OpenClaw agent framework that runs entirely within Disco.
...
But OpenClaw runs as a standalone process. What if I could take those patterns and run them **inside Disco**, gaining:
...
**That's exactly what disco-openclaw is.**

![Disco OpenClaw board showing the agent's first bootstrap conversation with zones for workflow management](/images/blog/disco-openclaw-board.png)
_The Disco board during bootstrap - zones for workflow automation (left), agent's first conversation establishing identity (right)_
...
2. **Retrofitted AGENTS.md for Disco** - replaced standalone agent execution with Disco MCP orchestration, oriented toward software development workflows
3. **Added repo context system** - `repos/` directory for per-repository documentation
4. **Imported to Disco as a private repo** - `disco-openclaw-private` (now renamed to `disco-claw`)
5. **Created a fresh board** - "Disco Claw Experiment"
...
> Hey there! 👋
>
> Looks like this is our first session together. I'm here to help automate your work through Disco (managing tasks across repos, coordinating AI agents, and generally making your life easier).
>
...
>
> **Which board should I use?** I'll do most of my work on a dedicated Disco board so everything stays organized and visible.
>
...
3. ✅ Set up the main board ("Disco Claw Experiment")
4. ✅ Configured repos (`preset-io/disco`, `mistercrunch/disco-claw`)
...
### Disco-First Architecture

Everything runs through Disco MCP. The agent doesn't spawn local processes. It orchestrates Disco resources through natural language:

- **Creating worktrees** - The agent asks Disco to create a new worktree for a task, and it appears on the board instantly
...
The main agent reads these files at the start of every session, syncs state with Disco MCP, and picks up exactly where it left off.
...
├── preset-io-disco.md           # Disco repo: workflow, tech stack, conventions
└── apache-superset.md          # (example) Another repo's context
```

Before working in a repo, the agent reads that repo's context file. It knows Max's exact PR workflow for Disco:

1. `pnpm install`
2. `pnpm check` (types, lint, tests, build)
3. Commit changes
4. `gh pr create`
5. Attach PR URL to worktree via Disco MCP
...
Everything is visual and introspectable on an Disco board.
...
- **Git worktrees** via Disco MCP
- **Zones** on the board (spatial organization, [see zones docs](https://disco.live/docs/concepts/zones))
...
- Tracks tasks as pointers to Disco worktrees and sessions
...
Disco's [session genealogy](https://disco.live/docs/concepts/sessions#genealogy) enables true multi-agent workflows.
...
2. **Sync Disco state** (query worktrees, sessions, tasks)
...
- Disco state synced at 22:24:18 PST

### Board Organization

All work properly organized on "Disco Claw Experiment" board with 7 zones. No orphaned worktrees.
...
Disco provides guarantees that standalone OpenClaw doesn't:

**Sandboxing & Permissions** - Agents run through [Claude Agent SDK](https://docs.anthropic.com/en/docs/agents), [Codex SDK](https://codex.io), [Gemini SDK](https://ai.google.dev), or [OpenCode](https://github.com/opencodeblock/opencode). These frameworks provide sandboxing and permission systems. Disco uses sensible defaults, meaning agents will ask for permissions or get blocked when attempting risky operations. You control how rogue agents can go.

**Structured Repository Management** - All coding jobs go through Disco's framework, making it easy to track where repos and worktrees are stored. You can archive worktrees, see filesystem status, and maintain clean repository hygiene without hunting through directories.

**[Managed Environments](https://disco.live/docs/guide/environments)** - When a repo includes an `.disco.yml` file, you can fire up dev environments instantly. Click play on a worktree widget and a full dev environment appears, perfect for QA, review, or testing agent changes in isolation.
...
### 2. Synced Disco State

The agent queried Disco MCP to get fresh data on all worktrees, sessions, and tasks across the tracked repositories.

Then it **compared** what Disco knows vs. what's in `memory/disco-state/*.json`.

**Found discrepancies?** Update the local files.

### 3. Checked Active Tasks

The agent read `memory/disco-state/sessions.json` and cross-referenced with Disco MCP:

**Task #1: MCP Repo Creation**

- Status in memory: "running"
- Status in Disco: "idle" (completed)
- PR attached: #536
- ✅ COMPLETED
...
**`memory/disco-state/sessions.json`**
  ...
  "sessions": [
    {
      ...
      "pr_url": "https://github.com/preset-io/disco/pull/536"
    }
    // ... etc
  ]
}
...
**`memory/disco-state/worktrees.json`**
...
- Session: 5a242ffa
- PR: https://github.com/preset-io/disco/pull/536
...
3. **Better memory tracking** - improved `disco-state/*.json` structure
...
| Feature                      | OpenClaw                  | Disco-OpenClaw                                 |
| ---------------------------- | ------------------------- | --------------------------------------------- |
...
| **Task tracking**            | ✅ taskr integration      | ✅ Disco worktrees + sessions                  |
...
| **Dev environments**         | ❌ Manual setup           | ✅ One-click via `.disco.yml`                  |

Both frameworks share the core insight: **agents need infrastructure for continuity.**

Disco-OpenClaw adds visibility, multi-agent coordination, git-native state tracking, and built-in safety guardrails.
...
**The public repo:** [github.com/preset-io/disco-teammate](https://github.com/preset-io/disco-teammate)

(Created autonomously by the agent on day one, btw.)

### Long-Term Goals

I want disco-openclaw agents to:
...
All while being **fully visible** on an Disco board.

### Community Iteration

I'm open-sourcing this experiment. The framework is MIT licensed. If you want to:

- Try running your own persistent agent in Disco
- Contribute new skills or patterns
- Share learnings from your own agents
- Suggest improvements

**Join me:** [github.com/preset-io/disco-teammate](https://github.com/preset-io/disco-teammate)
...
1. **An Disco instance** - self-hosted or [Disco Cloud](/blog/disco-cloud)
2. **The Disco teammate template** - [github.com/preset-io/disco-teammate](https://github.com/preset-io/disco-teammate)

Then:

1. Import the repo to Disco
...
The agent loop architecture (SOUL.md, MEMORY.md, persistent identity, self-improvement) is OpenClaw's innovation. I just adapted it to run inside Disco.
...
Disco-OpenClaw is an experiment in bringing those patterns to team-based software development workflows, with full visibility and multi-agent coordination.
...
OpenClaw figured this out. I'm learning from it and adapting it for Disco's multi-agent, spatial canvas paradigm.
...
**[Try Disco →](https://disco.live)** | **[Disco Teammate GitHub →](https://github.com/preset-io/disco-teammate)** | **[Join Disco Cloud Beta →](https://docs.google.com/forms/d/e/1FAIpQLSdXtZwQBHaLFa1LYHvOHXq9IUF_Qm3T12Hr6UZcMdEvjpm2PQ/viewform?usp=dialog)**
...
_If you're running disco-openclaw or building your own agent loops in Disco, I'd love to hear about it. Open a discussion or drop me a note. Your feedback shapes where this goes next._
