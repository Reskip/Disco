...
vi.mock('@disco/core/config', () => configMocks);
vi.mock('@disco/core/db', () => dbMocks);

import { BadRequest, Forbidden, NotAuthenticated } from '@disco/core/feathers';
import type { TaskID, UserID } from '@disco/core/types';
...
describe('ConfigService.resolveApiKey', () => {
  ...
  it('falls back to the server-wide Codex login for every user in trusted simple mode', async () => {
    configMocks.resolveApiKey.mockResolvedValue({
      apiKey: undefined,
      source: 'none',
      useNativeAuth: false,
    ...
    service.app = {
      service: vi.fn(() => ({
        get: vi.fn(async () => ({ created_by: 'new-family-user', session_id: 'session-1' })),
      })),
    } as never;

    await expect(
      service.resolveApiKey({
        taskId: 'task-shared' as TaskID,
  ...
  it('does not enable shared Codex native auth in auth-resolved tenancy', async () => {
    configMocks.resolveApiKey.mockResolvedValue({
      apiKey: undefined,
      source: 'none',
      useNativeAuth: false,
