...
Keep one overview page as the map, but use grouped sections rather than a flat list. It should position Disco as the team command center, then route readers to core workflow first.
...
These should remain prominent because they explain Disco's product model. If a reader only understands three pages, it should be these.
...
- **Disco MCP Server** — agent self-awareness and control surface.

Positioning: Teammates are the durable “who”; Knowledge is the workspace memory; MCP is the tool layer that lets sessions and teammates operate Disco.
