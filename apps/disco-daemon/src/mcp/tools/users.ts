import { ROLES, type User } from '@disco/core/types';
...
type UserFindField = 'username' | 'name' | 'unix_username';
...
export function registerUserTools(server: McpServer, ctx: McpContext): void {
  // Tool 1: disco_users_list
  server.registerTool(
    'disco_users_list',
    {
      description:
        'List users in the system with pagination and optional case-insensitive search across display name, username, and execution home key. Returns compact rows by default; pass lean:false for detailed user payloads.',
      ...
      inputSchema: z.strictObject({
        ...
        search: mcpOptionalString(
          'search',
          'Case-insensitive search across display name, username, and execution home key'
        ),
        lean: z
          .boolean()
          .optional()
  ...
  // Tool 2: disco_users_find
  server.registerTool(
    'disco_users_find',
    {
      description:
        'Find users by display name, username, or unix_username. Useful before admin updates: returns compact matching rows with user_id. Matching is case-insensitive substring.',
      annotations: { readOnlyHint: true },
      inputSchema: z.strictObject({
        search: mcpOptionalString(
          'search',
        ...
        username: mcpOptionalString(
          'username',
          'Username to search for (case-insensitive substring)'
        ),
    ...
    async (args) => {
      ...
      if (!searchTerm) {
        throw new Error('Provide one of: search, username, name, or unix_username');
      }
  ...
  // Tool 4: disco_users_get_current
  server.registerTool(
  ...
  // Tool 5: disco_users_update_current
  server.registerTool(
  ...
  // Tool 6: disco_users_update
  server.registerTool(
    'disco_users_update',
    {
      description:
        'Update any user account (admin operation). Only updates fields that are provided. Can update username, display name, role, password, unix_username, must_change_password, emoji, avatar, and preferences.',
      annotations: { idempotentHint: true },
      inputSchema: z.strictObject({
        userId: mcpRequiredId('userId', 'User', 'User ID to update (UUIDv7 or short ID)'),
        username: mcpOptionalString('username', 'New username (optional)'),
        ...
        role: z
          ...
          .optional()
          .describe(
            'New user role (optional). superadmin=full system and account administration, admin=manage allowed resources, member=standard user, viewer=read-only'
          ),
        unix_username: mcpOptionalString(
          'unix_username',
  ...
  // Tool 7: disco_user_create
  server.registerTool(
    'disco_user_create',
    {
      description:
        'Create a new user account. Requires username and password. Optionally set display name, emoji, avatar, unix_username, must_change_password, and role.',
      inputSchema: z.strictObject({
        username: mcpRequiredString('username', 'Username (must be unique)'),
        ...
        unix_username: mcpOptionalString(
          'unix_username',
          'Opaque execution home key (optional, derived from username when omitted)'
        ),
        must_change_password: z
          .boolean()
          .optional()
        ...
        role: z
          ...
          .optional()
          .describe(
            'User role (optional, defaults to "member"). Roles: superadmin=full system and account administration, admin=manage allowed resources, member=standard user, viewer=read-only'
          ),
      }),
    },
    async (args) => {
