...
export class ReactiveSessionHandle {
  ...
  constructor(client: DiscoClient, sessionId: string, options?: ReactiveSessionOptions) {
    this.client = client;
    this.options = {
  ...
  private async fetchTasksAtHighWater(): Promise<Task[]> {
    ...
    // @disco/core. The surrounding bootstrap/resync journal stays open through
 ...
 * `apps/disco-ui`'s `authErrors.ts` walks, but inlined here to avoid a UI →
...
export interface ReactiveDiscoClient extends DiscoClient {
  session(sessionId: string, options?: ReactiveSessionOptions): ReactiveSessionHandle;
}

export function attachReactiveSessionApi(client: DiscoClient): ReactiveDiscoClient {
  const reactiveClient = client as ReactiveDiscoClient;
  const target = reactiveClient as ReactiveDiscoClient & {
    session?: (sessionId: string, options?: ReactiveSessionOptions) => ReactiveSessionHandle;
  };
...
const CLIENT_STREAM_STATE = new WeakMap<DiscoClient, ClientStreamState>();

function getClientStreamState(client: DiscoClient): ClientStreamState {
 ...
 */
function getCanonicalSessionId(client: DiscoClient, sessionId: string): string | null {
...
async function removeSubscription(client: DiscoClient, sub: StreamSubscription): Promise<void> {
...
function ensureStreamDisconnectHandler(client: DiscoClient, state: ClientStreamState): void {
...
function retainSessionStream(client: DiscoClient, sessionId: string): Promise<void> {
 ...
 */
function resubscribeSessionStream(client: DiscoClient, sessionId: string): Promise<void> {
  const state = CLIENT_STREAM_STATE.get(client);
...
function releaseSessionStream(client: DiscoClient, sessionId: string): void {
  const state = CLIENT_STREAM_STATE.get(client);
  if (!state) return;
  const key = resolveStreamKey(state, sessionId);
  const sub = state.subs.get(key);
 ...
 */
export function __streamSubscriptionCountForTest(client: DiscoClient): number {
...
function disposeSharedReactiveSessionEntry(
  client: DiscoClient,
  clientSessions: Map<string, SharedReactiveSessionEntry>,
  cacheKey: string,
  entry: SharedReactiveSessionEntry
): void {
  if (entry.disposeTimer) clearTimeout(entry.disposeTimer);
  entry.disposeTimer = null;
  entry.handle.dispose();
  clientSessions.delete(cacheKey);
  if (clientSessions.size === 0) SHARED_REACTIVE_SESSIONS.delete(client);
}

function trimIdleReactiveSessionCache(
  client: DiscoClient,
  clientSessions: Map<string, SharedReactiveSessionEntry>
): void {
  if (clientSessions.size <= SHARED_REACTIVE_SESSION_MAX_IDLE_ENTRIES) return;
  const idleEntries = [...clientSessions.entries()]
    .filter(([, entry]) => entry.refCount === 0)
    .sort(([, left], [, right]) => left.lastReleasedAt - right.lastReleasedAt);
  for (const [cacheKey, entry] of idleEntries) {
    if (clientSessions.size <= SHARED_REACTIVE_SESSION_MAX_IDLE_ENTRIES) break;
    disposeSharedReactiveSessionEntry(client, clientSessions, cacheKey, entry);
  }
}
...
export function retainReactiveSession(
  ...
  clientSessions.set(cacheKey, {
    handle,
    refCount: 1,
    disposeTimer: null,
    lastReleasedAt: 0,
  });
  trimIdleReactiveSessionCache(client, clientSessions);
  return handle;
}

/**
 * Release a retained shared reactive session handle.
 * Keeps an idle handle briefly when ref count reaches zero. Switching back to
 * a recently viewed conversation can then reuse its hydrated transcript and
 * attachment URLs without repeating the full session bootstrap. The bounded
 * idle cache is eventually disposed and never survives a browser restart.
 */
export function releaseReactiveSession(
  ...
  entry.disposeTimer = setTimeout(() => {
    if (entry.refCount !== 0 || clientSessions.get(cacheKey) !== entry) return;
    disposeSharedReactiveSessionEntry(client, clientSessions, cacheKey, entry);
