...
const RETIRED_ARCHITECTURE_REMOVAL_POLICY: MigrationImpactPolicy = {
  requiresOfflineCutover: true,
  impact: defineMigrationImpact({
    classification: 'data',
    userAction: 'required',
    rollbackCompatibility: 'incompatible',
    summary: 'Deletes retired Agor-era carrier data and requires the old daemon to remain stopped.',
  }),
};
...
const MIGRATION_IMPACT_REGISTRY = createMigrationImpactRegistry([
  ...
  ['0099_drop_retired_agor_architecture', RETIRED_ARCHITECTURE_REMOVAL_POLICY],
  [
    '0102_drop_retired_agor_architecture',
    {
      ...RETIRED_ARCHITECTURE_REMOVAL_POLICY,
      // SQLite is single-daemon/local in supported deployments; its migration
      // runs transactionally during startup and does not need the PostgreSQL
      // fleet cutover acknowledgement.
      requiresOfflineCutover: false,
    },
  ],
]);
...
export class OfflineMigrationCutoverRequiredError extends MigrationError {
  ...
  constructor(migrations: string[]) {
    super(
      `Offline migration cutover required for: ${migrations.join(', ')}. Stop every daemon using this database, run \`disco db migrate --offline-cutover\` once, then start only daemons running the new version.`
    );
...
function getMigrationsFolder(db: Database): string {
  ...
  // inferring a topology from directory names; this also supports @disco/core
...
/**
 * Fresh Disco databases intentionally contain no product records. Keep the
 * startup hook while callers migrate, but do not recreate the retired default
 * Board or any other Agor-era carrier.
 */
export async function seedInitialData(_db: Database, _createdBy?: string): Promise<void> {
