...
export default defineConfig({
  entry: {
    ...
    'runtime-capabilities': 'src/runtime-capabilities.ts',
    ...
    'templates/session-context': 'src/templates/session-context.ts', // Disco system prompt rendering
    ...
    'agent-runtime': 'src/agent-runtime.ts', // Node-only agent context preload and snapshot rendering
  ...
  onSuccess: async () => {
    ...
    cpSync('src/templates/disco-system-prompt.md', 'dist/templates/disco-system-prompt.md');
    // Published templates must not preserve a restrictive development-worktree mode.
    chmodSync('dist/templates/disco-system-prompt.md', 0o644);
    console.log('✅ Copied disco-system-prompt.md template to dist/');
