...
describe('UI URL runtime resolution', () => {
  it('models bundled daemon UI as /ui basename and same-origin API', () => {
    const runtime = resolveUiRuntime({ baseUrl: '/ui/', pathname: '/ui/s/session-id' });
    ...
    expect(daemonUrlForRuntime(runtime, 'https://disco.example.com', '3030')).toBe(
      'https://disco.example.com'
    );
  });

  it('keeps same-origin API while accepting a bundled build reverse-proxied at root', () => {
    const runtime = resolveUiRuntime({ baseUrl: '/ui/', pathname: '/' });

    expect(runtime.mode).toBe('bundled-daemon-root-proxy');
    expect(routerBasenameForRuntime(runtime)).toBe('');
    expect(usesSameOriginDaemon(runtime)).toBe(true);
  });

  it('models root Vite dev as root basename and daemon-port API', () => {
    const runtime = resolveUiRuntime({ baseUrl: '/', pathname: '/s/session-id' });
  ...
  it('models canonical /ui deep links in Vite dev as /ui basename but daemon-port API', () => {
    const runtime = resolveUiRuntime({ baseUrl: '/', pathname: '/ui/s/session-id' });
