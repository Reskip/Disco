...
import type { MessageID, Task, TaskPendingDispatchStatus, UUID } from '@disco/core/types';
import { MessageRole, SessionStatus, TaskStatus } from '@disco/core/types';
...
/**
 * Create a user-owned Session for Task foreign-key coverage.
...
async function createSessionWithDeps(db: Database): Promise<UUID> {
  ...
  const session = await sessionRepo.create({
    ...
    working_directory: `/tmp/test/session-${sessionCounter++}`,
  });
...
describe('TaskRepository.findAll', () => {
  ...
  dbTest('should restrict by visibleToUserId through direct Session ownership', async ({ db }) => {
    const taskRepo = new TaskRepository(db);
    ...
    await users.create({
      user_id: viewerId,
      username: 'tasks-visible@example.com',
      name: 'Tasks Viewer',
    });
    const visibleSession = await sessions.create({
      ...
      agentic_tool: 'claude-code',
      created_by: viewerId,
      working_directory: `/tmp/tasks-visible/visible-${sessionCounter++}`,
    });
    const hiddenSession = await sessions.create({
      session_id: generateId(),
      agentic_tool: 'claude-code',
      created_by: 'test-user' as UUID,
      working_directory: `/tmp/tasks-visible/hidden-${sessionCounter++}`,
    });
...
describe('TaskRepository.createPending', () => {
  ...
  dbTest(
    'keeps later prompts queued in order while the active task is retrying provider capacity',
    async ({ db }) => {
      const taskRepo = new TaskRepository(db);
      const sessionRepo = new SessionRepository(db);
      ...
      const active = await taskRepo.createPending(
        createPendingInput({
          session_id: sessionId,
          status: TaskStatus.CREATED,
          full_prompt: 'long-running prompt',
        })
      );
      const activeClaim = await taskRepo.claimDispatchAndProjectSession(
        active.task_id,
        TaskStatus.CREATED,
        { status: TaskStatus.DISPATCHING }
      );
      expect(activeClaim.outcome).toBe('claimed');
      await taskRepo.connectExecutor(active.task_id);

      // Capacity retry is a durable transcript projection. It must not make
      // the active Task terminal or make the Session available to the queue.
      ...
      const firstQueued = await taskRepo.createPending(
        createPendingInput({
          session_id: sessionId,
          status: TaskStatus.QUEUED,
          full_prompt: 'queued first',
        })
      );
      const secondQueued = await taskRepo.createPending(
        createPendingInput({
          session_id: sessionId,
          status: TaskStatus.QUEUED,
          full_prompt: 'queued second',
        })
      );

      await expect(
        taskRepo.claimDispatchAndProjectSession(firstQueued.task_id, TaskStatus.QUEUED, {
          status: TaskStatus.DISPATCHING,
        })
      ).resolves.toMatchObject({
        outcome: 'condition_changed',
        task: { task_id: firstQueued.task_id, status: TaskStatus.QUEUED, queue_position: 1 },
      });
      expect((await taskRepo.findQueued(sessionId)).map((task) => task.task_id)).toEqual([
        firstQueued.task_id,
        secondQueued.task_id,
      ]);

      await taskRepo.update(active.task_id, { status: TaskStatus.COMPLETED });
      ...
      ).resolves.toMatchObject({ outcome: 'claimed', task: { task_id: firstQueued.task_id } });
      expect((await taskRepo.findQueued(sessionId)).map((task) => task.task_id)).toEqual([
        secondQueued.task_id,
      ]);

      await taskRepo.update(firstQueued.task_id, { status: TaskStatus.COMPLETED });
      await sessionRepo.update(sessionId, {
        status: SessionStatus.IDLE,
        ready_for_prompt: true,
      });
      await expect(
        taskRepo.claimDispatchAndProjectSession(secondQueued.task_id, TaskStatus.QUEUED, {
          status: TaskStatus.DISPATCHING,
        })
      ).resolves.toMatchObject({ outcome: 'claimed', task: { task_id: secondQueued.task_id } });
    }
  );
