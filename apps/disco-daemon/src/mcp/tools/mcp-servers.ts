import { NotFound } from '@disco/core/feathers';
import {
  isReservedMCPCustomHeaderName,
  isValidMCPHeaderName,
} from '@disco/core/tools/mcp/http-headers';
import type {
...
import { hasMinimumRole, ROLES } from '@disco/core/types';
 ...
 * lister (`disco_mcp_servers_list`) and the per-session attachment view that
 * `disco_sessions_get_current` / `disco_sessions_get` embeds as
...
async function getOAuthStatus(
  ...
  const { UserMCPOAuthTokenRepository } = await import('@disco/core/db');
 ...
 * junction), enriched with OAuth status. Used by `disco_sessions_get_current`
 * and `disco_sessions_get` to expose `attached_mcp_servers` in their payload.
 */
 ...
 *   authorization;
 * - Disco-provided session rows are discoverable to a caller with trusted
...
const mcpServerCreateSchema = z
  .strictObject({
    ...
    scope: z
      ...
      .optional()
      .describe(
        "Scope. Defaults to 'global' so it appears in disco_mcp_servers_list. 'session' is only for explicit session attachment workflows."
      ),
...
function createOrUpdateNextSteps(
  ...
  if (attach?.ok) {
    ...
    steps.push(
      'This session-scoped MCP server must be linked to a session with disco_sessions_add_mcp_server (or set on session creation) before that session can use it.'
    );
  }
  return steps;
}
...
async function resolveTargetSessionId(ctx: McpContext, sessionId?: string) {
  ...
  if (!targetSessionId) {
    return {
      error:
        'No current session context. Pass sessionId explicitly, or reconnect with X-Disco-Session-Id / a session-scoped MCP token.',
    };
  }
...
export function registerMcpServerTools(server: McpServer, ctx: McpContext): void {
  // Tool 1: disco_mcp_servers_list
  server.registerTool(
    'disco_mcp_servers_list',
    {
      description:
        'List the MCP-server catalog the current user can access (i.e. servers eligible to attach to a session). Each entry includes name, transport, auth type, custom-header presence, and OAuth status. Use this to discover IDs to pass to `disco_sessions_create({ mcpServerIds })`. To see which servers are currently ATTACHED to a session, read `attached_mcp_servers` from `disco_sessions_get_current` or `disco_sessions_get`. If the catalog is empty, do not invent an unvetted third-party server or request broad external-service tokens; ask an administrator to configure an approved server in Settings > MCP Servers.',
  ...
  // Tool 2: disco_mcp_servers_auth_status
  server.registerTool(
    'disco_mcp_servers_auth_status',
    {
      description:
        'Check the OAuth authentication status for an MCP server. Returns whether the current user is authenticated. If NOT authenticated, returns instructions for the user to complete OAuth via Settings → MCP Servers. Use disco_mcp_servers_list to get server IDs.',
      annotations: { readOnlyHint: true },
      inputSchema: z.strictObject({
        mcpServerId: mcpRequiredId(
          'mcpServerId',
  ...
  // Tool 4: disco_mcp_servers_update
  server.registerTool(
    'disco_mcp_servers_update',
    {
      description:
        'Update an existing MCP server definition. Permissions are service-enforced: admins always may, members only under the workspace `mcp_member_policy`, and nobody but an admin or the owner may touch a private server. Updating config does not create a session-specific link: enabled `global` servers are already effective for all sessions, while `session` scoped servers need `disco_sessions_add_mcp_server`. Provide only fields to change. Validation rejects incompatible combinations (e.g. stdio+url/auth/headers, remote+command/args, wrong auth fields). OAuth tip: keep only `auth:{type:"oauth"}` unless discovery fails or a provider requires endpoint/client overrides. Use `auth:{type:"none"}` to clear auth.',
      annotations: { destructiveHint: false, idempotentHint: false },
  ...
  // Tool 5: disco_sessions_add_mcp_server
  server.registerTool(
    'disco_sessions_add_mcp_server',
    {
      description:
        'Attach a registered MCP server to a session. Defaults to current session when `sessionId` is omitted. Permissions are service-enforced via the same session-scoped route as the UI: the caller must be allowed to edit that session config, and a server private to another user cannot be attached to it. Verify with `disco_sessions_get_current`/`disco_sessions_get`; the agent may need a restart/re-prompt before its MCP client sees new tools.',
  ...
  // Tool 6: disco_sessions_remove_mcp_server
  server.registerTool(
    'disco_sessions_remove_mcp_server',
    {
      description:
    ...
    async (args) => {
      ...
      return textResult({
        ...
        next_steps: [
          'Verify with disco_sessions_get_current/disco_sessions_get. Restart or re-prompt an existing agent if its MCP client does not hot-reload tool removals.',
        ],
      });
    }
  );

  // Tool 7: disco_sessions_set_mcp_servers
  server.registerTool(
    'disco_sessions_set_mcp_servers',
    {
      description:
    ...
    async (args) => {
      ...
      return textResult({
        ...
        next_steps: [
          'Verify with disco_sessions_get_current/disco_sessions_get. Enabled global MCP servers remain effective even if not listed here.',
