...
2. **Task Completion Detection** (`apps/disco-daemon/src/services/tasks.ts:93-124`)
...
3. **Message Queuing System** (`packages/core/src/db/repositories/messages.ts:203-258`)
   ...
   - Queue processed automatically after task completion (`apps/disco-daemon/src/index.ts:2133-2161`)
...
5. **MCP Server Integration**
   - Parent sessions have access to Disco MCP tools via `mcp_token`
   - Available tools: `disco_sessions_get`, `disco_tasks_get`, `disco_tasks_list`
   - Authenticated via session tokens (provides `userId` and `sessionId`)

### Key Hook Point

**Location:** `apps/disco-daemon/src/index.ts:2133-2161`
...
1. **Message Type and Role**
   ...
   - Add metadata flags: `is_disco_callback: true`, `source: 'disco'`
   - Benefits: Semantic clarity, right-side positioning, enables Disco logo/special styling in UI
...
9. **UI Integration - Spawn Button Behavior**
   ...
   - Advanced indicators (Disco logo, badges, timeline) in future phases
...
│  │  ┌────────────────────────────────────────────────┐      │   │
│  │  │ [Disco System] Child session 4a7b3c2d has       │      │   │
│  │  │ completed its task.                            │      │   │
│  │  │                                                │      │   │
│  │  │ **Original Prompt:**                           │      │   │
...
│  │  │                                                │      │   │
│  │  │ Use Disco MCP tools to inspect results...       │      │   │
│  │  └────────────────────────────────────────────────┘      │   │
│  └──────────────────────────────────────────────────────────┘   │
│                             │                                    │
│                             │ Parent becomes IDLE                │
...
│  │                                                          │   │
│  │  [Uses MCP: disco_tasks_get(taskId: "8f3e9a1c")]         │   │
...
```markdown
[Disco] Child session 4a7b3c2d has completed.
...
Use `disco_tasks_get` (taskId: "8f3e9a1c-1234-5678-90ab-cdef12345678") or `disco_sessions_get` (sessionId: "4a7b3c2d-e5f6-7890-abcd-ef1234567890") for more details.
```

**Rendered Message (Failure Case):**

```markdown
[Disco] Child session 4a7b3c2d has failed.
...
Investigate the failure using `disco_tasks_get` (taskId: "8f3e9a1c-1234-5678-90ab-cdef12345678") or `disco_sessions_get` (sessionId: "4a7b3c2d-e5f6-7890-abcd-ef1234567890").
...
- `metadata.is_disco_callback` = enables Disco logo/special styling in UI
 ...
 */
const DEFAULT_TEMPLATE = `[Disco] Child session {{childSessionId}} has {{#if (eq status "COMPLETED")}}completed{{else}}failed{{/if}}.
...
{{/if}}{{#if (eq status "COMPLETED")}}Use \`disco_tasks_get\` (taskId: "{{childTaskFullId}}") or \`disco_sessions_get\` (sessionId: "{{childSessionFullId}}") for more details.{{else}}Investigate the failure using \`disco_tasks_get\` (taskId: "{{childTaskFullId}}") or \`disco_sessions_get\` (sessionId: "{{childSessionFullId}}").
...
**File:** `apps/disco-daemon/src/services/tasks.ts`

Update the `patch()` method to check for parent and queue callback:

```typescript
import { renderChildCompletionCallback } from '@disco/core/callbacks/child-completion-template';
import { MessagesRepository } from '@disco/core/db/repositories/messages';
...
private async queueParentCallback(
  ...
  try {
    ...
    // Create queued message with Disco callback metadata
...
async createQueued(
  ...
  const message: MessageCreate = {
    ...
    metadata, // Include metadata (is_disco_callback, source, etc.)
  };
...
**File:** `apps/disco-daemon/src/services/sessions.ts`
...
- Decoupled from Disco internals
...
Fetch details: disco_tasks_get('8f3e9a1c-1234-5678-90ab-cdef12345678')
```
...
```
Queue Position 1: [Disco System] Child session 5c8d9e0f has completed...
Queue Position 2: [Disco System] Child session 4a7b3c2d has completed...
Queue Position 3: [Disco System] Child session 6f1a2b3c has completed...
```
...
2. **TasksService Callback Logic** (`apps/disco-daemon/src/services/tasks.test.ts`)
...
1. **CLI Workflow**
   ...
   pnpm disco session create --branch-id <wt> --agentic-tool claude-code
   # Get session ID
...
```markdown
[Disco System] 3 child sessions have completed:
