import { MigrationError, OfflineMigrationCutoverRequiredError } from '@disco/core/db';
...
describe('db migrate presentation', () => {
  ...
  it('keeps SQLite backup and verification guidance safely path-based', () => {
    const sqliteUrl = "file:/tmp/disco db's/runtime.db";
    ...
    expect(backup).toContain(`cp -- '/tmp/disco db'"'"'s/runtime.db'`);
    expect(diagnostics).toContain(`sqlite3 '/tmp/disco db'"'"'s/runtime.db'`);
    expect(diagnostics).toContain('PRAGMA table_info(sessions)');
  });
