 ...
 * local cwd = Disco branch worktree, SDK agent id persisted in sdk_session_id,
 * stream text/thinking/tool events into Disco messages, and cancel the active
 * Cursor run when Disco stops the executor.
 */
...
import { DEFAULT_CURSOR_MODEL } from '@disco/core/models';
...
export function normalizeCursorToolInput(
  ...
  // Preserve Cursor's native tool name when we map it to an Disco/Claude-ish
...
async function resolveCursorApiKey(client: DiscoClient, taskId: TaskID): Promise<string> {
...
async function getInitialUserMessages(client: DiscoClient, taskId: TaskID): Promise<Message[]> {
...
async function getNextMessageIndex(client: DiscoClient, sessionId: SessionID): Promise<number> {
  const result = await client.service('messages').find({
    query: {
...
export async function executeCursorTask(params: {
  ...
  if (params.permissionMode && params.permissionMode !== 'bypassPermissions') {
    console.warn(
      `[cursor] Ignoring permission mode "${params.permissionMode}"; @cursor/sdk currently runs autonomously in Disco.`
    );
  }
  ...
  try {
    ...
    if (!workingDirectory) {
      throw new Error('Cursor sessions require a Session working directory.');
    }
    ...
    const agent = session.sdk_session_id
      ? await Agent.resume(session.sdk_session_id, {
          apiKey,
          model,
          local: { cwd: workingDirectory },
          mcpServers,
        })
      : await Agent.create({
          apiKey,
          model,
          name: session.title || `Disco ${shortId(sessionId)}`,
          local: { cwd: workingDirectory },
          mcpServers,
        });
