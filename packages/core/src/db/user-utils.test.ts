...
function createUserData(overrides?: Partial<CreateUserData>): CreateUserData {
  return {
    username: overrides?.username ?? `test-${generateId().slice(0, 8)}@example.com`,
...
describe('createUser', () => {
  ...
  dbTest('should hash password using bcrypt', async ({ db }) => {
    ...
    const result = await (db as any).query.users.findFirst({
      where: (users: any, { eq }: any) => eq(users.username, 'hash@example.com'),
    });
  ...
  dbTest('should set star emoji for superadmin role', async ({ db }) => {
    const user = await createUser(
      db,
      createUserData({ username: 'sa@example.com', password: 'pass', role: 'superadmin' })
    );
  ...
  dbTest('should throw error if username already exists', async ({ db }) => {
    const data = createUserData({
    ...
    await expect(createUser(db, data)).rejects.toThrow(
      'User with username duplicate@example.com already exists'
    );
  });
  ...
  dbTest('should allow different case emails (SQLite is case-sensitive)', async ({ db }) => {
    ...
    expect(user1.username).toBe('case@example.com');
    expect(user2.username).toBe('CASE@example.com');
    expect(user1.user_id).not.toBe(user2.user_id);
  });

  dbTest('should create multiple users with unique emails', async ({ db }) => {
    const user1 = await createUser(db, createUserData({ username: 'user1@example.com' }));
    const user2 = await createUser(db, createUserData({ username: 'user2@example.com' }));
    const user3 = await createUser(db, createUserData({ username: 'user3@example.com' }));

    expect(user1.user_id).not.toBe(user2.user_id);
    expect(user2.user_id).not.toBe(user3.user_id);
    expect(user1.username).toBe('user1@example.com');
    expect(user2.username).toBe('user2@example.com');
    expect(user3.username).toBe('user3@example.com');
  });
  ...
  dbTest('should set created_at and updated_at to same timestamp', async ({ db }) => {
    ...
    const user = await createUser(db, createUserData({ username: 'time@example.com' }));
  ...
  dbTest('should handle special characters in username', async ({ db }) => {
    const data = createUserData({
    ...
    expect(user.username).toBe('test+tag@example.co.uk');
  });
  ...
  dbTest('should handle long passwords', async ({ db }) => {
    ...
    const result = await (db as any).query.users.findFirst({
      where: (users: any, { eq }: any) => eq(users.username, 'long@example.com'),
    });
...
describe('userExists', () => {
  dbTest('should return true for existing user', async ({ db }) => {
    const data = createUserData({ username: 'exists@example.com' });
    await createUser(db, data);
  ...
  dbTest('should be case-sensitive for username lookups', async ({ db }) => {
    const data = createUserData({ username: 'test@example.com' });
    await createUser(db, data);
  ...
  dbTest('should check multiple users independently', async ({ db }) => {
    await createUser(db, createUserData({ username: 'user1@example.com' }));
    await createUser(db, createUserData({ username: 'user2@example.com' }));
  ...
  dbTest('should handle special characters in username', async ({ db }) => {
    const username = 'test+tag@example.co.uk';
    await createUser(db, createUserData({ username }));
...
describe('getUserByUsername', () => {
  dbTest('should return user for existing username', async ({ db }) => {
    const data = createUserData({
    ...
    const found = await getUserByUsername(db, 'find@example.com');

    expect(found).not.toBeNull();
    expect(found?.user_id).toBe(created.user_id);
    expect(found?.username).toBe('find@example.com');
    expect(found?.name).toBe('Find Me');
    expect(found?.role).toBe('admin');
  });

  dbTest('should return null for non-existent username', async ({ db }) => {
    const found = await getUserByUsername(db, 'nonexistent@example.com');

    expect(found).toBeNull();
  });

  dbTest('should return null for empty database', async ({ db }) => {
    const found = await getUserByUsername(db, 'any@example.com');

    expect(found).toBeNull();
  });

  dbTest('should be case-sensitive for username lookups', async ({ db }) => {
    await createUser(db, createUserData({ username: 'case@example.com', name: 'Case Test' }));

    const found = await getUserByUsername(db, 'CASE@example.com');

    // SQLite TEXT columns are case-sensitive
    expect(found).toBeNull();

    const foundExact = await getUserByUsername(db, 'case@example.com');
  ...
  dbTest('should return complete user object with all fields', async ({ db }) => {
    ...
    const user = await getUserByUsername(db, 'complete@example.com');

    expect(user).not.toBeNull();
    expect(user?.user_id).toBeDefined();
    expect(user?.username).toBe('complete@example.com');
  ...
  dbTest('should handle user with undefined optional fields', async ({ db }) => {
    ...
    const user = await getUserByUsername(db, 'minimal@example.com');
  ...
  dbTest('should distinguish between multiple users', async ({ db }) => {
    await createUser(db, createUserData({ username: 'user1@example.com', name: 'User One' }));
    await createUser(db, createUserData({ username: 'user2@example.com', name: 'User Two' }));

    const user1 = await getUserByUsername(db, 'user1@example.com');
    const user2 = await getUserByUsername(db, 'user2@example.com');
  ...
  dbTest('should not expose password in returned user object', async ({ db }) => {
    await createUser(db, createUserData({ username: 'secure@example.com', password: 'secret123' }));

    const user = await getUserByUsername(db, 'secure@example.com');
...
describe('DEVELOPMENT_DEFAULT_ADMIN_USER', () => {
  it('should have expected development-only default values', () => {
    expect(DEVELOPMENT_DEFAULT_ADMIN_USER.username).toBe('admin');
...
describe('createDefaultAdminUser', () => {
  ...
  dbTest('should throw error if admin user already exists', async ({ db }) => {
    ...
    await expect(createDefaultAdminUser(db, { password: 'different-secret' })).rejects.toThrow(
      'Admin user already exists (username: admin)'
    );
  });

  dbTest('should throw error if admin username exists with different user', async ({ db }) => {
    // Create a user with the admin username manually
    await createUser(db, {
      username: 'admin',
      password: 'different',
      name: 'Different User',
  ...
  dbTest('should be idempotent check (fails on second call)', async ({ db }) => {
    ...
    const found = await getUserByUsername(db, 'admin');
...
describe('User utilities integration', () => {
  dbTest('should support complete user lifecycle', async ({ db }) => {
    ...
    expect(await getUserByUsername(db, 'lifecycle@example.com')).toBeNull();
    ...
    // Retrieve user
    const retrieved = await getUserByUsername(db, 'lifecycle@example.com');
  ...
  dbTest('should handle mixed user roles in same database', async ({ db }) => {
    const superadmin = await createUser(
      db,
      createUserData({ username: 'superadmin@example.com', role: 'superadmin' })
    );
    const admin = await createUser(
      db,
      createUserData({ username: 'admin@example.com', role: 'admin' })
    );
    const member = await createUser(
      db,
      createUserData({ username: 'member@example.com', role: 'member' })
    );
    const viewer = await createUser(
      db,
      createUserData({ username: 'viewer@example.com', role: 'viewer' })
    );
  ...
  dbTest('should work alongside default admin user', async ({ db }) => {
    ...
    expect(await userExists(db, 'admin')).toBe(true);
    expect(await userExists(db, 'user1@example.com')).toBe(true);
    expect(await userExists(db, 'user2@example.com')).toBe(true);
