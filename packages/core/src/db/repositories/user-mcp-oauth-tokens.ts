...
import type { MCPOAuthGrantBindingVersion, MCPServerID, UserID } from '@disco/core/types';
...
export class UserMCPOAuthTokenRepository {
  ...
  constructor(
    private db: Database,
    masterSecret = process.env.DISCO_MASTER_SECRET
  ) {
    ...
    if (this.postgres && !masterSecret) {
      throw new RepositoryError('PostgreSQL MCP OAuth grants require DISCO_MASTER_SECRET');
    }
  }
   ...
   * `@disco/core/tools/mcp/oauth-refresh`, and is orchestrated by the daemon
