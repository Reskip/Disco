...
**A session is an AI conversation.** Unlike a CLI where conversations scroll up and disappear, Disco sessions can **branch** (fork siblings, spawn children, or ask side questions), building a genealogy tree you can navigate, resume, and reason about.
...
For orchestrating agents, the same thing is exposed via `disco_sessions_prompt` with `mode: "btw"`. Agents can ask peer sessions quick questions without breaking their flow.
...
Spawning is more than "start a child." Subsessions in Disco have first-class **callback** semantics:
...
If you've used [Claude Code's subagent feature](https://docs.claudecode.com/subagents), Disco's subsessions go further:

| Feature                       | Claude Code Subagents            | Disco Subsessions                      |
| ----------------------------- | -------------------------------- | ------------------------------------- |
...
**Crucially**, neither Codex, Gemini, nor OpenCode SDKs natively support spawning subsessions. **Disco unlocks this for all supported agents.**
...
Disco previously included an experimental `claude-code-cli` runtime. That integration has been
...
Disco does **not** convert those rows into Claude Agent SDK (`claude-code`) sessions. Historical
...
- [Disco MCP Server](/guide/internal-mcp): How agents introspect and spawn each other programmatically
