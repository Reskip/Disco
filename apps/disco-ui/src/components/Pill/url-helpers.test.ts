...
const githubRepo = {
  slug: 'preset-io/disco',
  remote_url: 'https://github.com/preset-io/disco.git',
};

const localGithubRepo = {
  slug: 'local/disco',
  remote_url: 'git@github.com:preset-io/disco.git',
};

const gitlabSameSlugRepo = {
  slug: 'preset-io/disco',
  remote_url: 'https://gitlab.com/preset-io/disco.git',
};

describe('getGitHubRepoSlugFromRemoteUrl', () => {
  it('extracts GitHub repo identity from HTTPS remotes', () => {
    expect(getGitHubRepoSlugFromRemoteUrl('https://github.com/preset-io/disco.git')).toBe(
      'preset-io/disco'
    );
  });

  it('extracts GitHub repo identity from SSH remotes', () => {
    expect(getGitHubRepoSlugFromRemoteUrl('git@github.com:Preset-IO/Disco.git')).toBe(
      'preset-io/disco'
    );
  });

  it('does not extract identity from non-GitHub remotes with the same slug', () => {
    expect(getGitHubRepoSlugFromRemoteUrl('https://gitlab.com/preset-io/disco.git')).toBeUndefined();
  });
});
...
describe('getUrlDisplayLabel', () => {
  describe('GitHub URLs', () => {
    it('extracts org/repo#number for issues', () => {
      expect(getUrlDisplayLabel('https://github.com/preset-io/disco/issues/714')).toBe(
        'preset-io/disco#714'
      );
    });

    it('extracts org/repo#number for pull requests', () => {
      expect(getUrlDisplayLabel('https://github.com/preset-io/disco/pull/42')).toBe(
        'preset-io/disco#42'
      );
    });
    ...
    it('omits org/repo for issue URLs from the current repo', () => {
      expect(
        getUrlDisplayLabel('https://github.com/preset-io/disco/issues/714', {
    ...
    it('omits org/repo for pull request URLs from the current repo', () => {
      expect(
        getUrlDisplayLabel('https://github.com/preset-io/disco/pull/42', {
          currentRepo: githubRepo,
        })
      ).toBe('#42');
    });
    ...
    it('does not use slug-only identity for non-GitHub repos with matching slug', () => {
      ...
      ).toBe('preset-io/disco#714');
    });

    it('falls back to last segment for short GitHub URLs', () => {
      expect(getUrlDisplayLabel('https://github.com/preset-io/disco')).toBe('disco');
    });
  });
