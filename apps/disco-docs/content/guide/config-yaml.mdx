...
Disco reads `~/.disco/config.yaml` as **deployment-owned, immutable runtime input**. `disco init`
creates it once with mode `0600`. After initialization Disco does not silently rewrite it, preserving
...
`disco init` generates it. Every replica of one HA deployment must share it; a copied config used
to create a separate deployment must receive a new value. The daemon refuses to start without a
valid deployment ID. Interactive `disco daemon start` can back up and explicitly rewrite an older
...
After editing this list, run `disco install --sync`. It installs the exact matching package version, removes
unconfigured tools and packages from older Disco versions, and leaves `config.yaml` untouched. The
...
`DISCO_AGENTIC_TOOLS`); `all` and `none` are explicit shorthands.

If `agentic_tools.installed` is absent in an older config, local CLI installs use a backward-compatible
host-local managed manifest. Run interactive `disco install` and select at least one tool to create or
...
`DAEMON_HOST`, `DAEMON_URL`, `DISCO_JWT_SECRET`, `DISCO_MASTER_SECRET`, `DISCO_TELEMETRY`,
`DISCO_RBAC_ENABLED`, `DISCO_UNIX_USER_MODE`, and the sandbox overrides. They are
...
`disco config` shows the effective result after defaults and supported environment overrides are
applied. `disco config --yaml` emits machine-readable YAML with secrets redacted. Use
`--yaml --show-secrets` only when redirecting to a protected destination and intentionally
materializing a complete replacement file. Disco provides no `config set/get/unset` mutation
...
by `DiscoConfig` in `packages/core/src/config/types.ts`; unknown/removed keys fail validation. Init
