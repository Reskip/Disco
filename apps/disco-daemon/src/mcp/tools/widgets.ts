 ...
 *
 * v1 ships a single tool: `disco_widgets_request_env_vars`. The widget is
  ...
  server.registerTool(
    'disco_widgets_request_env_vars',
    {
      description:
        ...
        'FIRE-AND-FORGET: the widget renders inline; end your turn after calling. You will receive a user-role message ("[Disco] User submitted ...") when the user responds. ' +
    ...
    async (args) => {
      ...
      if (presentEverywhere) {
        ...
        if (params.auto_resume) {
          ...
          const prompt = `[Disco] ${namesList} ${verb} already configured. You can proceed.`;
