...
[`Daemon HA readiness: distributed work model and prerequisite plan`](disco://kb/document/019fd41f-0cbc-756c-a22d-8d2ecac0176b)
**Private Knowledge successor/mirror:**
[`Process-affine HA support matrix`](https://disco.sandbox.preset.zone/ui/kb/disco-cloud-team/architecture/process-affine-ha-support-matrix-2026-08.md)
**Longer-term durable interactive-permission design:**
[`Durable permission-boundary resume`](disco://kb/document/019fe832-cafa-7cbe-af21-728c260ff1ca)
...
1. **Resolved on this branch:** when the selected execution substrate survives
   ...
   (`apps/disco-daemon/src/services/session-token-service.ts:175-216,223-298,300-388`;
...
2. Task control rooms are Feathers `app.channel` objects
   (`apps/disco-daemon/src/utils/realtime-publish.ts:67-84,94-95,651-658`), not
...
- keep the built-in stateless MCP endpoint and PostgreSQL-backed GitHub App
  setup enabled while disabling web terminals, MCP OAuth browser flows,
...
(`apps/disco-daemon/src/setup/socketio-ack.integration.test.ts:46-90`), and the
...
with tabs per branch (`apps/disco-daemon/src/services/terminals.ts:97-105`). The
...
| Persisted MCP OAuth token + daemon cache  | PostgreSQL access/refresh/grant-client fields use OAuth-specific versioned envelopes bound to tenant/subject/server/generation. Full grant configuration binding is rechecked before use; relevant server mutations delete attempts/grants. No daemon bearer cache is used. PostgreSQL refresh uses a database-time claim ID plus grant/refresh generations; SQLite retains the local Promise coalescer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | **P+R** on PostgreSQL; standalone local on SQLite               | One daemon refreshes a rotating token while peers observe its commit. A stale owner becomes `ambiguous`; exact fences prevent a losing `invalid_grant` from deleting a newer row.                                                                                                                                                                                 | The active-active authority is implemented on PostgreSQL after `0078` with a fleet-shared master secret, but the constrained profile still gates the browser flow pending separate activation. Shared start/completion/replacement/refresh/disconnect are admin-only; local deletion is not provider revocation. Never put secrets in Redis.                                                                                                                                                                                                                                                |
...
| OAuth/GitHub third-party callback                | **No.** Redirects need not carry the originating affinity cookie.              | PostgreSQL covers both states; MCP OAuth remains disabled pending separate HA activation/certification.                  |
...
7. Task execution with live Disco-managed permission routing for Claude,
...
2. **Shared executor credential authority (P0) — implemented on current main.**
   ...
   a review environment with `DISCO_TEST_POSTGRES_URL`.
...
7. **Transient callback state (P1/P2) — durable prerequisites complete; GitHub
   ...
   tenant-qualified and no origin-only authorization-code cache remains. The
   constrained profile still gates MCP OAuth pending a separate activation
   decision; this change activates only GitHub App setup.
...
`DistributedWorkController`. `@disco/core/coordination` is intentionally only
...
1. The initial profile disables web terminal, Gemini/Codex provider-native
   ...
   introspection. Claude, Copilot, and OpenCode Disco-managed permission
...
- fast unit/contract tests cover SQLite local max-use/reuse, tenant claim,
  ...
  (`apps/disco-daemon/src/services/session-token-service.test.ts`);
...
- the env-gated PostgreSQL suite creates two independent service instances and
  ...
  pnpm --filter @disco/daemon exec vitest run src/services/session-token-service.postgres.test.ts
  ```

In the prerequisite branch's targeted run, 89 core tests and 56 daemon/auth tests passed; the 8
PostgreSQL tests are discovered but skipped because `DISCO_TEST_POSTGRES_URL` is
