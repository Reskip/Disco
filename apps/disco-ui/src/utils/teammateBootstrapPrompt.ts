...
function formatTeammateBootstrapPrompt(context: TeammateBootstrapPromptContext): string {
  const lines = [
    '### First-session onboarding instructions for Disco AI teammate',
    '',
    'Context:',
  ...
  if (context.user?.name) {
    lines.push(
      `- User: ${context.user.name}${context.user.username ? ` (${context.user.username})` : ''}`
    );
  } else if (context.user?.username) {
    lines.push(`- Username: ${context.user.username}`);
  }
  ...
  lines.push('');
  lines.push(
    "Read ONBOARDING.md if it exists; otherwise, read BOOTSTRAP.md. Then respond to the user. Use the supplied context and live Disco state, and ask only the next useful question if the user's goal is not already clear."
  );
