...
Disco-session binding, but it did not retain or compare an immutable tenant
...
`stopListening()`, and `sendMessage()`. Provider callbacks do not carry an Disco
...
4. **Immutable stateful binding:** an MCP Streamable HTTP session is bound at
   initialize time to `(tenant, user, optional Disco session)`. Every subsequent
   request re-authenticates; tenant and user must match. The optional Disco
