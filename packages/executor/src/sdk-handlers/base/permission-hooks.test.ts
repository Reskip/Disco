...
describe('createCanUseToolCallback', () => {
  ...
  describe('MCP auto-approve', () => {
    it('auto-allows tools from the built-in "disco" server without consulting the DB', async () => {
      const deps = createBaseDeps();
      const callback = createCanUseToolCallback(sessionId, taskId, deps);

      const toolInput = { sessionId };
      const result = await callback('mcp__disco__disco_sessions_get_current', toolInput, noopOptions);
      ...
      // The disco server is added dynamically — should NOT round-trip through the DB.
