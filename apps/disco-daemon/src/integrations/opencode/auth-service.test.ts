import { isTenantAgenticToolEnabled, loadConfigSync } from '@disco/core/config';
import {
...
vi.mock('@disco/core/config', async () => {
  const actual = await vi.importActual<typeof import('@disco/core/config')>('@disco/core/config');
  return {
    ...actual,
...
vi.mock('@disco/core/db', async () => {
  ...
  return { ...actual, SessionRepository: vi.fn(), UsersRepository: vi.fn() };
});

vi.mock('@disco/core/unix', async () => {
  const actual = await vi.importActual<typeof import('@disco/core/unix')>('@disco/core/unix');
  return {
    ...actual,
...
const sessionRepository = vi.mocked(SessionRepository);
...
beforeEach(() => {
  ...
  sessionRepository.mockImplementation(function repository() {
    return {
      findById: vi.fn(async () => ({
        session_id: 'session-1',
        created_by: 'same-user',
        working_directory: '/worktrees/session-1',
      })),
    };
  } as never);
...
describe('OpenCode provider auth service', () => {
  ...
  it('authorizes session-aware discovery and forwards only the resolved directory', async () => {
    await runWithTenantContext('tenant-a', () =>
      service().find({ ...params, query: { session_id: 'session-1' } } as never)
    );
    ...
    expect(runCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        params: {
          tool: 'opencode',
          request: { operation: 'discover', directory: '/worktrees/session-1' },
        },
      }),
      expect.any(Object)
    );
  ...
  it('rejects unsupported discovery scope and another user session before executor activity', async () => {
    sessionRepository.mockImplementationOnce(function repository() {
      return {
        findById: vi.fn(async () => ({
          session_id: 'session-1',
    ...
    await runWithTenantContext('tenant-a', async () => {
      ...
      ).rejects.toThrow(/only an optional session ID/i);
      await expect(
        service().find({ ...params, query: { session_id: 'session-1' } } as never)
  ...
  it('does not resolve another tenant session with the same identifier', async () => {
    sessionRepository.mockImplementation(function repository() {
      return {
        findById: vi.fn(async () =>
          requireCurrentTenantId() === 'tenant-a'
            ? {
                ...
                created_by: 'same-user',
                working_directory: '/worktrees/tenant-a',
              }
            : undefined
        ),
      };
    ...
    await runWithTenantContext('tenant-a', () =>
      service().find({ ...params, query: { session_id: 'shared-id' } } as never)
    );
