 ...
 *
 * Authenticates requests using personal API keys (disco_sk_...).
 * Supports both Authorization: Bearer and X-API-Key headers.
 */

import type { UserApiKeysRepository } from '@disco/core/db';
import { AuthenticationBaseStrategy, NotAuthenticated } from '@disco/core/feathers';
...
export class ApiKeyStrategy extends AuthenticationBaseStrategy {
  ...
  async authenticate(authentication: any, params: any): Promise<any> {
    ...
    if (!apiKey?.startsWith('disco_sk_')) {
   ...
   * Supports:
   * - Authorization: Bearer disco_sk_...
   * - X-API-Key: disco_sk_...
   */
  ...
  async parse(req: any): Promise<{ strategy: string; apiKey: string } | null> {
    ...
    if (xApiKey && typeof xApiKey === 'string' && xApiKey.startsWith('disco_sk_')) {
    ...
    if (authorization && typeof authorization === 'string') {
      ...
      if (scheme?.toLowerCase() === 'bearer' && token?.startsWith('disco_sk_')) {
