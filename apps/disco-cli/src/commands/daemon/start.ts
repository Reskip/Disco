/**
 * `disco daemon start` - Start daemon as a detached background process.
 *
 * Loads config up front, then spawns the daemon in the background via
 * daemon-manager. The CLI exits
 * immediately; logs go to ~/.disco/logs/daemon.log.
 *
...
import { assertConfiguredAgenticToolsReady } from '@disco/core/agentic-integrations';
import type { DiscoConfig } from '@disco/core/config';
import { resolveDaemonUrl } from '@disco/core/config';
import { resolveDatabaseUrl } from '@disco/core/db';
...
export default class DaemonStart extends Command {
  static description = 'Start the Disco daemon in the background';

  static examples = [
    '<%= config.bin %> daemon start',
    '<%= config.bin %> daemon start --config /etc/disco/config.yaml',
  ...
  static flags = {
    config: Flags.string({
      char: 'c',
      description: 'Path to config file (default: ~/.disco/config.yaml)',
    }),
  ...
  async run(): Promise<void> {
    ...
    if ((await probeDiscoDaemon(daemonUrl)).running) {
      this.error(
        `An Disco daemon is already running at ${daemonUrl}, but it is not managed by this CLI. Stop its service, container, or foreground terminal instead.`
      );
    }
    ...
    //    into ~/.disco/logs/daemon.log — so the error would be invisible at
    ...
    if (flags.foreground) {
      this.log(chalk.bold('Starting Disco daemon in foreground...'));
      try {
    ...
    this.log(chalk.bold('Starting Disco daemon...'));
    ...
    if (flags.config) {
      env.DISCO_CONFIG_PATH = resolve(flags.config);
    }
    ...
    try {
      ...
      this.log(chalk.dim('  Logs: ~/.disco/logs/daemon.log'));
    } catch (error) {
      this.log(chalk.red('Failed to start daemon:'));
      this.log(chalk.red(`  ${error instanceof Error ? error.message : String(error)}`));
      this.exit(1);
  ...
  private async failOnUnreadyAgenticTools(config: DiscoConfig): Promise<void> {
    try {
  ...
  private resolveDaemonEntrypoint(): string {
    ...
    this.log(
      chalk.yellow('Development mode detected. Use `pnpm dev` in apps/disco-daemon/ for hot-reload.')
    );
    this.log(chalk.yellow('Starting daemon without watch mode...'));

    // Resolve to compiled daemon entrypoint
    const here = dirname(fileURLToPath(import.meta.url));
    return resolve(here, '../../../disco-daemon/dist/main.js');
  }
}
