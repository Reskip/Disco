import type { MCPServer, SessionID } from '@disco/core/types';
...
describe('getMcpServersForSession', () => {
  ...
  it('resolves an OAuth server whose only template is oauth_client_secret', async () => {
    const prevKeys = process.env.DISCO_USER_ENV_KEYS;
    const prevSecret = process.env.OAUTH_CLIENT_SECRET;
    process.env.DISCO_USER_ENV_KEYS = 'OAUTH_CLIENT_SECRET';
    ...
    try {
      ...
      if (prevKeys === undefined) delete process.env.DISCO_USER_ENV_KEYS;
      else process.env.DISCO_USER_ENV_KEYS = prevKeys;
