...
describe('executorRuntimeScopeGuard', () => {
  ...
  it('rejects retired Branch endpoints', async () => {
    const context = ctx({ path: 'branches', method: 'find' });
    await expect(executorRuntimeScopeGuard()(context)).rejects.toThrow(/not valid/);
  });
  ...
  it('rejects retired Repo endpoints without consulting a carrier service', async () => {
    const context = ctx({
      path: 'repos',
      method: 'get',
      id: 'repo-1',
  ...
  it('rejects retired Repo endpoints even with a tenant scope', async () => {
    const context = ctx({
      path: 'repos',
      method: 'get',
      id: 'tenant-b-repo',
  ...
  it('allows a task executor to read one same-user Agent through service ownership checks', async () => {
    const context = ctx({ path: 'agents', method: 'get', id: 'agent-1' });

    await expect(executorRuntimeScopeGuard()(context)).resolves.toBe(context);
  });

  it.each(['find', 'create', 'patch', 'remove'])(
    'keeps Agent enumeration and mutation unavailable to executor tokens: agents.%s',
    async (method) => {
      const context = ctx({
        path: 'agents',
        method,
        id: method === 'find' || method === 'create' ? undefined : 'agent-1',
      });

      await expect(executorRuntimeScopeGuard()(context)).rejects.toThrow(
        /not valid for this agents request/
      );
    }
  );
