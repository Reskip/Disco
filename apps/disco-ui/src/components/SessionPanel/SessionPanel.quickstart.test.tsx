...
import type { Branch, Session } from '@disco-live/client';
...
beforeEach(() => {
  discoStore.getState().setAgenticToolSettings([]);
});
...
describe('SessionPanel inline title edit', () => {
  it('shows the localized untitled placeholder for a session with no title or description', () => {
  ...
  it('click-to-edit saves a new title on Enter', () => {
    ...
    fireEvent.click(screen.getByText('未命名对话'));
    const input = screen.getByRole('textbox', { name: '编辑当前对话标题' });
    expect(input).toHaveAttribute('maxlength', '100');
    expect(input).toHaveClass('disco-session-title-input');
...
describe('SessionPanel simplified actions', () => {
  it('does not expose the removed switch-tool menu', () => {
    renderPanel(makeSession({ tasks: [] }), { onChooseAgenticTool: vi.fn() });
    expect(screen.queryByText('Switch tool…')).not.toBeInTheDocument();
    expect(screen.queryByText('切换工具')).not.toBeInTheDocument();
  });
});
