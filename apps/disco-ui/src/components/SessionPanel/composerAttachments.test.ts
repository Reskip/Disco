...
describe('composerAttachments', () => {
  ...
  it('accepts arbitrary file types before send', () => {
    expect(
    ...
    expect(acceptedFiles.map((file) => file.name)).toEqual(['notes.txt', 'unsafe.svg']);
  ...
  it('infers supported file types from safe extensions when dropped files have empty MIME', () => {
    ...
    expect(acceptedFiles.map((file) => [file.name, file.type])).toEqual([
      ...
      ['unsafe.svg', 'application/octet-stream'],
    ]);
  });

  it('preserves browser-reported MIME types without filtering them', () => {
    const { acceptedFiles, rejections } = validateComposerFileIntake([
      new File(['<script>'], 'renamed.txt', { type: 'text/html' }),
    ]);

    expect(acceptedFiles.map((file) => [file.name, file.type])).toEqual([
      ['renamed.txt', 'text/html'],
    ]);
    expect(rejections).toEqual([]);
  });
  ...
  it('applies only the count cap when arbitrary files exceed one batch', () => {
    const files = [
    ...
    expect(summarizeComposerFileRejections(rejections)).toBe(
      'bad.svg: 最多可添加 10 个待上传文件（另有 11 个）'
    );
  });
