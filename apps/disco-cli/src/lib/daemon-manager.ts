/**
 * Daemon Manager - Lifecycle management for disco-daemon
 *
...
export function getPidFilePath(): string {
  return path.join(getDiscoHome(), 'daemon.pid');
}

export function getDaemonIdentityFilePath(): string {
  return path.join(getDiscoHome(), 'daemon.instance');
}
...
export function getLogFilePath(): string {
  const logsDir = path.join(getDiscoHome(), 'logs');
...
export function startDaemon(
  ...
  const child = spawn('node', [daemonPath], {
    ...
    env: {
      ...
      NODE_ENV: 'production',
      ...extraEnv,
      DISCO_MANAGED_DAEMON_INSTANCE_ID: managedInstanceId,
    },
  });
