...
export function formatRelativeTime(timestamp: string | Date): string {
  ...
  if (diffMin < 60) {
    return isChinese ? `${diffMin} 分钟前` : `${diffMin}m ago`;
  }
  if (diffHour < 24) {
    return isChinese ? `${diffHour} 小时前` : `${diffHour}h ago`;
  }
  if (diffDay < 7) {
    return isChinese ? `${diffDay} 天前` : `${diffDay}d ago`;
  }
  if (diffDay < 30) {
    const weeks = Math.floor(diffDay / 7);
    return isChinese ? `${weeks} 周前` : `${weeks}w ago`;
  }
  if (diffDay < 365) {
    const months = Math.floor(diffDay / 30);
    return isChinese ? `${months} 个月前` : `${months}mo ago`;
  }
