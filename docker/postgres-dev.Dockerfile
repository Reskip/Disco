# Development PostgreSQL image for Disco.
#
...
COPY docker/postgres-init-app-user.sql /tmp/disco-init-app-user.sql
RUN install -o root -g root -m 0644 \
  /tmp/disco-init-app-user.sql \
  /docker-entrypoint-initdb.d/10-disco-app-user.sql \
  && rm /tmp/disco-init-app-user.sql
