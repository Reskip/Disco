...
describe('isValidGitRepo', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'disco-git-test-'));
  });
...
describe('getRemoteUrl', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'disco-git-test-'));
    remoteDir = await fs.mkdtemp(path.join(os.tmpdir(), 'disco-git-remote-'));
  });
...
describe('getDefaultBranch', () => {
  ...
  it('should handle custom remote names', async () => {
    ...
    // Add another remote
    const otherRemoteDir = await fs.mkdtemp(path.join(os.tmpdir(), 'disco-git-other-'));
...
describe('createBranch', () => {
  ...
  it('should handle pullLatest parameter', async () => {
    const remoteDir = await fs.mkdtemp(path.join(os.tmpdir(), 'disco-git-remote-'));
...
describe('cloneRepo', () => {
  ...
  it('should clone repository to an explicitly resolved layout path', async () => {
    ...
    const result = await cloneRepo({
      url: remoteDir,
      targetDir: path.join(tempDir, '.disco', 'repos', path.basename(remoteDir)),
    });

    expect(result.path).toContain('.disco/repos');
  ...
  it('should throw if target exists but is not a git repo', async () => {
    const targetDir = path.join(tempDir, '.disco', 'repos', 'test-repo');
  ...
  // land on that branch (so `.disco.yml` etc. on a feature branch is
...
describe('createBranchAsClone', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'disco-git-test-clone-'));
  ...
  it('uses --reference when referencePath exists on disk (alternates pointer)', async () => {
    ...
    // base at `~/.disco/repos/<slug>/` that the daemon would manage.
