...
import type { DiscoClient, MCPMemberPolicy } from '@disco-live/client';
import { DEFAULT_MCP_MEMBER_POLICY } from '@disco-live/client';
...
export function useMcpMemberPolicy(client: DiscoClient | null): McpMemberPolicyState {
  ...
  useEffect(() => {
    if (!client) {
      ...
      setLoading(false);
      setError('Not connected to the Disco service');
      return;
    }
    let active = true;
    setLoading(true);
