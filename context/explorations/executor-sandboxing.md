...
> connectivity. Disabled by default; `disco init` offers a one-shot opt-in. Validated live: a real
> Codex agent gets **`Permission denied`** on `~/.disco/config.yaml`, an **empty** `~/.ssh`, and
...
One chokepoint. In `apps/disco-daemon/src/utils/spawn-executor.ts`, when `execution.sandbox.enabled`
...
- **Tool nesting avoided:** the wrap sets `DISCO_OUTER_SANDBOX=1`; Codex reads it and forces
...
--ro-bind /dev/null ~/.disco/config.yaml           # protect_secrets: mask daemon secrets…
--ro-bind /dev/null ~/.disco/disco.db
...
from Disco's own DB state — `repo.local_path` for the branch's repo, resolved in `register-services`
...
--ro-bind /dev/null <data_home>/disco.db
--setenv HOME <passwd-home>  --setenv TMPDIR /tmp
--chdir  <branch>
```

When the data root is below the passwd home, the overlay hides the **entire** daemon `.disco` tree
(config.yaml, disco.db, worktrees, repos) by construction. When a hosted deployment places the data
root elsewhere, Disco explicitly masks that root before re-exposing the authorized branch, base git
...
mount namespace — not Unix groups — enforces branch authorization, computed straight from Disco's
permission service.

---

## 3. Config (as shipped) — `~/.disco/config.yaml`

Filesystem-only. Snake_case Disco knobs; no network semantics.
...
execution:
  ...
  sandbox:
    enabled: false # master switch. Default OFF. Chosen at `disco init` (single y/N).
    include:
    ...
    protect_secrets: true # mask ~/.disco/{config.yaml,disco.db} + ~/.ssh, ~/.gnupg, ~/.aws, ~/.config/gcloud, ~/.npmrc
...
- `DISCO_SANDBOX_ENABLED=true` env → `execution.sandbox.enabled`; `DISCO_SANDBOX_HOME_MODE=per_user|shared`
  → `execution.sandbox.home_mode`. Both are used by the `sandbox` / `sandbox-peruser` `.disco.yml` dev
  variants and forwarded into the container via `docker-compose.yml` (in-memory resolve; config.yaml
  is never rewritten).
- `disco doctor` reports the sandbox row (enabled? `bwrap` present? Linux?).

---

## 4. Validated live (containerized dev env, real Codex agent)

With `enabled: true` in the `sandbox` `.disco.yml` variant, a real Codex session prompted to read
secrets produced (fresh, un-recallable output):

- `wc -c ~/.disco/config.yaml` → **empty**; `cat ~/.disco/config.yaml` → **`Permission denied`**
...
- `~/.disco/config.yaml` and `disco.db` → **`No such file`** (hidden by the overlay, no explicit mask)
...
- Hostile/injected agent reading daemon secrets (`jwtSecret`, `masterSecret`, API keys, `disco.db`).
...
- **No per-user OS identity.** Everything runs as the daemon user; `ps`/file-ownership no longer
  reflect the Disco user, and SSH-into-the-box no longer maps to branch access. This was only ever
  deliverable via the host-account apparatus and never survived HA/multi-pod. The correct model is
  "Disco owns its filesystem; access only via Disco or admins."
...
- **Process-side isolation via PID namespace (BEST-EFFORT).** When the host allows it we
  ...
  **Caveat:** many container runtimes refuse to mount proc in a nested PID namespace ("Operation not
  ...
  the host's `ptrace_scope` or — in Disco Cloud, where each tenant/user runs in its OWN container —
...
- **Per-task, branch-scoped.** Wraps prompts/terminals/commands that carry a branch work dir;
  repo-level ops (e.g. `git.repo.realign-origin`) run unwrapped (Disco's own trusted code).
...
Disco computes directly from its own RBAC config — instead of round-tripping the permission model
...
strict's whole-filesystem group model. At spawn, `register-services` asks Disco's permission service
...
5. ✅ **Migration tooling + "de-strict" recipe** — `scripts/strict-to-sandbox-migration.sh`
   ...
   flips `unix_user_mode: sandbox`, and optionally `groupdel disco_wt_*` + removes the sudoers file.
   ...
   **Pre-flight before relocating** — `scripts/sandbox-home-migration-preflight.sh` (read-only; prints
   ...
   requires zero auth/settings blockers. In Disco the executor cwd is the branch (re-exposed at its
...
6. ✅ **Soft-deprecate** — the daemon logs a deprecation warning on `strict`/`insulated` startup
   pointing to the migration script (`apps/disco-daemon/src/startup.ts`).
...
8. **Delete** — remove sudoers, `unix/run-as-user`, the sudo paths in `unix/user-manager`, password
   ...
   `sandbox` has real prod miles (keeps a fallback through the Disco Cloud rollout).
...
- `packages/core/src/config/types.ts` — `DiscoSandboxSettings` incl. `home_mode`.
...
- `packages/core/src/config/config-manager.ts` — `DISCO_SANDBOX_ENABLED` → `enabled`, `DISCO_SANDBOX_HOME_MODE` → `home_mode` (in `resolveEffectiveConfig`).
- `apps/disco-daemon/src/register-services.ts` — resolves authoritative `repo.local_path` (→ `sandboxBaseRepoPath`) + per-owner store path (→ `sandboxHomeStore`) into `payload.params`.
- `apps/disco-daemon/src/utils/spawn-executor.ts` — wrap at `spawnExecutorLocal` (keys on `payload.params.cwd`); threads `sandboxBaseRepoPath`/`sandboxHomeStore`.
- `apps/disco-daemon/src/utils/sandbox-wrap.ts` — `buildSandboxWrap()`: takes authoritative paths (no disk parsing), `mkdir`s the per-user store, resolves, drops missing-target masks, emits `bwrap …`.
- `packages/executor/src/sdk-handlers/codex/prompt-service.ts` — `DISCO_OUTER_SANDBOX` → `sandboxMode=danger-full-access` (no nesting).
- `apps/disco-cli/src/commands/doctor.ts` + `src/lib/sandbox-diagnostics.ts` — `disco doctor` sandbox row (bwrap check) (+ tests).
- `apps/disco-cli/src/commands/init.ts` — one-shot enable prompt.
- `docker/Dockerfile` — `bubblewrap` in the dev image.
- `docker-compose.yml` + `.disco.yml` — `DISCO_SANDBOX_ENABLED` + `DISCO_SANDBOX_HOME_MODE` passthrough + `sandbox` / `sandbox-peruser` dev variants.
- `packages/core/src/config/types.ts` — `UnixUserMode` adds `'sandbox'` (the strict-replacement switch).
- `packages/core/src/config/config-manager.ts` — `unix_user_mode: sandbox` derivation (implies RBAC + enabled per-user sandbox + fail-closed) + `DISCO_SANDBOX_HOME_MODE` env.
...
- `apps/disco-daemon/src/startup.ts` — `sandbox` startup banner + `strict`/`insulated` soft-deprecation warning.
