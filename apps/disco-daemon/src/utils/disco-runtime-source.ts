 ...
 *
 * Listens for `postMessage` queries from the Disco parent page and replies
 ...
 *
 * Wire format:
 *   parent → iframe: { type: 'disco:query', requestId, kind, args }
 *   iframe → parent: { type: 'disco:result', requestId, ok: bool, result?, error? }
 *
 * Supported `kind`:
 ...
 */
export const DISCO_RUNTIME_SOURCE = `// disco-runtime.js — injected by Disco at render time. Do not edit.
// Source: apps/disco-daemon/src/utils/disco-runtime-source.ts
(function () {
  if (typeof window === 'undefined') return;
  if (window.__disco_runtime_installed__) return;
  window.__disco_runtime_installed__ = true;
  ...
  window.addEventListener('message', function (event) {
    var data = event.data;
    if (!data || typeof data !== 'object' || data.type !== 'disco:query') return;
    ...
    function reply(payload) {
      try {
        var msg = { type: 'disco:result', requestId: requestId };
  ...
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'disco:ready' }, '*');
    }
  } catch (e) {
    /* parent closed */
  }
