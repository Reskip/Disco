...
export function estimateCodexCostUsd(input: CodexEstimatedCostInput): number | undefined {
  ...
  const cacheWriteCostPerToken = pricing.cache_creation_input_token_cost ?? inputCostPerToken;
  ...
  if (cacheWriteTokens > 0 && cacheWriteCostPerToken === undefined) return undefined;
  ...
  return (
    ...
    cacheWriteTokens * (cacheWriteCostPerToken ?? 0) +
