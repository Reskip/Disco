...
import type { PermissionDecision } from '@disco/core/permissions';
import type {
  HookContext,
  Message,
...
import { PermissionScope, PermissionStatus } from '@disco/core/types';
...
export async function deliverPermissionDecision(options: {
  ...
  const { app, sessionId, data, params } = options;
  ...
  // Executor control is user-owned. A caller who can merely see a shared
  // tenant resource must never answer permission prompts for another user.
  if (session.created_by !== decidedBy) {
    throw new Forbidden('Only the Session owner may respond to permission requests.');
  }
