# Disco home page design proposal

## Summary recommendation

Make the home page a **boards portal first, personalized dashboard second, onboarding hub when empty**.

The board remains Disco's primary work surface, so home should not replace the canvas. It should answer three questions quickly:
...
- **Boards/branches are primary work units**: `context/concepts/branches.md`, `apps/disco-ui/src/components/SessionCanvas/`, `apps/disco-ui/src/components/BranchCard/`.
...
Recommendation: keep the primary section named **Boards**. “Workspace” is not part of Disco’s information architecture. Teammates should appear as context inside board cards when they are primary/attached, with a small fallback for unassigned teammates if needed.
...
- Default label should be **Boards**, not “Your boards”. Disco is multiplayer; over-personalizing the primary section hides team work.
...
- Make actor and target text clickable: user links open profile/user context; branch/teammate/artifact/comment links deep-link to the relevant Disco object.
...
Use Disco's default Ant Design theme rather than a bespoke visual language. The default theme is defined in `apps/disco-ui/src/contexts/ThemeContext.tsx` as an AntD `ThemeConfig` with Disco teal (`colorPrimary: #2e9a92`), standard AntD status colors, `borderRadius: 8`, and the active light/dark algorithm.
...
- Build a real Home SPA page using AntD and existing Disco components/patterns.
...
- Use Disco's default AntD theme via `theme.useToken()`; avoid bespoke mock CSS.
...
1. Should the Disco logo navigate to Home, current board, or last visited board?
...
- `context/explorations/disco-home-page-mock/index.html`
- `context/explorations/disco-home-page-mock/index.js` (Sandpack artifact entry)
- `context/explorations/disco-home-page-mock/styles.css`

Open it directly in a browser, or publish it as an Disco artifact from the folder.
