...
description: Running Disco executors in Kubernetes, Docker, or other containerized environments with shared storage
---
...
This guide covers running Disco's executor component in containerized environments like Kubernetes, enabling scalable and isolated execution of AI coding agents.

---

## Overview

Disco's architecture separates the **daemon** (orchestration layer) from the **executor** (isolated execution environment). This separation enables flexible deployment models:
...
Running Disco in Kubernetes is fundamentally different from deploying static web applications:

| Static Deploys       | Disco Dev Environments   |
| -------------------- | ----------------------- |
...
Containerized Disco requires infrastructure that most organizations already have for multi-server environments. **Disco assumes this infrastructure exists - it does not provide it.**

### Storage and identity requirements

A delegated executor must receive trusted `{tenant_id}` and `{user_id}` values and bind them to isolated runtime storage. The launcher—not a host uid chosen by Disco—owns the mapping to container identities, persistent homes, credentials, and branch mounts.
...
**If the Disco database is persistent, every branch workspace it references must also be
...
on disk: ...`, see [issue
\#1109](https://github.com/preset-io/disco/issues/1109)), but the only
real fix is to restore the volume or, if the volume is unrecoverable,
clean up the orphan database rows using
`disco branch remove` / `disco repo remove` before re-cloning.
...
Disco's development model requires:
...
Disco supports separating daemon config from git data:
...
| `{unix_user}`     | Compatibility delegated home key        | `disco_alice`          |
| `{session_id}`    | Disco session ID (if available)          | `01abc123...`         |
| `{branch_id}`     | Branch ID (if available)                | `01xyz789...`         |
| `{user_id}`       | Trusted authenticated Disco user UUID    | `01user789...`        |
...
`{tenant_id}` but no tenant context is active, Disco refuses to execute the
...
`{unix_user}` is validated against the delegated home-key format and Disco refuses
...
> **Codex subscription credentials caveat:** Codex auth-file operations already run through an
> executor, but correctness requires every auth helper and Task for one trusted tenant/user identity
> to receive the same persistent home. Declare that guarantee as
> `execution.executor_storage.user_home: persistent-per-user`; the launcher must actually implement
> it. The constrained HA profile admits auth-file check/import/logout only with a consistent
> declared home. Declaring storage does not make the process-affine device-code poller durable, so
> device sign-in remains gated.
...
spec:
  securityContext:
    # Run as non-root user (mapped from Disco user)
    runAsNonRoot: true
    runAsUser: 1001
    runAsGroup: 1001
...
Disco does not create or select container users. A delegated launcher receives
...
{
  "command": "prompt",
  "daemonUrl": "http://disco-daemon.disco.svc.cluster.local:3030",
...
```yaml
# ~/.disco/config.yaml (read by daemon only)
daemon:
  port: 3030
...
Disco has an explicit constrained active-active daemon mode using PostgreSQL, Redis realtime fanout,
...
With shared storage (EFS/NFS) for both `DISCO_DATA_HOME` and `/home`, the daemon becomes largely stateless:
...
Disco no longer creates Unix users/groups or sends UID/GID template variables. Configure pod/container `securityContext` in the launcher or workload definition. Prefer stable `{tenant_id}` and `{user_id}` inputs and resolve them through trusted deployment configuration rather than mutable usernames.
...
In a fully containerized Disco deployment, there are three distinct tiers:

```
┌─────────────────────────────────────────────────────────────┐
│  Tier 1: Disco Control Plane                                 │
...
spec:
  ...
  template:
    spec:
      serviceAccountName: disco-daemon
      containers:
        - name: daemon
          image: ghcr.io/preset-io/disco:latest
          command: ['disco', 'daemon', 'start']
          volumeMounts:
            - name: config
              mountPath: /home/disco/.disco
            - name: data
              mountPath: /data/disco
```
...
| Metric                           | Description                  |
| -------------------------------- | ---------------------------- |
| `disco_executor_spawn_total`      | Total executor spawns        |
| `disco_executor_duration_seconds` | Execution duration histogram |
| `disco_executor_failures_total`   | Failed executions            |
| `disco_prompt_tokens_total`       | Token usage by session       |
...
```bash
kubectl exec -it executor-abc123 -- ls -la /data/disco/worktrees
```
...
```bash
kubectl logs -f deployment/disco-daemon | grep "EXECUTOR_TIMEOUT"
```
...
kubectl exec -it executor-abc123 -- \
  curl -s http://disco-daemon:3030/health
```
...
mv ~/.disco/repos /data/disco/repos
mv ~/.disco/worktrees /data/disco/worktrees

# Create symlinks for backward compatibility
ln -s /data/disco/repos ~/.disco/repos
ln -s /data/disco/worktrees ~/.disco/worktrees
```
...
runtime contract as the daemon. Upgrade remote executor images to the same Disco
...
Disco can detect an executor whose wrapper is alive but whose SDK stream stops
...
For locally spawned executors, Disco additionally verifies process-group absence
...
remains unverified. After an abrupt daemon restart, Disco keeps its existing
