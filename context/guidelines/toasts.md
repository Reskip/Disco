...
Source: [`apps/disco-ui/src/utils/message.tsx`](../../apps/disco-ui/src/utils/message.tsx).
...
- **Keyed loading→success/error.** Pass `{ key: 'download' }` to both `showLoading(...)` and the follow-up `showSuccess(...)` / `showError(...)`; antd replaces the toast in place instead of stacking. See `apps/disco-ui/src/components/BranchModal/tabs/FilesTab.tsx`.
