import { Forbidden } from '@disco/core/feathers';
import type { AuthenticatedParams, HookContext, Params } from '@disco/core/types';
import {
...
export function executorRuntimeScopeGuard() {
  return async (context: HookContext): Promise<HookContext> => {
    ...
    if (path === 'sessions') {
      ...
      // A task may need the current Agent profile to validate and refresh its
      // preloaded runtime snapshot. The Agents service still scopes the lookup
      // to params.user, so this read cannot cross the Disco user boundary; no
      // executor token may enumerate or mutate Agents.
      if (context.method !== 'get' || !id) {
        throw new Forbidden('Executor token is not valid for this agents request');
      }
    } else if (path === 'tasks') {
      const taskId = expectClaim(scope.taskId, 'task');
      if (context.method === 'find') {
      ...
      throw new Forbidden(
        `Executor token is not valid for this endpoint (${path || '<unknown>'}.${context.method})`
      );
    }
