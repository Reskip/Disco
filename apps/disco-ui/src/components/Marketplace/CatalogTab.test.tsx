...
import { sessionPath } from '@disco-live/client';
...
function makeClient(): DiscoClient {
  const service = (path: string) => {
    ...
    if (path === 'sessions') {
      return {
        findAll: async () => [{ session_id: SESSION_ID, title: 'Research session' }],
      };
    }
  ...
  return { service } as unknown as DiscoClient;
}
...
describe('catalog browsing', () => {
  ...
  it('says the daemon is unreachable rather than spinning silently', async () => {
    ...
    try {
      ...
      await act(() => Promise.resolve());
      expect(screen.queryByText('Not connected to the Disco daemon')).not.toBeInTheDocument();

      await act(async () => {
        await vi.advanceTimersByTimeAsync(2500);
      });

      expect(screen.getByText('Not connected to the Disco daemon')).toBeVisible();
      expect(screen.queryByText('No servers match')).not.toBeInTheDocument();
    } finally {
      vi.useRealTimers();
    }
...
describe('connect', () => {
  ...
  it('connects the catalog entry to the selected existing session', async () => {
    const drawer = await openDrawer();
    const connect = drawer.getByRole('button', { name: /Connect/ });
    fireEvent.click(drawer.getByRole('checkbox'));
    await waitFor(() => expect(connect).toBeEnabled());
    ...
    expect(localStorage.getItem(`disco-draft-${SESSION_ID}`)).toBe(DEEPWIKI.starter_prompt);
  });
