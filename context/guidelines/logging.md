...
Operational logs should tell a safe, bounded story of what Disco did. They are an event stream for
...
operational logging in `disco-ui`: browser output is user-controlled, inconsistently retained, and
...
diagnose. An existing narrow `DISCO_DEBUG_*` flag can be preferable to enabling all debug calls; do
...
- **Operator-configured analytics** (`@disco/core/analytics`, disabled by default) sends curated,
...
- **Open-source telemetry** (`@disco/core/telemetry`) is separate and opt-in, with an anonymous
...
to Disco's current implementation.
