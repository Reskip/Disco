...
**Status:** 🔬 Exploration
**Target:** Disco Cloud (initial), Disco Local (future)
**Complexity:** High
...
3. ✅ Executors run as separate Unix user (`disco_executor` or per-user `disco_alice`)
...
→ Tool: Bash("cat ~/.disco/config.yaml")
...
```bash
# Agent running via disco-executor (separate process, separate user)
→ Tool: Bash("cat ~/.disco/config.yaml")
← Error: Permission denied (disco_executor can't read disco user files) ✅
```
...
| **Terminal command injection** | ❌ Can read ~/.disco/config.yaml                 | High (users run arbitrary commands) |
...
Agent via Claude SDK:
  → Tool: Bash("sqlite3 ~/.disco/disco.db 'SELECT email FROM users'")
...
### Why This Is Critical for Disco Cloud

**Disco Cloud = Multi-tenant SaaS**
...
```
┌─────────────────────────────────────────────────────────┐
│  Disco Daemon (Trusted, Privileged)                      │
│  Unix User: disco                                        │
│                                                          │
│  Responsibilities:                                       │
...
└─────────────────────────────────────────────────────────┘
        ...
        /var/run/disco/executor.sock
              ↓
┌─────────────────────────────────────────────────────────┐
│  Disco Executor (Untrusted, Sandboxed)                   │
│  Unix User: disco_executor (or disco_alice)               │
│                                                          │
│  Responsibilities:                                       │
│  - Spawn terminals (PTY)                                │
...
│  - Home: /home/disco_executor (isolated)                 │
│  - Cannot access: /home/disco/, ~/.disco/config.yaml      │
│                                                          │
...
N Executor Processes
  - Per-user isolation (disco_alice, disco_bob)
...
- ✅ File permissions (only disco and disco_executor can connect)
...
```json
{
  ...
  "params": {
    ...
    "cwd": "/home/disco/.disco/worktrees/myapp/feature-x",
...
**Location:** `/apps/disco-daemon/src/services/executor-ipc.ts`

```typescript
import * as net from 'node:net';
import { logger } from '@disco/core';

export class ExecutorIPCService {
  private server: net.Server;
  private socketPath = '/var/run/disco/executor.sock';
  ...
  async start() {
    ...
    // Set permissions (only disco and disco_executor can connect)
...
```typescript
// apps/disco-daemon/src/index.ts
...
async function executeViaExecutor(session: Session, task: Task, prompt: string) {
  ...
  const executor = await executorPool.spawn({
    unix_user: 'disco_executor', // Or per-user: disco_alice
  });
...
// Entry point
async function main() {
  const executor = new DiscoExecutor();
  await executor.connect();

  logger.info('Disco Executor started');
}
...
```bash
#!/usr/bin/env node
// packages/executor/bin/disco-executor

const { DiscoExecutor } = require('../dist/index.js');
...
**Location:** `/apps/disco-daemon/src/services/executor-pool.ts`
...
export class ExecutorPool {
  ...
  async spawn(options: { unix_user: string }): Promise<ExecutorProcess> {
    ...
    if (useImpersonation) {
      ...
      command = 'sudo';
      args = ['-u', unix_user, '/usr/local/bin/disco-executor'];
    } else {
      // Spawn as current user (fallback)
      command = 'node';
      args = ['/usr/local/bin/disco-executor'];
    }
  ...
  private async shouldUseImpersonation(): Promise<boolean> {
    ...
    try {
      execSync('sudo -n -l /usr/local/bin/disco-executor', { stdio: 'ignore' });
      return true;
    } catch {
      return false;
    }
...
**Modified: `/apps/disco-daemon/src/services/terminals.ts`**
...
export class TerminalsService {
  ...
  private async createViaExecutor(data: CreateTerminalData, params: Params) {
    ...
    const unix_user = user?.unix_username || 'disco_executor';
...
│  - Runs as unprivileged user (disco_executor or per-user)  │
...
execution:
  ...
  # IPC settings
  ipc:
    socket_path: /var/run/disco/executor.sock
    socket_permissions: 0o660 # Only disco and executor users can connect
...
```bash
$ sudo disco setup-executor-isolation

Disco Executor Isolation Setup
==============================
...
Step 1: Create executor Unix user
  → sudo useradd -r -s /bin/bash disco_executor
  ✓ Created user disco_executor (uid: 999)

Step 2: Configure sudoers
  → /etc/sudoers.d/disco-executor
  ✓ Daemon user 'disco' can run executor as disco_executor
...
To create per-user Unix accounts (recommended):
  sudo disco user setup-unix alice@example.com
```
...
- [ ] Implement `DiscoExecutor` (executor side)
...
- [ ] Executor spawns via sudo (per-user: disco_alice)
...
- [ ] Setup command: `disco setup-executor-isolation`
- [ ] User linking: `disco user setup-unix <email>`

**Success Criteria:**

- Terminals run as correct Unix user (`whoami` = disco_alice)
...
**Week 1-2:** Internal testing (Disco team only)

- Enable on dev environment
- Fix critical bugs

**Week 3-4:** Beta testing (select Disco Cloud customers)
...
- Enable by default for new Disco Cloud instances
...
**Executor isolation** provides a robust security boundary for Disco Cloud by separating privileged daemon operations from untrusted user execution.
