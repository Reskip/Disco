import { normalizeAgenticToolModelConfiguration } from '@disco/agentic-tools/config';
import {
...
import { TENANT_AGENTIC_TOOL_NAMES } from '@disco/core/types';
...
export class AgenticToolPresetsService {
  ...
  async remove(id: string): Promise<AgenticToolPreset> {
    ...
    const [sessions, schedules, users] = await Promise.all([
