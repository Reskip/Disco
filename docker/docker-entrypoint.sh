...
echo "🚀 Starting Disco development environment..."
...
# 10 workspace packages (apps/disco-docs and packages/disco-live are neither installed
...
# "dubious ownership" guard inside the container). Harmless in Disco's
...
# setups feed DISCO_BUILD_SHA via env from .disco.yml's start command instead.
git config --global --add safe.directory /app 2>/dev/null || true

# Fix home directory permissions (volumes may have wrong UID/GID from previous builds)
echo "🔧 Fixing home directory permissions..."
mkdir -p /home/disco/.disco /home/disco/.cache
sudo -n chown -R disco:disco /home/disco 2>/dev/null || true
...
DIST_DIRS="/app/packages/git/dist /app/packages/core/dist /app/packages/agentic-tool-opencode/dist /app/packages/agentic-tools/dist /app/packages/executor/dist /app/packages/client/dist /app/apps/disco-daemon/dist /app/apps/disco-cli/dist /app/apps/disco-ui/dist"
if sudo -n true 2>/dev/null; then
  ...
  sudo -n chown disco:disco /app/packages/* /app/apps/* 2>/dev/null || true

  # Chown dist directories recursively (in case they have nested files)
  sudo -n chown -R disco:disco $DIST_DIRS
  ...
  sudo -n chown -R disco:disco $BUILD_INFO_DIRS
...
echo "🔨 Building @disco/git (initial build)..."
pnpm --filter @disco/git build

echo "⏳ Waiting for @disco/git type definitions..."
MAX_WAIT=30
WAITED=0
...
echo "✅ @disco/git initial build complete (including type definitions)"

echo "🔨 Building @disco/core (initial build)..."
pnpm --filter @disco/core build

# Wait for DTS files (tsup's rollup-plugin-dts runs async after main build)
echo "⏳ Waiting for @disco/core type definitions..."
MAX_WAIT=30
WAITED=0
...
echo "✅ @disco/core initial build complete (including type definitions)"

echo "🔨 Building @disco/agentic-tool-opencode (initial build)..."
pnpm --filter @disco/agentic-tool-opencode build

echo "⏳ Waiting for @disco/agentic-tool-opencode type definitions..."
MAX_WAIT=30
WAITED=0
...
echo "✅ @disco/agentic-tool-opencode initial build complete (including type definitions)"

echo "🔨 Building @disco/agentic-tools (initial build)..."
pnpm --filter @disco/agentic-tools build

echo "⏳ Waiting for @disco/agentic-tools type definitions..."
MAX_WAIT=30
WAITED=0
...
echo "✅ @disco/agentic-tools initial build complete (including type definitions)"

echo "🔨 Building @disco/executor (initial build)..."
pnpm --filter @disco/executor build

echo "⏳ Waiting for @disco/executor type definitions..."
MAX_WAIT=30
WAITED=0
...
echo "✅ @disco/executor initial build complete (including type definitions)"

export DISCO_EXECUTOR_PATH=/app/packages/executor/bin/disco-executor

echo "🔨 Building @disco-live/client (initial build)..."
pnpm --filter @disco-live/client build

echo "⏳ Waiting for @disco-live/client type definitions..."
MAX_WAIT=30
WAITED=0
...
echo "✅ @disco-live/client initial build complete (including type definitions)"

# Start watch modes for hot-reload
echo "🔄 Starting watch modes..."
pnpm --filter @disco/git dev &
GIT_PID=$!

pnpm --filter @disco/core dev &
CORE_PID=$!

pnpm --filter @disco/agentic-tool-opencode dev &
AGENTIC_TOOL_OPENCODE_PID=$!

pnpm --filter @disco/agentic-tools dev &
AGENTIC_TOOLS_PID=$!

pnpm --filter @disco/executor dev &
EXECUTOR_PID=$!
...
# `disco init` is the SQLite installation bootstrap and intentionally creates
# ~/.disco/disco.db. PostgreSQL deployments get their database state from the
# migrations and daemon bootstrap below, and receive stable development
# secrets from the postgres Compose overlay.
if [ "${DISCO_DB_DIALECT:-sqlite}" = "postgresql" ]; then
  echo "⏭️  Skipping SQLite disco init for PostgreSQL deployment"
else
  echo "📦 Initializing Disco environment..."
  pnpm disco init --skip-if-exists --non-interactive --agentic-tools "${DISCO_AGENTIC_TOOLS:-none}" --daemon-port "${DAEMON_PORT:-3030}" --daemon-host "${DAEMON_HOST:-0.0.0.0}"
fi
...
ADMIN_OUTPUT=$(pnpm --filter @disco/cli exec tsx bin/dev.ts local create-admin --dev-default 2>&1)
echo "$ADMIN_OUTPUT"
...
ADMIN_USER_ID=$(cd /app && DISCO_SILENT_PRAGMA_LOGS=true ./node_modules/.bin/tsx scripts/get-admin-id.ts || echo "")
...
PORT="${DAEMON_PORT:-3030}" pnpm --filter @disco/daemon dev:daemon-only &
DAEMON_PID=$!
...
# VITE_DAEMON_URL (when set by the .disco.yml dev variant) points the browser
...
VITE_DAEMON_PORT="${DAEMON_PORT:-3030}" VITE_DAEMON_URL="${VITE_DAEMON_URL:-}" pnpm --filter disco-ui dev --host 0.0.0.0 --port "${UI_PORT:-5173}"
