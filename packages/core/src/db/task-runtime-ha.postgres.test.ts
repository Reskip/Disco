 ...
 *   pnpm --filter @disco/core exec vitest run src/db/task-runtime-ha.postgres.test.ts
 */
...
async function seedTenant(db: Database, label: string): Promise<TenantSeed> {
  ...
  return runWithTenantDatabaseScope(db, tenantId, async (scoped) => {
    ...
    const session = await new SessionRepository(scoped).create({
      ...
      agentic_tool: 'codex',
      working_directory: `/tmp/task-runtime-ha/${tenantId}/${generateId()}`,
    });
    return {
      tenantId,
