...
`/demo/marketing-video` (disco-ui) into short, loopable hero videos for
disco.live. Sibling of `../launch-video/`; binaries are gitignored, the final
...
(see `apps/disco-ui/src/pages/marketing/`):
...
- `MarketingVideoPage.tsx` — fork of the screenshot fixture that samples the
  scene at `t` and exposes `window.__discoDemo = { setTime, getDuration, isReady }`
...
cd apps/disco-docs/demo-videos
npm install
npx playwright install chromium

# Terminal 1: UI dev server
pnpm --filter disco-ui dev        # vite on :5173

# Terminal 2: capture + encode
node capture.mjs                 # all scenes  (--scene <name> for one)
./encode.sh                      # per-scene mp4s + disco-hero.mp4 + poster
...
#   __discoDemo.setTime(4000)
```

## Adding a scene

1. Create `apps/disco-ui/src/pages/marketing/scenes/<name>.ts` exporting a
...
| `out/disco-hero.mp4`        | All scenes stitched with 0.25s fades — loop this in the hero |
| `out/disco-hero-poster.jpg` | Poster frame for the `<video>` element                       |

Embed shape (hero div, reduced-motion fallback to the static screenshot):

```html
<video autoplay muted loop playsinline poster="/videos/disco-hero-poster.jpg">
