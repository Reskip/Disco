...
import type { DiscoClient, Schedule, Session } from '@disco-live/client';
...
export const ScheduleRunsPanel: React.FC<ScheduleRunsPanelProps> = ({
  ...
  return (
    <Drawer
      ...
      rootClassName="disco-schedule-runs-drawer"
      title={schedule ? `运行记录 · ${schedule.name}` : '运行记录'}
      open={open}
      onClose={onClose}
      width={640}
      destroyOnClose
    >
      {loading ? (
        ...
        <Table
          className="disco-schedule-runs-table"
          rowKey="session_id"
          dataSource={runs}
          columns={columns}
          pagination={false}
