import { EXECUTOR_RESULT_PREFIX } from '@disco/core/executor-protocol';
...
export function completeExecutorResult<T extends { success: boolean }>(result: T): void {
  ...
  process.stdout.write(line, () => {
    // On Windows, exiting from inside libuv's write callback can race other closing
    // async handles and surface a native Node assertion window. Move the forced exit
    // to the next check phase: stdout is already flushed, while libuv gets one turn to
    // finish its close callbacks before teardown.
