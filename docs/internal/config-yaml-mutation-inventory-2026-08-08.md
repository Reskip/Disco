...
`~/.disco/config.yaml` is deployment-owned input. After initial creation, ordinary Disco
...
| `disco config set/get/unset`       | Operator CLI                    | Set/unset rewrote the document; get read one key      | Set/unset could run unattended without warning                                                        | Removed. `disco config` is read-only and can display/materialize the effective configuration as YAML.                                                             |
| JWT bootstrap                     | Daemon startup when missing     | Generated `daemon.jwtSecret`                          | Unattended runtime rewrite; daemon race                                                               | Removed. `DISCO_JWT_SECRET` wins over YAML; otherwise fail closed. Init creates a stable secret.                                                                  |
| Master-secret bootstrap           | Daemon startup when missing     | Generated `daemon.masterSecret`                       | Unattended rewrite/race could invalidate encrypted values                                             | Removed. `DISCO_MASTER_SECRET` wins over YAML; otherwise fail closed. Init creates a stable secret.                                                               |
| Telemetry env materialization     | Startup with `DISCO_TELEMETRY=1` | Enabled flag and instance ID                          | Unattended rewrite                                                                                    | Removed; override remains in memory.                                                                                                                             |
...
| `disco init`                       | Initial install                 | Defaults, telemetry choice, stable secrets            | Interactive/headless creation                                                                         | Allowed only during initialization. Uses exclusive create, mode 0600; existing-install `--set-config` is rejected.                                               |
...
MCP tools, installers, setup/repair helpers, and tests. `.disco.yml`, git config, Codex config, and
...
| telemetry                                                       | `DO_NOT_TRACK` and `DISCO_TELEMETRY*` → YAML → disabled/unconfigured | deployment-global         |
...
`disco config set`, config loading, and Docker materialization, so automation using set/get/unset
must migrate to explicit source-file management or read-only `disco config --yaml` inspection.
