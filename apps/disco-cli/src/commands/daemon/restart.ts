/**
 * `disco daemon restart` - Restart daemon
 */

import { assertConfiguredAgenticToolsReady } from '@disco/core/agentic-integrations';
import { type DiscoConfig, getDaemonUrl, resolveDaemonUrl } from '@disco/core/config';
...
export default class DaemonRestart extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (!isInstalledPackage()) {
      ...
      this.log(`  2. ${chalk.cyan('cd apps/disco-daemon && pnpm dev')}`);
      this.log('');
      this.exit(1);
    }
    ...
    if (!daemonPath) {
      ...
      this.log(`  ${chalk.cyan('npm install -g disco-live')}`);
      this.log('');
      this.exit(1);
    }
    ...
    try {
      if (
        ...
        (await probeDiscoDaemon(replacementDaemonUrl)).running
      ) {
        throw new Error(
          `An Disco daemon is already running at the replacement URL ${replacementDaemonUrl}. Stop its service, container, or foreground terminal before restarting.`
        );
      }
      ...
      if (existingPid !== null) {
        ...
        if (!identity) {
          ...
          throw new Error(
            `Refusing to signal PID ${existingPid}: it cannot be verified as the CLI-managed Disco daemon at ${oldDaemonUrl}.`
          );
        }
        stopped = stopDaemon();
      } else if ((await probeDiscoDaemon(oldDaemonUrl)).running) {
        throw new Error(
          `An Disco daemon is running at ${oldDaemonUrl}, but it is not managed by this CLI. Stop its service, container, or foreground terminal instead.`
        );
      }
      if (stopped) {
      ...
      const restartEnv = identity?.configPath
        ? { DISCO_CONFIG_PATH: identity.configPath }
        : undefined;
      ...
      this.log(`  ${chalk.cyan('disco daemon logs')}`);
      this.log('');
