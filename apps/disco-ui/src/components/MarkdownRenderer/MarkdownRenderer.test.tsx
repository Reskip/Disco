...
const doc = `# Project: Next Steps\n\n- Add semantic search once indexing is configured.\n- Introduce smart document chunking for long pages.\n- Preserve preferences, project context, decisions, and reusable prompts.\n- Support skill bundles and lightweight import/export.\n- Keep polishing authoring, history, and safer collaboration defaults.\n- Autocomplete references from sessions and other places.\n- Git syncing?`;
...
describe('MarkdownRenderer', () => {
  ...
  it('refreshes preview text when an earlier bullet list item changes', async () => {
    ...
    expect(
      await screen.findByText(
        'Add amazing semantic search once indexing is configured.'
      )
    ).toBeInTheDocument();
    expect(
      screen.queryByText('Add semantic search once indexing is configured.')
  ...
  it('renders internal upload links as authenticated preview and download actions', async () => {
    ...
    render(
      <MarkdownRenderer
        content={
          '[example.txt](https://disco.live/_uploads/upl_00000000-0000-4000-8000-000000000001) (text/plain, 12 B)'
        }
      />
    );
    ...
    expect(screen.getByRole('button', { name: '下载 example.txt' })).toBeInTheDocument();

    anchorClick.mockRestore();
  });

  it('renders Unicode internal upload filenames without exposing raw virtual URLs', async () => {
    render(
      <MarkdownRenderer
        content={
          '[报告最终版.txt](https://disco.live/_uploads/upl_00000000-0000-4000-8000-000000000001)'
        }
      />
    );

    expect(await screen.findByRole('button', { name: '报告最终版.txt' })).toBeInTheDocument();
    expect(screen.queryByText(/Blocked URL/i)).not.toBeInTheDocument();
  });

  it('renders historical image upload links inline', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
    ...
    vi.stubGlobal(
      'URL',
      Object.assign(URL, {
        createObjectURL: vi.fn(() => 'blob:inline-image'),
        revokeObjectURL: vi.fn(),
      })
    );
    ...
    render(
      <MarkdownRenderer
        content={
          '[sunflower.png](https://disco.live/_uploads/upl_00000000-0000-4000-8000-000000000002)'
        }
      />
    );

    expect(await screen.findByRole('img', { name: 'sunflower.png' })).toHaveAttribute(
      'src',
      'blob:inline-image'
    );
  ...
  it('renders host-local Markdown links as readable text beside published downloads', async () => {
    render(
      <MarkdownRenderer content={'已创建：[下载报告](E:\\Disco\\workspace\\报告.txt)'} />
    );
  ...
  it('keeps ordinary code syntax-highlighted with its controls', async () => {
    ...
    const { container } = render(<MarkdownRenderer content={fenced('typescript', lines)} inline />);
  ...
  it('renders common LLM LaTeX delimiters while preserving code examples', async () => {
    const { container } = render(
      <MarkdownRenderer
        inline
        content={'行内 \\(x^2 + y^2\\) 与 $z^2$。\n\n\\[\\frac{a}{b}\\]\n\n`\\(literal\\)`'}
      />
    );
  ...
  it('does not parse plain PowerShell variables as an unbreakable math span', async () => {
    const content =
      '只运行一次 PowerShell：$names="CODEX_HOME"; foreach($n in $names){ $value=$env:CODEX_HOME }';
    const { container } = render(<MarkdownRenderer inline content={content} />);

    await waitFor(() => expect(container).toHaveTextContent('$names="CODEX_HOME"'));
