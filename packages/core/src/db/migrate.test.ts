...
describe('migration status introspection', () => {
  ...
  it('describes retired carrier removal as an incompatible data cutover', () => {
    expect(getMigrationImpact('0099_drop_retired_agor_architecture')).toMatchObject({
      classification: 'data',
      userAction: 'required',
      rollbackCompatibility: 'incompatible',
    });
    expect(getMigrationImpact('0102_drop_retired_agor_architecture')).toMatchObject({
      classification: 'data',
      userAction: 'required',
      rollbackCompatibility: 'incompatible',
    });
  ...
  it('registers impact metadata for every offline cutover migration', () => {
    const offlineMigrations = pendingOfflineCutoverMigrations('postgresql', {
      ...
      pending: [
        ...
        '0099_drop_retired_agor_architecture',
      ],
    });

    expect(offlineMigrations).toHaveLength(5);
