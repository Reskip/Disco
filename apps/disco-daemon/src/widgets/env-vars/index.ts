...
import { type Database, SessionEnvSelectionRepository } from '@disco/core/db';
...
async function applyEnvVarsSubmit(
  ...
  // `canResolveWidget` (Session owner), so
...
export const envVarsWidget: WidgetRegistryEntry<EnvVarsParams, EnvVarsSubmit, EnvVarsResultMeta> = {
  ...
  buildDismissedPrompt: (params) =>
    `[Disco] User dismissed the request for ${orderedEnvVarNames(params.names).join(', ')}. ` +
