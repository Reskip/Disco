...
**Date:** 2026-08-14
**Related:** [[executor-expansion]], [[daemon-fs-decoupling]], [[executor-isolation]], [[web-terminal-ownership-ha]], [[clone-redesign]], [[task-runtime-state]], `apps/disco-docs/content/guide/containerized-execution.mdx`
...
> backends** used for hardened multi-tenant / Disco Cloud, reached via the `executor_backend` /
...
> Disco already models that split. Build a **first-class local Docker executor
...
1. **There is no single "stdout/sentinel + socket" behavior to preserve uniformly.**
   ...
   The stdout sentinel (`DISCO_EXECUTOR_RESULT `) is used **only** for one-shot commands
...
2. **"Wrap the executor invocation in a container" already exists as a config path.**
   ...
   `spawnExecutorWithTemplate` (`apps/disco-daemon/src/utils/spawn-executor.ts:504`), and
...
it actually simplify Disco while preserving Stop correctness, tenant boundaries, and the
Cloud direction?_
...
`apps/disco-daemon/src/utils/spawn-executor.ts`
...
- `findExecutorPath()` (`:320`) honors `DISCO_EXECUTOR_PATH`, else bundled `../executor/cli.js`.
...
`apps/disco-daemon/src/register-services.ts` (`createExecuteHandler`, ~`:855-1280`):
...
| **stdout sentinel** `DISCO_EXECUTOR_RESULT `          | one-shot commands (git/file/env/unix)                                | `console.log(prefix+json)` (`executor-output.ts`), parsed `spawn-executor.ts:564`; settle on **`close`** not `exit` (#2222) | `docker run -i` proxies stdout via docker CLI — must verify flush semantics survive |
...
- Fallback containment: `apps/disco-daemon/src/executor-tracking.ts:259-380` — `SIGTERM` to
...
- Durable fences under `~/.disco/runtime/executor-containment-fences/` (0600 fsync'd JSON);
...
- Path: `getBranchPath` → `$DISCO_DATA_HOME/worktrees/<repoSlug>/<branchName>`
  (`config-manager.ts:1791`); base repo at `~/.disco/repos/<slug>/` (`:1759`).
- **Two storage modes** (`branch.ts:362-388`, `config/types.ts:596-633`):
  - `worktree` (default): the branch's `.git` is a **`gitdir:` pointer into the base repo**
    (`~/.disco/repos/<slug>/.git/worktrees/…`); object store + `.git/config` live **outside** the
...
- `docker/Dockerfile` multi-stage: `base` (node:22-slim + git/zellij/gh/pnpm, **no agentic CLIs**;
  creates `disco` + unprivileged `disco_executor` users), `development` (`disco-dev`), `production`,
  `production-source` (**what CI ships**, `disco-live` bundle = daemon+cli+executor+core+ui), `…-ha`.
  **No `disco-executor` image stage exists.** The executor is bundled inside `disco-live`.
- `ghcr.io/preset-io/disco-executor:latest` appears **only** in config docstrings + `containerized-execution.mdx`
...
- Managed agentic tools: `disco install [--sync]` pins `@disco-live/<tool>@<discoVersion>` into
  `~/.disco/agentic-tools/<version>/<tool>/` (`agentic-integrations.ts`, `apps/disco-cli/src/commands/install.ts`);
  `DISCO_MANAGED_AGENTIC_TOOLS=1` selects managed vs workspace SDKs. This is the reproducible mechanism
  for baking pinned CLIs into an executor image.
- `.disco.yml` environment variants (`ha`, `docs`, `full`, `sqlite`, …) are **docker-compose commands that
  containerize _Disco-under-development_** — a different axis from Disco's own executor substrate (see §7).

---

## 2. Conceptual boundary: is `docker` a unix mode, a backend, or both?

**It is an execution backend/topology, orthogonal to identity policy.** Disco's own type system
...
execution:
  ...
  docker: # backend-specific block, only read when backend=docker
    image: ghcr.io/preset-io/disco-executor:<pinned>
...
| **stdout sentinel + flush**         | ⚠️ verify           | `DISCO_EXECUTOR_RESULT ` flows through docker's stdout proxy. The #2222 "settle on close" fix must still hold with docker's stream teardown. Needs a runtime smoke test (feasible; §13).                                                                                                                                                                                                                                            |
...
| **daemon restart / orphans**        | ⚠️                  | Orphan reaping must enumerate containers by **label** (`disco.task_id`, `disco.tenant_id`, `disco.daemon_boot_id`) via `docker ps --filter label=…`, not pgid. Restart reconciliation lists labeled containers and cross-checks the DB.                                                                                                                                                                                               |
...
Design principle: **mount the least, require clone storage, never mount the base repo, never mount `~/.disco`.**
...
| **Base repo (`~/.disco/repos`)**               | **Do not mount.** Require `storage_mode=clone`.                                                                    | Worktree `.git` pointer escapes the mount (§1.6); mounting the base repo exposes _all_ branches' objects/config → cross-branch/tenant disclosure. Clone mode is self-contained. Reuse the existing `base_repository: unavailable` → clone-only gate (`deployment.ts:398-413`). |
| **User home**                                 | Per-user named volume mounted at container `$HOME` (e.g. `disco_home_<uid>` → `/home/disco`), **not** the host home. | Persists tool caches/creds per user without exposing host `~/.disco/config.yaml`, `disco.db`, JWT secret. Matches `executor_storage.user_home: persistent-per-user`.                                                                                                             |
| **`~/.disco/config.yaml`, `disco.db`, secrets** | **Never mount.**                                                                                                   | These are the daemon's trust root. Executor gets credentials via env-file/token, not the config file.                                                                                                                                                                          |
| **Managed agentic tools**                     | Baked into the image (preferred) _or_ mounted read-only from `~/.disco/agentic-tools/<version>`                     | Baking = reproducible + offline; mounting = smaller image but couples host layout + version drift. Prefer bake with pinned version (§7).                                                                                                                                       |
...
**Agentic tools — bake, don't mount (recommended):** run `disco install --sync` at image build with
`DISCO_MANAGED_AGENTIC_TOOLS=1` and a pinned `DISCO_VERSION`, producing
`~/.disco/agentic-tools/<version>/…` inside the image. Reproducible, offline-capable, version-aligned by
construction. Mounting from the host coupling is the fallback for air-gapped/self-hosted who pre-populate.

**Version alignment:** the image tag must encode the Disco release; daemon refuses to launch a Docker
...
execution:
  ...
  docker:
    image: ghcr.io/preset-io/disco-executor@sha256:<digest> # digest required in prod
    ...
    extra_mounts: [] # opt-in, validated, never base repo / ~/.disco
```
...
**Doctor/readiness:** extend `disco doctor` with: docker daemon reachable, image present + version match,
...
**Should a repo `.disco.yml` `start` command ever build this image?** **No.** That conflates the
_application-under-development environment_ (what `.disco.yml` variants are — they run the app-under-test
in compose) with _Disco's execution substrate_ (a platform concern). The executor image is a platform
artifact built by Disco's release pipeline / `disco doctor --build`, not by a branch's environment lifecycle.
A `docker` **`.disco.yml` variant** is still fine and useful (for dev-environments), but it is a _different
...
| Disco Cloud k8s                         | ➖ orthogonal           | Cloud uses `template` (kubectl), not local Docker; but the **image** and **containment interface** are shared, so this work de-risks Cloud |
...
- **Audit/observability**: container labels + `docker events` give a clean audit trail; add container id
  to task telemetry; `disco doctor` reports orphaned labeled containers.
...
**What a Docker backend lets Disco delete later (for deployments that adopt it):** host Unix account
...
`~/.disco`. `host.docker.internal` daemon URL. Container-lifecycle containment (`kill`+`inspect`+label
...
in `docker inspect`; a hostile prompt cannot read `~/.disco/config.yaml` or another branch.
...
**Phase 3 — image + doctor:** new `executor` Dockerfile target (bundle + `disco install --sync` pinned),
release-pipeline build, `disco doctor` reachability/version/round-trip checks. (Multi-arch = optional.)
...
note this is Disco's substrate, so it's a `config.yaml`/`disco doctor --build` affordance, _not_ a
`.disco.yml` variant.
...
- `packages/core/src/config/types.ts` — `executor_backend`, `DiscoDockerBackendSettings`.
- `packages/core/src/config/config-manager.ts` / `deployment.ts` — resolve backend; clone-only + version + reachability validation.
- `apps/disco-daemon/src/utils/spawn-executor.ts` — backend fork (`local`/`docker`/`template`); daemon-URL-for-container computation; label + stdin plumbing.
- **new** `apps/disco-daemon/src/executor-backends/docker.ts` — typed Engine-API create/start/attach/wait/kill/remove.
- `apps/disco-daemon/src/executor-tracking.ts` + `termination-coordinator.ts` — `ContainmentStrategy` interface; container-inspect evidence; container-id fences.
- `apps/disco-daemon/src/services/task-runtime-reconciler.ts` — label-based orphan sweep.
- `packages/core/src/unix/id-lookups.ts` — reuse/extend `getUidGidFromUsername`.
- **new** `docker/executor.Dockerfile` + release pipeline entry; `disco doctor` checks in CLI.
- Docs: fold into `apps/disco-docs/content/guide/containerized-execution.mdx` (local-Docker section), not a new concept page.
...
- Existing images include `disco-dev` (4.78GB), plus externally-built `disco-executor:review-1` and
  `disco-cloud-executor-*` (~1.93GB) — **not defined in this repo** (no Dockerfile/CI builds them), so a
...
`unix_user_mode`.** Rationale: it composes cleanly with the identity model Disco already has (`delegated`
...
4. **Image ownership.** Does the executor image become a **release artifact** (built + digest-pinned +
   version-gated by Disco's pipeline)? Multi-arch now or amd64-only POC?
