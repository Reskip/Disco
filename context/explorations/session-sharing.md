...
their MCP-tool front-ends `disco_sessions_spawn` and
`disco_sessions_prompt(mode:"fork"|"subsession")` — created child sessions with:
...
### New rules (see `apps/disco-daemon/src/utils/branch-authorization.ts`
...
`apps/disco-daemon/src/utils/determine-spawn-identity.test.ts` covers the
...
branch-scoped directory, e.g. `<branch.path>/.disco-sdk/<tool>/`. Sessions
