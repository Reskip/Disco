# Hyperframes Composition Brief: disco

## Objective

Create a short (~20s) launch-style brag video for **disco** — a premium product film
that breaks out of the terminal into Disco's signature warm-orange multiplayer canvas.
...
- Project root: `/var/lib/disco/home/discopg/.disco/worktrees/preset-io/disco/stage-marketing-screenshots`
- Primary files read: `apps/disco-docs/components/LandingPage.tsx`,
  `apps/disco-docs/components/LandingPage.module.css`, the live site disco.live
  (captured via Playwright), `context/messaging-and-positioning.md`, and the marketing
  screenshots in `apps/disco-docs/public/screenshots/`.
- Product name: disco
...
- Key UI / visual moment to recreate: the **warm-orange Disco board** (`board-hero.png`)
  emerging from a cold dark-teal terminal — the cool→warm contrast IS the visual hook.
- Real product screenshots to show (all under `apps/disco-docs/public/screenshots/`):
  - `board-hero.png` — the hero board (spatial canvas, zones, branch cards, AI-spend &
    burndown dashboards). The centerpiece.
  - `marketing/disco-marketing-facepile-tooltip.png` — the live facepile (emoji avatars + "+7").
  - `marketing/disco-marketing-cursor-indicator.png` — the orange "Mina" live cursor.
  - `marketing/disco-marketing-social-comment-context.png` — a board comment thread on an
    active branch card ("Can we keep the facepile capped but still show the +7 overflow?").
  - `marketing/disco-marketing-slack-thread.png` — an agent pinged in Slack
    ("@DatDisco can you help me debug the Airflow job that failed this AM?").
  - Harness logos in `apps/disco-docs/public/tools/`: `claude-code.png`, `codex.png`,
...
- Angle: Everyone runs agents alone in a terminal. Disco breaks them onto one warm, living
...
- Hook: a lone terminal prompt on the dark-teal disco.live background; line 1 types
  "one agent in a terminal is fine.", then "five across a team? chaos." snaps in.
- Outro / punchline: dark-teal canvas + constellation resolve; disco wordmark + tagline
  "Team command center for all things agentic." and "fairly open · disco.live". One logo hit.
- Avoid:
  - Generic SaaS language ("streamline your workflow", "supercharge", "10x")
  - Abstract filler visuals / particle systems / equalizer bars
  - Unrelated visual redesign — match disco.live exactly (see Visual Identity + design.md)
...
- Signature motif: thin teal **constellation/network lines** connecting small dots,
  drifting slowly — the disco.live parallax background. Use subtly behind the text scenes
...
- Display font: heavy system sans (Inter / system-ui), weight 800–900, very tight tracking
  (~-0.05em) — mirrors disco.live's headline. Body: same family, regular.
...
5. **Outro / logo** — 3.0s — dark-teal canvas + constellation; disco wordmark + tagline
   - "fairly open · disco.live". One clean logo hit.
