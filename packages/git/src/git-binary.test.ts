...
describe('resolveGitBinary', () => {
  ...
  it('uses an executable space-free Program Files alias on Windows', () => {
    const longGit = 'C:\\Program Files\\Git\\cmd\\git.exe';
    const shortGit = 'C:\\PROGRA~1\\Git\\cmd\\git.exe';
    expect(
      resolveGitBinary({
        platform: 'win32',
        path: 'C:\\Program Files\\Git\\cmd',
        commonPaths: [],
        isExecutable: (candidate) => candidate === longGit || candidate === shortGit,
      })
    ).toBe(shortGit);
  });
