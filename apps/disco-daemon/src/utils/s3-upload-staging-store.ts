...
import { getManagedStorageSegments } from '@disco/core/config';
import type {
  UploadMetadata,
  UploadOwner,
  UploadReadInput,
...
const PROVENANCE = new Set(['browser']);
...
function parseStoredMetadata(metadata: S3Metadata): ParsedS3Metadata {
  ...
  return {
    ...
    createdBy: metadata['created-by'],
    agentId: metadata['agent-id'] || null,
    name: metadata.name,
...
export class S3UploadStagingStore implements UploadStagingStore {
  ...
  async stage(input: UploadStageInput): Promise<UploadMetadata> {
    ...
    const metadata: Record<string, string> = {
      ...
      'agent-id': input.owner.agentId ?? '',
      name: sanitizeUploadFilename(input.name),
