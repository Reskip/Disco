...
Disco does not distribute, migrate, or durably resume PTYs. A web terminal is an
...
Owner loss ends the Disco attachment. The UI displays **Disconnected** and
requires an explicit **Reconnect**. Reconnect creates a fresh PTY bridge. It
may reattach to a surviving Zellij shell on the same runtime, but Disco makes no
...
For first creation Disco uses Zellij's new-session path, not `attach --create`:
...
- Closing the modal removes the attachment and asks its executor to shut down.
  Disco intentionally quits the live Zellij server rather than retaining an
...
| Two daemons, shared host/storage              | Supported, ephemeral transport with optional shell resurrection                        | PTY bytes stay owner-local; the effective home and its Disco-managed Zellij socket directory must be the same local mount in every replica |
...
| Disco Cloud / managed private cloud            | Disabled unless deployed as supported shared-local; otherwise future workspace runtime | Capability endpoint hides UI when unsupported                                                                                             |
...
effective uid 1000, and `/home/disco` home. Both daemon containers were healthy.
...
A separate Zellij check closed the Disco attachment, created a new terminal ID,
...
process memory. This is the intended distinction: Disco transport persistence
...
branch cwd, uid/gid 1000, `/home/disco`, and a branch-scoped Zellij session.
...
The bundled Zellij 0.43.1 was tested separately from the Disco transport. Its
`attach --create` path did not leave an exited, resurrectable session, while a
true new-session launch with serialization enabled did. Disco now preflights the
