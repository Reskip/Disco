 ...
 *
 * 1. `scopeSchedulesToCaller()` — every external list is owner-scoped.
 * 2. `ensureScheduleLoaded(repo)` — lazy-loads the current schedule onto
 *    `params.schedule` for get/patch/remove/run-now ownership checks.
 *
 * 3. `validateScheduleConfig()` — cron + IANA + prompt-template
 ...
 *
 * 4. `recomputeNextRunAt()` — keeps the `next_run_at` invariant fresh
...
import type { ScheduleRepository } from '@disco/core/db';
import { BadRequest, Forbidden, NotAuthenticated } from '@disco/core/feathers';
import type { HookContext, Schedule } from '@disco/core/types';
import { isTimezoneMode } from '@disco/core/types';

/** Force external Schedule lists to the authenticated owner. */
export function scopeSchedulesToCaller() {
  return (context: HookContext): HookContext => {
    ...
    context.params.query = { ...(context.params.query ?? {}), created_by: userId };
    return context;
  };
}

/**
 * Lazy-load the current schedule into `context.params.schedule` for
 * item operations. Idempotent: no-op when already present.
 */
export function ensureScheduleLoaded(scheduleRepo: ScheduleRepository) {
  return async (context: HookContext) => {
    if (context.method === 'find' || context.method === 'create') return context;
 ...
 *
 * Must run AFTER `ensureScheduleLoaded` on patch so the merged
...
export function validateScheduleConfig() {
  return async (context: HookContext) => {
    ...
    if (data.cron_expression !== undefined) {
      const { isValidCron } = await import('@disco/core/utils/cron');
 ...
 * an agent that will execute with that user's Disco MCP token and credential route.
 *
 * External callers may therefore only modify/run schedules they created.
 * The run-as field is immutable for everyone and even superadmins cannot
 * operate another user's schedule through the user-facing service. Internal
 * scheduler ticks bypass this hook.
 */
export function ensureScheduleRunsAsCaller() {
  return (context: HookContext): HookContext => {
 ...
 *
 * Must run AFTER `ensureScheduleLoaded` on patch (it reads
 * `params.schedule`).
 */
export function recomputeNextRunAt() {
  return async (context: HookContext) => {
    ...
    const { getNextRunTime } = await import('@disco/core/utils/cron');
    ...
    if (!current) return context; // No current row (shouldn't happen post-ensureScheduleLoaded).
