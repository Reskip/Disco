...
import { type DiscoConfig, type ApiKeyName, resolveApiKey } from '@disco/core/config';
import { runWithTenantDatabaseScope, type TenantScopeAwareDatabase } from '@disco/core/db';
...
function getExecutorTokenPayload(params?: Params): ExecutorTokenPayload | undefined {
  ...
  if (authParams?.authentication?.strategy === 'jwt' && authParams.task_id) {
    const scopedParams = params as
      | (Params & { session_id?: string; sessionId?: string; task_id?: string })
      | undefined;
    return {
...
export class ConfigService {
  ...
  async resolveApiKey(
    ...
    // In trusted local simple mode every account runs the same Codex CLI home.
    // A user's own API key remains an explicit override; otherwise the shared
    // server auth.json is the native credential for old and newly-created users
    // alike, independent of their legacy per-user auth-method flag.
    const useServerSharedCodexAuth =
      tool === 'codex' && !result.apiKey && usesServerSharedCodexAuth(this.config);
    const useNativeAuth = result.useNativeAuth || useServerSharedCodexAuth;
    if (useNativeAuth) {
