...
export class CopilotTool implements ITool {
  ...
  async checkInstalled(): Promise<boolean> {
    try {
      ...
      execSync('which copilot', { encoding: 'utf-8', windowsHide: true });
      return true;
    } catch {
      return false;
    }
  ...
  private async captureSessionId(sessionId: SessionID, sdkSessionId: string): Promise<void> {
    console.log(`🔑 Captured Copilot session ID for Disco session ${sessionId}: ${sdkSessionId}`);
    ...
    if (this.sessionsRepo) {
      ...
      console.log(`💾 Stored Copilot session ID in Disco session`);
    }
  }
