# Brag Plan: disco (v2 — 40s cinematic product tour)

## What is this app?

Disco is a multiplayer, Figma-like spatial canvas where a whole team runs their AI coding
...
A premium, cinematic product tour that **follows the disco.live flow**: break out of the
...
A lone terminal on the dark-teal disco.live background (soft music intro). A command types,
...
- Track: `assets/music/disco-epic-40s.mp3` — "Adventure Epic" (Kornev, Pixabay, royalty-free,
...
- Cue preset (bed-relative): `assets/music/cues/disco-epic-40s.music-cues.json`.
...
PRNG). Minimal monospace terminal: a command types (e.g. `disco ▸ run agent…`) then the line
...
(`marketing/disco-marketing-facepile-tooltip.png`) popping in one by one; the orange Mina
cursor (`marketing/disco-marketing-cursor-indicator.png`) gliding; the board comment popover
(`marketing/disco-marketing-social-comment-context.png`) popping. Caption:
...
sub-beat: the Slack thread `marketing/disco-marketing-slack-thread.png` slides in with
...
Back to dark-teal + settling constellation. The `disco` wordmark (heavy display, teal "o"
glyph) resolves on the **38.05s** hit, with tagline **"Team command center for all things
agentic."** and **"fairly open · disco.live"**. One clean bell; music fades 38.8→40.0s.
