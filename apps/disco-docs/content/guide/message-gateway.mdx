...
description: Route messages from Slack, GitHub, and other platforms directly into Disco sessions, turning any chat thread or PR comment into a portal to an autonomous coding agent.
heroImage: '/screenshots/marketing/disco-marketing-slack-thread.png'
---
...
> **The mental model:** a gateway channel is a remote terminal into an agent that holds the same authority as a human prompting that branch. Configure who can reach it, what it can do, and which identity it runs as, and the rest is normal Disco surface area.

</div>

The Message Gateway connects external messaging platforms to Disco's agent sessions. Mention your Slack bot in a channel thread, mention `@disco` on a GitHub PR, or mention the configured assistant in a Shortcut story comment, and Disco spins up a coding session on the right branch with the right agent, then routes responses back to the platform. In Slack channels, every prompt requires an explicit bot mention; Disco catches the bot up on intervening thread replies when it is mentioned again.

> Currently supports **Slack**, **GitHub**, **Shortcut**, and **Microsoft Teams**. Discord, WhatsApp, and Telegram connectors are planned.

<img
  src="/screenshots/marketing/disco-marketing-slack-thread.png"
  alt="Slack thread where a user mentions an Disco teammate and the bot creates an Disco session, replies with a view-session link, and starts reading context"
  style={{
    maxWidth: '920px',
    width: '100%',
    borderRadius: '14px',
...
Gateway channels keep an internal per-channel key for Disco's generic inbound endpoint, but supported integrations usually do not require you to copy it anywhere. Slack uses Socket Mode, while GitHub and Shortcut use API polling; no public webhook URL is needed for those connectors.

## How It Works

When a message arrives from a platform (e.g., a Slack bot mention or a GitHub `@disco` mention), the gateway processes it through a simple pipeline:

1. **Authenticate.** The gateway looks up the channel by its key and verifies it's enabled.
2. **Map thread to session.** If this is a new platform thread, create a fresh Disco session on the channel's target branch with the channel's agentic tool configuration. If this thread has been seen before for this gateway channel, route the mention to the existing session. Multiple Slack bots can participate in the same Slack thread; each bot gets its own mapping.
3. **Send prompt.** The message text is sent to the session via Disco's standard prompt flow (task creation, executor spawn, the full pipeline).
...
5. **Route response back.** When the agent produces a response, Disco's outbound routing hook intercepts it and posts it back into the same platform thread. Formatting is adapted per platform (Slack mrkdwn conversion, GitHub markdown pass-through).
6. **Repeat.** Slack follow-ups require another explicit mention. When mentioned again, Disco fetches intervening Slack thread replies and includes them as catch-up context before prompting the agent.

```
Slack DM                    Disco Daemon                    Agent
────────                    ───────────                    ─────
  │                              │                           │
  │  "Fix the login bug"         │                           │
  ├─────────────────────────────>│                           │
...
> **Provider limit in PostgreSQL mode:** Slack, GitHub, and Shortcut listeners
> support distributed ownership. Microsoft Teams currently depends on a local
> Bot Framework port and in-memory conversation references, so Disco fails its
...
GitHub and Shortcut polling watermarks are persisted behind the same owner
fence, with a short overlap during takeover. Their create-comment APIs do not
accept an idempotency key, so a crash after the provider accepts an
acknowledgement but before Disco records it can still produce a duplicate
acknowledgement; the Disco Session and Task remain deduplicated.
...
and tenant-RLS provider-event table. Apply normal Disco migrations before
...
Proactive outbound emits (`disco_gateway_emit_message`) and Slack thread history reads made from a session are bound to that session's own branch: the gateway channel's target branch must match the calling session's branch, and there is no admin-role bypass. `disco_gateway_outbound_targets_list` only returns channels a session can actually use. Calls made without session context (personal API keys) follow the regular user-permission model instead.

## Slack thread behavior

In Slack channels, Disco uses a hard mention rule:

- A Slack channel or group thread only starts an Disco session when the bot is explicitly `@mentioned`.
- Later prompts in that Slack thread also require an explicit mention. Ordinary human replies do not wake the agent.
- On each mention, Disco looks up Slack thread replies since the bot last received a prompt and includes them as catch-up context.
- The same Slack thread can involve multiple Disco/Slack bots. Each explicitly mentioned bot maps the thread to its own Disco session.
...
Slack setup is **manifest-based**. Instead of clicking through OAuth scopes, event subscriptions, and Socket Mode by hand, you describe what the bot should do and Disco produces a complete Slack app manifest. Pasting that manifest into Slack's "Create app from manifest" flow preconfigures every scope and event in one step, so the scope list can never drift from what the connector actually calls.

There are two ways to get the manifest:

- **The Add Channel wizard (UI).** When you create a Slack channel in Disco, a guided wizard builds the manifest from your choices and shows it (with a live scope/event preview) ready to copy.
- **The `disco_gateway_slack_manifest_generate` MCP tool (agent-driven).** Ask an agent to generate the manifest for you. The tool is the programmatic equivalent of the wizard, backed by the same generator: from a set of capability toggles it returns the manifest JSON, the derived bot scopes and events, ordered setup steps, and the channel config to pass to `disco_gateway_channels_create`. It is pure. It creates no Slack app, no Disco channel, and validates no tokens. When an agent completes setup, it creates the channel disabled and without tokens via `disco_gateway_channels_create` (`enabled: false`), then calls `disco_widgets_request_gateway_token` so you enter the `xoxb-`/`xapp-` tokens in a secure inline form. The agent never asks you to paste tokens into the chat or passes them as `disco_gateway_channels_create` arguments, because raw secrets in tool arguments would leak into the MCP transcript.
...
3. On the **Channel** step, set the basics: **Channel Type** (Slack), a **Name** for the Disco channel, the **Target Branch** where sessions run, and whether the channel is **Enabled**. Click **Continue**.
4. On the **Options** step, choose what the bot can do:
   ...
   - **Align Slack users.** Match each Slack profile's email to an Disco user (unmatched senders are rejected). When off, pick a single Disco user to run every session as.
...
When someone mentions `@disco` (or your app's name) in a PR or issue comment, Disco picks it up via API polling and creates a session. The session runs on the target branch, and the agent's final response is posted back as a PR/issue comment under the app's bot identity (e.g., `disco[bot]`).

Key differences from Slack:

- **Polling, not WebSocket.** Disco polls the GitHub API every 15 seconds (configurable). No public endpoint needed.
- **Per-PR/issue sessions.** Each PR or issue gets its own session. All `@disco` mentions within the same PR route to the same session.
- **Last-message-only responses.** Unlike Slack (which streams every message), only the agent's final response is posted to GitHub. Intermediate progress is visible in the Disco UI.
- **Bot identity.** Responses are posted as the GitHub App's bot user, not as any human user.

### 1. Create a GitHub App

You can create the app through Disco's setup wizard or manually:

**Via Disco UI (recommended):**
...
4. Name your app (default: "Disco") and click **Create**.
5. Disco captures the credentials automatically.
...
4. Disable webhooks (set `Active` to false). Disco uses polling.
5. After creating, generate a private key and note the App ID.

### 2. Install the App

1. Go to your GitHub App's page → **Install App**.
2. Select your organization and choose which repositories Disco should access.
3. Click **Install**.

### 3. Create a Channel in Disco
...
3. Optionally configure:
   ...
   - **Mention Name.** The keyword to trigger on (default: `disco`, matches `@disco`).
4. Save. Polling starts immediately.

### 4. Mention @disco on a PR

Comment `@disco review this PR` on any PR in a watched repo. Within 15 seconds:

1. The comment gets a 👀 reaction (instant feedback).
2. A "Processing..." comment appears with a link to the Disco session.
3. The agent reviews the PR.
4. The "Processing..." comment is replaced with the agent's response.

Follow-up `@disco` mentions on the same PR continue the same session. The agent has full conversation history.
...
```
You handle GitHub mentions for preset-io/disco.
...
- Include links to Disco sessions in your replies
...
With a capable model (like Claude Opus) and Disco MCP tools available, even a minimal prompt produces good results. The agent figures out how to use branches, spawn sessions, and interact with GitHub. Over time, the teammate learns the codebase, team conventions, and review patterns across PRs.

## Setting Up Shortcut Integration

### How It Works

When someone mentions the configured assistant in a Shortcut story comment, Disco creates or resumes a session for that comment thread. It immediately posts a 👀 acknowledgement, adds the Disco session link, and replaces that acknowledgement with the assistant's final response when the turn completes.

Key behavior:

- **Polling, not webhooks.** Disco polls the Shortcut API every 15 seconds by default, so the daemon does not need a public endpoint.
- **Threaded sessions.** Each Shortcut comment thread maps to one Disco session. Later mentions in the same thread continue that session.
...
- **Per-user attribution.** With **Align Shortcut users** enabled, Disco first checks the channel's member-ID-to-email map, then the member's Shortcut email. Unmatched senders are rejected instead of running as another user.
- **Last-message-only responses.** Intermediate progress stays in Disco; only the assistant's final message is written back to Shortcut.
...
3. Copy the token. Treat it like a password; Disco encrypts it at rest after the channel is saved.

### 2. Create the Channel in Disco
...
7. Choose the identity behavior:
   - Enable **Align Shortcut users** to run each session as the matching Disco user.
   - Add **User Map** entries when a member's Shortcut email differs from their Disco email. Keys are Shortcut member IDs and values are Disco email addresses.
   - If alignment is disabled, select the single Disco user under whose identity all channel sessions run.
8. Save the channel. Polling starts immediately when the channel is enabled.

### 3. Mention the Assistant

Add a comment to a story and `@mention` the Shortcut member resolved during setup. Disco posts an acknowledgement and session link, runs the assistant on the target branch, then edits the acknowledgement into the final threaded response.
...
- **1:1 DM vs channel threads.** In a personal (1:1) chat, every message routes to that conversation's session. In a channel or group chat, Disco replies in the same thread and, by default, only responds when `@mentioned` (configurable). The bot mention is stripped before the text reaches the agent.
...
4. Under the bot's **Configuration**, set the **Messaging endpoint** to `https://<your-daemon-host>/api/messages` (matching the webhook path and port you configure in Disco).
...
### 4. Message the Bot or @mention disco

In a 1:1 chat, just message the bot. In a channel, post `@disco review this thread`. Disco:
...
- **Thread-Session Map** (`/thread-session-map`). Persists the mapping between platform thread IDs and Disco session IDs.
...
- **Multiple workspaces / Enterprise Grid.** Each workspace needs its own Slack app and its own bot + app token pair, configured as a separate Disco channel. One token pair maps to one workspace.
...
- **Bot doesn't respond to `@disco` mentions.** Check daemon logs for `[github] Watching N repos` and `[github] Starting poll loop`. Verify the App ID, private key, and installation ID are correct. Ensure the app is installed on the org with the right repos selected.
...
- **Mentions detected but no session created.** Check that `mention_name` matches your app's slug (default: `disco`). Mentions inside code blocks are ignored by design.
- **Response posted as wrong user.** GitHub comments should appear as the app's bot identity (e.g., `disco[bot]`). If comments appear as a human user, something is bypassing the connector and using a personal `GITHUB_TOKEN` instead.
...
- **A commenter is rejected as unlinked.** With user alignment enabled, the commenter's Shortcut email must match an Disco account. Add their Shortcut member ID and Disco email to **User Map** when the emails differ.
...
- [Disco MCP Server](./internal-mcp). Give gateway-spawned agents full self-awareness and orchestration capabilities.
