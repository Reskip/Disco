...
describe('open-source telemetry config', () => {
  it('honors kill-switch environment variables', () => {
    ...
    expect(isTelemetryFullyDisabledByEnv({ DISCO_TELEMETRY: '0' } as NodeJS.ProcessEnv)).toBe(true);
    expect(isTelemetryEnabledByEnv({ DISCO_TELEMETRY: '1' } as NodeJS.ProcessEnv)).toBe(true);
    expect(
      isTelemetryEnabledByEnv({
        DISCO_TELEMETRY: '1',
        DO_NOT_TRACK: '1',
  ...
  it('allows environment to enable telemetry over config', () => {
    const resolved = resolveOpenSourceTelemetryConfig({
      env: { DISCO_TELEMETRY: '1', DISCO_TELEMETRY_WRITE_KEY: 'wk' } as NodeJS.ProcessEnv,
