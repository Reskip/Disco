...
const tempDir = mkdtempSync(path.join(os.tmpdir(), 'disco-client-pack-'));
...
if (packedManifest.dependencies && Object.hasOwn(packedManifest.dependencies, '@disco/core')) {
  fail('Packed manifest must not contain @disco/core as a runtime dependency');
}
...
const workspaceImportPattern =
  /(?:^|[\s;])(?:import|export|require)\s.*['"]@disco\/(?:core|agentic-tools|agentic-tool-opencode)(?:\/[^'"]*)?['"]/m;
