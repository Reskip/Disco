...
beforeAll(() => {
  process.env.DISCO_MASTER_SECRET ||= 'agentic-tool-preset-resolver-test-secret';
});
...
describe('agentic tool preset resolution', () => {
  ...
  dbTest('resolves a fresh user implicit default through the built-in fallback', async ({ db }) => {
    const user = await new UsersRepository(db).create({
      username: `preset-default-${Date.now()}-${Math.random()}@example.com`,
      name: 'Fresh User',
    });
    await expect(
      resolveAgenticConfigurationReference(
  ...
  dbTest('resolves the user default inline configuration', async ({ db }) => {
    const user = await new UsersRepository(db).create({
      username: `preset-user-default-${Date.now()}-${Math.random()}@example.com`,
  ...
  dbTest('materializes the exact pair selected by the execution owner', async ({ db }) => {
    const user = await new UsersRepository(db).create({
      username: `materialized-default-${Date.now()}-${Math.random()}@example.com`,
      name: 'Configured User',
      default_agentic_config: {
        opencode: {
  ...
  dbTest(
    ...
    async ({ db }) => {
      const user = await new UsersRepository(db).create({
        username: `materialized-atomic-${Date.now()}-${Math.random()}@example.com`,
        name: 'Configured User',
        default_agentic_config: {
          codex: {
            permissionMode: 'ask',
  ...
  dbTest('rejects a preloaded execution owner with a mismatched subject', async ({ db }) => {
    const user = await new UsersRepository(db).create({
      username: `materialized-owner-${Date.now()}-${Math.random()}@example.com`,
      name: 'Configured User',
    });
  ...
  dbTest(
    ...
    async ({ db }) => {
      const user = await new UsersRepository(db).create({
        username: `atomic-preset-${Date.now()}-${Math.random()}@example.com`,
        name: 'Configured User',
        default_agentic_config: {
          opencode: {
  ...
  dbTest(
    ...
    async ({ db }) => {
      const user = await new UsersRepository(db).create({
        username: `atomic-workspace-${Date.now()}-${Math.random()}@example.com`,
        name: 'Configured User',
        default_agentic_config: {
          opencode: {
            modelConfig: { mode: 'exact', provider: 'owner', model: 'owner-model' },
