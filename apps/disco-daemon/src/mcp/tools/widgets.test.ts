/**
 * disco_widgets_request_env_vars MCP tool — handler tests.
 *
...
import type { MessageID } from '@disco/core/types';
...
describe('disco_widgets_request_env_vars', () => {
  ...
  it('normal path: creates a pending widget_request and returns status "requested"', async () => {
    ...
    const handler = captured.disco_widgets_request_env_vars.cb;
  ...
  it('generates the persisted widget id before the MCP message create', async () => {
    ...
    const res = await captured.disco_widgets_request_env_vars.cb({
      names: ['HUBSPOT_API_KEY'],
      reason: 'call hubspot',
      auto_resume: true,
    });
  ...
  it('tool description tells the agent this is fire-and-forget', () => {
    ...
    const desc = captured.disco_widgets_request_env_vars.cfg.description ?? '';
  ...
  it('tool description steers the agent to PREFER the widget over Settings instructions', () => {
    ...
    const desc = (captured.disco_widgets_request_env_vars.cfg.description ?? '').toLowerCase();
  ...
  it('renders a grammatical singular/plural preview line for the widget row', async () => {
    ...
    await captured.disco_widgets_request_env_vars.cb({
      names: ['HUBSPOT_API_KEY'],
      reason: 'call hubspot',
      auto_resume: true,
    });
    ...
    await captured2.disco_widgets_request_env_vars.cb({
      names: ['STRIPE_SECRET_KEY', 'HUBSPOT_API_KEY'],
      reason: 'two integrations',
      auto_resume: true,
    });
...
describe('disco_widgets_request_gateway_token', () => {
  const appendStub = appendSystemMessage as unknown as ReturnType<typeof vi.fn>;
  ...
  it('rejects a non-admin caller before creating any widget (admin-only)', async () => {
    ...
    await expect(
      captured.disco_widgets_request_gateway_token.cb({ gatewayChannelId: 'channel-1' })
  ...
  it('tool description marks it admin-only and prefers the widget over manual setup', () => {
    ...
    const desc = (captured.disco_widgets_request_gateway_token.cfg.description ?? '').toLowerCase();
