...
CREATE POLICY "knowledge_embedding_migration_0074_select" ON "kb_document_units"
  FOR SELECT
  USING (
    current_setting('disco.system_scope', true) = 'knowledge_embedding_migration_0074'
  );
--> statement-breakpoint
...
--> statement-breakpoint
SELECT set_config('disco.system_scope', 'knowledge_embedding_migration_0074', true);
--> statement-breakpoint
...
--> statement-breakpoint
SELECT set_config('disco.system_scope', '', true);
--> statement-breakpoint
...
CREATE POLICY "knowledge_embedding_discovery" ON "kb_document_units"
  FOR SELECT
  USING (
    current_setting('disco.system_scope', true) = 'knowledge_embedding_discovery'
