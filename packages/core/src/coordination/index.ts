...
export interface DistributedWorkIdentityEnvironment {
  readonly DISCO_DAEMON_INSTANCE_ID?: string;
 ...
 *
 * Instance precedence is DISCO_DAEMON_INSTANCE_ID, HOSTNAME, then the safe
...
export function createDistributedWorkIdentity(
  ...
  const instanceId =
    normalizedNonEmpty(environment.DISCO_DAEMON_INSTANCE_ID) ??
