 ...
 * `disco:mcp:internal`). Each issued token carries:
 *
 * - `sub`  — session id
 * - `uid`  — user id
 * - `tid`  — tenant id
 * - `aud`  — `disco:mcp:internal`
 * - `iss`  — `disco`
...
// Re-exported so daemon callers don't have to reach into @disco/core/types.
export { MCP_TOKEN_AUDIENCE, MCP_TOKEN_ISSUER } from '@disco/core/types';
