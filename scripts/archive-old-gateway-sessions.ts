...
import { createDatabase, SessionRepository } from '@disco/core/db';
import { getGatewaySource, isGatewaySession } from '@disco/core/types';
...
async function main() {
  ...
  if (dialect === 'postgresql') {
    databaseUrl = process.env.DATABASE_URL || 'postgresql://localhost:5432/disco';
  } else {
    const dbPath = path.join(os.homedir(), '.disco', 'disco.db');
