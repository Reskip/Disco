...
} from '@disco/core/callbacks/child-completion-template';
import {
  PAGINATION,
...
import { type Application, BadRequest, Conflict } from '@disco/core/feathers';
import { getOfficialTokenPricing } from '@disco/core/models';
import { deriveTitleFromPrompt, extractTitlePromptText } from '@disco/core/sessions';
import type {
  AuthenticatedParams,
  ContentBlock,
...
const tokenPricingLookupsInFlight = new Set<string>();
...
export class TasksService extends DrizzleService<Task, Partial<Task>, TaskParams> {
  ...
  async find(params?: TaskParams): Promise<Task[] | Paginated<Task>> {
    ...
    if (params?._discoSqlSessionAccessUserId) {
      pageOptions.visibleToUserId = params._discoSqlSessionAccessUserId;
    }
    if (params?._discoSqlSessionOwnerUserId) {
      pageOptions.ownerSessionUserId = params._discoSqlSessionOwnerUserId;
    }
  ...
  async autoTitleSession(task: Task, params?: TaskParams): Promise<void> {
    ...
    try {
      ...
      if (fresh.title == null) {
        ...
        this.scheduleTitleRefinement(task, autoTitle, params);
      }
    } catch (titleError) {
  ...
  /**
   * Replace the immediate heuristic title with a fast model summary without
   * delaying task creation. The provisional title doubles as a compare-and-set
   * token: any manual rename makes the deferred result ineligible to write.
   */
  private scheduleTitleRefinement(task: Task, provisionalTitle: string, params?: TaskParams): void {
    const config = this.app.get?.('config');
    const prompt = extractTitlePromptText(task.full_prompt || '').trim().slice(0, 4000);
    // Object.create-based unit fixtures intentionally have no runtime DB/config.
    if (!this.db || !config || !task.created_by || !prompt) return;

    const trustedParams = { ...(params ?? {}), provider: undefined } as TaskParams;
    deferWithTenantContext(
      params,
      async () => {
        const tenantId = getCurrentTenantId();
        if (!tenantId) return;
        const withTenantDatabase = <T>(
          work: (tenantDb: TenantScopedDatabase) => Promise<T>
        ): Promise<T> => runWithTenantDatabaseScope(this.db, tenantId, work);

        const route = await resolveCodexCredentialRoute(
          task.created_by as UUID,
          withTenantDatabase,
          config
        );
        if (!route.ok) {
          console.warn(
            `[TasksService] Title refinement skipped for session ${shortId(task.session_id)}: ${route.reason}`
          );
          return;
        }

        const result = await runExecutorCommand(
          { command: 'codex.generate-title', params: { prompt } },
          {
            ...
            ...(route.codexHome
              ? {
                  env: {
                    ...
                    CODEX_HOME: route.codexHome,
                  },
                }
              : {}),
            sensitiveOutput: true,
            timeoutMs: 30_000,
            logPrefix: '[SessionTitleExecutor]',
          }
        );
        const generatedTitle =
          result.success &&
          result.data &&
          typeof (result.data as Record<string, unknown>).title === 'string'
            ? String((result.data as Record<string, unknown>).title).trim()
            : '';
        if (!generatedTitle || generatedTitle === provisionalTitle) return;

        await runWithTenantDatabaseScope(this.db, tenantId, async (tenantDb) => {
          await assertTenantWritable(tenantDb, tenantId);
          const sessions = this.app.service('sessions');
          const fresh = await sessions.get(task.session_id, trustedParams);
          if (fresh.title !== provisionalTitle) return;
          // Re-read immediately before the trusted patch so a rename that
          // arrives while the model is answering wins over automation.
          const confirmed = await sessions.get(task.session_id, trustedParams);
          if (confirmed.title !== provisionalTitle) return;
          await sessions.patch(task.session_id, { title: generatedTitle }, trustedParams);
        });
      },
      (error) => {
        const message = error instanceof Error ? error.message : String(error);
        console.warn(
          `[TasksService] Title refinement failed for session ${shortId(task.session_id)}: ${message}`
        );
      }
    );
  }
  ...
  /**
   * Incrementally fill pricing for a newly observed model. The lookup runs in
   * the owning user's Codex credential context and never delays task
   * completion. Manual or already-known rates always win.
   */
  private scheduleUnknownModelPricing(task: Task, params?: TaskParams): void {
    const model = (task.model ?? task.normalized_sdk_response?.primaryModel ?? '').trim();
    const config = this.app.get?.('config');
    if (!model || getOfficialTokenPricing(model) || !this.db || !config || !task.created_by) return;
    ...
    deferWithTenantContext(
      ...
      async () => {
        const tenantId = getCurrentTenantId();
        if (!tenantId) return;
        const lookupKey = `${tenantId}:${task.created_by}:${model}`;
        if (tokenPricingLookupsInFlight.has(lookupKey)) return;
        tokenPricingLookupsInFlight.add(lookupKey);
        try {
          const users = this.app.service('users');
          const user = await users.get(task.created_by, trustedParams);
          const preferences = user.preferences ?? {};
          const tokenPricing = preferences.tokenPricing;
          if (tokenPricing?.autoUpdate === false || tokenPricing?.models?.[model]) return;
          ...
          const result = await runExecutorCommand(
            { command: 'codex.lookup-token-pricing', params: { model } },
            {
              delegatedHomeKey: route.delegatedHomeKey,
              templateVariables: {
                unix_user: route.delegatedHomeKey ?? undefined,
              ...
              sensitiveOutput: true,
              timeoutMs: 50_000,
              logPrefix: '[TokenPricingExecutor]',
            }
          );
          const data = result.success ? (result.data as Record<string, unknown> | undefined) : undefined;
          const input = Number(data?.inputUsdPerMillion);
          const cached = Number(data?.cachedInputUsdPerMillion);
          const output = Number(data?.outputUsdPerMillion);
          ...
          await runWithTenantDatabaseScope(this.db, tenantId, async (tenantDb) => {
            await assertTenantWritable(tenantDb, tenantId);
            const fresh = await users.get(task.created_by, trustedParams);
            const freshPreferences = fresh.preferences ?? {};
            const freshPricing = freshPreferences.tokenPricing ?? {};
            if (freshPricing.autoUpdate === false || freshPricing.models?.[model]) return;
            await users.patch(
              task.created_by,
              {
                preferences: {
                  ...
                  tokenPricing: {
                    ...freshPricing,
                    autoUpdate: freshPricing.autoUpdate !== false,
                    models: { ...(freshPricing.models ?? {}), [model]: rate },
                  },
                },
              },
              trustedParams
          ...
          tokenPricingLookupsInFlight.delete(lookupKey);
        }
      },
      (error) => {
        const message = error instanceof Error ? error.message : String(error);
        console.warn(`[TasksService] Token pricing lookup failed for ${model}: ${message}`);
      }
    );
  }
  ...
  private baseTaskAnalyticsProperties(task: Task): Record<string, unknown> {
    return {
      ...
      is_callback: task.metadata?.is_disco_callback === true,
  ...
  private async processCompletionSideEffects(
    ...
    try {
      ...
      if (status === TaskStatus.COMPLETED) {
        ...
        this.scheduleUnknownModelPricing(task, params);
      }
  ...
  private async queueCallbackToSession(
    ...
    try {
      ...
      // re-stamps `is_disco_callback` and `source` onto the synthesized
