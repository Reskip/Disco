 ...
 *   pnpm --filter @disco/daemon exec vitest run \
...
import type { MCPServerID, UserID } from '@disco/core/types';
import { isMCPOAuthGrantBindingVersion } from '@disco/core/types';
      ...
      await expect(
        runWithTenantDatabaseScope(dbA, owner.tenantId, (scoped) =>
          new MCPOAuthPendingFlowRepository(scoped).create({
            ...
            envelopeVersion: 1,
            sealedMaterial: 'disco-mcp-oauth:v1:pending-exchange:test',
            ttlMs: 60_000,
          })
        )
