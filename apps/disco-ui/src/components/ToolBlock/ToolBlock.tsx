...
export interface ToolBlockProps {
  ...
  /** Hide the leading status/icon slot. AgentChain uses this because its
   * outer summary is the single source of running/completed state. */
  showIcon?: boolean;
...
export const ToolBlock: React.FC<ToolBlockProps> = ({
  ...
  const header = (
    <div
      className="disco-tool-block-header"
    ...
    >
      {/* Status icon */}
      {showIcon && (
        <span style={{ flexShrink: 0, fontSize: 14, color: statusColor, lineHeight: 1 }}>
          {icon}
        </span>
      )}
  ...
  return (
    <div className="disco-tool-block" style={{ minWidth: 0, maxWidth: '100%' }}>
      {header}
      ...
      {children && (
        <div
          className={`disco-tool-block-collapse${expanded ? ' is-expanded' : ''}`}
          aria-hidden={!expanded}
        >
          <div className="disco-tool-block-collapse-inner">
            <div
              className="disco-tool-block-body"
              style={{
                marginTop: 2,
