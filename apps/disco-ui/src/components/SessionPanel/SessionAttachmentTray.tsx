...
import { Alert, Button, Modal, Progress, Space, Tooltip, Typography, theme } from 'antd';
  ...
  return (
    <>
      <Space
        ...
        className="disco-composer-attachment-tray"
        style={{ width: '100%' }}
        size={6}
      >
        {disabled && (
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: token.paddingSM, flex: 1 }}>
            {attachments.map((attachment) => {
              ...
              return (
                <div
                  key={attachment.id}
                  className="disco-composer-attachment-card"
                  style={{
                    position: 'relative',
                    width: 96,
                    height: 72,
                    ...
                    {isImage ? (
                      <img
                        ...
                        style={{
                          ...
                          display: 'block',
                          borderRadius: token.borderRadiusLG,
                          opacity: isUploading || isPending || isFailed ? 0.42 : 1,
                        }}
                      />
                    ) : (
                      <div
                    ...
                    {(isUploading || isPending || isFailed) && (
                      ...
                      >
                        {isUploading || isPending ? (
                          <Progress
                            type="circle"
                            percent={isPending ? 0 : (attachment.progress ?? 0)}
                            size={38}
                            strokeWidth={8}
                            railColor={token.colorBorderSecondary}
                            format={(percent) => (
                              <span style={{ fontSize: 10 }}>{Math.round(percent ?? 0)}%</span>
                            )}
                          />
                        ) : (
                          <ExclamationOutlined />
                        )}
                        <span>{isUploading || isPending ? '上传中' : '失败'}</span>
                      </div>
                    )}
                  </button>
                  <Tooltip title={disabled ? '文件正在上传…' : '移除文件'}>
                    <Button
                      className="disco-composer-attachment-remove"
        ...
        {failedCount > 0 && (
          <Alert
            type="error"
            showIcon
            message={`${failedCount} 个文件上传失败，请移除后再发送。`}
      ...
      {previewAttachment && (
        <Modal
          ...
          className="disco-attachment-preview-modal"
          open
          title={`预览 ${previewAttachment.file.name}`}
          footer={null}
          centered
          width={720}
        ...
        >
          {previewAttachment.previewUrl ? (
            <img
              ...
              alt={`${previewAttachment.file.name} 的预览`}
              style={{
                display: 'block',
                maxWidth: '100%',
                maxHeight: '70vh',
            ...
            <Space direction="vertical" align="center" style={{ width: '100%', padding: 24 }}>
              ...
              <Typography.Text type="secondary">
                {previewAttachment.file.type || '未知文件类型'}
              </Typography.Text>
            </Space>
          )}
        </Modal>
