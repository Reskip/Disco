 ...
 * `~/.disco/config.yaml` the machine happens to have — so results depend on the
 ...
 */
const testHome = mkdtempSync(path.join(os.tmpdir(), 'disco-test-home-'));
...
delete process.env.DISCO_OUTER_SANDBOX;
