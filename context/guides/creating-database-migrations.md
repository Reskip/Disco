...
Disco uses **Drizzle ORM** for schema migrations against both SQLite and PostgreSQL. Schemas live in two parallel files; migrations are generated locally with `pnpm` and applied via `disco db migrate`. No Docker is required for the dev loop.
...
pnpm disco db migrate

# 5. (Optional) Test against a live disco-managed env.
#    The container's docker-entrypoint.sh runs `pnpm disco db migrate --yes`
...
`disco tenant import` restores a whole tenant inside one transaction with
...
with the daemon **stopped** (`systemctl stop` → `disco db migrate` → `start`);
...
every daemon and runs `disco db migrate --offline-cutover`. Fresh databases may
...
```bash
sqlite3 ~/.disco/disco.db ".schema <table>"
...
# From repo root:
pnpm disco db status          # show pending migrations
pnpm disco db status --json   # emit the versioned automation contract
pnpm disco db migrate         # apply pending migrations to local DB
```
...
- Auto-apply on container boot: `docker/docker-entrypoint.sh` (calls `pnpm disco db migrate --yes`)
