...
import { DISCO_MCP_SERVER_NAME } from '@disco/core/mcp';
 ...
 * Shows Disco's custom permission UI via WebSocket and uses SDK's built-in permission persistence.
 */
  ...
  return async (
    ...
    if (toolName.startsWith('mcp__')) {
      ...
      if (configuredPermission === 'ask') {
        ...
        // Built-in "disco" server is always auto-approved (it's added dynamically, not in DB)
        if (serverName === DISCO_MCP_SERVER_NAME) {
          console.log(
            `✅ [canUseTool] Auto-approving MCP tool: ${toolName} (built-in "disco" server)`
          );
          return {
            behavior: 'allow',
    ...
    // We show Disco's UI and let SDK handle persistence via updatedPermissions
    ...
    try {
      ...
      if (decision.remember && decision.scope) {
        // Map Disco's scopes to SDK destinations
