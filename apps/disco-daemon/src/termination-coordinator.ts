...
import { isAgenticToolName, isTerminalTaskStatus, TaskStatus } from '@disco/core/types';
...
function unverifiedMessage(taskId: string, detail: string): string {
  return (
    `${detail} Disco could not verify that this executor stopped. It may still be running ` +
