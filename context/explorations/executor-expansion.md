...
**Created:** 2025-12-17
**Target:** Disco Cloud + Disco Local
**Complexity:** High
...
5. [Directory Structure: DISCO_HOME vs DISCO_DATA_HOME](#directory-structure-disco_home-vs-disco_data_home)
...
| **Directory separation**   | DISCO_HOME vs DISCO_DATA_HOME | Enables shared storage (EFS), local daemon config    |
...
4. **Breaking changes to public CLI** - `disco` CLI remains the user-facing interface
...
| Git clone        | `@disco/core/git`        | sudo wrapper script              |
| Git branch       | `@disco/core/git`        | sudo wrapper script              |
| Unix groups      | `@disco/core/unix`       | `SudoCliExecutor`                |
| ACL management   | `@disco/core/unix`       | `SudoCliExecutor`                |
...
│  DISCO_HOME: ~/.disco/                                                │
│                                                                      │
│  - config.yaml, disco.db, logs/                                      │
│  - REST/WebSocket API                                                │
│  - Session/Task orchestration                                        │
│  - NEVER touches DISCO_DATA_HOME directly                            │
│                                                                      │
│  Spawns executor via:                                                │
│  - Local: spawn('disco-executor', ['--stdin'])                       │
│  - Remote: template → kubectl run ... | disco-executor --stdin       │
└─────────────────────────────────────────────────────────────────────┘
                              │
...
│  DISCO_DATA_HOME: /data/disco/ (or EFS mount)                         │
│                                                                      │
...
## Directory Structure: DISCO_HOME vs DISCO_DATA_HOME

### Motivation

**Current:** `~/.disco/` contains everything mixed together:
...
```bash
# DISCO_HOME (daemon operating directory)
# Default: ~/.disco/
...
├── disco.db              # SQLite database (or postgres connection)
├── logs/                # Daemon logs
├── cache/               # Temporary cache
└── run/                 # Runtime files (PID, sockets)
    └── executor.sock    # IPC socket (local execution only)

# DISCO_DATA_HOME (git data directory)
# Default: $DISCO_HOME (backward compatible)
...
paths:
  ...
  # Default: same as disco_home (backward compatible)
  # Can be set to shared storage for k8s deployments
  data_home: /data/disco/

  # Explicit overrides (rarely needed)
  repos_dir: /data/disco/repos/ # Default: $DISCO_DATA_HOME/repos/
  branches_dir: /data/disco/worktrees/ # Default: $DISCO_DATA_HOME/branches/
  zellij_dir: /data/disco/zellij/ # Default: $DISCO_DATA_HOME/zellij/
```
...
DISCO_HOME=/home/disco/.disco
DISCO_DATA_HOME=/data/disco
...
1. **Default behavior unchanged** - `DISCO_DATA_HOME` defaults to `DISCO_HOME`
...
echo '{"command":"prompt","params":{...}}' | disco-executor --stdin

# Debug mode: read from file
disco-executor --stdin < payload.json

# Legacy mode (backward compat during migration)
disco-executor --session-token xxx --prompt "hello" --tool claude-code
```
...
async function main() {
  ...
  if (values.stdin) {
    ...
    console.error('Usage: disco-executor --stdin < payload.json');
    process.exit(1);
  }
      ...
      --overrides='{
        "spec": {
          ...
          "containers": [{
            ...
            "volumeMounts": [{
              "name": "data",
              "mountPath": "/data/disco"
            }]
          }],
          "volumes": [{
            "name": "data",
            "persistentVolumeClaim": {
              "claimName": "disco-data"
            }
          }]
        }
      }' \
...
async function spawnWithTemplate(
  ...
  const command = template
    ...
    .replace('{unix_user}', payload.asUser || 'disco');
...
- `DISCO_DATA_HOME` mounted as PVC (EFS, NFS, etc.)
...
export async function withImpersonation<T>(
  ...
  if (process.env.DISCO_IMPERSONATED === 'true') {
...
async function runAsUser(asUser: string, payload: ExecutorPayload): Promise<any> {
  // Build command: sudo su - $USER -c 'disco-executor --stdin'
  const cmd = 'sudo';
  const args = [
    '-n', // Non-interactive
    'su',
    ...
    asUser,
    '-c',
    'DISCO_IMPERSONATED=true disco-executor --stdin',
  ];
...
```bash
# /etc/sudoers.d/disco-executor

# Allow disco user to run executor as any disco_* user
disco ALL=(disco_*) NOPASSWD: /usr/local/bin/disco-executor

# Allow disco user to run privileged commands
disco ALL=(root) NOPASSWD: /usr/sbin/groupadd disco_*
disco ALL=(root) NOPASSWD: /usr/sbin/groupdel disco_*
disco ALL=(root) NOPASSWD: /usr/sbin/usermod -aG disco_* *
disco ALL=(root) NOPASSWD: /usr/bin/setfacl *
disco ALL=(root) NOPASSWD: /usr/bin/chgrp -R disco_* *
```
...
**Goal:** Decouple DISCO_HOME from DISCO_DATA_HOME

- [ ] Add `paths.data_home` config option
- [ ] Update all path resolution to use appropriate base
- [ ] Update `@disco/core/git` to use DISCO_DATA_HOME
- [ ] Update daemon to use DISCO_HOME for config/db
- [ ] Default DISCO_DATA_HOME = DISCO_HOME (backward compat)
...
execution:
  ...
  executor_unix_user: disco_executor
...
DISCO_DAEMON_PORT=3030
DISCO_DAEMON_HOST=0.0.0.0
...
async function executeGitClone(payload: GitClonePayload, client: DiscoClient) {
...
```
Daemon → spawn('disco-executor', ['--stdin']) → pipe JSON → read stdout
```
...
**Current implementation location:** `apps/disco-daemon/src/services/session-token-service.ts`
...
- **2025-12-17:** Initial design document created
  ...
  - Added DISCO_HOME/DISCO_DATA_HOME separation
