...
The original branch goal (build a "deserialize artifact → folder" MCP tool) was a no-op: `disco_artifacts_land` already shipped in PR #1052. This branch was repurposed for the refactor below.

---

## Goals

1. **One canonical format.** Files map + DB metadata. No `sandpack.json` sidecar, no `dependencies` column-as-truth, no Disco-only manifest fields scattered across the file tree.
...
3. **Explicit daemon capabilities.** `disco_grants` metadata enumerates what the daemon will inject (JWT, API URL, and artifact metadata). Each grant maps to a well-known env var name. Auditable and stricter consent treatment for high-power grants.
...
- Drop `selfHostedBundlerURL` resolution in `ArtifactsService` (`apps/disco-daemon/src/services/artifacts.ts:113`).
...
- Drop `payload.bundlerURL` plumbing in the render path (`apps/disco-ui/src/components/SessionCanvas/canvas/ArtifactNode.tsx:467,486`). The CodeSandbox-hosted bundler is the only path.

### 2. `sandpack.json` as a serialized file format

- `readFilesRecursive` no longer special-cases `sandpack.json` (`services/artifacts.ts:1098`) — it's just a regular file if a user happens to have one, but Disco doesn't read or write it.
...
### 3. `disco.config.js` Handlebars rendering

- Remove the per-fetch Handlebars rendering layer (`services/artifacts.ts:46-62, 664, 897-987`).
- Anything that today relied on `{{user.env.X}}`, `{{disco.token}}`, `{{disco.apiUrl}}`, `{{user.email}}`, `{{artifact.id}}`, `{{artifact.boardId}}` migrates to the new `required_env_vars` + `disco_grants` system below.
...
**Block (Disco-controlled or unsafe):**

- `options.bundlerURL` — Disco sets this.
...
| ------------------------ | ------------------ | ------------------------------- |
| `disco_api_url: true`     | `DISCO_API_URL`     | Inject the daemon's base URL.   |
| `disco_user_email: true`  | `DISCO_USER_EMAIL`  | Inject viewer's email.          |
| `disco_artifact_id: true` | `DISCO_ARTIFACT_ID` | Inject this artifact's ID.      |
| `disco_board_id: true`    | `DISCO_BOARD_ID`    | Inject the artifact's board ID. |

Conventional name + explicit declaration. Grants are part of the consent surface, except informational grants (`disco_artifact_id`, `disco_board_id`) which need no consent.
...
artifact_trust_grants
  ...
  disco_grants_set   — JSON object of grants this grant covers
...
- Reuses `FileCollection` (`apps/disco-ui/src/components/FileCollection/FileCollection.tsx`) — adapter from `Record<string, string>` files map to `FileItem[]`.
...
- **`disco_grants` editor** in the artifact settings panel: checkboxes for each grant key. Author-only (other viewers can't edit).
...
- `sandpack.json` files in their `files` map (an Disco-specific sidecar that's no longer parsed).
- `dependencies` / `entry` populated in DB columns (still readable as cache).
- `disco.config.js` files with Handlebars syntax (`{{user.env.X}}`, `{{disco.token}}`).
- No `required_env_vars`, no `disco_grants`, no `sandpack_config`.
...
{
  isLegacy: boolean
  signals: ('has_sandpack_json' | 'has_disco_config_js' | 'no_sandpack_config' | 'has_handlebars_disco_grant' | 'has_handlebars_user_env')[]
  detectedEnvVars: string[]      // names parsed out of {{user.env.X}}
  detectedGrants: string[]       // grant keys parsed out of {{disco.X}}
}
```
...
>
> ```
> Use the disco_artifacts_get tool to read artifact <id>'s files and current
> required_env_vars. Then use disco_artifacts_publish to republish with:
>   - sandpack_config populated (template, dependencies, entry from package.json)
>   - required_env_vars: [<detected var names>]
>   - disco_grants: { <detected grants> }
>   - sandpack.json and disco.config.js removed from the file map
...
> See disco_search_tools for full schemas.
> ```
...
- ADD `disco_grants jsonb DEFAULT '{}'`
...
```
disco_artifacts_export_codesandbox(artifactId)
...
3. Drop `disco.config.js` if present (will be Handlebars literals, broken on CodeSandbox).
4. POST to `https://codesandbox.io/api/v1/sandboxes/define?json=1` with `{ files, template: artifact.sandpack_config.template }`.
5. Return sandbox URL and instructions to set `required_env_vars` / `disco_grants` values via CodeSandbox's Secret Keys UI (the names will already match because the export uses the same prefix-per-template convention).
...
- **`preset-io/disco-artifact-examples` repo.** Curated examples consumable via `git clone + disco_artifacts_publish`. Empty repo + seed examples + PR template + CI to validate. Optional `disco_artifacts_publish_from_examples_repo` helper later.
- **Static MCP tool reference page on disco.live.** The artifact tools (and others) live only in code; a guide page would help human discoverability when the examples repo lands.
...
- **Migration**: legacy artifact (with `sandpack.json` + `disco.config.js`) renders in safe-degraded mode, banner appears with correct interpolated upgrade instructions.
...
- **Consent**: artifact requires `OPENAI_KEY` + `disco_api_url`; first render shows modal; granting "this author" persists; second render of same author's artifact requesting same set is silent; second render with new `STRIPE_KEY` re-prompts. Revocation removes grant; next render re-prompts.
...
- `packages/core/src/types/artifact.ts` — drop `SandpackManifest`, add `SandpackConfig`, `DiscoGrants`, `ArtifactTrustGrant`
- `packages/core/src/db/repositories/artifact.ts` — new fields
- `packages/core/src/db/repositories/artifact-trust.ts` — new repo

**Daemon:**

- `apps/disco-daemon/src/services/artifacts.ts` — rip `disco.config.js` rendering, rip `sandpack.json` handling, rip `use_local_bundler`, add `.env` synthesis with prefix logic, add `disco_grants` injection, add legacy detection
- `apps/disco-daemon/src/services/artifact-trust.ts` — new service for grants
- `apps/disco-daemon/src/mcp/tools/artifacts.ts` — update `publish` + `get` schemas, add `export_codesandbox`, add `review` (or expose via REST)
- `apps/disco-daemon/src/services/artifact-review.ts` — new one-shot review endpoint
- `apps/disco-daemon/src/utils/sandpack-config.ts` — `sanitizeSandpackConfig`, `envVarPrefixForTemplate`, defaults

**UI:**

- `apps/disco-ui/src/components/SessionCanvas/canvas/ArtifactNode.tsx` — render path uses `sandpack_config` blob, drops `bundlerURL`, shows trust state badge
- `apps/disco-ui/src/components/ArtifactConsentModal/` — new component (single modal w/ inline file viewer, reuses `FileCollection`)
- `apps/disco-ui/src/components/SettingsModal/TrustedArtifactsTab.tsx` — new settings tab
- `apps/disco-ui/src/components/SettingsModal/ArtifactsTable.tsx` — add `required_env_vars` / `disco_grants` editors
- RTL/component coverage or live-dev QA cases for the consent modal

**Docs:**

- `apps/disco-docs/pages/guide/artifacts.mdx` — rewrite for new format
...
- `disco_artifacts_land` already exists and stays. Round-trip via land → edit → publish should still work post-refactor, just with the new format.
