...
import type { Database } from '@disco/core/db';
import { createDatabaseAsync, generateId } from '@disco/core/db';
import {
...
import type { AuthenticatedParams, HookContext, User } from '@disco/core/types';
...
describe('POST /authentication/impersonate - Handler Logic', () => {
  ...
  describe('Token claims', () => {
    it('includes impersonation claims in JWT', async () => {
      ...
      expect(decoded.iss).toBe('disco');
      expect(decoded.aud).toBe('https://disco.dev');
    });
