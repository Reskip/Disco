...
describe('validateCron', () => {
  it('throws with a useful message on invalid cron', () => {
    expect(() => validateCron('bogus')).toThrow(/Cron 表达式无效/);
  });
});

describe('humanizeCron', () => {
  it('returns Chinese descriptions and Chinese preset labels', () => {
    expect(humanizeCron('0 * * * *')).toContain('每小时');
    expect(humanizeCron('0 9 * * 1-5')).toMatch(/09:00|9:00/u);
    expect(humanizeCron('0 9 * * 1-5')).not.toMatch(/Monday|Friday|At /u);
    expect(Object.values(CRON_PRESETS).map((preset) => preset.label)).toEqual([
      '每小时',
      '每 4 小时',
      '每天 09:00',
      '工作日 09:00',
