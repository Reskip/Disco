...
Guide for adding custom methods to FeathersJS services in Disco.
...
Disco uses a **hybrid approach** for extending FeathersJS services:
...
File: `apps/disco-daemon/src/services/boards.ts`
...
> (`apps/disco-daemon/src/adapters/drizzle.ts:376-385`) and the single-id branch
...
```typescript
// Use registerAuthenticatedRoute helper (apps/disco-daemon/src/utils/authorization.ts)
registerAuthenticatedRoute(
  app,
  '/sessions/:id/prompt',
  {
...
```typescript
// In apps/disco-daemon/src/services/sessions.ts
...
- `apps/disco-daemon/src/services/boards.ts` (service methods example)
- `apps/disco-daemon/src/index.ts` (custom routes examples)
- `apps/disco-daemon/src/utils/authorization.ts` (helper functions)
