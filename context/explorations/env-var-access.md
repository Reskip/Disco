...
Today Disco env vars are **ambient**. The daemon's (or impersonated user's) full env flows into every executor child process. From the agent's side, `env` / `printenv` / `cat /proc/self/environ` dumps all of it in a single tool call.
...
- **Consumers declare** what env vars they can use. Just metadata / discovery hints. An MCP server can declare "I know how to use `SHORTCUT_API_TOKEN`." An artifact feature can declare "I need `DISCO_API_KEY`."
...
Your env vars:
  ...
  GITHUB_TOKEN
    Exposed to: ☑ any consumer in [datdisco repo ▾]
  HUBSPOT_API_KEY
...
| `repo`             | Sessions in branches of this repo                              | repo uuid       | `FIVETRAN_API_KEY` in `datdisco`                      |
| `mcp_server`       | Only when this MCP is in session scope                         | mcp server uuid | `SHORTCUT_API_TOKEN` ↔ Shortcut MCP                  |
| `artifact_feature` | Only when this Disco feature is active                          | feature name    | `DISCO_API_KEY` for Artifacts                         |
...
```
agent: disco.run_with_env(cmd: "gh pr create ...", env: ["GITHUB_TOKEN"])
...
4. **Consumer declaration hints** for MCP servers and Disco features so the UI offers sensible pickers.
5. **UI**:
...
- Tool-only wrapper (`disco.run_with_env`) and per-call audit log.
...
### Tool-mediated access (`disco.run_with_env`)
...
Pairs with tool-mediated access. First `disco.run_with_env` call for a `tool_only + approve` var pauses the session, emits a permission-request event, UI shows modal with scope options, user approves, execution resumes. Reuses the existing permission system — no new infra.
...
- **Disco-defined vs user-defined consumers.** MCP servers have stable IDs; users can add/remove them. Artifact features and executors are platform-defined strings. The `resource_id` column needs to gracefully handle both — probably foreign keys only for stable-table targets, with runtime existence check plus cleanup on target deletion for the rest.
